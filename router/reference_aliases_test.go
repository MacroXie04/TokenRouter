...
func TestReferencePasswordResetTokenContract(t *testing.T) {
	...
	createResetFlow := func(code string) {
		t.Helper()
		_, createErr := service.CreateAuthFlow(
			service.PasswordResetPurpose, "email", "", user.Id, "", code, 15*time.Minute,
		)
	...
	assert.True(t, common.PasswordVerify(generatedBody.Data, user.Password))
	assert.False(t, common.PasswordVerify("old-password", user.Password))
	...
	require.NoError(t, db.First(&user, user.Id).Error)
	assert.True(t, common.PasswordVerify("chosen-password", user.Password))
	...
	restoreEntropy := common.SetSecureRandomReaderForTesting(resetEntropyFailureReader{})
	...
	require.NoError(t, db.First(&user, user.Id).Error)
	assert.True(t, common.PasswordVerify("retry-password", user.Password), "entropy failure must not consume the token")
}
