...
func TestCreateUserWithReferralRejectsOverflowAtomically(t *testing.T) {
	...
	inviter := referralUser("inviter", 7, 0)
	require.NoError(t, model.DB.Create(inviter).Error)
	invitee := referralUser("overflow", int(quotamath.MaxQuota)-10, inviter.Id)
	require.NoError(t, setting.UpdateOption(setting.QuotaForNewUserOption, fmt.Sprintf("%d", quotamath.MaxQuota-10)))
...
func TestCreateUserWithReferralUsesCanonicalQuotaAndCreatesTokenInTransaction(t *testing.T) {
	...
	assert.Equal(t, userssvc.GroupAuto, tokens[0].Group)
	assert.True(t, tokens[0].UnlimitedQuota)
	assert.Equal(t, quotamath.QuotaPerUnit, tokens[0].RemainQuota)
...
func TestTransferAffQuotaRejectsOverflowWithoutDebiting(t *testing.T) {
	setupAffiliateDB(t)
	user := referralUser("affiliate", int(quotamath.MaxQuota)-5, 0)
	user.AffQuota = quotamath.QuotaPerUnit
	require.NoError(t, model.DB.Create(user).Error)

	err := TransferAffQuota(user.Id, quotamath.QuotaPerUnit)
	require.ErrorIs(t, err, ErrReferralCreditOverflow)
	var got model.User
	require.NoError(t, model.DB.First(&got, user.Id).Error)
	assert.Equal(t, quotamath.QuotaPerUnit, got.AffQuota)
	assert.Equal(t, int(quotamath.MaxQuota)-5, got.Quota)
}
...
func TestTransferAffQuotaRequiresCurrentPaymentComplianceWithoutDebiting(t *testing.T) {
	...
	err := TransferAffQuota(user.Id, quotamath.QuotaPerUnit)
	require.ErrorIs(t, err, billingsvc.ErrPaymentComplianceRequired)

	var stored model.User
	require.NoError(t, model.DB.First(&stored, user.Id).Error)
	assert.Equal(t, quotamath.QuotaPerUnit, stored.AffQuota)
...
func TestTransferAffQuotaUsesImmutableQuotaUnit(t *testing.T) {
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), fmt.Sprintf("%d", quotamath.QuotaPerUnit))
	...
	require.NoError(t, TransferAffQuota(user.Id, quotamath.QuotaPerUnit))
	require.NoError(t, model.DB.First(&stored, user.Id).Error)
	assert.Zero(t, stored.AffQuota)
	assert.Equal(t, 25+quotamath.QuotaPerUnit, stored.Quota)
}
