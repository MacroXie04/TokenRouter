...
func prepareVerifiedRegistration(t *testing.T, email string) (code string, flow model.AuthFlow) {
	...
	previous := mailtransport.Mail
	mock := &mockMailer{}
	mailtransport.Mail = mock
	t.Cleanup(func() { mailtransport.Mail = previous })
...
func TestCreateUserWithReferralAndVerifiedEmailCommitsOneAtomicRegistration(t *testing.T) {
	...
	assert.Equal(t, cryptoutil.SHA256Hex("verified.user@example.com"), flow.Provider)
	...
	assert.Equal(t, cryptoutil.SHA256Hex(stored.Email), *stored.VerifiedEmailKey)
...
func TestCreateUserWithVerifiedEmailConcurrentSingleWinner(t *testing.T) {
	...
	_, err := CreateAuthFlow(
		EmailVerificationPurpose, cryptoutil.SHA256Hex(email), "email", 0, "", code, time.Minute,
	)
