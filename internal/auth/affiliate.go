...
func CreditInviter(inviterId, inviteeId int) error {
	if inviterId == 0 || !billingsvc.PaymentComplianceConfirmed() {
		return nil
	}
...
func parseReferralCreditOption(values map[string]string, key string) (int, error) {
	...
	if raw != strings.TrimSpace(raw) || len(raw) > 10 {
		return 0, fmt.Errorf("%w: %s is outside 0..%d", ErrInvalidReferralCredit, key, quotamath.MaxQuota)
	}
	...
	if parseErr != nil || parsed < 0 || parsed > quotamath.MaxQuota {
		return 0, fmt.Errorf("%w: %s is outside 0..%d", ErrInvalidReferralCredit, key, quotamath.MaxQuota)
	}
...
func creditInviterTx(tx *gorm.DB, inviterId, inviteeId int) error {
	...
	if inviterQuota > 0 {
		...
		newCount, countOK := quotamath.AddQuotaWithinBounds(inviter.AffCount, 1)
		newAffQuota, quotaOK := quotamath.AddQuotaWithinBounds(inviter.AffQuota, inviterQuota)
		newHistoryQuota, historyOK := quotamath.AddQuotaWithinBounds(inviter.AffHistoryQuota, inviterQuota)
	...
	if inviteeQuota > 0 {
		...
		if err := locking.SubscriptionLockForUpdate(tx).Select("id", "quota").First(&invitee, inviteeId).Error; err != nil {
			return err
		}
		newQuota, ok := quotamath.AddQuotaWithinBounds(invitee.Quota, inviteeQuota)
		if !ok {
			return ErrReferralCreditOverflow
		}
		result := tx.Model(&model.User{}).
...
func TransferAffQuota(userId, quota int) error {
	...
	minQuota := quotamath.QuotaPerUnit
	if userId < 1 || quota <= 0 || int64(quota) > quotamath.MaxQuota || minQuota <= 0 {
	...
	if !billingsvc.PaymentComplianceConfirmed() {
		return billingsvc.ErrPaymentComplianceRequired
	}
	if quota < minQuota {
	...
	return model.DB.Transaction(func(tx *gorm.DB) error {
		...
		if !quotamath.QuotaWithinBounds(user.AffQuota) || !quotamath.QuotaWithinBounds(user.Quota) {
			return ErrReferralCreditOverflow
		}
