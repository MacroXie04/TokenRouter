...
func TestResolveSpecialUsableGroupsIsDeterministicAndRequiresPositiveRatio(t *testing.T) {
	...
	resolved := userssvc.ResolveSpecialUsableGroups(policy, "team", base, lookup)
	assert.Equal(t, []userssvc.ResolvedUsableGroup{
	...
	assert.Equal(t, "default", userssvc.ResolveSpecialUsableGroups(policy, "team", base, lookup)[0].Name)
	assert.Empty(t, userssvc.ResolveSpecialUsableGroups(policy, "team", base, nil))
}
...
func TestRegistrationMutationPlanCapturesExactQuotaAndDefaultTokenBeforeMutation(t *testing.T) {
	...
	assert.Equal(t, billingsvc.TokenStatusEnabled, token.Status)
	...
	assert.Equal(t, userssvc.GroupAuto, again.Group)
...
func TestRegistrationMutationPlanMaterializationIsConcurrentAndDetached(t *testing.T) {
	...
	for worker := 1; worker <= workers; worker++ {
		go func(userID int) {
			...
			if token.UserId != userID || token.Group != userssvc.GroupAuto || token.Key != "sk-"+strings.Repeat("A", defaultTokenCredentialBytes) {
