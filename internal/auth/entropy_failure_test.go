...
func TestSecurityIssuanceEntropyFailureLeavesNoMutation(t *testing.T) {
	...
	user := model.User{
		Username: "entropy-user", Password: "hash", Role: roles.RoleCommonUser,
		Status: model.UserStatusEnabled, Group: userssvc.GroupDefault, AuthVersion: 1, Quota: 1000,
	...
	originalBackup := model.TwoFABackupCode{
		UserId: user.Id, CodeHash: cryptoutil.SHA256Hex("existing-backup"), CreatedAt: time.Now(),
	}
	...
	assert.ErrorIs(t, err, cryptoutil.ErrSecureRandomUnavailable)
	...
	assert.ErrorIs(t, SendPasswordResetEmail(user.Email), cryptoutil.ErrSecureRandomUnavailable)
	assert.ErrorIs(t, SendEmailVerificationCode("new@example.com"), cryptoutil.ErrSecureRandomUnavailable)
	...
	redemptionKeys, err := billingsvc.CreateRedemptionBatch(user.Id, "entropy", 100, 0, 2)
	assert.ErrorIs(t, err, cryptoutil.ErrSecureRandomUnavailable)
	...
	topUp, err := billingsvc.CreateTopUp(user.Id, 100, 1, "stripe", billingsvc.PaymentProviderStripe)
	assert.ErrorIs(t, err, cryptoutil.ErrSecureRandomUnavailable)
	assert.Nil(t, topUp)
	assert.ErrorIs(t, billingsvc.PurchaseSubscriptionWithBalance(user.Id, 1), cryptoutil.ErrSecureRandomUnavailable)
	assert.ErrorIs(t, billingsvc.RecordSystemLogChecked(user.Id, billingsvc.LogTypeSystem, "entropy audit"), cryptoutil.ErrSecureRandomUnavailable)
