...
func disableTwoFAWithCodeTxWithClock(tx *gorm.DB, userId int, code string, clock twoFADatabaseClock) (verificationErr, storageErr error) {
	...
	if !verified {
		...
		err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).
			Where("user_id = ? AND code_hash = ? AND is_used = ?", userId, cryptoutil.SHA256Hex(code), false).
			First(&backup).Error
		switch {
		case err == nil:
			verified = true
...
func TwoFAStatus(userId int) bool {
	...
	if err != nil {
		logging.SysError("query 2FA status failed: " + err.Error())
		return false
	}
	return enabled
}
...
func createTwoFABackupCodesAt(tx *gorm.DB, userId int, now time.Time) ([]string, error) {
	...
	for i := 0; i < backupCodeCount; i++ {
		code, err := cryptoutil.SecureRandomNumeric(8)
		if err != nil {
			return nil, err
		}
		codes = append(codes, code)
		records = append(records, model.TwoFABackupCode{
			UserId: userId, CodeHash: cryptoutil.SHA256Hex(code), CreatedAt: now,
		})
	}
