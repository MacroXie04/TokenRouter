// Package operations owns scheduling and the lease-guarded system-task queue.
...
func StartSystemTaskRunner() {
	systemTaskRunnerOnce.Do(func() {
		...
		go func() {
			...
			runnerUUID, err := cryptoutil.SecureRandomUUID()
			if err != nil {
				logging.SysError("start system task runner: " + err.Error())
				return
			}
			...
			for {
				...
				if err := ExpireStaleSystemTaskLocks(); err != nil {
					logging.SysError("expire stale system tasks: " + err.Error())
				}
				if err := runSystemTaskClaimPass(runnerID, false); err != nil {
					logging.SysError("run system task claim pass: " + err.Error())
				}
			}
		}()
	})
...
func runSystemTaskClaimPass(runnerID string, wait bool) error {
	...
	report := func(err error) {
		...
		logging.SysError("system task runner: " + err.Error())
		if wait {
			errCh <- err
		}
	}
...
func runChannelTestTask(ctx context.Context, task *model.SystemTask) (string, error) {
	...
	if task.Payload != "" && task.Payload != "null" {
		if err := jsonutil.ValidateJSONNoDuplicateKeys([]byte(task.Payload)); err != nil {
			return "", fmt.Errorf("invalid channel test payload: %w", err)
		}
		if err := jsonutil.UnmarshalJsonStr(task.Payload, &payload); err != nil {
			return "", fmt.Errorf("invalid channel test payload: %w", err)
		}
	}
	...
	selected, err := channelssvc.SelectChannelsForReliabilityTest(channels, payload.Mode)
	if err != nil {
		return "", err
	}
	...
	for i := range selected {
		...
		ch := &selected[i]
		summary.Tested++
		success, _, recordErr := channelssvc.TestAndRecordChannelContextWithMode(ctx, ch, now, allowDisable)
		if recordErr != nil {
			return "", recordErr
		}
		if success {
		...
		if err := model.DB.First(&refreshed, ch.Id).Error; err == nil {
			if refreshed.Status == channelcatalog.ChannelStatusEnabled {
				summary.Enabled++
			} else {
				summary.Disabled++
			}
		...
		if i%5 == 0 || i == len(selected)-1 {
			...
			if b, err := jsonutil.Marshal(state); err == nil {
	...
	resultBytes, err := jsonutil.Marshal(summary)
	if err != nil {
		return "", err
	}
	if payload.Notify {
		if err := userssvc.NotifyRootUser(userssvc.UserNotification{
			Type: "channel_test", Title: "通道测试完成", Content: "所有通道测试已完成",
		}); err != nil {
			logging.SysError("channel test completion notification failed")
		}
	}
...
func runModelUpdateTask(ctx context.Context, task *model.SystemTask) (string, error) {
	...
	report := func(processed, total int) error {
		...
		state, err := jsonutil.Marshal(modelUpdateTaskState{Processed: processed, Total: total, Progress: progress})
		if err != nil {
			return err
		}
		return model.UpdateSystemTaskState(task.TaskID, task.LockedBy, string(state))
	}
	summary, err := channelssvc.RunChannelUpstreamModelUpdateTask(ctx, payload.Manual, !payload.Manual, report)
	if err != nil {
		return "", err
	}
	result, err := jsonutil.Marshal(summary)
	if err != nil {
		return "", err
	}
...
func runLogCleanupTask(ctx context.Context, task *model.SystemTask) (string, error) {
	...
	if task.State != "" {
		if err := jsonutil.UnmarshalJsonStr(task.State, &state); err != nil {
			return "", err
		}
	}
	for {
