...
func buildCheckinSetting(options map[string]string) (CheckinSetting, error) {
	...
	if config.MinQuota <= 0 || int64(config.MinQuota) > quotamath.MaxQuota {
		return CheckinSetting{}, fmt.Errorf("%s must be between 1 and %d", CheckinMinQuotaOption, quotamath.MaxQuota)
	}
	if config.MaxQuota < config.MinQuota || int64(config.MaxQuota) > quotamath.MaxQuota {
		return CheckinSetting{}, fmt.Errorf("%s must be between %d and %d", CheckinMaxQuotaOption, config.MinQuota, quotamath.MaxQuota)
	}
	return config, nil
}
