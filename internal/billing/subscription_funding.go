...
func preConsumeUserSubscriptionTx(tx *gorm.DB, requestId string, userId int, amount int64) (*SubscriptionPreConsumeResult, error) {
	...
	query := locking.SubscriptionLockForUpdate(tx).Where("request_id = ?", requestId).Limit(1).Find(&existing)
	if query.Error != nil {
	...
	for i := range subs {
		...
		newUsed, ok := quotamath.AddQuotaWithinBounds(used, int(amount))
		if !ok {
			return nil, ErrSubscriptionQuotaOverflow
		}
		...
		if create.RowsAffected == 0 {
			...
			if err := locking.SubscriptionLockForUpdate(tx).Where("request_id = ?", requestId).First(&duplicate).Error; err != nil {
				return nil, err
			}
...
func populateSubscriptionPreConsumeReplay(
	...
	if record.Status != SubscriptionPreConsumeStatusConsumed || record.PreConsumed <= 0 || record.PreConsumed > quotamath.MaxQuota {
		return ErrSubscriptionQuotaOverflow
	}
	var sub model.UserSubscription
	if err := locking.SubscriptionLockForUpdate(tx).Where("id = ? AND user_id = ?", record.UserSubscriptionId, userId).
