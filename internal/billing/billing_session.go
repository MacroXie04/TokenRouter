	...
	err := transact(func(tx *gorm.DB) error {
		...
		if s.source == BillingSourceWallet {
			...
			case s.reserved > actual:
				...
				var ok bool
				newQuota, ok = quotamath.AddQuotaWithinBounds(user.Quota, refund)
				if !ok {
					return ErrUserQuotaOverflow
				}
			}
		...
		if tokenId > 0 && (actual > 0 || tokenReserved > 0) {
			...
			newTokenUsed, usedOK := quotamath.AddQuotaWithinBounds(token.UsedQuota, actual)
			if !usedOK || (!tokenUnlimited && !quotamath.QuotaWithinBounds(token.RemainQuota)) {
			...
			if !tokenUnlimited {
				...
				case tokenReserved > actual:
					...
					var ok bool
					newRemain, ok = quotamath.AddQuotaWithinBounds(token.RemainQuota, refund)
					if !ok {
						return ErrTokenQuotaOverflow
					}
				}
...
func boundedSubscriptionQuota(value int64) (int, bool) {
	if value < 0 || value > quotamath.MaxQuota {
		return 0, false
	}
	converted := int(value)
	return converted, quotamath.QuotaWithinBounds(converted)
}
...
func applySubscriptionQuotaDelta(value, delta int64) (int64, bool) {
	...
	switch {
	case delta > 0:
		if delta > quotamath.MaxQuota {
			return 0, false
		}
		updated, ok := quotamath.AddQuotaWithinBounds(quota, int(delta))
...
func settleFundingSubscriptionDeltaTx(tx *gorm.DB, userId, subscriptionId int, expectedEpoch, delta int64, now int64) error {
	...
	case delta > 0:
		...
		var ok bool
		newUsed, ok = quotamath.AddQuotaWithinBounds(used, int(delta))
		if !ok {
			return ErrSubscriptionQuotaOverflow
		}
	case delta < 0:
...
func (s *FundingSession) Refund() {
	if err := s.RefundChecked(); err != nil {
		logging.SysError("funding refund failed for user " + textutil.Int2Str(s.userId) + ": " + err.Error())
	}
}
