...
func TestQuotaDataHistogramWritePath(t *testing.T) {
	...
	base := wallclock.NowTimestamp() - wallclock.NowTimestamp()%3600
...
func TestSaveQuotaDataCacheRetainsUpdateFailureAndRetriesOnce(t *testing.T) {
	...
	seed := model.QuotaData{
		...
		UseGroup: "default", TokenID: 7, ChannelID: 3, NodeName: runtimeinfo.NodeName(),
...
func TestLogQuotaDataCheckedRejectsAggregateOverflow(t *testing.T) {
	...
	defer ResetQuotaDataCache()
	base := wallclock.NowTimestamp() - wallclock.NowTimestamp()%3600
	require.NoError(t, LogQuotaDataChecked(1, "alice", "gpt-4o", "default", 7, 3, int(quotamath.MaxQuota), 1, base))
	...
	for _, cached := range quotaDataCache {
		assert.Equal(t, int(quotamath.MaxQuota), cached.Quota)
