...
func IsUserSelectableGroup(userGroup, group string) bool {
	if group == "" || group == userssvc.GroupAuto || group != strings.TrimSpace(group) {
		return false
	}
	if userGroup == "" {
...
func GetUserUsableGroups(userGroup string) map[string]string {
	...
	autoDescription, autoAllowed := base[userssvc.GroupAuto]
	delete(base, userssvc.GroupAuto)
	policy := setting.GetRegistrationGroupPolicy()
	for _, rule := range policy.SpecialUsableGroupRules(userGroup) {
		if rule.Group != userssvc.GroupAuto {
			continue
		}
	...
	resolved := userssvc.ResolveSpecialUsableGroups(policy, userGroup, base, func(group string) (float64, bool) {
	...
	if autoAllowed && userssvc.ValidResolvedGroupText(autoDescription, userssvc.MaxResolvedGroupDescriptionBytes, true) {
		groups[userssvc.GroupAuto] = autoDescription
	}
	return groups
}
...
func ValidateUserAutoGroups(userGroup string, groups []string, maxCount int) error {
	...
	for _, group := range groups {
		...
		if trimmed == "" || trimmed == userssvc.GroupAuto || trimmed != group {
...
func GetGroupsModels(groups []string) map[string]bool {
	...
	for _, group := range groups {
		for modelName := range channelssvc.GetGroupModels(group) {
