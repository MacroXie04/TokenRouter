...
func CheckTokenUsable(token *model.Token) error {
	...
	if token.ExpiredTime > 0 && token.ExpiredTime < wallclock.NowTimestamp() {
...
func ReserveTokenQuota(id, quota int) error {
	...
	result := model.DB.Model(&model.Token{}).
		Where("id = ? AND remain_quota >= ? AND remain_quota <= ?", id, quota, quotamath.MaxQuota).
		UpdateColumn("remain_quota", locking.GormExpr("remain_quota - ?", quota))
	if result.Error != nil {
		return result.Error
	}
...
func RefundTokenQuotaReservation(id, quota int) error {
	...
	return model.DB.Transaction(func(tx *gorm.DB) error {
		var token model.Token
		if err := locking.SubscriptionLockForUpdate(tx).Unscoped().Select("id", "remain_quota").First(&token, id).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
				return ErrTokenNotFound
			}
			return err
		}
		newRemainQuota, ok := quotamath.AddQuotaWithinBounds(token.RemainQuota, quota)
		if !ok {
...
func CommitTokenQuotaReservation(id, quota int) error {
	...
	return model.DB.Transaction(func(tx *gorm.DB) error {
		var token model.Token
		if err := locking.SubscriptionLockForUpdate(tx).Unscoped().Select("id", "used_quota").First(&token, id).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
				return ErrTokenNotFound
			}
			return err
		}
		newUsedQuota, ok := quotamath.AddQuotaWithinBounds(token.UsedQuota, quota)
		if !ok {
			return ErrTokenQuotaOverflow
		}
		result := tx.Unscoped().Model(&model.Token{}).
...
func CreateUserTokenWithinLimit(token *model.Token, limit int) error {
	...
	return model.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := locking.SubscriptionLockForUpdate(tx).Select("id").First(&owner, token.UserId).Error; err != nil {
			return err
		}
