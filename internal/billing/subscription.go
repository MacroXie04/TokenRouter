...
func calcSubscriptionBalanceQuota(priceAmount float64) (int, error) {
	...
	quota := decimal.NewFromFloat(priceAmount).
		...
		Ceil()
	return quotamath.QuotaFromDecimalStrict(quota)
}
...
func calcSubscriptionBalanceQuotaDecimal(price decimal.Decimal) (int, error) {
	...
	quota := price.Mul(decimal.NewFromInt(quotamath.QuotaPerUnit)).Ceil()
	return quotamath.QuotaFromDecimalStrict(quota)
}
...
func PurchaseSubscriptionWithBalance(userId, planId int) error {
	...
	err = model.DB.Transaction(func(tx *gorm.DB) error {
		...
		var user model.User
		if err := locking.SubscriptionLockForUpdate(tx).Where("id = ?", userId).First(&user).Error; err != nil {
			return err
		}
		if requiredQuota > 0 {
	...
	if err := DeliverAuditLogOutboxEvent(auditEventID); err != nil {
		logging.SysError("balance subscription audit delivery deferred: " + err.Error())
	}
	return nil
}
...
func ResetSubscriptionQuotaContext(ctx context.Context, sub *model.UserSubscription) error {
	...
	return model.DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if err := locking.SubscriptionLockForUpdate(tx).Where("id = ?", sub.Id).First(&current).Error; err != nil {
			return err
		}
