...
func TestFreeModelReservationSkipsWalletAndSubscriptionHoldsDurably(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "wallet", preference: userssvc.BillingPreferenceWalletOnly},
		{name: "subscription", preference: userssvc.BillingPreferenceSubscriptionOnly, withSub: true},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			_, err := userssvc.UpdateUserBillingPreference(user.Id, test.preference)
			...
			channel := model.Channel{
				...
				Status: channelcatalog.ChannelStatusEnabled, UsedQuota: 3,
			}
