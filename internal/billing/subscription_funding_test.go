...
func TestPreConsumeSubscriptionValidationAndLedger(t *testing.T) {
	...
	_, err = PreConsumeUserSubscription("req-overflow", u.Id, quotamath.MaxQuota+1)
	...
	for name, values := range map[string]struct{ total, used int64 }{
		...
		t.Run(name, func(t *testing.T) {
			corrupt := seedSub(t, second.Id, plan.Id, values.total, values.used, func(sub *model.UserSubscription) {
				sub.EndTime = wallclock.NowTimestamp() + 10
			})
...
func TestRefundSubscriptionPreConsumeRejectsCorruptLedgerWithoutTransition(t *testing.T) {
	...
	record := model.SubscriptionPreConsumeRecord{
		...
		CreatedAt: wallclock.NowTimestamp(), UpdatedAt: wallclock.NowTimestamp(),
	}
...
func TestFundingSessionPreferenceDispatch(t *testing.T) {
	newUser := func(t *testing.T, name string, quota int, pref string) *model.User {
		...
		if pref != "" {
			_, err := userssvc.UpdateUserBillingPreference(u.Id, pref)
			require.NoError(t, err)
		}
		return u
	}
	...
	t.Run("wallet_only", func(t *testing.T) {
		initSubDB(t)
		u := newUser(t, "wonly", 100, userssvc.BillingPreferenceWalletOnly)
	...
	t.Run("subscription_only", func(t *testing.T) {
		initSubDB(t)
		u := newUser(t, "sonly", 100, userssvc.BillingPreferenceSubscriptionOnly)
	...
	t.Run("wallet_first falls back to subscription", func(t *testing.T) {
		initSubDB(t)
		u := newUser(t, "wfirst", 10, userssvc.BillingPreferenceWalletFirst)
		plan := seedRawPlan(t, nil)
		sub := seedSub(t, u.Id, plan.Id, 1000, 0, nil)
	...
	t.Run("zero reservation still reserves 1 from subscription", func(t *testing.T) {
		initSubDB(t)
		u := newUser(t, "zerores", 0, userssvc.BillingPreferenceSubscriptionOnly)
		plan := seedRawPlan(t, nil)
		sub := seedSub(t, u.Id, plan.Id, 100, 0, nil)
...
func TestFundingSessionSettleAndRefund(t *testing.T) {
	t.Run("subscription settle up and down", func(t *testing.T) {
		...
		_, err := userssvc.UpdateUserBillingPreference(u.Id, userssvc.BillingPreferenceSubscriptionOnly)
		require.NoError(t, err)
		plan := seedRawPlan(t, nil)
		sub := seedSub(t, u.Id, plan.Id, 1000, 0, nil)
		...
		assert.Equal(t, userssvc.BillingPreferenceSubscriptionOnly, fields["billing_preference"])
...
func TestCleanupSubscriptionPreConsumeRecords(t *testing.T) {
	...
	old := wallclock.NowTimestamp() - 8*24*3600
...
func TestUserSettingsMergePreservesFields(t *testing.T) {
	...
	pref, err := userssvc.UpdateUserBillingPreference(u.Id, userssvc.BillingPreferenceWalletFirst)
	require.NoError(t, err)
	assert.Equal(t, userssvc.BillingPreferenceWalletFirst, pref)
	require.NoError(t, userssvc.UpdateUserSetting(u.Id, 12345, "email"))
	assert.Equal(t, userssvc.BillingPreferenceWalletFirst, userssvc.GetUserBillingPreference(u.Id))

	var got model.User
	require.NoError(t, model.DB.First(&got, u.Id).Error)
	assert.Equal(t, 12345, userssvc.QuotaWarningThreshold(&got))

	// Updating the preference keeps the quota warning too.
	_, err = userssvc.UpdateUserBillingPreference(u.Id, userssvc.BillingPreferenceWalletOnly)
	require.NoError(t, err)
	require.NoError(t, model.DB.First(&got, u.Id).Error)
	assert.Equal(t, 12345, userssvc.QuotaWarningThreshold(&got))
	assert.Equal(t, userssvc.BillingPreferenceWalletOnly, userssvc.GetUserBillingPreference(u.Id))

	// Unknown values normalize to the default silently.
	pref, err = userssvc.UpdateUserBillingPreference(u.Id, "bogus")
	require.NoError(t, err)
	assert.Equal(t, userssvc.BillingPreferenceSubscriptionFirst, pref)
}

func TestNormalizeBillingPreference(t *testing.T) {
	assert.Equal(t, "subscription_first", userssvc.NormalizeBillingPreference(""))
	assert.Equal(t, "subscription_first", userssvc.NormalizeBillingPreference("nonsense"))
	assert.Equal(t, "wallet_first", userssvc.NormalizeBillingPreference(" wallet_first "))
	assert.Equal(t, "subscription_only", userssvc.NormalizeBillingPreference("subscription_only"))
	assert.Equal(t, "wallet_only", userssvc.NormalizeBillingPreference("wallet_only"))
}
