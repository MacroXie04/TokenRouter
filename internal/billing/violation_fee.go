...
func CalculateGrokViolationFeeQuota(policy setting.GrokSetting, groupRatio float64) (int, error) {
	...
	quotaDecimal := amount.
		...
		Round(0)
	quota, err := quotamath.QuotaFromDecimalStrict(quotaDecimal)
	if err != nil {
...
func validateGrokViolationFeeInput(input GrokViolationFeeInput) error {
	...
	if input.UseTime < 0 || int64(input.UseTime) > quotamath.MaxQuota {
...
func chargeGrokViolationFeeTx(
	...
	nextAttempts, attemptsOK := quotamath.AddQuotaWithinBounds(reservation.ViolationFeeAttempts, 1)
	if !attemptsOK {
		return ErrViolationFeeOperation
	}
...
func chargeViolationFeeSubscriptionTx(tx *gorm.DB, reservation *model.RelayQuotaReservationRecord, quota int, now int64) error {
	...
	newUsed, ok := quotamath.AddQuotaWithinBounds(used, quota)
	if !ok {
		return ErrSubscriptionQuotaOverflow
	}
...
func chargeViolationFeeUserTx(tx *gorm.DB, reservation *model.RelayQuotaReservationRecord, quota int) (string, error) {
	...
	newUsed, usedOK := quotamath.AddQuotaWithinBounds(user.UsedQuota, quota)
	newCount, countOK := quotamath.AddQuotaWithinBounds(user.RequestCount, 1)
	if !usedOK || !countOK {
...
func chargeViolationFeeTokenTx(tx *gorm.DB, reservation *model.RelayQuotaReservationRecord, quota int) (string, error) {
	...
	newUsed, ok := quotamath.AddQuotaWithinBounds(token.UsedQuota, quota)
	if !ok {
...
func validateStoredGrokViolationFeeIntent(
	...
	if record == nil || record.ViolationFeeCode != relaycommon.GrokViolationErrorCode ||
		...
		record.ViolationFeeAttempts < 0 || int64(record.ViolationFeeAttempts) > quotamath.MaxQuota {
		return ErrViolationFeeOperation
	}
...
func classifyGrokViolationFeeFailure(err error) string {
	...
	case errors.Is(err, userssvc.ErrUserNotFound), errors.Is(err, ErrTokenNotFound), errors.Is(err, gorm.ErrRecordNotFound):
...
func persistGrokViolationFeeReview(input GrokViolationFeeInput, plan grokViolationFeePlan, failureCode string) error {
	...
	return defaultViolationFeeTransactionRunner(func(tx *gorm.DB) error {
		...
		nextAttempts, ok := quotamath.AddQuotaWithinBounds(reservation.ViolationFeeAttempts, 1)
		if !ok {
			return ErrViolationFeeOperation
		}
		now, err := model.DatabaseUnixTimestamp(tx)
...
func grokViolationFeeIntentFromRecord(
	...
	if record == nil || record.ViolationFeeQuota <= 0 || !quotamath.QuotaWithinBounds(record.ViolationFeeQuota) {
	...
	if record.ViolationFeeAmount == "" || len(record.ViolationFeeAmount) > 64 ||
		...
		!quotamath.IsSafeDecimalLiteral(record.ViolationFeeAmount) ||
		!quotamath.IsSafeDecimalLiteral(record.ViolationFeeGroupRatio) {
		return GrokViolationFeeInput{}, grokViolationFeePlan{}, ErrViolationFeeOperation
	}
	...
	if amountFloatErr != nil || ratioFloatErr != nil || math.IsNaN(amountFloat) || math.IsInf(amountFloat, 0) ||
		...
		amountFloat > float64(quotamath.MaxQuota)/float64(quotamath.QuotaPerUnit) ||
		amountFloat*float64(quotamath.QuotaPerUnit) > float64(quotamath.MaxQuota)/ratioFloat {
		return GrokViolationFeeInput{}, grokViolationFeePlan{}, ErrViolationFeeOperation
	}
	...
	recomputed, err := quotamath.QuotaFromDecimalStrict(amount.
		Mul(decimal.NewFromInt(int64(quotamath.QuotaPerUnit))).Mul(ratio).Round(0))
	if err != nil || recomputed != record.ViolationFeeQuota {
		return GrokViolationFeeInput{}, grokViolationFeePlan{}, ErrViolationFeeOperation
	}
	expectedAuditID := "violation_fee_" + cryptoutil.SHA256Hex(record.ReservationID)[:48]
