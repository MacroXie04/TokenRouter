...
func TestReferenceUsageSettlementFailsClosedOnInvalidOrOverflowingUsage(t *testing.T) {
	...
	_, err = plan.SettlementUsageQuota(&protocolkit.Usage{
		PromptTokens:                int(quotamath.MaxQuota),
		PromptCacheCreation5mTokens: int(quotamath.MaxQuota),
	...
	result, err := overflow.SettlementUsageQuota(
		&protocolkit.Usage{PromptTokens: int(quotamath.MaxQuota), TotalTokens: int(quotamath.MaxQuota)},
