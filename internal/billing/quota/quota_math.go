...
func quotaFromFloat(f float64, outClamp **QuotaClamp) int64 {
	if math.IsNaN(f) {
		...
		logging.SysError("quota: NaN input in QuotaFromFloat")
		return 0
	}
	if math.IsInf(f, 1) {
		setQuotaClamp(outClamp, &QuotaClamp{Reason: "inf"})
		logging.SysError("quota: +Inf input in QuotaFromFloat")
		return MaxQuota
	}
	if math.IsInf(f, -1) {
		setQuotaClamp(outClamp, &QuotaClamp{Reason: "inf"})
	...
	if clamped != t {
		...
		logging.SysError("quota: overflow clamped in QuotaFromFloat")
	}
	return clamped
}
...
func quotaRound(f float64, outClamp **QuotaClamp) int64 {
	if math.IsNaN(f) {
		setQuotaClamp(outClamp, &QuotaClamp{Reason: "nan"})
		logging.SysError("quota: NaN input in QuotaRound")
		return 0
	}
	if math.IsInf(f, 1) {
		setQuotaClamp(outClamp, &QuotaClamp{Reason: "inf"})
		logging.SysError("quota: +Inf input in QuotaRound")
		return MaxQuota
	}
	if math.IsInf(f, -1) {
		setQuotaClamp(outClamp, &QuotaClamp{Reason: "inf"})
	...
	if clamped != r {
		setQuotaClamp(outClamp, &QuotaClamp{Reason: "overflow"})
		logging.SysError("quota: overflow clamped in QuotaRound")
	}
	return clamped
}
...
func quotaFromDecimal(d decimal.Decimal, outClamp **QuotaClamp) int64 {
	...
	if d.GreaterThanOrEqual(maxExclusive) {
		...
		logging.SysError("quota: decimal overflow clamped in QuotaFromDecimal")
		return MaxQuota
	}
	if d.LessThanOrEqual(minInclusive) {
		setQuotaClamp(outClamp, &QuotaClamp{Reason: "overflow", Value: d.String()})
		logging.SysError("quota: decimal underflow clamped in QuotaFromDecimal")
		return MinQuota
	}
	return d.IntPart()
}
