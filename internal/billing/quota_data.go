...
func LogQuotaData(userId int, username, modelName, group string, tokenId, channelId int,
	...
	if err := LogQuotaDataChecked(userId, username, modelName, group, tokenId, channelId, quota, tokenUsed, createdAt); err != nil {
		logging.SysError("cache quota data failed: " + err.Error())
	}
}
...
func LogQuotaDataChecked(userId int, username, modelName, group string, tokenId, channelId int,
	...
	if cached, ok := quotaDataCache[key]; ok {
		newCount, countOK := quotamath.AddQuotaWithinBounds(cached.Count, qd.Count)
		newQuota, quotaOK := quotamath.AddQuotaWithinBounds(cached.Quota, qd.Quota)
		newTokenUsed, tokenOK := quotamath.AddQuotaWithinBounds(cached.TokenUsed, qd.TokenUsed)
...
func SaveQuotaDataCache() {
	if err := SaveQuotaDataCacheChecked(); err != nil {
		logging.SysError("save quota data cache failed: " + err.Error())
	}
}
...
func SaveQuotaDataCacheChecked() error {
	...
	if flushed > 0 {
		logging.SysLog(fmt.Sprintf("保存数据看板数据成功，共保存%d条数据", flushed))
	}
	return errors.Join(flushErrors...)
}

func flushQuotaDataEntry(qd *model.QuotaData) error {
	if qd == nil || qd.Count <= 0 || int64(qd.Count) > quotamath.MaxQuota ||
		qd.Quota < 0 || int64(qd.Quota) > quotamath.MaxQuota ||
		qd.TokenUsed < 0 || int64(qd.TokenUsed) > quotamath.MaxQuota {
	...
	return model.DB.Transaction(func(tx *gorm.DB) error {
		...
		err := locking.SubscriptionLockForUpdate(tx.Table(model.QuotaData{}.TableName())).Where(where, args...).First(&existing).Error
		...
		if _, ok := quotamath.AddQuotaWithinBounds(existing.Count, qd.Count); !ok {
			return ErrQuotaDataOverflow
		}
		if _, ok := quotamath.AddQuotaWithinBounds(existing.Quota, qd.Quota); !ok {
			return ErrQuotaDataOverflow
		}
		if _, ok := quotamath.AddQuotaWithinBounds(existing.TokenUsed, qd.TokenUsed); !ok {
			return ErrQuotaDataOverflow
		}
		result := tx.Table(model.QuotaData{}.TableName()).Where(where, args...).
			Where("count >= 0 and count <= ? and quota >= 0 and quota <= ? and token_used >= 0 and token_used <= ?",
				quotamath.MaxQuota-int64(qd.Count), quotamath.MaxQuota-int64(qd.Quota), quotamath.MaxQuota-int64(qd.TokenUsed)).
