...
type PricingCatalogItem struct {
	...
	SupportedEndpointTypes []channelcatalog.EndpointType `json:"supported_endpoint_types"`
...
type PricingCatalog struct {
	...
	SupportedEndpoint map[string]channelcatalog.EndpointInfo
	AutoGroups        []string
	Version           string
}
...
func BuildPricingCatalog(userGroup string) (PricingCatalog, error) {
	...
	sort.Strings(groups)
	if len(groups) > channelssvc.EndpointCatalogMaxGroups {
		return PricingCatalog{}, channelssvc.ErrEndpointCatalogTooLarge
	}

	modelGroups := make(map[string]map[string]struct{})
	models := make(map[string]bool)
	for _, group := range groups {
		for modelName, enabled := range channelssvc.GetGroupModels(group) {
	...
	if len(models) > channelssvc.EndpointCatalogMaxModels {
		return PricingCatalog{}, channelssvc.ErrEndpointCatalogTooLarge
	}

	endpointTypes, err := channelssvc.GetModelSupportedEndpointTypes(groups, models)
	if err != nil {
	...
	for _, modelName := range modelNames {
		...
		item := PricingCatalogItem{
			...
			SupportedEndpointTypes: append([]channelcatalog.EndpointType(nil), endpointTypes[modelName]...),
		}
		if !populatePricingCatalogPrice(&item, billing) {
			continue
		}
		if item.SupportedEndpointTypes == nil {
			item.SupportedEndpointTypes = []channelcatalog.EndpointType{}
		}
		if meta != nil {
...
func parsePricingCatalogBillingExpressions(raw string) (map[string]string, error) {
	...
	if err := jsonutil.UnmarshalJsonStr(raw, &expressions); err != nil || expressions == nil {
...
func populatePricingCatalogPrice(item *PricingCatalogItem, billing pricingCatalogBilling) bool {
	...
	modelRatio := promptPrice * float64(quotamath.QuotaPerUnit) / 1_000_000
	if !validCatalogRatio(modelRatio) || !validCatalogRatio(completionRatio) {
		return false
	}
	item.QuotaType = 0
...
func referenceRatioUSDPerMillion(modelRatio float64) float64 {
	return modelRatio * 1_000_000 / float64(quotamath.QuotaPerUnit)
}

func pricingSupportedEndpoints(items []PricingCatalogItem) map[string]channelcatalog.EndpointInfo {
	result := make(map[string]channelcatalog.EndpointInfo)
	for _, item := range items {
		for _, endpointType := range item.SupportedEndpointTypes {
			info, ok := channelcatalog.GetDefaultEndpointInfo(endpointType)
			if ok {
...
func PricingCatalogMetadata(models map[string]bool) (map[string]*model.Model, error) {
	...
	if model.DB == nil {
		return nil, channelssvc.ErrEndpointCatalogUnavailable
	}
	...
	sort.Strings(names)
	for start := 0; start < len(names); start += channelssvc.EndpointCatalogBatchSize {
		end := min(start+channelssvc.EndpointCatalogBatchSize, len(names))
	...
	if err := model.DB.Where("name_rule <> ?", model.ModelNameRuleExact).
		Order("id ASC").Limit(channelssvc.EndpointCatalogMaxModels + 1).Find(&patterns).Error; err != nil {
		return nil, err
	}
	if len(patterns) > channelssvc.EndpointCatalogMaxModels {
		return nil, channelssvc.ErrEndpointCatalogTooLarge
	}
...
func pricingCatalogVendors(ids map[int]struct{}) ([]PricingCatalogVendor, error) {
	...
	for start := 0; start < len(ordered); start += channelssvc.EndpointCatalogBatchSize {
		end := min(start+channelssvc.EndpointCatalogBatchSize, len(ordered))
