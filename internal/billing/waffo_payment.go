...
func transitionBoundWaffoTopUp(settlement WaffoSettlement, paid bool) error {
	...
	err := model.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := locking.SubscriptionLockForUpdate(tx).Where("trade_no = ?", settlement.MerchantOrderID).First(&order).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
				return ErrTopUpNotFound
			}
			return err
	...
	if credited {
		if err := DeliverAuditLogOutboxEvent(auditEventID); err != nil {
			logging.SysError(fmt.Sprintf("Waffo audit delivery deferred trade_no=%s user_id=%d: %v", order.TradeNo, order.UserId, err))
		}
	}
	return nil
}
...
func validateWaffoTopUpSnapshot(order *model.TopUp) error {
	if order == nil || order.PaymentProvider != PaymentProviderWaffo || order.PaymentMethod != PaymentMethodWaffo ||
		...
		order.CreditQuotaVersion != WaffoTopUpCreditQuotaVersion || order.CreditQuota <= 0 || order.CreditQuota > quotamath.MaxQuota ||
...
func parseWaffoSnapshot(raw, fingerprint string) (WaffoCheckoutRequest, error) {
	...
	if err := jsonutil.Unmarshal([]byte(raw), &snapshot); err != nil {
		return WaffoCheckoutRequest{}, ErrWaffoCheckoutBindingMismatch
	}
	return snapshot, nil
}
