...
func InitializeSessionSecret() error {
	...
	if !configured || secret == "" {
		...
		logging.Logger.Warn("SESSION_SECRET is unset while DEBUG=true; using an ephemeral key that invalidates sessions and encrypted data on restart and is unsuitable for multiple nodes")
		return nil
	}
