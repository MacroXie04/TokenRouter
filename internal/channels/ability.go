...
func GetRandomSatisfiedChannel(group, modelName string, ignoreChannel map[int]struct{}, r *rand.Rand) (*model.Channel, error) {
	...
	for i, c := range pool {
		w := c.weight
		if w == 0 {
			w = channelcatalog.DefaultChannelWeight
		}
		weights[i] = w
	}
...
func collectCandidates(group, modelName string, ignore map[int]struct{}) []*selectableChannel {
	...
	if err := model.DB.Where("id IN ? AND status = ?", channelIDs, channelcatalog.ChannelStatusEnabled).Find(&channels).Error; err != nil {
		return nil
	}
...
func GetChannelName(id int) string {
	...
	return channelcatalog.ChannelTypeName(channelcatalog.ChannelType(ch.Type))
}
