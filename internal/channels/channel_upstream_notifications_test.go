...
func TestChannelUpstreamNotificationOptInBoundsAndSuppression(t *testing.T) {
	...
	users := []model.User{
		{Username: "watcher", Password: "password8", Email: "watcher@example.com", EmailVerified: true,
			Role: roles.RoleAdminUser, Status: model.UserStatusEnabled, Setting: `{"upstream_model_update_notify_enabled":true}`},
		{Username: "ordinary", Password: "password8", Email: "ordinary@example.com", EmailVerified: true,
			Role: roles.RoleCommonUser, Status: model.UserStatusEnabled, Setting: `{"upstream_model_update_notify_enabled":true}`},
		...
		{Username: "opted-out", Password: "password8", Email: "out@example.com", EmailVerified: true,
			Role: roles.RoleAdminUser, Status: model.UserStatusEnabled},
	}
	for i := range users {
...
func TestChannelUpstreamNotificationPreferenceIsAdminOnlyAndPreserved(t *testing.T) {
	...
	require.NoError(t, userssvc.UpdateUserSettingWithUpstreamNotification(user.Id, roles.RoleCommonUser, 100, "email", &enabled))
	settings, err := userssvc.LoadUserSettings(user.Id)
	require.NoError(t, err)
	...
	require.NoError(t, userssvc.UpdateUserSettingWithUpstreamNotification(user.Id, roles.RoleAdminUser, 200, "email", &enabled))
	settings, err = userssvc.LoadUserSettings(user.Id)
	require.NoError(t, err)
	assert.False(t, settings.UpstreamModelUpdateNotifyEnabled)
	require.NoError(t, model.DB.Model(&user).Updates(map[string]any{
		"role": roles.RoleAdminUser, "email": "failure@example.com", "email_verified": true,
	}).Error)
	require.NoError(t, userssvc.UpdateUserSettingWithUpstreamNotification(user.Id, roles.RoleAdminUser, 200, "email", &enabled))
	settings, err = userssvc.LoadUserSettings(user.Id)
	require.NoError(t, err)
...
func TestChannelUpstreamNotificationContentIsBoundedAndSanitized(t *testing.T) {
	...
	// Status notification delivery belongs to the channel domain; task completion
	// notification is exercised separately by the operations task lifecycle test.
	t.Run("root channel status delivery", func(t *testing.T) {
		setupChannelUpstreamUpdateTestDB(t)
		require.NoError(t, model.DB.AutoMigrate(&model.User{}))
		...
		require.NoError(t, notifyRootChannelStatus(7, "Primary", channelcatalog.ChannelStatusAutoDisabled, "余额不足"))
		assert.Equal(t, []string{"root@example.test"}, mailer.to)
		assert.Equal(t, []string{"通道「Primary」（#7）已被禁用"}, mailer.subject)
	})
}
...
func TestConcurrentUserSettingUpdatesPreserveIndependentFields(t *testing.T) {
	...
	user := model.User{Username: "concurrent-settings", Password: "password8", Status: model.UserStatusEnabled,
		Role: roles.RoleAdminUser, Setting: `{"future_setting":{"keep":true}}`}
	require.NoError(t, model.DB.Create(&user).Error)
	enabled := true
	...
	go func() {
		<-start
		_, err := userssvc.UpdateUserBillingPreference(user.Id, userssvc.BillingPreferenceWalletOnly)
		errors <- err
	}()
	go func() {
		<-start
		errors <- userssvc.UpdateUserSettingWithUpstreamNotification(user.Id, roles.RoleAdminUser, 321, "email", &enabled)
	}()
	close(start)
	...
	settings, err := userssvc.LoadUserSettings(user.Id)
	require.NoError(t, err)
	assert.Equal(t, userssvc.BillingPreferenceWalletOnly, settings.BillingPreference)
