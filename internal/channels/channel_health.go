...
func probeChannelHealthContext(ctx context.Context, channelId int, modelOverride string) channelHealthProbeResult {
	...
	if err != nil {
		return channelHealthProbeResult{DisplayReason: logging.RedactSensitiveText(err.Error()), LocalError: true}
	}
...
func buildChannelHealthRequest(ctx context.Context, channel *model.Channel, modelName string) (*http.Request, error) {
	...
	if !channelHealthSupported(channelType) {
		return nil, fmt.Errorf("%s channel test is not supported", channelcatalog.ChannelTypeName(channelType))
	}
	...
	if channelType == channelcatalog.ChannelTypeAws {
	...
	if base == "" && channel.Type >= 0 && channel.Type < len(channelcatalog.ChannelBaseURLs) {
		base = strings.TrimRight(channelcatalog.ChannelBaseURLs[channel.Type], "/")
	}
	if base == "" {
	...
	case channelcatalog.ChannelTypeZhipuV4:
		...
		requestURL, err := zhipu.V4RequestURL(base, channelcatalog.RelayModeChatCompletions, channelcatalog.RelayFormatOpenAI)
		if err != nil {
			return nil, err
		}
		payload = openAIHealthPayload(mappedModel)
	...
	case channelcatalog.ChannelTypeDify:
		...
		payload = map[string]any{
			...
			"user": cryptoutil.BestEffortUUID(), "auto_generate_name": false, "files": []any{},
		}
		req, err := newJSONHealthRequest(ctx, requestURL, payload)
		if err != nil {
			return nil, err
	...
	case channelcatalog.ChannelTypeMokaAI:
		meta := &relaycommon.Meta{
			Context: ctx, Channel: channel,
			Mode: channelcatalog.RelayModeEmbeddings, Format: channelcatalog.RelayFormatEmbedding,
	...
	default:
		...
		if channelType == channelcatalog.ChannelTypeOpenRouter {
...
func channelHealthSupported(channelType channelcatalog.ChannelType) bool {
	return channelType == channelcatalog.ChannelTypeAnthropic ||
		channelType == channelcatalog.ChannelTypeAws ||
		channelType == channelcatalog.ChannelTypeGemini ||
		channelType == channelcatalog.ChannelTypeCodex ||
		channelType == channelcatalog.ChannelTypeAli ||
		channelType == channelcatalog.ChannelTypeZhipu ||
		channelType == channelcatalog.ChannelTypeZhipuV4 ||
		channelType == channelcatalog.ChannelTypeMiniMax ||
		channelType == channelcatalog.ChannelTypeCohere ||
		channelType == channelcatalog.ChannelTypeCoze ||
		channelType == channelcatalog.ChannelTypeDify ||
		channelType == channelcatalog.ChannelTypeBaidu ||
		channelType == channelcatalog.ChannelTypeBaiduV2 ||
		channelType == channelcatalog.ChannelTypeMokaAI ||
		channelType == channelcatalog.ChannelTypePaLM ||
		channelType == channelcatalog.ChannelTypeSubmodel ||
		channelcatalog.IsOpenAICompatibleChannelType(channelType)
}
...
func buildAWSHealthRequest(ctx context.Context, channel *model.Channel, originalModel, mappedModel, credential string) (*http.Request, error) {
	...
	meta := &relaycommon.Meta{
		Context: ctx, Channel: channel, Mode: channelcatalog.RelayModeChatCompletions,
		Format: channelcatalog.RelayFormatOpenAI, OriginalModelName: originalModel, ModelName: mappedModel,
...
func mapChannelModel(channel *model.Channel, modelName string) string {
	...
	if err := jsonutil.UnmarshalJsonStr(channel.ModelMapping, &mapping); err != nil {
		return modelName
	}
...
func parseCodexOperationalCredential(raw string) (string, string, error) {
	...
	if err := jsonutil.UnmarshalJsonStr(strings.TrimSpace(raw), &key); err != nil {
...
func TestAndRecordChannelContextWithMode(ctx context.Context, ch *model.Channel, now int64, allowDisable bool) (bool, int, error) {
	...
	err := model.DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if statusChanged {
			if err := tx.Model(&model.Ability{}).Where("channel_id = ?", ch.Id).
				Update("enabled", newStatus == channelcatalog.ChannelStatusEnabled).Error; err != nil {
				return err
			}
		}
		return nil
	...
	if statusChanged {
		...
		if err := notifyRootChannelStatus(ch.Id, ch.Name, newStatus, transition.Reason); err != nil {
			logging.SysError(fmt.Sprintf("notify root about channel %d health status failed", ch.Id))
		}
	}
