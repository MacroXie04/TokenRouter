...
func createChannelForUpstreamUpdate(t *testing.T, serverURL, name, models, settings string) model.Channel {
	...
	channel := model.Channel{
		Type: int(channelcatalog.ChannelTypeOpenAI), Key: "secret-" + name,
		BaseURL: serverURL, Name: name, Models: models, Group: userssvc.GroupDefault,
...
func TestChannelUpstreamDetectionAndApplyAreAtomicAndPreserveUnknownSettings(t *testing.T) {
	...
	require.NoError(t, jsonutil.UnmarshalJsonStr(reloaded.OtherSettings, &rawSettings))
...
func TestApplyAllChannelUpstreamModelUpdatesScopesAndConcurrentReplay(t *testing.T) {
	...
	require.NoError(t, model.DB.Model(&model.Channel{}).Where("id = ?", disabled.Id).Update("status", channelcatalog.ChannelStatusManuallyDisabled).Error)
...
func TestChannelUpstreamModelInputBoundsAndCancellation(t *testing.T) {
	setupChannelUpstreamUpdateTestDB(t)
	err := persistChannelUpstreamCheckTime(context.Background(), nil, wallclock.NowTimestamp(), true)
