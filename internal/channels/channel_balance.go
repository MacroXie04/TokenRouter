...
func fetchChannelBalanceValue(ctx context.Context, ch *model.Channel, credential string) (float64, error) {
	...
	switch channelcatalog.ChannelType(ch.Type) {
	case channelcatalog.ChannelTypeOpenAI:
		return fetchOpenAIBalance(ctx, ch, credential)
	case channelcatalog.ChannelTypeCustom:
		if strings.TrimSpace(ch.BaseURL) == "" {
			return 0, errors.New("custom channel has no balance base URL")
		}
		return fetchOpenAIBalance(ctx, ch, credential)
	case channelcatalog.ChannelTypeAIProxy:
	...
	case channelcatalog.ChannelTypeAPI2GPT:
	...
	case channelcatalog.ChannelTypeAIGC2D:
...
func fetchOpenAIBalance(ctx context.Context, ch *model.Channel, credential string) (float64, error) {
	...
	if err := jsonutil.Unmarshal(subBody, &subscription); err != nil {
	...
	if err := jsonutil.Unmarshal(usageBody, &usage); err != nil {
