...
func ListRatioSyncChannels(ctx context.Context) ([]RatioSyncChannel, error) {
	...
	channels = append(channels,
		RatioSyncChannel{
			...
			BaseURL: ratioSyncOfficialPresetURL, Status: channelcatalog.ChannelStatusEnabled,
		},
		RatioSyncChannel{
			ID: ratioSyncModelsDevPresetID, Name: ratioSyncModelsDevPresetName,
			BaseURL: ratioSyncModelsDevPresetURL, Status: channelcatalog.ChannelStatusEnabled,
		},
	)
	return channels, nil
}
...
func effectiveRatioSyncBaseURL(channelType int, configured string) string {
	...
	if baseURL == "" && channelType >= 0 && channelType < len(channelcatalog.ChannelBaseURLs) {
		baseURL = channelcatalog.ChannelBaseURLs[channelType]
	}
...
func normalizeRatioSyncUpstream(ctx context.Context, upstream RatioSyncUpstream) (RatioSyncUpstream, error) {
	...
	if upstream.Endpoint == ratioSyncOpenRouterEndpoint && upstream.ID > 0 {
		...
		if channelcatalog.ChannelType(channel.Type) != channelcatalog.ChannelTypeOpenRouter {
			return RatioSyncUpstream{}, ErrRatioSyncInvalidRequest
		}
...
func fetchRatioSyncUpstream(parent context.Context, index int, upstream RatioSyncUpstream, timeout time.Duration) ratioSyncFetchResult {
	...
	if upstream.Endpoint == ratioSyncOpenRouterEndpoint {
		...
		if parsedTarget == nil || (parsedTarget.Scheme != "https" && !httpx.SSRFDisabled()) {
		...
		var channel model.Channel
		if err := model.DB.WithContext(ctx).First(&channel, upstream.ID).Error; err != nil ||
			channelcatalog.ChannelType(channel.Type) != channelcatalog.ChannelTypeOpenRouter {
...
func readRatioSyncResponse(response *http.Response) ([]byte, error) {
	...
	body, err := httpx.ReadAllLimited(response.Body, ratioSyncMaxResponseBytes)
	if err != nil {
...
func parseStandardRatioSync(body []byte) (map[string]any, error) {
	...
	if err := jsonutil.Unmarshal(body, &envelope); err != nil || !envelope.Success {
	...
	if err := jsonutil.Unmarshal(envelope.Data, &rawFields); err == nil {
	...
	if err := jsonutil.Unmarshal(envelope.Data, &items); err != nil {
		return nil, err
	}
...
func decodeRatioSyncNumericMap(raw []byte) (map[string]float64, error) {
	...
	if err := jsonutil.Unmarshal(raw, &encoded); err != nil || encoded == nil {
	...
	for modelName, valueRaw := range encoded {
		if jsonutil.GetJsonType(valueRaw) != "number" {
			return nil, errors.New("pricing value must be numeric")
		}
		var value float64
		if err := jsonutil.Unmarshal(valueRaw, &value); err != nil || !validRatioSyncNumber(value) {
			return nil, errors.New("invalid numeric pricing value")
		}
...
func decodeRatioSyncStringMap(raw []byte) (map[string]string, error) {
	...
	for modelName, valueRaw := range encoded {
		if jsonutil.GetJsonType(valueRaw) != "string" {
			return nil, errors.New("pricing value must be a string")
		}
		var value string
		if err := jsonutil.Unmarshal(valueRaw, &value); err != nil {
			return nil, errors.New("invalid string pricing value")
		}
		values[modelName] = value
	}
...
func pricingItemsToRatioSyncData(items []ratioSyncPricingItem) (map[string]any, error) {
	...
	for _, item := range items {
		...
		if item.BillingMode == billingsvc.BillingModeTieredExpr && strings.TrimSpace(item.BillingExpr) != "" {
			put("billing_mode", item.ModelName, billingsvc.BillingModeTieredExpr)
...
func parseOpenRouterRatioSync(body []byte) (map[string]any, error) {
	...
	if err := jsonutil.Unmarshal(body, &response); err != nil || len(response.Data) > ratioSyncMaxEntries {
	...
	for _, entry := range response.Data {
		...
		modelRatio := roundRatioSync(prompt * float64(quotamath.QuotaPerUnit))
...
func parseModelsDevRatioSync(body []byte) (map[string]any, error) {
	...
	if err := jsonutil.Unmarshal(body, &providers); err != nil || len(providers) == 0 {
	...
	for modelName, candidate := range candidates {
		...
		modelRatio := roundRatioSync(candidate.input * float64(quotamath.QuotaPerUnit) / 1_000_000)
...
func localRatioSyncData() map[string]any {
	...
	modelRatios := make(map[string]any)
	completionRatios := make(map[string]any)
	for modelName, price := range billingsvc.ExportedModelPrices() {
		if !validRatioSyncModelName(modelName) || !validRatioSyncNumber(price.Prompt) ||
			!validRatioSyncNumber(price.Completion) {
			continue
		}
		modelRatio := roundRatioSync(price.Prompt * float64(quotamath.QuotaPerUnit) / 1_000_000)
		if validRatioSyncNumber(modelRatio) {
			modelRatios[modelName] = modelRatio
		}
		if price.Prompt > 0 {
