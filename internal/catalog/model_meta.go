...
func ValidateModelMetadata(metadata *model.Model, statusOnly bool) error {
	...
	if metadata.Endpoints != "" {
		var decoded any
		if err := jsonutil.Unmarshal([]byte(metadata.Endpoints), &decoded); err != nil {
...
func EnrichModelMetadata(metadata []*model.Model) error {
	...
	for _, item := range metadata {
		...
		for _, candidate := range catalog {
			if billingsvc.ModelRuleMatches(item.NameRule, item.ModelName, candidate) {
	...
	priced := billingsvc.ExportedModelPrices()
	...
	for _, item := range metadata {
		...
		item.SupportedEndpointTypes = channelssvc.EndpointTypesForModelAndChannelTypes(item.ModelName, channelTypes)
	}
	return nil
}
...
func modelCatalogNames() ([]string, error) {
	...
	for name := range billingsvc.ExportedModelPrices() {
		set[name] = struct{}{}
	}
