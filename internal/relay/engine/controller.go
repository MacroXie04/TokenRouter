...
func relayAndSettleWithDispatch(c *gin.Context, info *RelayInfo, dispatch func(*gin.Context, *RelayInfo) (*protocolkit.Usage, error)) error {
	...
	if len(policy.Groups) == 0 && info.Group != "" && info.Group != userssvc.GroupAuto {
	...
	for attempt := 0; attempt <= retryTimes; attempt++ {
		...
		c.Set(requestctx.ContextKeyGroup, selectedGroup)
	...
	logErr := billingsvc.RecordConsumeLogChecked(
		userId,
		requestctx.GetUsername(c),
		requestctx.GetString(c, requestctx.ContextKeyTokenName),
		info.ModelName,
		...
		info.Group,
		c.ClientIP(),
		requestctx.GetRequestId(c),
		"",
		requestctx.GetInt(c, requestctx.ContextKeyTokenId),
