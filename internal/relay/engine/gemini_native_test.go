...
func setupChannelIntegration(t *testing.T, mockURL string, channelType channelcatalog.ChannelType, modelName string) string {
	t.Helper()
	...
	user := model.User{Username: "guser", Password: "x", Role: roles.RoleCommonUser, Status: 1, Group: "default", Quota: 500000, AuthVersion: 1}
	...
	w := uint(1)
	channel := model.Channel{Name: "mock", Type: int(channelType), Key: "sk-upstream", Status: channelcatalog.ChannelStatusEnabled, BaseURL: mockURL, Models: modelName, Group: "default", Weight: &w}
...
func TestGeminiNativeTieredMultimodalUsageSettlesExactAccounting(t *testing.T) {
	...
	key := setupChannelIntegration(t, mock.URL, channelcatalog.ChannelTypeGemini, modelName)
	previousPrices := billingsvc.ExportedModelPrices()
	previousRatios := billingsvc.ExportedGroupRatios()
	billingsvc.SetModelPriceRegistry(map[string]billingsvc.ModelPrice{modelName: {Prompt: 2, Completion: 2}})
...
func TestGeminiNativeAppliesVersionThinkingAndFunctionResponsePolicies(t *testing.T) {
	...
	key := setupChannelIntegration(t, mock.URL, channelcatalog.ChannelTypeGemini, clientModel)
	require.NoError(t, setting.UpdateOptions(map[string]string{
...
func TestGeminiNativeV1RoutePassthroughAndAccounting(t *testing.T) {
	...
	previousPrices := billingsvc.ExportedModelPrices()
	previousRatios := billingsvc.ExportedGroupRatios()
	billingsvc.SetModelPriceRegistry(map[string]billingsvc.ModelPrice{modelName: {Prompt: 1, Completion: 3}})
	billingsvc.SetGroupRatios(map[string]float64{"default": 1})
	t.Cleanup(func() {
		billingsvc.SetModelPriceRegistry(previousPrices)
		billingsvc.SetGroupRatios(previousRatios)
	...
	require.NoError(t, model.DB.Where("username = ?", "guser").First(&user).Error)
	var log model.Log
	require.NoError(t, model.LOG_DB.Where("user_id = ? AND type = ?", user.Id, billingsvc.LogTypeConsume).First(&log).Error)
...
func TestGeminiNativeViaOpenAIChannel(t *testing.T) {
	...
	key := setupChannelIntegration(t, mock.URL, channelcatalog.ChannelTypeOpenAI, modelName)
	r := router.SetUpRouter()
...
func TestRelayAlphaSearchPassthrough(t *testing.T) {
	...
	key := setupChannelIntegration(t, mock.URL, channelcatalog.ChannelTypeSub2API, "gpt-4")
	r := router.SetUpRouter()
...
func TestRelayAlphaSearchChannelGate(t *testing.T) {
	...
	key := setupChannelIntegration(t, mock.URL, channelcatalog.ChannelTypeOpenAI, "gpt-4")
	r := router.SetUpRouter()
