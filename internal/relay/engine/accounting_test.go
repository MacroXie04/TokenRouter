...
func TestXAIExactSafetyViolationSkipsRetryRefundsThenChargesAtomically(t *testing.T) {
	...
	require.NoError(t, model.DB.Model(&model.Channel{}).Where("id = ?", fixture.channel.Id).
		Update("type", int(channelcatalog.ChannelTypeXai)).Error)
	second := model.Channel{
		Name: "second-xai", Type: int(channelcatalog.ChannelTypeXai), Key: "upstream-2",
		Status: channelcatalog.ChannelStatusEnabled, BaseURL: "https://example.invalid", Models: "accounting-model",
...
func TestOrdinaryRelayReservationPreventsConcurrentTokenOverspend(t *testing.T) {
	...
	actual := billingsvc.ComputeQuota("accounting-model", "default", 1, 1)
...
func TestQuotaSaturationMarkerPersistsInConsumptionAuditLog(t *testing.T) {
	...
	info.QuotaClamp = &quotamath.QuotaClamp{Reason: "overflow", Value: "1e100"}

	require.NoError(t, billingsvc.RecordConsumeLogChecked(
...
func TestAutoGroupRelayUsesAuthorizedFallbackAndActualGroupBilling(t *testing.T) {
	const maxTokens = 64
	fixture := newRelayAccountingFixture(t, 100_000, 100_000)
	billingsvc.SetGroupRatios(map[string]float64{"default": 1, "staff": 3, "vip": 2})
	...
	require.NoError(t, channelssvc.InitAbilityCache())
	fixture.token.Group = userssvc.GroupAuto
	c, recorder, info := newRelayAccountingContext(t, &fixture.token, maxTokens)
	middleware.SetRelayGroupPolicy(c, billingsvc.RelayGroupPolicy{
	...
	actual := billingsvc.ComputeQuota("accounting-model", "vip", 4, 2)
	var gotUser model.User
	...
	assert.NotEqual(t, userssvc.GroupAuto, log.Group)
}
...
func TestOrdinaryRelayUsesUserGroupSpecialRatioForHoldSettlementAndLog(t *testing.T) {
	...
	actual := billingsvc.ComputeQuotaForUser("accounting-model", "default", "vip", 1_000, 100)
...
func TestAutoGroupRetryRecordsAffinityForActualGroup(t *testing.T) {
	...
	fixture := newRelayAccountingFixture(t, 100_000, 100_000)
	billingsvc.SetGroupRatios(map[string]float64{"staff": 1, "vip": 2})
	require.NoError(t, setting.Init())
	rules, err := jsonutil.Marshal([]setting.ChannelAffinityRule{{
		Name:              "actual-group",
	...
	vipChannel := model.Channel{
		Name: "vip-channel", Type: int(channelcatalog.ChannelTypeOpenAI), Key: "vip-upstream-key",
		Status: channelcatalog.ChannelStatusEnabled, BaseURL: "https://example.invalid",
	...
	preferred, found := channelssvc.GetPreferredChannelByAffinity(lookup, "accounting-model", "vip", nil)
...
func TestOrdinaryRelayLogSinkFailureFallsBackWithoutTurningSuccessIntoRetry(t *testing.T) {
	...
	actual := billingsvc.ComputeQuota("accounting-model", "default", 3, 2)
	var gotUser model.User
	var gotToken model.Token
	require.NoError(t, model.DB.First(&gotUser, fixture.user.Id).Error)
	require.NoError(t, model.DB.First(&gotToken, fixture.token.Id).Error)
...
func TestOrdinaryRelayRejectsOversizedUsageWithoutRefundingAcceptedWork(t *testing.T) {
	oversized := int(quotamath.MaxQuota + 1)
	tests := []struct {
		name  string
		usage *protocolkit.Usage
	}{
