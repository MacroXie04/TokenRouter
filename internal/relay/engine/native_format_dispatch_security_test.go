...
func TestGeminiNativeViaAzureUsesProviderContractAndMappedDeployment(t *testing.T) {
	...
	key := setupChannelIntegration(t, mock.URL, channelcatalog.ChannelTypeAzure, requestedModel)
...
func TestNativeFormatsRejectUnsupportedProviderBeforeCredentialedNetwork(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			channelType: channelcatalog.ChannelTypeVertexAi,
		...
		},
		{
			...
			channelType: channelcatalog.ChannelTypePerplexity,
			modelName:   "sonar-pro",
			request: func(key string) *http.Request {
		...
		},
		{
			...
			channelType: channelcatalog.ChannelTypeJina,
