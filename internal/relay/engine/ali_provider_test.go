...
func withAliPrices(t *testing.T, modelNames ...string) {
	...
	prices := make(map[string]billingsvc.ModelPrice, len(modelNames)+1)
	prices["gpt-provider-contract"] = billingsvc.ModelPrice{Prompt: 1, Completion: 2}
	for _, modelName := range modelNames {
		prices[modelName] = billingsvc.ModelPrice{Prompt: 1, Completion: 2}
	}
	billingsvc.SetModelPriceRegistry(prices)
...
func configureAliIntegration(t *testing.T, upstreamURL, clientModel, upstreamModel string) (string, int, model.Channel) {
	...
	require.NoError(t, model.DB.Model(&channel).Updates(map[string]any{
		"type": int(channelcatalog.ChannelTypeAli), "models": clientModel,
...
func TestAliClaudeNativeAndConvertedLifecycleEndToEnd(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			defer upstream.Close()
			key, userID := setupClaudeRelay(t, upstream.URL, int(channelcatalog.ChannelTypeAli), "gpt-provider-contract")
