...
func TestRealtimeReferencePricingUsesOneCapturedAudioSnapshotForEveryResponse(t *testing.T) {
	...
	assert.Equal(t, billingsvc.BillingModeReference, logged.other["billing_mode"])
...
func TestRealtimeReferenceZeroPriceCapturesExplicitFreeModelPolicy(t *testing.T) {
	...
	require.NoError(t, model.DB.Where("reservation_id = ?", reservation.(*billingsvc.RelayQuotaReservation).ReservationID()).First(&record).Error)
