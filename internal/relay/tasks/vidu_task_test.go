...
func configureViduLifecycleFixture(t *testing.T, fixed bool) relayAccountingFixture {
	...
	require.NoError(t, model.DB.Model(&model.Channel{}).Where("id = ?", fixture.channel.Id).
		Updates(map[string]any{"type": int(channelcatalog.ChannelTypeVidu), "key": "upstream-vidu-key",
	...
	for _, modelName := range vidu.ModelList {
		require.NoError(t, model.DB.Create(&model.Ability{Group: userssvc.GroupDefault, Model: modelName,
...
func TestViduAmbiguousDispatchIsHeldForExplicitReview(t *testing.T) {
	...
	assert.Equal(t, billingsvc.RelayQuotaReviewKindViduTask, review.ReviewKind)
...
func createPreparedViduTaskForRecovery(t *testing.T, fixture relayAccountingFixture) (*model.Task, *billingsvc.RelayQuotaReservation) {
	t.Helper()
	pricing, enabled, err := billingsvc.ResolveReferenceAsyncTaskBillingPlan("viduq1", "default", "default")
	require.NoError(t, err)
