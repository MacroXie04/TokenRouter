...
func persistJimengRecovery(envelope jimengRecoveryEnvelope) error {
	...
	data, err := jsonutil.Marshal(jimengRecoveryDiskEnvelope{
...
func loadJimengRecovery(taskID string) (jimengRecoveryEnvelope, error) {
	...
	versioned := jsonutil.Unmarshal(data, &disk) == nil && disk.Version != 0
	if !versioned {
...
func decodeJimengRecoveryDiskEnvelope(data []byte) (jimengRecoveryEnvelope, error) {
	var disk jimengRecoveryDiskEnvelope
	if err := jsonutil.Unmarshal(data, &disk); err == nil && disk.Version != 0 {
			...
			if promoteErr == nil {
				...
				if clockErr != nil {
					...
					if released.Error != nil {
						...
						promoteErr = billingsvc.ErrRelayQuotaReservationBusy
					}
				}
			}
		}
...
func cleanupJimengRecoveryRecords(now int64, limit int) error {
	...
	retentionDays := env.GetEnvInt("JIMENG_RECOVERY_RETENTION_DAYS", jimengRecoveryRetentionDays)
