...
const (
	viduOperationRetrySeconds    = int64(45)
	viduDispatchRecoverySeconds  = int64(120)
	viduReservationSafetySeconds = int64(8 * 24 * time.Hour / time.Second)
	viduUnknownDispatchReason    = "provider submission outcome requires manual review"
)

func createViduReservedTask(
	task *model.Task,
	token *model.Token,
	...
	if task == nil || privateData == nil || task.TaskID == "" || task.UserId <= 0 ||
		task.ChannelId <= 0 || task.Platform != viduTaskPlatform || task.Quota < 0 {
		return nil, errors.New("invalid Vidu task reservation")
	}
	if privateData.RelayReservationID != "" || privateData.EncryptedProviderTaskID != "" ||
		...
		return nil, errors.New("Vidu task reservation metadata is not pristine")
	}
	...
	return billingsvc.NewRelayQuotaReservationWithFreeModelAndPersistence(
		...
		func(tx *gorm.DB, creation billingsvc.RelayQuotaReservationCreation) error {
			...
			encoded, err := marshalViduTaskPrivateData(*privateData)
			if err != nil {
				return err
			}
			...
			minimumExpiry := now + viduReservationSafetySeconds
			...
			return tx.Create(&model.TaskOperation{
				...
				Platform: viduTaskPlatform, UserID: task.UserId, ChannelID: task.ChannelId,
				State: model.TaskOperationPrepared, NextAttemptAt: now + viduOperationRetrySeconds,
...
func markViduTaskDispatching(task *model.Task, reservation *billingsvc.RelayQuotaReservation) error {
	if task == nil || reservation == nil || task.ID <= 0 {
		return errors.New("invalid Vidu dispatch transition")
	}
	err := reservation.MarkDispatchedWithPersistence(func(tx *gorm.DB) error {
		...
		operation := tx.Model(&model.TaskOperation{}).
			Where("task_id = ? AND reservation_id = ? AND platform = ? AND state = ? AND lease_owner = ?",
				task.TaskID, reservation.ReservationID(), viduTaskPlatform, model.TaskOperationPrepared, "").
			Updates(map[string]any{
				"state": model.TaskOperationDispatching, "settlement_pending": true,
				"next_attempt_at": now + viduDispatchRecoverySeconds, "updated_at": now, "last_error": "",
			})
		...
		result := tx.Model(&model.Task{}).
			Where("id = ? AND task_id = ? AND platform = ? AND status = ?", task.ID, task.TaskID,
				viduTaskPlatform, model.TaskStatusNotStart).
	...
	if err == nil || viduOperationStateMatches(task.TaskID, reservation.ReservationID(), model.TaskOperationDispatching) {
		return nil
	}
	return err
}

func refundRejectedViduTask(task *model.Task, reservation *billingsvc.RelayQuotaReservation, reason string) error {
	if task == nil || reservation == nil {
		return errors.New("invalid rejected Vidu task refund")
	}
	reason = boundedViduFailReason(reason)
	err := reservation.RefundWithPersistence(func(tx *gorm.DB) error {
		var currentTask model.Task
		if err := tx.Where("id = ? AND task_id = ? AND platform = ?", task.ID, task.TaskID, viduTaskPlatform).
		...
		if err := tx.Where("task_id = ? AND reservation_id = ? AND platform = ?", task.TaskID,
			reservation.ReservationID(), viduTaskPlatform).First(&operation).Error; err != nil {
			return err
		}
		if !viduTaskOperationIdentityMatches(&currentTask, &operation) {
			return errors.New("rejected Vidu task identity mismatch")
		}
	...
	if err == nil || viduOperationStateMatches(task.TaskID, reservation.ReservationID(), model.TaskOperationRefunded) {
		return model.DB.Where("id = ?", task.ID).First(task).Error
	}
	return err
}

func settleAcceptedViduTask(
	task *model.Task,
	...
	if task == nil || reservation == nil || strings.TrimSpace(providerTaskID) == "" {
		return errors.New("invalid accepted Vidu task settlement")
	}
	...
	if err != nil || actualQuota < 0 {
		return errors.New("invalid Vidu settlement quota")
	}
	encryptedProviderID, err := asyncTaskEncryptBound(
		providerTaskID,
		viduProviderTaskBinding(task.TaskID, reservation.ReservationID(), task.UserId, task.ChannelId),
	)
	if err != nil {
		return err
	}
	...
	if _, err := validateViduTaskPricingSnapshot(task, privateData, actualQuota); err != nil {
		return err
	}
	...
	err = reservation.SettleWithChannelAndPersistence(actualQuota, task.ChannelId, func(tx *gorm.DB) error {
		...
		if !viduTaskOperationIdentityMatches(&currentTask, &operation) {
			return errors.New("accepted Vidu task identity mismatch")
		}
		currentPrivate, currentPrivateErr := decodeViduTaskPrivateData(currentTask.PrivateData)
		if currentPrivateErr != nil {
			return currentPrivateErr
		}
		if _, snapshotErr := validateViduTaskPricingSnapshot(&currentTask, currentPrivate, actualQuota); snapshotErr != nil {
			return snapshotErr
		}
		if operation.State == model.TaskOperationSubmitted && !operation.SettlementPending &&
			...
			stored, decryptErr := asyncTaskDecryptBound(operation.EncryptedProviderTaskID,
				viduProviderTaskBinding(task.TaskID, operation.ReservationID, operation.UserID, operation.ChannelID))
			if decryptErr != nil || stored != providerTaskID {
				return errors.Join(errors.New("Vidu provider task id conflicts with durable state"), decryptErr)
			}
			audit, auditErr := newViduConsumeAudit(currentTask, privateData,
		...
		result = tx.Model(&model.TaskOperation{}).
			...
			Updates(map[string]any{
				...
				"next_attempt_at":            now + viduOperationRetrySeconds, "updated_at": now,
		...
		audit, auditErr := newViduConsumeAudit(auditTask, privateData,
			reservation.ReservationID(), actualQuota)
		if auditErr != nil {
			return auditErr
		}
	...
	if err == nil || viduAcceptedSettlementMatches(task.TaskID, reservation.ReservationID(), providerTaskID) {
		return model.DB.Where("id = ?", task.ID).First(task).Error
	}
	return err
}

func persistAcceptedViduFallback(task *model.Task, reservationID, providerTaskID string) error {
	if task == nil || reservationID == "" || strings.TrimSpace(providerTaskID) == "" {
		return errors.New("invalid accepted Vidu fallback")
	}
	encryptedProviderID, err := asyncTaskEncryptBound(providerTaskID,
		viduProviderTaskBinding(task.TaskID, reservationID, task.UserId, task.ChannelId))
	if err != nil {
		return err
	}
	...
	err = model.DB.Transaction(func(tx *gorm.DB) error {
		...
		var operation model.TaskOperation
		if err := tx.Where("task_id = ? AND reservation_id = ? AND platform = ?", task.TaskID,
			reservationID, viduTaskPlatform).First(&operation).Error; err != nil {
			return err
		}
		if !viduTaskOperationIdentityMatches(&currentTask, &operation) || operation.LeaseOwner != "" ||
		...
		privateData, err := decodeViduTaskPrivateData(currentTask.PrivateData)
		if err != nil {
			return err
		}
		if _, err := validateViduTaskPricingSnapshot(&currentTask, privateData, currentTask.Quota); err != nil {
			return err
		}
		privateData.EncryptedProviderTaskID = encryptedProviderID
	...
	if err == nil || viduAcceptedFallbackMatches(task.TaskID, reservationID, providerTaskID) {
		return nil
	}
	return err
}

func markViduAmbiguousDispatch(task *model.Task, reservationID, reason string) error {
	return markViduAmbiguousDispatchWithLease(task, reservationID, reason, "")
}

// markViduAmbiguousDispatch fences a request that may have reached Vidu but
...
func markViduAmbiguousDispatchWithLease(task *model.Task, reservationID, reason, leaseOwner string) error {
	if task == nil || task.ID <= 0 || reservationID == "" {
		return errors.New("invalid unknown Vidu dispatch")
	}
	reason = boundedViduFailReason(reason)
	err := model.DB.Transaction(func(tx *gorm.DB) error {
		var currentTask model.Task
		...
		if !viduTaskOperationIdentityMatches(&currentTask, &operation) {
			return errors.New("unknown Vidu task identity mismatch")
		}
	...
	if err == nil || viduOperationStateMatches(task.TaskID, reservationID, model.TaskOperationManualReview) {
		return nil
	}
	return err
}

func reverseFailedViduTask(task *model.Task, operation *model.TaskOperation, reason string, data string) error {
	if task == nil || operation == nil || operation.ReservationID == "" {
		return errors.New("invalid failed Vidu task reversal")
	}
	reason = boundedViduFailReason(reason)
	return billingsvc.ReverseSettledRelayQuotaReservationWithPersistence(operation.ReservationID, func(tx *gorm.DB) error {
		...
		if err := tx.Where("id = ? AND reservation_id = ? AND platform = ?", operation.ID,
			operation.ReservationID, viduTaskPlatform).First(&currentOperation).Error; err != nil {
			return err
		}
		if !viduTaskOperationIdentityMatches(&currentTask, &currentOperation) {
			return errors.New("failed Vidu task identity mismatch")
		}
		if currentOperation.State == model.TaskOperationReversed && currentTask.Status == model.TaskStatusFailure && currentTask.Quota == 0 {
			audit, auditErr := newViduRefundAudit(tx, currentTask, operation.ReservationID,
		...
		audit, auditErr := newViduRefundAudit(tx, currentTask, operation.ReservationID, reason, now)
		if auditErr != nil {
			return auditErr
		}
		return billingsvc.EnqueueAuditLogTx(tx, audit)
...
func viduTaskOperationIdentityMatches(task *model.Task, operation *model.TaskOperation) bool {
	if !isViduTask(task) || operation == nil || operation.Platform != viduTaskPlatform ||
...
func viduOperationStateMatches(taskID, reservationID, state string) bool {
	var operation model.TaskOperation
	return model.DB.Where("task_id = ? AND reservation_id = ? AND platform = ? AND state = ?",
		taskID, reservationID, viduTaskPlatform, state).First(&operation).Error == nil
}

func viduAcceptedSettlementMatches(taskID, reservationID, providerID string) bool {
	var task model.Task
	if err := model.DB.Where("task_id = ? AND platform = ?", taskID, viduTaskPlatform).
	...
	stored, err := asyncTaskDecryptBound(operation.EncryptedProviderTaskID,
		viduProviderTaskBinding(taskID, reservationID, operation.UserID, operation.ChannelID))
	...
	if err := model.DB.Model(&model.AuditLogOutbox{}).
		Where("event_id = ?", viduAuditEventID(reservationID)).Count(&auditCount).Error; err != nil {
		return false
	}
	return auditCount == 1
}

func viduAcceptedFallbackMatches(taskID, reservationID, providerID string) bool {
	var task model.Task
	if err := model.DB.Where("task_id = ? AND platform = ?", taskID, viduTaskPlatform).
		First(&task).Error; err != nil || task.Status != model.TaskStatusSubmitted {
		return false
...
func isViduTerminalStatus(status string) bool {
	return status == model.TaskStatusSuccess || status == model.TaskStatusFailure || status == model.TaskStatusUnknown
}

func loadViduTaskOperation(taskID string) (*model.TaskOperation, error) {
	var operation model.TaskOperation
	if err := model.DB.Where("task_id = ? AND platform = ?", taskID, viduTaskPlatform).First(&operation).Error; err != nil {
		return nil, err
	}
	return &operation, nil
}

func ensureViduPrivateProviderCipher(task *model.Task, operation *model.TaskOperation) error {
	if task == nil || operation == nil || operation.EncryptedProviderTaskID == "" {
		return errors.New("Vidu provider task id is unavailable")
	}
	privateData, err := decodeViduTaskPrivateData(task.PrivateData)
	if err != nil {
		return err
	...
	if privateData.EncryptedProviderTaskID != operation.EncryptedProviderTaskID {
		return fmt.Errorf("Vidu provider task ciphertext mismatch")
	}
	return nil
}

func newViduConsumeAudit(task model.Task, privateData viduTaskPrivateData,
	reservationID string, quota int) (*model.Log, error) {
	properties, err := decodeViduTaskProperties(task.Properties)
	if err != nil || reservationID == "" || quota < 0 {
		return nil, errors.New("invalid Vidu consume audit")
	}
	eventID := viduAuditEventID(reservationID)
...
func newViduRefundAudit(tx *gorm.DB, task model.Task, reservationID, reason string,
	createdAt int64) (*model.Log, error) {
	if tx == nil || reservationID == "" || createdAt <= 0 {
		return nil, errors.New("invalid Vidu refund audit")
	}
	properties, err := decodeViduTaskProperties(task.Properties)
	if err != nil {
		return nil, err
	...
	eventID := "vidu-refund:" + reservationID
