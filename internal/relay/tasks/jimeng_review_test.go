...
func TestResolveJimengManualReviewSettleIsAtomicAuditedAndIdempotent(t *testing.T) {
	...
	result, err := ResolveJimengRelayQuotaReservationReview(
		fixture.reservation.ReservationID(), 7001, billingsvc.RelayQuotaReviewResolutionSettle,
	)
	require.NoError(t, err)
	...
	replay, err := ResolveJimengRelayQuotaReservationReview(
		record.ReservationID, 7002, billingsvc.RelayQuotaReviewResolutionSettle,
	)
	require.NoError(t, err)
	...
	_, err = ResolveJimengRelayQuotaReservationReview(
		record.ReservationID, 7002, billingsvc.RelayQuotaReviewResolutionRefund,
	)
...
func TestResolveJimengManualReviewRefundIsAtomicAuditedAndIdempotent(t *testing.T) {
	fixture := createJimengManualReviewFixture(t)
	result, err := ResolveJimengRelayQuotaReservationReview(
		fixture.reservation.ReservationID(), 7101, billingsvc.RelayQuotaReviewResolutionRefund,
	)
	require.NoError(t, err)
	assert.True(t, result.Changed)
	...
	assert.Equal(t, billingsvc.RelayQuotaReviewActionResolveRefund, reviewEvent.Action)
	...
	replay, err := ResolveJimengRelayQuotaReservationReview(
		record.ReservationID, 7102, billingsvc.RelayQuotaReviewResolutionRefund,
	)
	require.NoError(t, err)
	assert.False(t, replay.Changed)
	assert.Equal(t, result.AuditEventID, replay.AuditEventID)
	_, err = ResolveJimengRelayQuotaReservationReview(
		record.ReservationID, 7102, billingsvc.RelayQuotaReviewResolutionSettle,
	)
	assert.ErrorIs(t, err, billingsvc.ErrRelayQuotaReviewInvalidState)
...
func TestResolveJimengManualReviewRejectsDriftAndAuditFailureRollsBack(t *testing.T) {
	t.Run("immutable funding snapshot mismatch", func(t *testing.T) {
		...
		_, err = ResolveJimengRelayQuotaReservationReview(
			fixture.reservation.ReservationID(), 7201, billingsvc.RelayQuotaReviewResolutionSettle,
		)
	...
	t.Run("review audit append failure", func(t *testing.T) {
		...
		_, err := ResolveJimengRelayQuotaReservationReview(
			fixture.reservation.ReservationID(), 7202, billingsvc.RelayQuotaReviewResolutionSettle,
		)
		require.Error(t, err)
		assertJimengUnresolvedHold(t, fixture)
