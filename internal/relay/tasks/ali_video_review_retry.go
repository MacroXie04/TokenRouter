...
var aliWanTaskManualReviewRetryMu sync.Mutex

type AliWanTaskManualReviewRetryResult struct {
...
// RetryAliWanTaskManualReview reopens only authenticated, already-settled AliWan
// polling. It never changes accounting and cannot replay a submission.
func RetryAliWanTaskManualReview(reservationID string, operatorUserID int) (*AliWanTaskManualReviewRetryResult, error) {
	...
	aliWanTaskManualReviewRetryMu.Lock()
	defer aliWanTaskManualReviewRetryMu.Unlock()
	...
	err = model.DB.Transaction(func(tx *gorm.DB) error {
		...
		result := tx.Clauses(clause.Locking{Strength: "UPDATE"}).
			Where("reservation_id = ? AND platform = ?", reservationID, aliWanTaskPlatform).Limit(1).Find(&operation)
		if result.Error != nil {
			return result.Error
		}
		...
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).
			Where("task_id = ? AND platform = ?", operation.TaskID, aliWanTaskPlatform).Limit(2).Find(&tasks).Error; err != nil {
			return err
		}
		if len(tasks) != 1 {
			return fmt.Errorf("%w: AliWan task identity is unavailable or ambiguous", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
		}
		task := &tasks[0]
		...
		if record.Status != model.RelayQuotaReservationStatusSettled ||
			...
			return fmt.Errorf("%w: AliWan task is not awaiting a polling retry", billingsvc.ErrRelayQuotaReviewInvalidState)
		}
		if err := validateActiveAliWanTaskPollReview(&record, task, &operation); err != nil {
			return err
		}
		...
		taskResult := tx.Model(&model.Task{}).
			Where("id = ? AND task_id = ? AND platform = ? AND user_id = ? AND channel_id = ? AND quota = ?",
				task.ID, task.TaskID, aliWanTaskPlatform, task.UserId, task.ChannelId, task.Quota).
		...
		opResult := tx.Model(&model.TaskOperation{}).
			Where("id = ? AND task_id = ? AND reservation_id = ? AND platform = ?", operation.ID,
				operation.TaskID, operation.ReservationID, aliWanTaskPlatform).
	...
	result, err := verifyAliWanTaskManualReviewRetry(reservationID, eventID)
...
func validateActiveAliWanTaskPollReview(record *model.RelayQuotaReservationRecord, task *model.Task,
	...
	if record == nil || task == nil || operation == nil ||
		...
		task.Platform != aliWanTaskPlatform || task.TaskID != operation.TaskID || task.UserId != record.UserID ||
		task.Status != model.TaskStatusUnknown || task.Quota != record.ActualQuota || task.FinishTime <= 0 ||
		operation.Platform != aliWanTaskPlatform || operation.ReservationID != record.ReservationID ||
		...
		return fmt.Errorf("%w: AliWan polling review tuple is inconsistent", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || privateData.RelayReservationID != record.ReservationID || privateData.SettlementPending ||
		...
		return fmt.Errorf("%w: AliWan recovery metadata does not match accounting", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	properties, err := validateAliWanTaskPricingSnapshot(task, privateData, record.ActualQuota)
	if err != nil {
		return fmt.Errorf("%w: AliWan pricing snapshots do not match accounting", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || settled != record.ActualQuota {
		return fmt.Errorf("%w: AliWan pricing snapshot does not match accounting", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	baseURL, err := aliWan.EffectiveBaseURL(privateData.ChannelBaseURL)
	if err != nil || baseURL != privateData.ChannelBaseURL {
		return fmt.Errorf("%w: AliWan provider endpoint is invalid", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || strings.TrimSpace(key) == "" || len(key) > 8<<10 || strings.ContainsAny(key, "\r\n\x00") {
		return fmt.Errorf("%w: AliWan channel credential is not recoverable", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	binding := aliWanProviderTaskBinding(task.TaskID, operation.ReservationID, task.UserId, task.ChannelId)
	for _, encrypted := range []string{operation.EncryptedProviderTaskID, privateData.EncryptedProviderTaskID} {
		...
		if err != nil || !validAliWanRecoveryOpaqueID(providerID, aliWan.MaxProviderTaskIDBytes) {
			return fmt.Errorf("%w: AliWan provider id is not recoverable", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
		}
	}
	return nil
}

func verifyAliWanTaskManualReviewRetry(reservationID, eventID string) (*AliWanTaskManualReviewRetryResult, error) {
	var record model.RelayQuotaReservationRecord
	if err := model.DB.Where("reservation_id = ?", reservationID).First(&record).Error; err != nil {
		return nil, err
	}
	var operation model.TaskOperation
	if err := model.DB.Where("reservation_id = ? AND platform = ?", reservationID, aliWanTaskPlatform).
		First(&operation).Error; err != nil {
		return nil, err
	}
	var task model.Task
	if err := model.DB.Where("task_id = ? AND platform = ?", operation.TaskID, aliWanTaskPlatform).
	...
	return &AliWanTaskManualReviewRetryResult{ReservationID: reservationID, TaskID: task.TaskID,
