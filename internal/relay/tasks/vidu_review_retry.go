...
func RetryViduTaskManualReview(reservationID string, operatorUserID int) (*ViduTaskManualReviewRetryResult, error) {
	...
	err = model.DB.Transaction(func(tx *gorm.DB) error {
		...
		if len(tasks) != 1 {
			return fmt.Errorf("%w: Vidu task identity is unavailable or ambiguous", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
		}
		task := &tasks[0]
		...
		if record.Status != model.RelayQuotaReservationStatusSettled ||
			...
			return fmt.Errorf("%w: Vidu task is not awaiting a polling retry", billingsvc.ErrRelayQuotaReviewInvalidState)
		}
...
func validateActiveViduTaskPollReview(record *model.RelayQuotaReservationRecord, task *model.Task,
	...
	if record == nil || task == nil || operation == nil ||
		...
		return fmt.Errorf("%w: Vidu polling review tuple is inconsistent", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || privateData.RelayReservationID != record.ReservationID || privateData.SettlementPending ||
		...
		return fmt.Errorf("%w: Vidu recovery metadata does not match accounting", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	if _, err := validateViduTaskPricingSnapshot(task, privateData, record.ActualQuota); err != nil {
		return fmt.Errorf("%w: Vidu pricing snapshots do not match accounting", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	settled, err := privateData.Pricing.SettlementQuota(0)
	if err != nil || settled != record.ActualQuota {
		return fmt.Errorf("%w: Vidu pricing snapshot does not match accounting", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	baseURL, err := vidu.EffectiveBaseURL(privateData.ChannelBaseURL)
	if err != nil || baseURL != privateData.ChannelBaseURL {
		return fmt.Errorf("%w: Vidu provider endpoint is invalid", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || strings.TrimSpace(key) == "" || strings.ContainsAny(key, "\r\n") {
		return fmt.Errorf("%w: Vidu channel credential is not recoverable", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	for _, encrypted := range []string{operation.EncryptedProviderTaskID, privateData.EncryptedProviderTaskID} {
		...
		if err != nil || !validViduRecoveryOpaqueID(providerID, vidu.MaxProviderTaskIDBytes) {
			return fmt.Errorf("%w: Vidu provider id is not recoverable", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
		}
	}
	return nil
}
