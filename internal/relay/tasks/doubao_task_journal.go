...
func persistDoubaoRecoveryJournal(record doubaoRecoveryJournalRecord) error {
	...
	data, err := jsonutil.Marshal(doubaoRecoveryJournalDiskEnvelope{
...
func loadDoubaoRecoveryJournal(taskID string) (doubaoRecoveryJournalRecord, error) {
	...
	if err := jsonutil.Unmarshal(data, &disk); err != nil || disk.Version != doubaoRecoveryJournalDiskVersion ||
	...
	if err := jsonutil.Unmarshal([]byte(plaintext), &record); err != nil || record.TaskID != taskID ||
