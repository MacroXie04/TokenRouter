...
var jimengTaskPlatform = strconv.Itoa(int(channelcatalog.ChannelTypeJimeng))
...
func RelayJimengSubmit(c *gin.Context, state relaycommon.RequestState, request jimeng.Request, rawBody []byte) {
	...
	initialKey := channelssvc.GetChannelKey(selected)
	...
	effectiveGroupRatio, specialGroupRatio := billingsvc.EffectiveGroupRatio(userGroup, group)
	...
	if err != nil {
		...
		if billingsvc.IsSubscriptionFundingErr(err) {
			message = "订阅额度不足或未配置订阅: " + err.Error()
			code = "insufficient_quota"
			status = http.StatusBadRequest
		} else if errors.Is(err, billingsvc.ErrInsufficientQuota) || errors.Is(err, billingsvc.ErrInsufficientTokenQuota) {
			message = "用户或令牌额度不足"
			code = "insufficient_quota"
			status = http.StatusBadRequest
		} else {
			logging.SysError("create atomic Jimeng reservation: " + err.Error())
		}
	...
	defer func() {
		...
		if cleanupErr := refundJimengTaskReservation(&task, reservation, "submission aborted before provider acceptance", ""); cleanupErr != nil {
			logging.SysError("Jimeng atomic reservation cleanup failed for " + task.TaskID + ": " + cleanupErr.Error())
		}
	}()

	retryTimes := channelssvc.RetryTimes()
	preferredChannelID, affinityFound := channelssvc.GetPreferredChannelByAffinity(c, originModel, group, rawBody)
	...
	for attempt := 0; attempt <= retryTimes; attempt++ {
		...
		if attempt > 0 {
			var selectErr error
			channel, usedAffinity, selectErr = channelssvc.GetSatisfiedChannelWithPreferred(group, originModel, preferredChannelID, ignore, nil)
			if selectErr != nil {
				lastErr = selectErr
				break
			}
		...
		if channel.Type != int(channelcatalog.ChannelTypeJimeng) {
			lastErr = fmt.Errorf("channel %d does not support Jimeng tasks", channel.Id)
			ignore[channel.Id] = struct{}{}
			continue
		}
		if usedAffinity {
			channelssvc.MarkChannelAffinityUsed(c, group, channel.Id)
		}
	...
	if lastErr != nil || selected == nil || upstreamResult == nil {
		...
		if providerMayHaveAccepted && selected != nil {
			...
			if marshalErr != nil {
				...
				logging.SysError("persist ambiguous Jimeng task " + task.TaskID + ": " + marshalErr.Error())
				data := gin.H{
				...
				channelssvc.RecordChannelAffinity(c, initialChannelID, selected.Id)
			...
			logErr := billingsvc.DeliverAuditLogOutboxEvent(jimengAuditEventID(reservation.ReservationID()))
			if logErr != nil {
				logging.SysError("deliver ambiguous Jimeng consume audit: " + logErr.Error())
			}
			channelssvc.RecordChannelAffinity(c, initialChannelID, selected.Id)
		...
		if err := refundJimengTaskReservation(&task, reservation, message, ""); err != nil {
			reservationFinalized = true
			logging.SysError("persist rejected Jimeng task " + task.TaskID + ": " + err.Error())
			...
			if jimengSafeRefundTransitionMatches(task.TaskID, reservation.ReservationID()) {
				...
				if recoveryErr := persistJimengRecovery(jimengRecoveryEnvelope{
					...
					logging.SysError("persist secondary Jimeng rejection recovery for " + task.TaskID + ": " + recoveryErr.Error())
				} else {
	...
	if err != nil {
		...
		if fallbackErr != nil {
			logging.SysError("Jimeng accepted task metadata recovery write failed for " + task.TaskID + ": " + fallbackErr.Error())
			data["status"] = "unknown"
			...
			if recoveryErr := persistJimengRecovery(jimengRecoveryEnvelope{
				...
				logging.SysError("persist secondary Jimeng metadata recovery for " + task.TaskID + ": " + recoveryErr.Error())
			} else {
				data["node_local_recovery"] = true
			}
		}
	...
	if err := settleJimengAcceptedTask(
		...
		logging.SysError("commit accepted Jimeng task " + task.TaskID + ": " + err.Error())
		data := gin.H{
			"task_id": task.TaskID, "status": "accepted",
			"settlement_pending": true,
			"recovery_durable":   true,
		...
		if pendingErr != nil {
			logging.SysError("Jimeng accepted task database recovery write failed for " + task.TaskID + ": " + pendingErr.Error())
			fallbackErr := persistJimengAcceptedOperationFallback(
				&task, reservation.ReservationID(), upstreamResult.Data.TaskID,
			)
			if fallbackErr != nil {
				...
				if recoveryErr := persistJimengRecovery(jimengRecoveryEnvelope{
					...
					logging.SysError("persist secondary Jimeng recovery for " + task.TaskID + ": " + recoveryErr.Error())
				} else {
					data["node_local_recovery"] = true
				}
			}
	...
	logErr := billingsvc.DeliverAuditLogOutboxEvent(jimengAuditEventID(reservation.ReservationID()))
	if logErr != nil {
		logging.SysError("deliver accepted Jimeng consume audit: " + logErr.Error())
	}
	channelssvc.RecordChannelAffinity(c, initialChannelID, selected.Id)
...
func selectInitialJimengChannel(c *gin.Context, groups []string, modelName string, rawBody []byte) (*model.Channel, string, bool, bool, error) {
	for _, group := range groups {
		preferredChannelID, affinityFound := channelssvc.GetPreferredChannelByAffinity(c, modelName, group, rawBody)
		ignored := make(map[int]struct{})
		for {
			channel, usedAffinity, err := channelssvc.GetSatisfiedChannelWithPreferred(group, modelName, preferredChannelID, ignored, nil)
			if err != nil {
...
func commitJimengAcceptedTask(
	task *model.Task,
	funding *billingsvc.FundingSession,
	tokenId int,
	tokenUnlimited bool,
	quota, channelId int,
	privateJSON string,
...
func settlePendingJimengTaskWithFence(
	...
	if privateData.BillingSource == billingsvc.BillingSourceSubscription && expectedFundingReservation == 0 {
	...
	if privateData.RelayReservationID != "" {
		reservation, err := billingsvc.RestoreRelayQuotaReservation(privateData.RelayReservationID)
		if err != nil {
			return err
		}
	...
	funding, err := billingsvc.RestoreFundingSession(task.UserId, billingsvc.FundingReservation{
...
func marshalJimengRecoveryEnvelope(envelope jimengRecoveryEnvelope) ([]byte, error) {
	return jsonutil.Marshal(envelope)
}
...
func restoreLegacyJimengRecoveryToken(
	...
	plaintext, err := cryptoutil.DecryptByAES(recoveryToken)
	if err != nil {
		return jimengTaskPrivateData{}, errors.New("invalid Jimeng recovery token")
	}
	var envelope jimengRecoveryEnvelope
	if err := jsonutil.UnmarshalJsonStr(plaintext, &envelope); err != nil {
		return jimengTaskPrivateData{}, errors.New("invalid Jimeng recovery token")
	}
...
func resolveJimengTaskChannel(task model.Task, privateData jimengTaskPrivateData) (string, string, error) {
	...
	return jimengChannelBaseURL(&channel), channelssvc.GetChannelKey(&channel), nil
}
...
func RelayJimengFetch(c *gin.Context, state relaycommon.RequestState, request jimeng.Request) {
	...
	if privateData.RelayReservationID == "" && !privateData.SettlementPending && task.Status != model.TaskStatusNotStart {
		if err := removeJimengRecovery(task.TaskID); err != nil {
			logging.SysError("remove stale Jimeng recovery " + task.TaskID + ": " + err.Error())
	...
	if hadPendingSettlement {
		...
		billingsvc.CheckAndSendQuotaReminder(userID)
	}
	if privateData.RelayReservationID == "" && (recoveredFromJournal || hadPendingSettlement) {
		if err := removeJimengRecovery(task.TaskID); err != nil {
			logging.SysError("remove settled Jimeng recovery " + task.TaskID + ": " + err.Error())
			writeJimengTaskErrorData(c, http.StatusInternalServerError, "task_recovery_cleanup_failed",
	...
	if err := persistJimengTaskResultWithOperation(&task, previousStatus, modernOperation); err != nil {
		logging.SysError("persist Jimeng provider result for " + task.TaskID + ": " + err.Error())
...
func newJimengVideoResponse(task model.Task, modelName string) jimengVideoResponse {
	var properties jimengTaskProperties
	_ = jsonutil.UnmarshalJsonStr(task.Properties, &properties)
	var privateData jimengTaskPrivateData
	if isJimengTerminalTaskStatus(task.Status) {
...
func decodeJimengTaskMetadataChecked(task model.Task) (jimengTaskProperties, jimengTaskPrivateData, error) {
	var properties jimengTaskProperties
	if err := jsonutil.UnmarshalJsonStr(task.Properties, &properties); err != nil {
...
func decodeJimengTaskPrivateDataStored(value string) (jimengTaskPrivateData, error) {
	...
	if err := jsonutil.UnmarshalJsonStr(value, &privateData); err != nil {
		return jimengTaskPrivateData{}, err
	}
	return privateData, nil
}
...
func marshalJimengTaskProperties(properties jimengTaskProperties) (string, error) {
	value, err := jsonutil.Marshal(properties)
	if err != nil {
		return "", err
	}
...
func marshalJimengTaskPrivateData(privateData jimengTaskPrivateData) (string, error) {
	...
	value, err := jsonutil.Marshal(storedPrivateData)
	if err != nil {
		return "", err
	}
	encoded := string(value)
...
func durableJimengProviderPayload(raw []byte) []byte {
	...
	encoded, err := jsonutil.Marshal(canonical)
...
func jimengChannelBaseURL(channel *model.Channel) string {
	...
	index := int(channelcatalog.ChannelTypeJimeng)
	if index >= 0 && index < len(channelcatalog.ChannelBaseURLs) {
		return channelcatalog.ChannelBaseURLs[index]
	}
	return ""
}
