...
func RelaySunoTask(c *gin.Context) {
	...
	if prepared.Value.TaskID != "" {
		...
		if err != nil || channel == nil || channel.Status != channelcatalog.ChannelStatusEnabled ||
			channelcatalog.ChannelType(channel.Type) != channelcatalog.ChannelTypeSunoAPI {
	...
	if err != nil {
		...
		if billingsvc.IsSubscriptionFundingErr(err) || errors.Is(err, billingsvc.ErrInsufficientQuota) ||
			...
			logging.SysError("create atomic Suno reservation: " + err.Error())
		}
	...
	defer func() {
		...
		if cleanupErr := refundRejectedSunoTask(&task, reservation, "submission aborted before provider dispatch"); cleanupErr != nil {
			logging.SysError("Suno reservation cleanup failed for " + task.TaskID + ": " + cleanupErr.Error())
		}
	}()
	if err := markSunoTaskDispatching(&task, reservation); err != nil {
		logging.SysError("mark Suno dispatch: " + err.Error())
	...
	if submitErr != nil {
		if suno.IsDefinitiveRejection(submitErr) {
			...
			if journalErr := persistRejectedSunoRecoveryJournal(
				...
				logging.SysError("persist rejected Suno recovery journal " + task.TaskID + ": " + journalErr.Error())
			}
			if refundErr := refundRejectedSunoTask(&task, reservation, reason); refundErr != nil {
				logging.SysError("persist rejected Suno task " + task.TaskID + ": " + refundErr.Error())
			...
			if cleanupErr := removeSunoRecoveryJournal(task.TaskID); cleanupErr != nil {
				logging.SysError("remove rejected Suno recovery journal " + task.TaskID + ": " + cleanupErr.Error())
			}
			finalized = true
		...
		if !suno.SubmitWasDispatched(submitErr) {
			...
			if journalErr := persistRejectedSunoRecoveryJournal(
				...
				logging.SysError("persist undispatched Suno recovery journal " + task.TaskID + ": " + journalErr.Error())
			}
			if refundErr := refundRejectedSunoTask(&task, reservation, reason); refundErr != nil {
				logging.SysError("persist undispatched Suno task " + task.TaskID + ": " + refundErr.Error())
				writeSunoTaskError(c, http.StatusInternalServerError, "task_cleanup_failed", "Suno submission failed and quota cleanup is pending")
				return
			}
			if cleanupErr := removeSunoRecoveryJournal(task.TaskID); cleanupErr != nil {
				logging.SysError("remove undispatched Suno recovery journal " + task.TaskID + ": " + cleanupErr.Error())
			}
			finalized = true
		...
		if settleErr := settleUnknownSunoDispatch(&task, reservation, sunoUnknownDispatchReason, ""); settleErr != nil {
			logging.SysError("persist ambiguous Suno task " + task.TaskID + ": " + settleErr.Error())
	...
	if journalErr != nil {
		logging.SysError("persist accepted Suno recovery journal " + task.TaskID + ": " + journalErr.Error())
	}
	if err := settleAcceptedSunoTask(&task, reservation, providerTaskID, model.TaskOperationDispatching, ""); err != nil {
		logging.SysError("settle accepted Suno task " + task.TaskID + ": " + err.Error())
		fallbackErr := persistAcceptedSunoFallback(&task, reservation.ReservationID(), providerTaskID)
		if fallbackErr != nil {
			logging.SysError("persist accepted Suno fallback " + task.TaskID + ": " + fallbackErr.Error())
		} else if cleanupErr := removeSunoRecoveryJournal(task.TaskID); cleanupErr != nil {
			logging.SysError("remove accepted Suno recovery journal " + task.TaskID + ": " + cleanupErr.Error())
		}
		writeSunoTaskEnvelope(c, http.StatusAccepted, "task_commit_pending",
...
func selectSunoChannel(groups []string, modelName string) (*model.Channel, string, error) {
	...
	for attempt := 0; attempt < 64; attempt++ {
		...
		if channel != nil && channel.Id > 0 && channelcatalog.ChannelType(channel.Type) == channelcatalog.ChannelTypeSunoAPI {
...
func writeSunoTaskSuccess(c *gin.Context, data any) {
	payload, err := jsonutil.Marshal(gin.H{"code": "success", "message": "", "data": data})
