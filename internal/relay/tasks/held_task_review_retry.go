...
import (
	...
	klingprovider "github.com/tokenrouter/tokenrouter/internal/relay/providers/kling"
	doubaoprovider "github.com/tokenrouter/tokenrouter/internal/relay/providers/task/doubao"
...
func retryHeldAsyncTaskPollReview(
	...
	err = model.DB.Transaction(func(tx *gorm.DB) error {
		...
		if len(operations) != 1 {
			return fmt.Errorf("%w: %s operation identity is ambiguous", billingsvc.ErrRelayQuotaReviewUnsafeRetry, spec.name)
		}
		...
		if len(tasks) != 1 {
			return fmt.Errorf("%w: %s task identity is unavailable or ambiguous", billingsvc.ErrRelayQuotaReviewUnsafeRetry, spec.name)
		}
		task := &tasks[0]
		...
		if record.Status != model.RelayQuotaReservationStatusDispatched ||
			...
			return fmt.Errorf("%w: %s task is not awaiting a polling retry",
				billingsvc.ErrRelayQuotaReviewInvalidState, spec.name)
		}
		...
		if consumeAuditCount != 0 {
			return fmt.Errorf("%w: %s accounting audit already exists",
				billingsvc.ErrRelayQuotaReviewUnsafeRetry, spec.name)
		}
		...
		if taskResult.Error != nil || taskResult.RowsAffected != 1 {
			return errors.Join(taskResult.Error,
				fmt.Errorf("%w: %s task snapshot changed", billingsvc.ErrRelayQuotaReservationBusy, spec.name))
		}
		...
		if opResult.Error != nil || opResult.RowsAffected != 1 {
			return errors.Join(opResult.Error,
				fmt.Errorf("%w: %s operation snapshot changed", billingsvc.ErrRelayQuotaReservationBusy, spec.name))
		}
...
func validateActiveKlingTaskPollReview(record *model.RelayQuotaReservationRecord, task *model.Task,
	...
	if err := validateHeldTaskPollReviewTuple(record, task, operation, klingTaskPlatform,
		...
		return fmt.Errorf("%w: Kling %v", billingsvc.ErrRelayQuotaReviewUnsafeRetry, err)
	}
	if !klingTaskOperationIdentityMatches(task, operation, record.ReservationID) {
		return fmt.Errorf("%w: Kling task identity is inconsistent", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	properties, err := decodeKlingTaskProperties(task.Properties)
	if err != nil || task.Action != string(properties.Action) {
		return fmt.Errorf("%w: Kling task provenance is invalid", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || privateData.RelayReservationID != record.ReservationID || !privateData.SettlementPending ||
		...
		return fmt.Errorf("%w: Kling recovery metadata does not match accounting", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || preconsume != record.RequestedQuota || privateData.Pricing.ModelName != properties.OriginModelName {
		return fmt.Errorf("%w: Kling pricing snapshot does not match accounting", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || strings.TrimSpace(key) == "" || strings.ContainsAny(key, "\r\n\x00") {
		return fmt.Errorf("%w: Kling channel credential is not recoverable", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	baseURL, _, err := klingprovider.EffectiveBaseURL(privateData.ChannelBaseURL, key)
	if err != nil || baseURL != privateData.ChannelBaseURL {
		return fmt.Errorf("%w: Kling provider endpoint is invalid", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	for _, encrypted := range []string{operation.EncryptedProviderTaskID, privateData.EncryptedUpstreamTaskID} {
		...
		if err != nil || !validKlingRecoveryOpaqueID(providerID, klingprovider.MaxProviderTaskIDBytes) {
			return fmt.Errorf("%w: Kling provider id is not recoverable", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
		}
	}
	if _, err := decodeKlingStoredTaskData(task.Data); err != nil {
		return fmt.Errorf("%w: Kling provider state is invalid", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	return nil
}
...
func validateActiveDoubaoTaskPollReview(record *model.RelayQuotaReservationRecord, task *model.Task,
	...
	if !model.IsDoubaoVideoTaskOperationPlatform(operation.Platform) || operation.Platform != task.Platform {
		return fmt.Errorf("%w: Doubao platform is unsupported", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	if err := validateHeldTaskPollReviewTuple(record, task, operation, operation.Platform,
		doubaoPollingManualReviewReason); err != nil {
		return fmt.Errorf("%w: Doubao %v", billingsvc.ErrRelayQuotaReviewUnsafeRetry, err)
	}
	if !doubaoTaskOperationIdentityMatches(task, operation, record.ReservationID) {
		return fmt.Errorf("%w: Doubao task identity is inconsistent", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	properties, err := decodeDoubaoTaskProperties(task.Properties)
	if err != nil || task.Action != string(properties.Action) {
		return fmt.Errorf("%w: Doubao task provenance is invalid", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || privateData.RelayReservationID != record.ReservationID || !privateData.SettlementPending ||
		...
		privateData.BillingSource != record.FundingSource || privateData.SubscriptionID != record.SubscriptionID ||
		privateData.FundingUsageEpoch != record.UsageEpoch || privateData.TokenID != record.TokenID {
		return fmt.Errorf("%w: Doubao recovery metadata does not match accounting", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	preconsume, err := privateData.Pricing.PreConsumeQuota()
	if err != nil || preconsume != record.RequestedQuota || privateData.Pricing.ModelName != properties.OriginModelName {
		return fmt.Errorf("%w: Doubao pricing snapshot does not match accounting", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	baseURL, err := doubaoprovider.EffectiveBaseURL(privateData.ChannelBaseURL)
	if err != nil || baseURL != privateData.ChannelBaseURL {
		return fmt.Errorf("%w: Doubao provider endpoint is invalid", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	key, err := asyncTaskDecryptBound(privateData.EncryptedChannelKey,
		doubaoChannelCredentialBinding(task.TaskID, task.Platform, task.UserId, task.ChannelId, privateData.ChannelBaseURL))
	if err != nil || strings.TrimSpace(key) == "" || strings.ContainsAny(key, "\r\n\x00") {
		return fmt.Errorf("%w: Doubao channel credential is not recoverable", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	for _, encrypted := range []string{operation.EncryptedProviderTaskID, privateData.EncryptedUpstreamTaskID} {
		...
		if err != nil || !validDoubaoRecoveryOpaqueID(providerID, doubaoprovider.MaxProviderTaskIDBytes) {
			return fmt.Errorf("%w: Doubao provider id is not recoverable", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
		}
	}
	if _, err := decodeDoubaoStoredTaskData(task.Data); err != nil {
		return fmt.Errorf("%w: Doubao provider state is invalid", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	return nil
}
...
func verifyHeldAsyncTaskPollRetry(reservationID, eventID string,
	...
	if len(operations) != 1 {
		return nil, billingsvc.ErrRelayQuotaReviewUnsafeRetry
	}
	operation := &operations[0]
	var tasks []model.Task
