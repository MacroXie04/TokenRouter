...
type geminiVeoPricingSnapshot struct {
	Plan                 billingsvc.ReferenceAsyncTaskBillingPlan `json:"plan"`
	Duration             int                                      `json:"duration"`
	ResolutionMultiplier string                                   `json:"resolution_multiplier"`
}

func newGeminiVeoPricingSnapshot(plan billingsvc.ReferenceAsyncTaskBillingPlan, upstreamModel string,
...
func (pricing geminiVeoPricingSnapshot) quota(upstreamModel, resolution string) (int, error) {
	...
	return quotamath.QuotaFromDecimalStrict(decimal.NewFromInt(int64(base)).
