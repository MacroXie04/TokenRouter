...
func configureHailuoLifecycleFixture(t *testing.T, fixedPrice bool) relayAccountingFixture {
	...
	require.NoError(t, model.DB.Model(&model.Channel{}).Where("id = ?", fixture.channel.Id).
		Updates(map[string]any{
			"type": int(channelcatalog.ChannelTypeMiniMax), "key": "upstream-hailuo-key",
	...
	require.NoError(t, model.DB.Create(&model.Ability{Group: userssvc.GroupDefault, Model: hailuoTestModel,
...
func TestHailuoRecoveryPollUsesSnapshotsAndContentIsOwnerScoped(t *testing.T) {
	...
	assert.Equal(t, quotamath.QuotaPerUnit, task.Quota)
	assert.Equal(t, quotamath.QuotaPerUnit, reservation.ActualQuota)
...
func TestHailuoDefinitiveRejectionRefundsAndAmbiguityNeverReplays(t *testing.T) {
	...
	t.Run("ambiguous dispatch retained for explicit review", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, billingsvc.RelayQuotaReviewKindHailuoTask, review.ReviewKind)
...
func createPreparedHailuoTaskForRecovery(t *testing.T, fixture relayAccountingFixture) (*model.Task, *billingsvc.RelayQuotaReservation) {
	t.Helper()
	pricing, enabled, err := billingsvc.ResolveReferenceAsyncTaskBillingPlan(hailuoTestModel, "default", "default")
	require.NoError(t, err)
