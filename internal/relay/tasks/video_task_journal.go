...
func persistVideoRecoveryJournal(record videoRecoveryJournalRecord) error {
	...
	data, err := jsonutil.Marshal(videoRecoveryJournalDiskEnvelope{
...
func loadVideoRecoveryJournal(taskID string) (videoRecoveryJournalRecord, error) {
	...
	if err := jsonutil.Unmarshal(data, &disk); err != nil ||
	...
	if err := jsonutil.Unmarshal([]byte(plaintext), &record); err != nil {
