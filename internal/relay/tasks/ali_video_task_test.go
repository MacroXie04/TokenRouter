...
func configureAliWanLifecycleFixture(t *testing.T, fixedPrice bool) relayAccountingFixture {
	...
	require.NoError(t, model.DB.Model(&model.Channel{}).Where("id = ?", fixture.channel.Id).
		Updates(map[string]any{
			"type": int(channelcatalog.ChannelTypeAli), "key": "upstream-ali-wan-key",
	...
	require.NoError(t, model.DB.Create(&model.Ability{Group: userssvc.GroupDefault, Model: aliWanTestModel,
...
func TestAliWanRecoveryPollUsesSnapshotsAndContentIsOwnerScoped(t *testing.T) {
	...
	assert.Equal(t, quotamath.QuotaPerUnit/2, task.Quota)
	assert.Equal(t, quotamath.QuotaPerUnit/2, reservation.ActualQuota)
...
func TestAliWanDefinitiveRejectionRefundsAndAmbiguityNeverReplays(t *testing.T) {
	...
	t.Run("ambiguous dispatch retained for explicit review", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, billingsvc.RelayQuotaReviewKindAliWanTask, review.ReviewKind)
...
func createPreparedAliWanTaskForRecovery(t *testing.T, fixture relayAccountingFixture) (*model.Task, *billingsvc.RelayQuotaReservation) {
	t.Helper()
	pricing, enabled, err := billingsvc.ResolveReferenceAsyncTaskBillingPlan(aliWanTestModel, "default", "default")
	require.NoError(t, err)
