...
func RetryMidjourneyTaskManualReview(reservationID string, operatorUserID int) (*HeldAsyncTaskManualReviewRetryResult, error) {
	...
	err = model.DB.Transaction(func(tx *gorm.DB) error {
		...
		if len(operations) != 1 {
			return fmt.Errorf("%w: Midjourney operation identity is ambiguous", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
		}
		...
		if len(tasks) != 1 {
			return fmt.Errorf("%w: Midjourney task identity is unavailable or ambiguous", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
		}
		task := &tasks[0]
		...
		if len(mirrors) != 1 {
			return fmt.Errorf("%w: Midjourney mirror identity is unavailable or ambiguous", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
		}
		mirror := &mirrors[0]
		...
		if consumeAuditCount != 0 {
			return fmt.Errorf("%w: Midjourney accounting audit already exists", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
		}
		...
		if taskResult.Error != nil || taskResult.RowsAffected != 1 {
			return errors.Join(taskResult.Error,
				fmt.Errorf("%w: Midjourney task snapshot changed", billingsvc.ErrRelayQuotaReservationBusy))
		}
		...
		if mirrorResult.Error != nil || mirrorResult.RowsAffected != 1 {
			return errors.Join(mirrorResult.Error,
				fmt.Errorf("%w: Midjourney mirror snapshot changed", billingsvc.ErrRelayQuotaReservationBusy))
		}
		...
		if opResult.Error != nil || opResult.RowsAffected != 1 {
			return errors.Join(opResult.Error,
				fmt.Errorf("%w: Midjourney operation snapshot changed", billingsvc.ErrRelayQuotaReservationBusy))
		}
...
func validateActiveMidjourneyTaskPollReview(record *model.RelayQuotaReservationRecord, task *model.Task,
	...
	if err := validateHeldTaskPollReviewTuple(record, task, operation, midjourneyTaskPlatform,
		...
		return fmt.Errorf("%w: Midjourney %v", billingsvc.ErrRelayQuotaReviewUnsafeRetry, err)
	}
	...
	if err != nil || task.Action != string(properties.Action) || mirror.Action != task.Action {
		return fmt.Errorf("%w: Midjourney task provenance is invalid", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || baseURL != privateData.ChannelBaseURL {
		return fmt.Errorf("%w: Midjourney provider endpoint is invalid", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if _, err := decodeMidjourneyStoredTaskData(task.Data); err != nil {
		return fmt.Errorf("%w: Midjourney provider state is invalid", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	if _, err := midjourneyTaskDTOFromModel(mirror, task); err != nil {
		return fmt.Errorf("%w: Midjourney mirror state is invalid", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	return nil
}
