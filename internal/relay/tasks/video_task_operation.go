...
func settleAcceptedVideoTask(
	...
	response := sanitizedVideoResponse(
		task.TaskID, properties, provider, status, failReason, task.CreatedAt, wallclock.NowTimestamp(),
	)
	responseData, err := jsonutil.Marshal(response)
	...
	err = reservation.SettleWithChannelAndPersistence(task.Quota, task.ChannelId, func(tx *gorm.DB) error {
		...
		if currentOperation.State == model.TaskOperationUnknown {
			...
			if decodeErr != nil || currentTask.Status != model.TaskStatusUnknown ||
				...
				return errors.Join(billingsvc.ErrRelayQuotaReservationBusy, decodeErr)
			}
		...
		data, marshalErr := jsonutil.Marshal(response)
...
func persistAcceptedVideoFallback(
	...
	response := sanitizedVideoResponse(
		task.TaskID, properties, provider, status, "", task.CreatedAt, wallclock.NowTimestamp(),
	)
...
func reverseFailedVideoTask(
	...
	err = billingsvc.ReverseSettledRelayQuotaReservationWithPersistence(operation.ReservationID, func(tx *gorm.DB) error {
		var currentTask model.Task
		if err := tx.Where("id = ? AND task_id = ?", task.ID, task.TaskID).First(&currentTask).Error; err != nil {
			return err
		}
		...
		encoded, marshalErr := jsonutil.Marshal(response)
...
func newVideoConsumeAudit(
	...
	other, err := jsonutil.Marshal(fields)
...
func newVideoRefundAudit(
	...
	other, err := jsonutil.Marshal(map[string]any{
		"relay_reservation_id": reservationID,
		"task_id":              task.TaskID,
		"task_platform":        task.Platform,
		"reason":               boundedVideoFailReason(reason),
	...
	return &model.Log{
		...
		Type: billingsvc.LogTypeRefund, ModelName: properties.OriginModelName,
