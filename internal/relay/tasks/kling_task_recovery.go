...
func finishPolledKlingTask(
	...
	if err == nil {
		if provider.Status == kling.StatusSucceeded {
			if deliverErr := billingsvc.DeliverAuditLogOutboxEvent(klingAuditEventID(operation.ReservationID)); deliverErr != nil {
				logging.SysError("deliver recovered Kling consume audit: " + deliverErr.Error())
			}
		}
		return nil
	}
	...
	if fallbackErr := persistKlingProviderStatePending(task, operation.ReservationID, provider,
		...
		logging.SysError("Kling terminal accounting remains pending for " + task.TaskID + ": " + err.Error())
		return nil
	} else {
