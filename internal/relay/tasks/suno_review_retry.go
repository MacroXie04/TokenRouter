...
func RetrySunoTaskManualReview(reservationID string, operatorUserID int) (*SunoTaskManualReviewRetryResult, error) {
	...
	err = model.DB.Transaction(func(tx *gorm.DB) error {
		...
		if len(operations) != 1 {
			return fmt.Errorf("%w: Suno operation identity is ambiguous", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
		}
		...
		if len(tasks) != 1 {
			return fmt.Errorf("%w: Suno task identity is unavailable or ambiguous", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
		}
		task := &tasks[0]
		...
		if record.Status != model.RelayQuotaReservationStatusSettled ||
			...
			return fmt.Errorf("%w: Suno task is not awaiting a polling retry", billingsvc.ErrRelayQuotaReviewInvalidState)
		}
...
func validateActiveSunoTaskPollReview(record *model.RelayQuotaReservationRecord, task *model.Task,
	...
	if record == nil || task == nil || operation == nil ||
		...
		return fmt.Errorf("%w: Suno polling review tuple is inconsistent", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || privateData.RelayReservationID != record.ReservationID || privateData.SettlementPending ||
		...
		return fmt.Errorf("%w: Suno recovery metadata does not match accounting", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	properties, err := decodeSunoTaskProperties(task.Properties)
	if err != nil || task.Action != string(properties.Action) {
		return fmt.Errorf("%w: Suno task properties are invalid", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if preconsumeErr != nil || settleErr != nil || preconsume != record.RequestedQuota || settled != record.ActualQuota {
		return fmt.Errorf("%w: Suno pricing snapshot does not match accounting", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	baseURL, err := suno.ValidateBaseURL(privateData.ChannelBaseURL)
	if err != nil || baseURL != privateData.ChannelBaseURL {
		return fmt.Errorf("%w: Suno provider endpoint is invalid", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || strings.TrimSpace(key) == "" || strings.ContainsAny(key, "\r\n\x00") {
		return fmt.Errorf("%w: Suno channel credential is not recoverable", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	for _, encrypted := range []string{operation.EncryptedProviderTaskID, privateData.EncryptedProviderTaskID} {
		...
		if err != nil || !validSunoRecoveryOpaqueID(providerID, suno.MaxTaskIDBytes) {
			return fmt.Errorf("%w: Suno provider id is not recoverable", billingsvc.ErrRelayQuotaReviewUnsafeRetry)
		}
	}
	return nil
}
