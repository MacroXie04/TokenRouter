...
func settlePendingHailuoOperation(task *model.Task, reservation *billingsvc.RelayQuotaReservation, operation *model.TaskOperation) error {
	if operation.EncryptedProviderTaskID == "" {
	...
	if err != nil {
		return markHailuoOperationManualReview(operation, "Hailuo provider id cannot be decrypted", wallclock.NowTimestamp())
	}
...
func refundPreparedHailuoTaskWithLease(task *model.Task, reservation *billingsvc.RelayQuotaReservation,
