...
func xunfeiMeta(model string, stream bool) *relaycommon.Meta {
	...
	return &relaycommon.Meta{
		Context: context.Background(), Channel: nil, Mode: channelcatalog.RelayModeChatCompletions,
		Format: channelcatalog.RelayFormatOpenAI, OriginalModelName: model, ModelName: model,
...
func TestStrictRequestValidationAndBounds(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"wrong format", func(meta *relaycommon.Meta) { meta.Format = channelcatalog.RelayFormatClaude }},
...
func TestProductionDialerIsBoundedDirectAndSSRFSafe(t *testing.T) {
	...
	assert.Equal(t, reflect.ValueOf(httpx.SafeDialContext).Pointer(), reflect.ValueOf(dialer.NetDialContext).Pointer())
