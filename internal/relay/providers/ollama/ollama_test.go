...
func ollamaMeta(mode channelcatalog.RelayMode) *relaycommon.Meta {
	return &relaycommon.Meta{
		Channel: &model.Channel{Type: int(channelcatalog.ChannelTypeOllama)},
...
func TestOllamaRequestURLsHeadersAndModeGate(t *testing.T) {
	for _, test := range []struct {
		...
		{channelcatalog.RelayModeChatCompletions, "/prefix/api/chat"},
		{channelcatalog.RelayModeCompletions, "/prefix/api/generate"},
		{channelcatalog.RelayModeEmbeddings, "/prefix/api/embed"},
	} {
		t.Run(channelcatalog.RelayModeName(test.mode), func(t *testing.T) {
	...
	meta := ollamaMeta(channelcatalog.RelayModeImagesGenerations)
	adaptor := &Adaptor{}
	adaptor.Init(meta)
	...
	for _, invalid := range []string{
		...
		meta := ollamaMeta(channelcatalog.RelayModeChatCompletions)
...
func TestOllamaGenerateAndEmbeddingRequestConversion(t *testing.T) {
	maximum := 32
	meta := ollamaMeta(channelcatalog.RelayModeCompletions)
	meta.Request = &protocolkit.GeneralOpenAIRequest{
	...
	meta = ollamaMeta(channelcatalog.RelayModeEmbeddings)
...
func TestOllamaRequestConversionRejectsMalformedProviderFields(t *testing.T) {
	for _, test := range []struct {
		...
		{
			name: "remote image", mode: channelcatalog.RelayModeChatCompletions,
			request: &protocolkit.GeneralOpenAIRequest{
				Messages: []protocolkit.Message{{
					Role: "user",
		...
		{
			name: "invalid tool arguments", mode: channelcatalog.RelayModeChatCompletions,
			request: &protocolkit.GeneralOpenAIRequest{
				Messages: []protocolkit.Message{{
					Role: "assistant",
		...
		{name: "fractional top k", mode: channelcatalog.RelayModeChatCompletions, request: &protocolkit.GeneralOpenAIRequest{Extra: map[string]any{"top_k": 1.5}}},
		{name: "bad reasoning", mode: channelcatalog.RelayModeChatCompletions, request: &protocolkit.GeneralOpenAIRequest{ReasoningEffort: "extreme", Extra: map[string]any{}}},
		{name: "numeric embedding", mode: channelcatalog.RelayModeEmbeddings, request: &protocolkit.GeneralOpenAIRequest{Extra: map[string]any{"input": []any{"ok", float64(1)}}}},
	} {
...
func TestOllamaResponsesFailClosedAtBoundsAndMalformedUsage(t *testing.T) {
	tests := []struct {
		...
		{name: "malformed JSON", mode: channelcatalog.RelayModeChatCompletions, body: strings.NewReader(`{"done":`), want: "decode Ollama response"},
		{name: "missing terminal", mode: channelcatalog.RelayModeChatCompletions, body: strings.NewReader(`{"message":{"content":"x"},"done":false}`), want: "missing a terminal"},
		{name: "negative usage", mode: channelcatalog.RelayModeChatCompletions, body: strings.NewReader(`{"done":true,"prompt_eval_count":-1,"eval_count":2}`), want: "invalid usage"},
		{name: "usage overflow", mode: channelcatalog.RelayModeChatCompletions, body: strings.NewReader(`{"done":true,"prompt_eval_count":` + strconv.Itoa(math.MaxInt) + `,"eval_count":1}`), want: "invalid usage"},
		{name: "data after terminal", mode: channelcatalog.RelayModeChatCompletions, body: strings.NewReader("{\"done\":true}\n{\"response\":\"late\",\"done\":false}"), want: "after its terminal"},
		{name: "oversized buffered", mode: channelcatalog.RelayModeChatCompletions, body: io.LimitReader(zeroReader{}, relaycommon.MaxUpstreamJSONBodyBytes+1), want: "exceeds limit"},
		{name: "malformed first stream", mode: channelcatalog.RelayModeChatCompletions, stream: true, body: strings.NewReader("not-json\n"), want: "decode Ollama stream"},
		{name: "negative first stream usage", mode: channelcatalog.RelayModeChatCompletions, stream: true, body: strings.NewReader(`{"done":true,"prompt_eval_count":-1,"eval_count":2}`), want: "invalid usage"},
		{name: "oversized stream line", mode: channelcatalog.RelayModeChatCompletions, stream: true, body: io.LimitReader(zeroReader{}, int64(relaycommon.MaxUpstreamSSEEventBytes+2)), want: "maximum line"},
		{name: "empty embedding response", mode: channelcatalog.RelayModeEmbeddings, body: strings.NewReader(`{"embeddings":[]}`), want: "no vectors"},
		{name: "ragged embedding response", mode: channelcatalog.RelayModeEmbeddings, body: strings.NewReader(`{"embeddings":[[0.1],[0.2,0.3]]}`), want: "inconsistent vector dimensions"},
	}
