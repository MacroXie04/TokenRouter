...
func TestAdaptorPreservesGatewayPathsAndDynamicModelCatalog(t *testing.T) {
	tests := []struct {
		...
		{channelcatalog.RelayModeChatCompletions, "/v1/chat/completions"},
		{channelcatalog.RelayModeEmbeddings, "/v1/embeddings"},
		{channelcatalog.RelayModeImagesGenerations, "/v1/images/generations"},
		{channelcatalog.RelayModeResponses, "/v1/responses"},
		{channelcatalog.RelayModeResponsesCompact, "/v1/responses/compact"},
		{channelcatalog.RelayModeAlphaSearch, "/v1/alpha/search"},
	}
	for _, test := range tests {
		t.Run(test.path, func(t *testing.T) {
			meta := &relaycommon.Meta{
				Channel: &model.Channel{Type: int(channelcatalog.ChannelTypeNewAPI)},
...
func TestAdaptorRejectsMismatchedAndUnsupportedGatewayPaths(t *testing.T) {
	for _, test := range []struct {
		...
		{channelcatalog.RelayModeResponses, "/v1/chat/completions"},
		{channelcatalog.RelayModeRerank, "/v1/rerank"},
		{channelcatalog.RelayModeAudioSpeech, "/v1/audio/speech"},
	} {
...
func TestGatewayHeadersRequireCredentialAndPreserveNativeAuth(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Claude defaults version", format: channelcatalog.RelayFormatClaude,
		...
		{
			name: "Claude preserves bounded version", format: channelcatalog.RelayFormatClaude,
		...
		{
			name: "Gemini dual auth", format: channelcatalog.RelayFormatGemini,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			meta := &relaycommon.Meta{
				Channel: &model.Channel{Type: int(channelcatalog.ChannelTypeNewAPI)},
				Mode:    channelcatalog.RelayModeChatCompletions, Format: test.format, APIKey: "gateway-key", ClientHeaders: test.client,
			}
			adaptor := &Adaptor{}
			adaptor.Init(meta)
	...
	meta := &relaycommon.Meta{Channel: &model.Channel{Type: int(channelcatalog.ChannelTypeNewAPI)}, Mode: channelcatalog.RelayModeChatCompletions}
	adaptor := &Adaptor{}
	adaptor.Init(meta)
