...
func (a *Adaptor) ConvertRequest(meta *relaycommon.Meta) ([]byte, error) {
	...
	if mode == channelcatalog.RelayModeImagesGenerations {
	...
	if mode == channelcatalog.RelayModeImagesEdits && !isMultipart(meta.RequestContentType) {
		return nil, errors.New("Replicate image edits require multipart/form-data")
	}
	if mode == channelcatalog.RelayModeImagesEdits && len(meta.RawBody) > maxEditRequestBytes {
	...
	if mode == channelcatalog.RelayModeImagesEdits {
...
func (a *Adaptor) validate(meta *relaycommon.Meta) error {
	...
	if format != channelcatalog.RelayFormatOpenAIImage {
...
func validatePredictionInput(input map[string]any, mode channelcatalog.RelayMode) error {
	...
	if rawImage, exists := input["image_prompt"]; exists {
		...
		if mode == channelcatalog.RelayModeImagesEdits && strings.TrimSpace(imageURL) == "" {
