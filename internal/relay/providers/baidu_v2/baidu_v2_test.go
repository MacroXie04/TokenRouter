...
func v2Meta() *relaycommon.Meta {
	return &relaycommon.Meta{
		Channel:   &model.Channel{Type: int(channelcatalog.ChannelTypeBaiduV2)},
...
func TestBaiduV2URLHeadersCatalogAndFailClosedModes(t *testing.T) {
	...
	for _, mode := range []channelcatalog.RelayMode{
		channelcatalog.RelayModeEmbeddings,
		channelcatalog.RelayModeImagesGenerations,
		channelcatalog.RelayModeImagesEdits,
		channelcatalog.RelayModeRerank,
		channelcatalog.RelayModeAudioSpeech,
	} {
		unsupported := v2Meta()
		unsupported.Mode = mode
		unsupported.Format = relaycommon.GetRelayFormat(channelcatalog.ChannelTypeBaiduV2, mode)
