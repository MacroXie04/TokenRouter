...
func openAIMetaFromJSON(t *testing.T, raw string, mappedModel string) *relaycommon.Meta {
	...
	request.Extra = extra
	meta := testMeta(request.Model, channelcatalog.RelayFormatOpenAI, request.Stream)
...
func TestConvertNativeClaudeStrictWire(t *testing.T) {
	meta := testMeta(testClaudeModel, channelcatalog.RelayFormatClaude, false)
	meta.RawBody = []byte(`{
...
func TestConvertClaudePassThroughPreservesFutureFieldsButProtectsEnvelope(t *testing.T) {
	meta := testMeta(testClaudeModel, channelcatalog.RelayFormatClaude, true)
...
func TestSettingsAndFormatValidation(t *testing.T) {
	meta := testMeta(testClaudeModel, channelcatalog.RelayFormatOpenAI, false)
	tests := []struct {
		name   string
		mutate func()
	}{
		{"unsupported mode", func() { meta.Mode = channelcatalog.RelayModeEmbeddings }},
		{"unsupported format", func() {
			meta.Mode = channelcatalog.RelayModeChatCompletions
			meta.Format = channelcatalog.RelayFormatGemini
		}},
		{"duplicate key setting", func() {
			meta.Format = channelcatalog.RelayFormatOpenAI
