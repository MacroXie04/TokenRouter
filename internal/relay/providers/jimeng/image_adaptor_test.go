...
func jimengImageMeta() *relaycommon.Meta {
	...
	return &relaycommon.Meta{
		Channel: &model.Channel{Type: int(channelcatalog.ChannelTypeJimeng)},
		Mode:    channelcatalog.RelayModeImagesGenerations, Format: channelcatalog.RelayFormatOpenAIImage,
...
func TestJimengImageConversionValidatesProviderOptionsAndBounds(t *testing.T) {
	tests := []struct {
		...
		{"wrong mode", func(meta *relaycommon.Meta) { meta.Mode = channelcatalog.RelayModeChatCompletions }},
		{"wrong format", func(meta *relaycommon.Meta) { meta.Format = channelcatalog.RelayFormatOpenAI }},
