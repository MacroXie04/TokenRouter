...
func cohereTestMeta(mode channelcatalog.RelayMode) *relaycommon.Meta {
	return &relaycommon.Meta{
		Channel:   &model.Channel{Type: int(channelcatalog.ChannelTypeCohere)},
		Mode:      mode,
...
func TestCohereURLsHeadersModesAndModelCatalog(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "chat default", mode: channelcatalog.RelayModeChatCompletions, expected: "https://api.cohere.ai/v1/chat", wantAccept: "application/json"},
		{name: "chat stream custom path", mode: channelcatalog.RelayModeChatCompletions, stream: true, base: "https://cohere.example/gateway/", expected: "https://cohere.example/gateway/v1/chat", wantAccept: "text/event-stream"},
		{name: "rerank", mode: channelcatalog.RelayModeRerank, base: "https://cohere.example", expected: "https://cohere.example/v1/rerank", wantAccept: "application/json"},
	} {
	...
	unsupported := cohereTestMeta(channelcatalog.RelayModeEmbeddings)
	adaptor := &Adaptor{}
	...
	rerankStream := cohereTestMeta(channelcatalog.RelayModeRerank)
	...
	badBase := cohereTestMeta(channelcatalog.RelayModeChatCompletions)
	...
	require.NoError(t, err)
	badKey := cohereTestMeta(channelcatalog.RelayModeChatCompletions)
	badKey.APIKey = "\r\n"
...
func TestCohereChatRequestConversion(t *testing.T) {
	...
	meta := cohereTestMeta(channelcatalog.RelayModeChatCompletions)
...
func TestCohereRerankRequestConversionAndBounds(t *testing.T) {
	meta := cohereTestMeta(channelcatalog.RelayModeRerank)
	...
	chat := cohereTestMeta(channelcatalog.RelayModeChatCompletions)
