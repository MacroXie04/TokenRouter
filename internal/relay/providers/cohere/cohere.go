...
func (a *Adaptor) relayMode(meta *relaycommon.Meta) channelcatalog.RelayMode {
	if meta != nil && meta.Mode != channelcatalog.RelayModeUnknown {
		return meta.Mode
	}
	return a.mode
}
...
func cohereMaxTokens(request *protocolkit.GeneralOpenAIRequest) (int, error) {
	...
	if value < 0 || int64(value) > quotamath.MaxQuota/2 {
...
func configuredSafetyMode() (string, error) {
	value := strings.ToUpper(strings.TrimSpace(env.GetEnv("COHERE_SAFETY_SETTING", "NONE")))
	switch value {
	case "", "NONE":
		return "", nil
...
func convertRerankRequest(meta *relaycommon.Meta) (*rerankRequest, error) {
	...
	if int64(topN) > quotamath.MaxQuota {
...
func estimateRerankPromptTokens(request *protocolkit.GeneralOpenAIRequest) (int, error) {
	...
	if tokens < 0 || int64(tokens) > quotamath.MaxQuota {
...
func chatStreamResponse(c *gin.Context, resp *http.Response, meta *relaycommon.Meta) (*protocolkit.Usage, error) {
	...
	responseID := cohereStreamResponseIDPrefix + cryptoutil.BestEffortRandomAlphanumeric(24)
	created := time.Now().Unix()
	var usage *protocolkit.Usage
...
func usageFromBilledUnits(units billedUnits) (*protocolkit.Usage, error) {
	...
	if units.InputTokens < 0 || units.OutputTokens < 0 ||
		int64(units.InputTokens) > quotamath.MaxQuota || int64(units.OutputTokens) > quotamath.MaxQuota ||
		total < 0 || total > quotamath.MaxQuota {
		usage.TotalTokens = -1
...
func fallbackUsage(promptTokens int, contentBytes int64) (*protocolkit.Usage, error) {
	...
	if promptTokens < 0 || int64(promptTokens) > quotamath.MaxQuota || contentBytes < 0 ||
		contentBytes > quotamath.MaxQuota*4-3 {
		usage.TotalTokens = -1
...
func validateMode(mode channelcatalog.RelayMode, stream bool) error {
	switch mode {
	case channelcatalog.RelayModeChatCompletions:
		return nil
	case channelcatalog.RelayModeRerank:
