...
func (a *Adaptor) Init(meta *relaycommon.Meta) {
	if meta.Channel != nil {
		a.ChannelType = channelcatalog.ChannelType(meta.Channel.Type)
	}
...
func (a *Adaptor) GetRequestURL(meta *relaycommon.Meta) (string, error) {
	...
	base := meta.BaseURL
	if channelType == channelcatalog.ChannelTypeCustom {
	...
	if channelType == channelcatalog.ChannelTypeDeepSeek {
		switch a.Mode {
	...
	if channelType == channelcatalog.ChannelTypePerplexity && a.Mode == channelcatalog.RelayModeChatCompletions {
	...
	case channelcatalog.RelayModeEdits:
	...
	case channelcatalog.RelayModeAudioTranscription:
		return joinProviderURL(base, "/v1/audio/transcriptions"), nil
	case channelcatalog.RelayModeAudioTranslation:
...
func (a *Adaptor) SetupRequestHeader(req *http.Request, meta *relaycommon.Meta) error {
	...
	if a.providerType(meta) == channelcatalog.ChannelTypeAzure {
	...
	channelType := a.providerType(meta)
	if channelType == channelcatalog.ChannelTypeOpenAI && meta.Channel != nil && meta.Channel.OpenAIOrganization != "" {
...
func azureRequestURL(base string, meta *relaycommon.Meta, mode channelcatalog.RelayMode) (string, error) {
	...
	if mode == channelcatalog.RelayModeRealtime {
	...
	if mode == channelcatalog.RelayModeResponses || mode == channelcatalog.RelayModeResponsesCompact {
		...
		if mode == channelcatalog.RelayModeResponsesCompact {
			path += "/compact"
		}
...
func azureAPIVersion(meta *relaycommon.Meta) string {
	...
	return env.GetEnv("AZURE_DEFAULT_API_VERSION", "2025-04-01-preview")
}
...
func azureTaskPath(mode channelcatalog.RelayMode) (string, error) {
	switch mode {
	case channelcatalog.RelayModeChatCompletions:
...
func (a *Adaptor) ConvertRequest(meta *relaycommon.Meta) ([]byte, error) {
	...
	includeTypedFields := a.Mode == channelcatalog.RelayModeChatCompletions || a.Mode == channelcatalog.RelayModeCompletions
...
func (a *Adaptor) nonStreamResponse(c *gin.Context, resp *http.Response, meta *relaycommon.Meta) (*protocolkit.Usage, error) {
	...
	if a.Mode == channelcatalog.RelayModeAudioSpeech {
	...
	} else if a.Mode == channelcatalog.RelayModeRerank && channelType == channelcatalog.ChannelTypeSiliconFlow {
	...
	if usage == nil || usage.PromptTokens == 0 && usage.CompletionTokens == 0 {
		...
		case channelcatalog.RelayModeAudioTranscription, channelcatalog.RelayModeAudioTranslation:
...
func (a *Adaptor) bufferedResponseLimit() int64 {
	switch a.Mode {
	case channelcatalog.RelayModeEmbeddings, channelcatalog.RelayModeImagesGenerations, channelcatalog.RelayModeImagesEdits:
...
func (a *Adaptor) streamResponse(c *gin.Context, resp *http.Response, meta *relaycommon.Meta) (*protocolkit.Usage, error) {
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(line, "data:") {
			...
			if data == "[DONE]" {
				if hooks := meta.ToolHooks(); a.Mode == channelcatalog.RelayModeResponses && hooks != nil && hooks.FinishResponses != nil {
