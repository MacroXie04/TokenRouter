...
import (
	...
	geminitask "github.com/tokenrouter/tokenrouter/internal/relay/providers/task/gemini"
	vertexcore "github.com/tokenrouter/tokenrouter/internal/relay/providers/vertex"
...
func (client *Client) requestAdapter(config Config, modelName string) (*vertexcore.Adaptor, *relaycommon.Meta, error) {
	...
	meta := &relaycommon.Meta{
		Context: context.Background(), Channel: &model.Channel{
			Type: int(channelcatalog.ChannelTypeVertexAi), Other: config.Snapshot.Region, OtherSettings: config.OtherSettings,
		},
		Mode: channelcatalog.RelayModeChatCompletions, OriginalModelName: modelName,
