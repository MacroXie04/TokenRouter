...
func TestLoginFailsClosedWhenTwoFAStatusCannotBeRead(t *testing.T) {
	...
	model.DB = db
	model.LOG_DB = db
	hash, err := cryptoutil.PasswordHash("correct-password")
	require.NoError(t, err)
	require.NoError(t, db.Create(&model.User{
		Username: "factor-db-failure", Password: hash, Role: roles.RoleCommonUser,
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			r.GET(test.path, func(c *gin.Context) {
				requestctx.SetUserId(c, 1)
				c.Set("dashboard_session_claims", &cryptoutil.JWTClaims{
