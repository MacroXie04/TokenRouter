...
func setupTwoFAMutationErrorTest(t *testing.T) (*model.User, string) {
	...
	model.LOG_DB = db
	user := &model.User{
		Username: "twofa-error-user", Password: "hash", Role: roles.RoleCommonUser,
...
func twoFAMutationContext(user *model.User, code string) (*gin.Context, *httptest.ResponseRecorder) {
	...
	requestctx.SetUserId(context, user.Id)
	context.Set("dashboard_session_claims", &cryptoutil.JWTClaims{
