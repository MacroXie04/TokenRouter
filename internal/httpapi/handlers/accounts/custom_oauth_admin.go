...
func CreateCustomOAuthProvider(c *gin.Context) {
	...
	if authsvc.IsBuiltInOAuthProvider(req.Slug) {
...
func DeleteCustomOAuthProvider(c *gin.Context) {
	...
	if err != nil {
		logging.SysError("failed to get binding count for provider " + strconv.Itoa(id) + ": " + err.Error())
...
func FetchCustomOAuthDiscovery(c *gin.Context) {
	...
	targetURL, err := authsvc.BuildOAuthDiscoveryURL(wellKnownURL, issuerURL)
	if err != nil {
	...
	body, err := httpx.ReadAllLimited(resp.Body, maxCustomOAuthDiscoveryBodyBytes)
	if err != nil {
...
func GetUserOAuthBindingsByAdmin(c *gin.Context) {
	...
	target, err := userssvc.GetUserByID(userId)
	if err != nil {
	...
	bindings, err := authsvc.ListOAuthBindings(userId)
	if err != nil {
...
func UnbindCustomOAuthByAdmin(c *gin.Context) {
	...
	billingsvc.RecordSystemLog(requestctx.GetUserId(c), billingsvc.LogTypeManage,
		"user.oauth_unbind id="+textutil.Int2Str(userId)+" provider="+textutil.Int2Str(providerId))
