...
func performanceErrorMessage(err error, fallback string) string {
	switch {
	case errors.Is(err, operationssvc.ErrPerformanceInvalidConfig):
		return "invalid performance configuration"
	case errors.Is(err, operationssvc.ErrPerformanceUnsafePath):
		return "unsafe directory configuration"
	case errors.Is(err, operationssvc.ErrPerformanceDirectoryLimit):
...
func performanceError(c *gin.Context, err error, fallback string) {
	logging.LogError("performance management operation failed", "operation", c.FullPath(), "error_type", fmt.Sprintf("%T", err))
...
func GetPerformanceStats(c *gin.Context) {
	stats, err := operationssvc.GetPerformanceStats()
	if err != nil {
...
func ClearDiskCache(c *gin.Context) {
	...
	deleted, err := operationssvc.CleanupInactiveDiskCache(10 * time.Minute)
	performanceControllerMutationMu.Unlock()
	if err != nil {
		performanceError(c, err, "unable to clear inactive disk cache")
		return
	}
	logging.LogInfo("performance management operation completed",
		"action", "performance.clear_disk_cache", "operator_id", requestctx.GetUserId(c), "deleted_count", deleted)
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": "不活跃的磁盘缓存已清理",
	})
...
func ResetPerformanceStats(c *gin.Context) {
	performanceControllerMutationMu.Lock()
	operationssvc.ResetPerformanceStats()
	performanceControllerMutationMu.Unlock()
	logging.LogInfo("performance management operation completed",
		"action", "performance.reset_stats", "operator_id", requestctx.GetUserId(c))
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": "统计信息已重置",
	})
...
func ForceGC(c *gin.Context) {
	...
	performanceControllerMutationMu.Unlock()
	logging.LogInfo("performance management operation completed",
		"action", "performance.gc", "operator_id", requestctx.GetUserId(c))
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": "GC 已执行",
	})
...
func GetLogFiles(c *gin.Context) {
	files, err := operationssvc.GetPerformanceLogFiles()
	if err != nil {
...
func CleanupLogFiles(c *gin.Context) {
	...
	performanceControllerMutationMu.Lock()
	result, err := operationssvc.CleanupPerformanceLogFiles(mode, value)
	performanceControllerMutationMu.Unlock()
	if err != nil {
		if errors.Is(err, operationssvc.ErrPerformanceLogNotConfigured) {
	...
	logging.LogInfo("performance management operation completed",
		"action", "performance.clear_logs", "operator_id", requestctx.GetUserId(c),
