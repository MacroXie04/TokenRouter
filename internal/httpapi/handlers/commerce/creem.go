...
func RequestCreemPay(c *gin.Context) {
	...
	if req.PaymentMethod != billingsvc.PaymentMethodCreem {
	...
	suffix, err := cryptoutil.SecureRandomAlphanumeric(4)
	if err != nil {
	...
	order, checkoutRequest, err := billingsvc.CreateBoundCreemTopUpWithTradeNo(userID, billingsvc.CreemProductEconomics{
	...
	if err := billingsvc.BindCreemTopUpCheckout(order.TradeNo, checkout.ID); err != nil {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "拉起支付失败"})
		return
	}
...
func SubscriptionRequestCreemPay(c *gin.Context) {
	...
	userID := requestctx.GetUserId(c)
	if user, userErr := userssvc.GetUserByID(userID); userErr != nil || user == nil {
	...
	order, checkoutRequest, err := billingsvc.CreateBoundCreemSubscriptionOrder(userID, plan.Id, tradeNo)
	if err != nil {
	...
	if err := billingsvc.BindCreemSubscriptionCheckout(order.TradeNo, checkout.ID); err != nil {
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "拉起支付失败"})
		return
	}
	c.JSON(http.StatusOK, gin.H{
...
func CreemWebhook(c *gin.Context) {
	config, err := setting.GetCreemConfigChecked()
	if err != nil || !billingsvc.PaymentComplianceConfirmed() || config.APIKey == "" ||
		config.WebhookSecret == "" || len(config.Products) == 0 {
		c.AbortWithStatus(http.StatusForbidden)
		return
	}
	body, err := httpx.ReadAllLimited(c.Request.Body, creemWebhookMaxBodyBytes)
	if err != nil {
	...
	if jsonutil.Unmarshal(body, &event) != nil || len(event.EventType) > creemWebhookMaxEventType || len(event.ID) > creemWebhookMaxString {
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}
	...
	settlement := billingsvc.CreemSettlement{
	...
	case strings.HasPrefix(settlement.ReferenceID, "sub_ref_"):
		err = billingsvc.CompleteBoundCreemSubscriptionOrder(settlement.ReferenceID, string(body), settlement)
	case strings.HasPrefix(settlement.ReferenceID, "ref_"):
		if settlement.OrderType != "onetime" {
			c.Status(http.StatusOK)
			return
		}
		err = billingsvc.CompleteBoundCreemTopUpOrder(settlement.ReferenceID, settlement)
	default:
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}
	if err != nil {
		if creemWebhookRequestInvalid(err) {
			...
			logging.SysError("Creem webhook settlement failed trade_no=" + settlement.ReferenceID + ": " + err.Error())
...
func validPaidCreemEvent(event *CreemWebhookEvent) bool {
	if event == nil || !billingsvc.ValidCreemCheckoutID(event.ID) || !billingsvc.ValidCreemCheckoutID(event.Object.ID) ||
		!billingsvc.ValidCreemCheckoutID(event.Object.Order.ID) || event.Object.RequestID == "" ||
...
func creemWebhookRequestInvalid(err error) bool {
	return errors.Is(err, billingsvc.ErrTopUpNotFound) ||
		errors.Is(err, billingsvc.ErrSubscriptionOrderNotFound) ||
		errors.Is(err, billingsvc.ErrCreemPaymentMismatch) ||
		errors.Is(err, billingsvc.ErrCreemCheckoutBindingMismatch)
}
