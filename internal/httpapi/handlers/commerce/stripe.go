...
func StripeWebhook(c *gin.Context) {
	...
	secret := env.GetEnv("STRIPE_WEBHOOK_SECRET", "")
	if secret == "" {
	...
	if err := jsonutil.Unmarshal(body, &event); err != nil {
	...
	case "checkout.session.completed":
		...
		if err := fulfillStripeCheckout(event.Type, &event.Data.Object); err != nil {
			logging.SysLog(fmt.Sprintf("Stripe 订单处理失败 trade_no=%s event_type=%s error=%v",
	...
	case "checkout.session.async_payment_succeeded":
		...
		if err := fulfillStripeCheckout(event.Type, &event.Data.Object); err != nil {
			logging.SysLog(fmt.Sprintf("Stripe 异步订单处理失败 trade_no=%s event_type=%s error=%v",
				event.Data.Object.ClientReferenceID, event.Type, err))
			c.JSON(http.StatusServiceUnavailable, dto.Fail("支付回调处理失败，请重试"))
			return
		}
	case "checkout.session.async_payment_failed":
		if err := updateStripeCheckoutTerminal(&event.Data.Object, billingsvc.TopUpStatusFailed); err != nil {
			logging.SysLog(fmt.Sprintf("Stripe 异步订单失败标记失败 trade_no=%s error=%v", event.Data.Object.ClientReferenceID, err))
			c.JSON(http.StatusServiceUnavailable, dto.Fail("支付回调处理失败，请重试"))
			return
		}
	case "checkout.session.expired":
		...
		if err := updateStripeCheckoutTerminal(&event.Data.Object, billingsvc.TopUpStatusExpired); err != nil {
			logging.SysLog(fmt.Sprintf("Stripe 订单过期处理失败 trade_no=%s error=%v", event.Data.Object.ClientReferenceID, err))
			c.JSON(http.StatusServiceUnavailable, dto.Fail("支付回调处理失败，请重试"))
			return
		}
	case "charge.refunded", "charge.dispute.created", "charge.dispute.updated", "charge.dispute.closed":
		...
		if tradeNo == "" {
			if err := billingsvc.RecordUnmatchedStripePaymentReversalReview(event.ID, event.Type, event.Created); err != nil {
				logging.SysLog(fmt.Sprintf("Stripe reversal correlation audit failed event_id=%s event_type=%s error=%v", event.ID, event.Type, err))
				c.JSON(http.StatusServiceUnavailable, dto.Fail("支付冲正记录失败，请重试"))
				return
			}
			break
		}
		if err := billingsvc.FlagStripePaymentReversalReview(tradeNo, event.ID, event.Type, event.Created); err != nil {
			logging.SysLog(fmt.Sprintf("Stripe reversal review marker failed trade_no=%s event_id=%s event_type=%s error=%v",
...
func fulfillStripeCheckout(eventType string, object *stripeCheckoutObject) error {
	...
	if orderType == billingsvc.StripeOrderTypeSubscription {
		payload, err := jsonutil.Marshal(gin.H{
		...
		return billingsvc.CompleteBoundStripeSubscriptionOrder(referenceId, string(payload), object.ID, object.Mode,
			metadataOrderType, metadataPriceID, object.Customer, object.AmountTotal, object.Currency)
	}
	return billingsvc.CompleteBoundStripeTopUpOrderWithCustomer(referenceId, object.ID, object.Mode, metadataOrderType,
...
func updateStripeCheckoutTerminal(object *stripeCheckoutObject, status string) error {
	...
	metadataOrderType := strings.TrimSpace(object.Metadata["order_type"])
	if orderType == billingsvc.StripeOrderTypeSubscription {
		return billingsvc.UpdateBoundPendingSubscriptionOrderStatus(referenceID, object.ID, object.Mode, metadataOrderType,
			strings.TrimSpace(object.Metadata["price_id"]), status, object.AmountTotal, object.Currency)
	}
	return billingsvc.UpdateBoundPendingTopUpStatus(referenceID, object.ID, object.Mode, metadataOrderType, status,
...
func stripeOrderTypeForReference(referenceID string) (string, error) {
	...
	case strings.HasPrefix(referenceID, "sub_ref_"):
		return billingsvc.StripeOrderTypeSubscription, nil
	case strings.HasPrefix(referenceID, "ref_"):
		return billingsvc.StripeOrderTypeWallet, nil
	default:
