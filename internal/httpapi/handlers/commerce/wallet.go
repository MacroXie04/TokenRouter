...
func CreateRedemption(c *gin.Context) {
	...
	keys, err := billingsvc.CreateRedemptionBatch(requestctx.GetUserId(c), req.Name, req.Quota, req.ExpiredTime, req.Count)
	if err != nil {
	...
	billingsvc.RecordSystemLog(requestctx.GetUserId(c), billingsvc.LogTypeManage,
		"redemption.create name="+req.Name+" count="+textutil.Int2Str(req.Count))
...
func GetRedemptions(c *gin.Context) {
	...
	items, total, err := billingsvc.GetPagedRedemptions(pi.Offset(), pi.PageSize)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": err.Error()})
		return
	}
...
func Redeem(c *gin.Context) {
	...
	quota, err := billingsvc.Redeem(requestctx.GetUserId(c), req.Key)
	if err != nil {
...
func CheckIn(c *gin.Context) {
	result, err := billingsvc.CheckInWithResult(requestctx.GetUserId(c))
	if err != nil {
...
func CheckInStatus(c *gin.Context) {
	checkedIn, err := billingsvc.CheckInStatusChecked(requestctx.GetUserId(c))
	if err != nil {
...
func TopUp(c *gin.Context) {
	...
	quota, err := billingsvc.Redeem(userId, req.Key)
	if err != nil {
		...
		logging.SysLog("top-up redemption failed for user " + textutil.Int2Str(userId) + ": " + err.Error())
...
func GetSelfTopUps(c *gin.Context) {
	...
	orders, total, err := billingsvc.ListUserTopUps(requestctx.GetUserId(c), c.Query("keyword"), page.Page, page.PageSize)
	if err != nil {
