...
func loadSetupState(db *gorm.DB) (setupState, error) {
	...
	var rootCount int64
	if err := db.Model(&model.User{}).Where("role >= ?", roles.RoleRootUser).Count(&rootCount).Error; err != nil {
		return setupState{}, err
	}
...
func PostSetup(c *gin.Context) {
	...
	var user *model.User
	var registrationPlan *authsvc.RegistrationMutationPlan
	if !state.RootInitialized {
		...
		hash, hashErr := cryptoutil.PasswordHash(req.Password)
		if hashErr != nil {
			c.JSON(http.StatusInternalServerError, dto.Fail("密码加密失败"))
			return
		}
		plan, planErr := authsvc.PlanRegistrationMutationFromEnvironment(username)
		if planErr != nil {
...
func createInitialSetup(user *model.User, registrationPlan *authsvc.RegistrationMutationPlan, selfUseMode, demoSite bool) error {
	err := model.DB.Transaction(func(tx *gorm.DB) error {
		claim := model.Setup{ID: 1, Version: buildinfo.Version, InitializedAt: wallclock.NowTimestamp()}
		...
		var rootCount int64
		if err := tx.Model(&model.User{}).Where("role >= ?", roles.RoleRootUser).Count(&rootCount).Error; err != nil {
			return err
		}
		if rootCount == 0 {
			...
			if err := authsvc.InsertPlannedRegistrationUserWithTx(tx, user, *registrationPlan); err != nil {
				return err
			}
		}
	...
	if err := setting.Sync(); err != nil {
		logging.SysError("setup committed but settings cache refresh failed: " + err.Error())
	}
	return nil
}
