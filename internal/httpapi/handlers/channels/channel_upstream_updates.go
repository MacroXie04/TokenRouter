...
func ApplyChannelUpstreamModelUpdates(c *gin.Context) {
	...
	result, err := channelssvc.ApplyChannelUpstreamModelUpdates(
...
func ApplyAllChannelUpstreamModelUpdates(c *gin.Context) {
	summary, err := channelssvc.ApplyAllChannelUpstreamModelUpdates(c.Request.Context())
	if err != nil {
		channelUpstreamOperationError(c, err)
		return
	}
...
func DetectChannelUpstreamModelUpdates(c *gin.Context) {
	...
	result, _, err := channelssvc.DetectChannelUpstreamModelUpdates(c.Request.Context(), request.ID, true, false)
	if err != nil {
		channelUpstreamOperationError(c, err)
		return
	}
...
func DetectAllChannelUpstreamModelUpdates(c *gin.Context) {
	task, created, err := operationssvc.EnqueueSystemTask(model.SystemTaskTypeModelUpdate, map[string]any{"manual": true})
	if err != nil {
		channelUpstreamOperationError(c, err)
		return
	}
