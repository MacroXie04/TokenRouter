...
func setupSubscriptionAdminTest(t *testing.T) func(method, path, body string) *httptest.ResponseRecorder {
	...
	admin := model.User{Username: "subadmin", Password: "pw", Role: roles.RoleAdminUser,
		Status: model.UserStatusEnabled, Group: "default", AuthVersion: 1}
	require.NoError(t, model.DB.Create(&admin).Error)
	sid, access, refresh, err := auth.CompleteLogin(&admin, "127.0.0.1", "ua", "test")
	require.NoError(t, err)
...
func TestLegacySubscriptionPlanCreateUsesComplianceAndComprehensiveValidation(t *testing.T) {
	...
	subExpectFail(t, do(http.MethodPost, "/api/admin/subscription/plan",
		...
		billingsvc.ErrPaymentComplianceRequired.Error())
...
func TestAdminSubscriptionPlanCRUD(t *testing.T) {
	...
	for body, msg := range map[string]string{
		...
		fmt.Sprintf(`{"plan":{"title":"P","total_amount":%d}}`, quotamath.MaxQuota+1): fmt.Sprintf("总额度不能超过%d", quotamath.MaxQuota),
	} {
...
func TestAdminSubscriptionBindResetInvalidateDelete(t *testing.T) {
	...
	require.NoError(t, model.DB.Model(&model.Log{}).
		Where("user_id = ? AND type = ? AND content LIKE ?", u1.Id, billingsvc.LogTypeManage, "管理员重置订阅套餐%").
...
func TestAdminSubscriptionRoutesEnforceTargetRoleHierarchy(t *testing.T) {
	...
	peer := model.User{Username: "subscription-peer", Password: "pw", Role: roles.RoleAdminUser,
		Status: model.UserStatusEnabled, Group: "default", AuthVersion: 1}
	root := model.User{Username: "subscription-root", Password: "pw", Role: roles.RoleRootUser,
		Status: model.UserStatusEnabled, Group: "default", AuthVersion: 1}
	lower := model.User{Username: "subscription-lower", Password: "pw", Role: roles.RoleCommonUser,
		Status: model.UserStatusEnabled, Group: "default", AuthVersion: 1}
	...
	peerSub := model.UserSubscription{UserId: peer.Id, PlanId: planID, AmountTotal: 1000, AmountUsed: 400,
		StartTime: now, EndTime: now + 3600, Status: billingsvc.SubscriptionStatusActive, Source: "admin",
		QuotaResetPeriodSnapshot: billingsvc.SubscriptionResetMonthly, EntitlementVersion: billingsvc.UserSubscriptionEntitlementVersion,
		EntitlementMigrationState: billingsvc.SubscriptionEntitlementMigrationBackfilled}
	rootSub := peerSub
	rootSub.Id = 0
	...
	assertForbidden := func(rec *httptest.ResponseRecorder) {
		...
		body := subDecode(t, rec)
		assert.Equal(t, false, body["success"])
		assert.Equal(t, billingsvc.ErrSubscriptionTargetForbidden.Error(), body["message"])
	}
	...
	for _, subscription := range unchanged {
		...
		assert.Equal(t, billingsvc.SubscriptionStatusActive, subscription.Status)
	}
}
