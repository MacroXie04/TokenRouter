...
func TestRelayQuotaReservationManualReviewRootWorkflow(t *testing.T) {
	...
	token := model.Token{UserId: rootID, Key: "sk-review-route", Status: billingsvc.TokenStatusEnabled, RemainQuota: 1000}
	require.NoError(t, model.DB.Create(&token).Error)
	channel := model.Channel{Name: "review-route-channel", Key: "provider-secret", Status: channelcatalog.ChannelStatusEnabled}
	...
	list := do(http.MethodGet, "/api/relay-quota-reservations/manual-review?page_size=1&operation=settle&user_id="+textutil.Int2Str(rootID), "")
...
func TestGrokViolationFeeManualReviewRootRetryUsesImmutableStoredPlan(t *testing.T) {
	...
	channel := model.Channel{
		...
		Type: int(channelcatalog.ChannelTypeXai), Status: channelcatalog.ChannelStatusEnabled, UsedQuota: 7,
	}
	require.NoError(t, model.DB.Create(&channel).Error)
	reservation, err := billingsvc.NewRelayQuotaReservation(rootID, &token, 5)
	require.NoError(t, err)
	...
	_, err = billingsvc.ChargeGrokViolationFee(billingsvc.GrokViolationFeeInput{
		ReservationID: reservation.ReservationID(), ChannelID: channel.Id,
		ModelName: "grok-4", Group: userssvc.GroupDefault,
		RequestID:  "00000000-0000-4000-8000-000000000099",
		StatusCode: http.StatusBadRequest, UseTime: 2, GroupRatio: 1,
	})
	require.ErrorIs(t, err, billingsvc.ErrInsufficientQuota)
	...
	assert.Equal(t, billingsvc.RelayQuotaReviewKindGrokViolationFee, listed["review_kind"])
...
func TestRelayQuotaReservationJimengResolutionRootOnlyAndImmutable(t *testing.T) {
	...
	token := model.Token{
		UserId: rootID, Key: "sk-jimeng-review-route", Status: billingsvc.TokenStatusEnabled, RemainQuota: 1000,
	}
	require.NoError(t, model.DB.Create(&token).Error)
	channel := model.Channel{
		Name: "jimeng-review-route", Key: "provider-secret", Type: int(channelcatalog.ChannelTypeJimeng),
	...
	task := model.Task{
		...
		Platform: textutil.Int2Str(int(channelcatalog.ChannelTypeJimeng)), UserId: rootID,
	...
	require.NoError(t, model.DB.Where("event_id = ?", auditID).First(&event).Error)
	assert.Equal(t, rootID, event.OperatorUserID)
	assert.Equal(t, billingsvc.RelayQuotaReviewActionResolveSettle, event.Action)
}
...
func TestRelayQuotaReservationVideoPollReviewListsAndRejectsMissingProviderID(t *testing.T) {
	...
	token := model.Token{
		UserId: rootID, Key: "sk-video-review-route", Status: billingsvc.TokenStatusEnabled, RemainQuota: 1000,
	}
	require.NoError(t, model.DB.Create(&token).Error)
	channel := model.Channel{
		Name: "video-review-route", Key: "provider-secret", Type: int(channelcatalog.ChannelTypeSora),
		Status: channelcatalog.ChannelStatusEnabled, BaseURL: "https://provider.example.invalid",
	}
	require.NoError(t, model.DB.Create(&channel).Error)
	reservation, err := billingsvc.NewRelayQuotaReservation(rootID, &token, 10)
	require.NoError(t, err)
	...
	require.Len(t, items, 1)
	item := items[0].(map[string]any)
	assert.Equal(t, billingsvc.RelayQuotaReviewKindOpenAIVideoPoll, item["review_kind"])
