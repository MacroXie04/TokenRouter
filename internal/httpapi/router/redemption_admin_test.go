...
func createRedemptionRow(t *testing.T, name string, status int, quota, expiredTime int64) model.Redemption {
	t.Helper()
	r := model.Redemption{
		UserId: 1, Key: cryptoutil.BestEffortUUID(), Status: status, Name: name,
		Quota: int(quota), CreatedTime: wallclock.NowTimestamp(), ExpiredTime: expiredTime,
	}
...
func TestRedemptionStatusCodesMatchReference(t *testing.T) {
	assert.Equal(t, 1, billingsvc.RedemptionStatusEnabled)
	assert.Equal(t, 2, billingsvc.RedemptionStatusDisabled)
	assert.Equal(t, 3, billingsvc.RedemptionStatusUsed)
}
...
func TestRedemptionCreateContract(t *testing.T) {
	...
	assert.Equal(t, billingsvc.ErrPaymentComplianceRequired.Error(), body["message"])
	...
	// Expiry validation.
	rec = do(http.MethodPost, "/api/redemption", fmt.Sprintf(`{"name":"x","count":1,"quota":10,"expired_time":%d}`, wallclock.NowTimestamp()-10))
	...
	for i, r := range rows {
		assert.Equal(t, billingsvc.RedemptionStatusEnabled, r.Status)
...
func TestRedemptionListAndSearch(t *testing.T) {
	...
	createRedemptionRow(t, "gamma-code", billingsvc.RedemptionStatusDisabled, 10, 0)
	createRedemptionRow(t, "delta-code", billingsvc.RedemptionStatusEnabled, 10, now-100)
	createRedemptionRow(t, "epsilon-code", billingsvc.RedemptionStatusEnabled, 10, now+1000)
	...
	rec = do(http.MethodGet, "/api/redemption/search?keyword="+textutil.Int2Str(used.Id), "")
	data = decodeBody(t, rec)["data"].(map[string]any)
	require.Len(t, data["items"].([]any), 1)
...
func TestRedemptionGetUpdateDelete(t *testing.T) {
	...
	// Get by id.
	rec := do(http.MethodGet, "/api/redemption/"+textutil.Int2Str(r.Id), "")
	require.Equal(t, http.StatusOK, rec.Code, rec.Body.String())
	body := decodeBody(t, rec)
	assert.Equal(t, true, body["success"])
	data, ok := body["data"].(map[string]any)
	...
	rec = do(http.MethodPut, "/api/redemption?status_only=1", fmt.Sprintf(`{"id":%d,"name":"other","quota":1,"status":%d}`, r.Id, billingsvc.RedemptionStatusDisabled))
	require.Equal(t, http.StatusOK, rec.Code, rec.Body.String())
	updated = decodeBody(t, rec)["data"].(map[string]any)
	assert.Equal(t, float64(billingsvc.RedemptionStatusDisabled), updated["status"])
	assert.Equal(t, "renamed", updated["name"], "name untouched in status_only")
	assert.Equal(t, float64(77), updated["quota"])

	// Delete one.
	rec = do(http.MethodDelete, "/api/redemption/"+textutil.Int2Str(r.Id), "")
	require.Equal(t, http.StatusOK, rec.Code, rec.Body.String())
	...
	createRedemptionRow(t, "keep-me", billingsvc.RedemptionStatusEnabled, 10, now+1000)
	createRedemptionRow(t, "sweep-disabled", billingsvc.RedemptionStatusDisabled, 10, now+1000)
