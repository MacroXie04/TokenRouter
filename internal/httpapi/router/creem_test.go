...
func sendCreemWebhook(handler http.Handler, payload, secret string) *httptest.ResponseRecorder {
	...
	req.Header.Set(commerce.CreemSignatureHeader, signCreem([]byte(payload), secret))
...
func TestCreemWalletAndSubscriptionHTTPContractEndToEnd(t *testing.T) {
	...
	restore := creempayments.SetCheckoutTransportForTesting(creemRoundTripFunc(func(request *http.Request) (*http.Response, error) {
		...
		require.NoError(t, jsonutil.Unmarshal(body, &decoded))
		mu.Lock()
	...
	assert.Equal(t, billingsvc.TopUpStatusPending, walletOrder.Status)
	assert.Equal(t, int64(12_345), walletOrder.CreditQuota)
	assert.Equal(t, int64(1234), walletOrder.ProviderAmountMinor)
	assert.Equal(t, billingsvc.PaymentProviderCreem, walletOrder.PaymentProvider)
...
func TestCreemWebhookSignatureAvailabilityAndBodyLimits(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodPost, "/api/creem/webhook", strings.NewReader(payload))
	req.Header.Set(commerce.CreemSignatureHeader, strings.Repeat("0", 64))
	recorder = httptest.NewRecorder()
	handler.ServeHTTP(recorder, req)
	...
	req.Header.Set(commerce.CreemSignatureHeader, signCreem([]byte(oversized), "creem_secret"))
	recorder = httptest.NewRecorder()
	handler.ServeHTTP(recorder, req)
	assert.Equal(t, http.StatusRequestEntityTooLarge, recorder.Code)

	valid := []byte(`{"id":"evt","eventType":"checkout.completed"}`)
	assert.True(t, commerce.VerifyCreemSignature(valid, signCreem(valid, "creem_secret"), "creem_secret"))
	assert.False(t, commerce.VerifyCreemSignature(valid, strings.ToUpper(signCreem(valid, "creem_secret")), "creem_secret"))
	assert.False(t, commerce.VerifyCreemSignature(valid, signCreem(valid, "wrong"), "creem_secret"))
}
...
func TestCreemCheckoutFailureClassificationPreservesAmbiguousOrdersAndReleasesDefiniteRejections(t *testing.T) {
	...
	require.NoError(t, model.DB.Where("user_id = ? AND payment_provider = ?", userID, billingsvc.PaymentProviderCreem).
		Order("id desc").First(&wallet).Error)
	assert.Equal(t, billingsvc.TopUpStatusPending, wallet.Status)
	assert.Equal(t, billingsvc.CreemReconciliationCreate, wallet.ReconciliationState)
	...
	assert.Equal(t, billingsvc.TopUpStatusSuccess, wallet.Status)
	...
	assert.Equal(t, billingsvc.TopUpStatusFailed, order.Status)
	...
	assert.Equal(t, billingsvc.TopUpStatusSuccess, order.Status)
	assert.False(t, order.CapacityReserved)
}
