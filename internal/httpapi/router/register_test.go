...
func TestPasswordAuthPolicyAndVerifiedRegistration(t *testing.T) {
	...
	const code = "482913"
	_, err := authsvc.CreateAuthFlow(
		authsvc.EmailVerificationPurpose, "email", email, 0, "", code, time.Minute,
	)
	require.NoError(t, err)
	...
	require.NoError(t, model.DB.Where("purpose = ? AND payload = ?", authsvc.EmailVerificationPurpose, code).First(&flow).Error)
...
func TestRegisterAndLoginShareUTF8UsernameBoundary(t *testing.T) {
	...
	for _, username := range []string{strings.Repeat("a", 64), strings.Repeat("界", 21)} {
		...
		user, err := authsvc.AuthenticatePassword(username, "password123")
		require.NoError(t, err)
