...
func TestAdminClearUserBindingRoute(t *testing.T) {
	...
	target := model.User{
		Username: "bound-target", Password: "password8", Role: roles.RoleCommonUser,
	...
	recorder := do(http.MethodDelete, "/api/user/"+textutil.Int2Str(target.Id)+"/bindings/github", "")
	...
	recorder = do(http.MethodDelete, "/api/user/"+textutil.Int2Str(target.Id)+"/bindings/email", "")
	require.Equal(t, http.StatusOK, recorder.Code, recorder.Body.String())
	require.NoError(t, model.DB.First(&refreshed, target.Id).Error)
	...
	recorder = do(http.MethodDelete, "/api/user/"+textutil.Int2Str(target.Id)+"/bindings/unknown", "")
	assert.Equal(t, http.StatusBadRequest, recorder.Code)
}

func TestAdminDeleteUserRouteAndRoleGuard(t *testing.T) {
	_, do, rootID, _ := setupAuthAdjacent(t, roles.RoleRootUser)
	target := createManagedUser(t, "delete-route-target", roles.RoleCommonUser, model.UserStatusEnabled, "default")
	...
	require.NoError(t, model.DB.Create(&model.UserSession{
		...
		Status: "active", RefreshHash: "delete-route-refresh", ExpiresAt: wallclock.NowTimestamp() + 3600,
	}).Error)

	self := do(http.MethodDelete, "/api/user/"+textutil.Int2Str(rootID), "")
	assert.Equal(t, http.StatusForbidden, self.Code)

	recorder := do(http.MethodDelete, "/api/user/"+textutil.Int2Str(target.Id), "")
	require.Equal(t, http.StatusOK, recorder.Code, recorder.Body.String())
