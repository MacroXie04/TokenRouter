...
func setupSecureTest(t *testing.T, role int) (http.Handler, func(method, path, body string, headers map[string]string) *httptest.ResponseRecorder, int, string) {
	...
	require.NoError(t, auth.InitWebAuthn())
...
func createTestChannel(t *testing.T, name, key string) model.Channel {
	...
	w := uint(1)
	ch := model.Channel{
		Name: name, Type: int(channelcatalog.ChannelTypeOpenAI), Key: key,
		Status: channelcatalog.ChannelStatusEnabled, BaseURL: "https://api.example.com", Models: "gpt-4", Group: "default", Weight: &w,
	}
...
func seedProtectedTwoFA(t *testing.T, userId int) {
	...
	require.NoError(t, model.DB.Create(&model.TwoFABackupCode{
		UserId: userId, CodeHash: cryptoutil.SHA256Hex("87654321"), CreatedAt: time.Now(),
	}).Error)
}
...
func TestUniversalVerify2FA(t *testing.T) {
	_, do, userId, _ := setupSecureTest(t, roles.RoleAdminUser)
	enableTwoFA(t, userId)
...
func TestUniversalVerifyRejectsBadInput(t *testing.T) {
	...
	// User without 2FA.
	_, do2, _, _ := setupSecureTest(t, roles.RoleAdminUser)
...
func TestTwoFAFirstSetupRemainsSessionAuthenticated(t *testing.T) {
	_, do, userId, _ := setupSecureTest(t, roles.RoleCommonUser)
...
func TestTwoFAFirstSetupRejectsPATWithoutSession(t *testing.T) {
	r, _, userId, _ := setupSecureTest(t, roles.RoleCommonUser)
...
func TestTwoFAEnabledResetRequiresBoundProofWithoutStateLoss(t *testing.T) {
	...
	t.Run("wrong scope", func(t *testing.T) {
		_, do, userId, _ := setupSecureTest(t, roles.RoleCommonUser)
		seedProtectedTwoFA(t, userId)
		proof := issueProof(t, do, auth.SecurityProofScopeChannelKeyRead)
		before := snapshotTwoFA(t, userId)
	...
	t.Run("different session", func(t *testing.T) {
		_, do, userId, sid := setupSecureTest(t, roles.RoleCommonUser)
		seedProtectedTwoFA(t, userId)
		var user model.User
		var session model.UserSession
		require.NoError(t, model.DB.First(&user, userId).Error)
		require.NoError(t, model.DB.Where("sid = ?", sid).First(&session).Error)
		proof, _, err := auth.IssueSecurityProof(auth.SessionIdentity{
			UserID: userId, SessionID: "another-session",
			UserAuthVersion: user.AuthVersion, SessionVersion: session.Version,
		}, auth.SecurityProofMethod2FA, []string{auth.SecurityProofScopeTwoFAReset})
		require.NoError(t, err)
		before := snapshotTwoFA(t, userId)
	...
	t.Run("stale auth version", func(t *testing.T) {
		_, do, userId, sid := setupSecureTest(t, roles.RoleCommonUser)
		seedProtectedTwoFA(t, userId)
		proof := issueProof(t, do, auth.SecurityProofScopeTwoFAReset)
		before := snapshotTwoFA(t, userId)
		require.NoError(t, auth.BumpAuthVersionKeepSession(userId, sid))
...
func TestRegenerateBackupCodesRequiresScopedProof(t *testing.T) {
	...
	wrongScope := issueProof(t, do, auth.SecurityProofScopeTwoFAReset)
	...
	proof := issueProof(t, do, auth.SecurityProofScopeBackupCodeReset)
	rec = do(http.MethodPost, "/api/user/2fa/backup_codes", "",
		map[string]string{"X-Security-Proof": proof})
	require.Equal(t, http.StatusOK, rec.Code, "body: %s", rec.Body.String())
	after := snapshotTwoFA(t, userId)
...
func TestChannelKeyMaskedAndStepUp(t *testing.T) {
	_, do, userId, _ := setupSecureTest(t, roles.RoleRootUser)
	...
	rec = do(http.MethodGet, "/api/channel/"+textutil.Int2Str(ch.Id), "", nil)
	assert.Equal(t, "", decode(t, rec)["data"].(map[string]any)["key"])

	// Key disclosure requires a step-up proof.
	rec = do(http.MethodPost, "/api/channel/"+textutil.Int2Str(ch.Id)+"/key", "", nil)
	require.Equal(t, http.StatusForbidden, rec.Code)
	assert.Equal(t, "SECURITY_PROOF_REQUIRED", decode(t, rec)["code"])

	// A forged proof is rejected.
	rec = do(http.MethodPost, "/api/channel/"+textutil.Int2Str(ch.Id)+"/key", "",
		map[string]string{"X-Security-Proof": "not.a.jwt"})
...
func TestLegacyMultiKeyCredentialsStayOffOrdinaryChannelResponses(t *testing.T) {
	...
	channel := model.Channel{
		Name: "legacy-multi-key", Type: int(channelcatalog.ChannelTypeOpenAI), Key: "",
		Other: `["legacy-secret-one","legacy-secret-two"]`, Status: channelcatalog.ChannelStatusEnabled,
	...
	for _, path := range []string{
		"/api/channel",
		"/api/channel/" + textutil.Int2Str(channel.Id),
	...
	enableTwoFA(t, userId)
	proof := issueProof(t, do, auth.SecurityProofScopeChannelKeyRead)
	rec := do(http.MethodPost, "/api/channel/"+textutil.Int2Str(channel.Id)+"/key", "",
		map[string]string{"X-Security-Proof": proof})
...
func TestPasskeyDeleteWithout2FA(t *testing.T) {
	_, do, _, _ := setupSecureTest(t, roles.RoleCommonUser)
...
func TestPasskeyVerifyFinishFlowBinding(t *testing.T) {
	...
	userA := model.User{Username: "a", Password: "pw", Role: roles.RoleCommonUser, Status: model.UserStatusEnabled, Quota: 1000, AuthVersion: 1}
	userB := model.User{Username: "b", Password: "pw", Role: roles.RoleCommonUser, Status: model.UserStatusEnabled, Quota: 1000, AuthVersion: 1}
	require.NoError(t, model.DB.Create(&userA).Error)
	require.NoError(t, model.DB.Create(&userB).Error)
	sidA, accessA, refreshA, err := auth.CompleteLogin(&userA, "127.0.0.1", "ua", "test")
	require.NoError(t, err)
	sidB, accessB, refreshB, err := auth.CompleteLogin(&userB, "127.0.0.1", "ua", "test")
	require.NoError(t, err)
