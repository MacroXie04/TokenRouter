...
func TestLogsStatContract(t *testing.T) {
	...
	insertLogRow(t, 1, 1, 5, "alice", "tok-a", "gpt-4o", "default", 500, 0, 0, now-3600, nil, billingsvc.LogTypeManage)
...
func TestLogsListAndSelfContracts(t *testing.T) {
	...
	sid, access, refresh, err := authsvc.CompleteLogin(&other, "127.0.0.9", "ua", "test")
	require.NoError(t, err)
