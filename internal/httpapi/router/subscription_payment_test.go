...
func TestSubscriptionStripePayContract(t *testing.T) {
	...
	assert.Equal(t, billingsvc.PaymentMethodStripe, order.PaymentMethod)
	...
	assert.Equal(t, billingsvc.StripeSubscriptionCheckoutBindingVersion, order.ProviderBindingVersion)
...
func TestSubscriptionStripeWebhookCompletion(t *testing.T) {
	...
	assert.Equal(t, billingsvc.TopUpStatusPending, pendingOrder.Status)
	...
	assert.Equal(t, billingsvc.TopUpStatusSuccess, topup.Status)
	assert.Equal(t, 40.0, topup.Money)
	assert.Equal(t, billingsvc.PaymentMethodStripe, topup.PaymentMethod)
	...
	assert.Equal(t, billingsvc.TopUpStatusExpired, order2.Status)
