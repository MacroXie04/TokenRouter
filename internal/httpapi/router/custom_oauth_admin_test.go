...
func TestAdminUserOAuthBindingsContract(t *testing.T) {
	...
	require.Equal(t, http.StatusOK, rec.Code, rec.Body.String())
	providerId := int(decodeBody(t, rec)["data"].(map[string]any)["id"].(float64))
	target := model.User{Username: "oauth-target", Password: "password8", Role: roles.RoleCommonUser,
	...
	adminUser := model.User{Username: "oauthadmin", Password: "pw", Role: roles.RoleAdminUser,
		Status: model.UserStatusEnabled, Quota: 1000, AuthVersion: 1}
	require.NoError(t, model.DB.Create(&adminUser).Error)
	sid, access, refresh, err := authsvc.CompleteLogin(&adminUser, "127.0.0.7", "ua", "test")
	require.NoError(t, err)
...
func TestCustomOAuthRoutesRequireRoot(t *testing.T) {
	...
	plain := model.User{Username: "oauthplain", Password: "pw", Role: roles.RoleCommonUser,
		Status: model.UserStatusEnabled, Quota: 1000, AuthVersion: 1}
	require.NoError(t, model.DB.Create(&plain).Error)
	sid, access, refresh, err := authsvc.CompleteLogin(&plain, "127.0.0.6", "ua", "test")
	require.NoError(t, err)
