...
func TestJimengOfficialTaskContract(t *testing.T) {
	...
	token := model.Token{
		...
		Status: billingsvc.TokenStatusEnabled, RemainQuota: 100000, Group: "default",
	}
	require.NoError(t, model.DB.Create(&token).Error)
	priority := int64(10)
	channel := model.Channel{
		Type: int(channelcatalog.ChannelTypeJimeng), Key: "access|secret", Name: "jimeng-local",
	...
	billingsvc.SetGroupRatios(map[string]float64{"default": 2})
	billingsvc.SetGroupGroupRatios(map[string]map[string]float64{"default": {"default": 1}})
	require.NoError(t, setting.UpdateOption(billingsvc.DataExportEnabledOption, "false"))
	t.Cleanup(func() {
	...
	assert.Equal(t, strconv.Itoa(int(channelcatalog.ChannelTypeJimeng)), task.Platform)
	...
	require.NoError(t, model.LOG_DB.Where("type = ?", billingsvc.LogTypeConsume).First(&consumption).Error)
	...
	legacyFailed := model.Task{
		TaskID: "task_legacy_jimeng_failure", Platform: strconv.Itoa(int(channelcatalog.ChannelTypeJimeng)),
	...
	backupChannel := model.Channel{
		Type: int(channelcatalog.ChannelTypeJimeng), Key: "access|secret", Name: "jimeng-backup",
		Status: channelcatalog.ChannelStatusEnabled, BaseURL: upstream.URL,
...
func TestJimengMiddlewareAndOwnershipFailures(t *testing.T) {
	...
	token := model.Token{UserId: userID, Key: "sk-jimeng-owner", Name: "owner", Status: billingsvc.TokenStatusEnabled,
	...
	otherUser := model.User{Username: "jimeng-other", Password: "pw", Status: model.UserStatusEnabled,
		Role: roles.RoleCommonUser, Quota: 10000, Group: "default", AuthVersion: 1}
	...
	otherToken := model.Token{UserId: otherUser.Id, Key: "sk-jimeng-other", Name: "other",
		Status: billingsvc.TokenStatusEnabled, RemainQuota: 10000, Group: "default"}
	require.NoError(t, model.DB.Create(&otherToken).Error)
	task := model.Task{TaskID: model.GenerateTaskID(), Platform: strconv.Itoa(int(channelcatalog.ChannelTypeJimeng)), UserId: userID, Status: model.TaskStatusNotStart,
...
func TestJimengAcceptedTaskCommitFailureRecoversOnFetch(t *testing.T) {
	...
	priority := int64(10)
	channel := model.Channel{
		Type: int(channelcatalog.ChannelTypeJimeng), Key: "access|secret", Name: "jimeng-commit-failure",
		Status: channelcatalog.ChannelStatusEnabled, BaseURL: upstream.URL,
	...
	previousPrices := billingsvc.ExportedModelPrices()
	previousRatios := billingsvc.ExportedGroupRatios()
	billingsvc.SetModelPriceRegistry(map[string]billingsvc.ModelPrice{"jimeng-commit-model": {Prompt: 0.01}})
	...
	pollToken := model.Token{
		...
		Status: billingsvc.TokenStatusEnabled, UnlimitedQuota: true, Group: "default",
	}
...
func TestJimengFallbackWriteFailureRecoversFromEncryptedDatabaseOperation(t *testing.T) {
	...
	require.NoError(t, model.DB.Create(&token).Error)
	priority := int64(10)
	channel := model.Channel{Type: int(channelcatalog.ChannelTypeJimeng), Key: "access|secret", Name: "recovery-token",
		Status: channelcatalog.ChannelStatusEnabled, BaseURL: upstream.URL, Models: "jimeng-recovery-model",
	...
	previousPrices := billingsvc.ExportedModelPrices()
	previousRatios := billingsvc.ExportedGroupRatios()
	billingsvc.SetModelPriceRegistry(map[string]billingsvc.ModelPrice{"jimeng-recovery-model": {Prompt: 0.01}})
	billingsvc.SetGroupRatios(map[string]float64{"default": 1})
	t.Cleanup(func() {
		billingsvc.SetModelPriceRegistry(previousPrices)
		billingsvc.SetGroupRatios(previousRatios)
	...
	logging.SetLogger(slog.New(slog.NewJSONHandler(&logSink, nil)))
	...
	require.NoError(t, model.LOG_DB.Model(&model.Log{}).
		Where("type = ?", billingsvc.LogTypeConsume).Count(&preSettlementLogCount).Error)
	...
	require.NoError(t, channelssvc.InitAbilityCache())
	handler = router.SetUpRouter()
	require.NoError(t, tasks.ReconcileJimengTaskOperations(), "another process must safely replay completed recovery")
	assert.Equal(t, int32(1), fetchCalls.Load())
	...
	require.NoError(t, billingsvc.DeliverAuditLogOutbox())
	var logCount int64
...
func setupJimengDurabilityFixture(t *testing.T, upstreamURL, modelName string) (http.Handler, int, model.Token, model.Channel) {
	...
	priority := int64(10)
	channel := model.Channel{
		Type: int(channelcatalog.ChannelTypeJimeng), Key: "access|secret", Name: modelName,
	...
	previousPrices := billingsvc.ExportedModelPrices()
	previousRatios := billingsvc.ExportedGroupRatios()
	billingsvc.SetModelPriceRegistry(map[string]billingsvc.ModelPrice{modelName: {Prompt: 0.01}})
	billingsvc.SetGroupRatios(map[string]float64{"default": 1})
	t.Cleanup(func() {
		billingsvc.SetModelPriceRegistry(previousPrices)
		billingsvc.SetGroupRatios(previousRatios)
...
func TestJimengRefundFailureRemainsDurableForAnotherWorker(t *testing.T) {
	...
	require.NoError(t, tasks.ReconcileJimengTaskOperations())
	require.NoError(t, model.DB.First(&user, userID).Error)
	require.NoError(t, model.DB.First(&token, token.Id).Error)
	assert.Equal(t, 100000, user.Quota)
	assert.Equal(t, 100000, token.RemainQuota)
