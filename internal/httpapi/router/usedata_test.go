...
// covered by internal/billing/quota_data_test.go).
...
func TestDataFlowContract(t *testing.T) {
	...
	adminUser := model.User{Username: "flowadmin", Password: "pw", Role: roles.RoleAdminUser,
		Status: model.UserStatusEnabled, Quota: 1000, AuthVersion: 1}
	require.NoError(t, model.DB.Create(&adminUser).Error)
	sid, access, refresh, err := authsvc.CompleteLogin(&adminUser, "127.0.0.8", "ua", "test")
	require.NoError(t, err)
	doAdmin := doAsUser(t, handler, sid, access, refresh)

	ch1 := createSearchChannel(t, "flowchan-a", 1, channelcatalog.ChannelStatusEnabled, "default", "", "", 0)
	ch2 := createSearchChannel(t, "", 1, channelcatalog.ChannelStatusEnabled, "default", "", "", 0)
	tok := model.Token{UserId: uid, Key: "sk-flow-token", Name: "flow-tok", Status: 1,
		CreatedTime: wallclock.NowTimestamp(), UnlimitedQuota: true}
	...
	for _, endpoint := range endpoints {
		t.Run(strings.TrimPrefix(endpoint, "/api/data/"), func(t *testing.T) {
			...
			rec = do(http.MethodGet, fmt.Sprintf(
				...
				endpoint, start, start+billingsvc.DashboardDataMaxRangeSeconds+1,
			), "")
			body = decodeBody(t, rec)
			assert.Equal(t, false, body["success"], rec.Body.String())
			assert.Equal(t, "时间跨度不能超过 1 个月", body["message"])
			...
			rec = do(http.MethodGet, fmt.Sprintf(
				"%s?start_timestamp=%d&end_timestamp=%d",
				endpoint, start, start+billingsvc.DashboardDataMaxRangeSeconds,
			), "")
			body = decodeBody(t, rec)
