...
func TestPlaygroundChatCompletionsContract(t *testing.T) {
	...
	channel := model.Channel{
		Type: int(channelcatalog.ChannelTypeOpenAI), Key: "upstream-secret", Name: "playground-upstream",
		Status: channelcatalog.ChannelStatusEnabled, BaseURL: upstream.URL, Models: "playground-model", Group: "vip",
		Priority: &priority,
	}
	...
	billingsvc.SetGroupRatios(map[string]float64{"vip": 1.5, "staff": 2})
	...
	require.NoError(t, model.LOG_DB.Model(&model.Log{}).Where("type = ?", billingsvc.LogTypeConsume).Count(&consumeCount).Error)
	assert.Equal(t, int64(2), consumeCount)
	assert.Equal(t, int32(2), calls.Load())
	var latestConsumption model.Log
	require.NoError(t, model.LOG_DB.Where("type = ?", billingsvc.LogTypeConsume).Order("id DESC").First(&latestConsumption).Error)
