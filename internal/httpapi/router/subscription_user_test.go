...
func TestSubscriptionBalancePay(t *testing.T) {
	...
	subExpectFail(t, rec, billingsvc.ErrPaymentComplianceRequired.Error())
	...
	var log model.Log
	require.NoError(t, model.LOG_DB.Where("user_id = ? AND type = ?", user.Id, billingsvc.LogTypeTopup).
		First(&log).Error)
...
func TestSubscriptionSelfView(t *testing.T) {
	...
	expired := model.UserSubscription{UserId: user.Id, PlanId: plan.Id, AmountTotal: 1000,
		StartTime: now - 7200, EndTime: now - 3600, Status: billingsvc.SubscriptionStatusActive}
