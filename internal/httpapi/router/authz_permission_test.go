...
func setupPermissionTest(t *testing.T) (root, admin, plain permissionSession) {
	...
	makeSession := func(username string, role int, ip string) permissionSession {
		...
		sid, access, refresh, err := auth.CompleteLogin(&u, ip, "ua", "test")
	...
	return makeSession("permroot", roles.RoleRootUser, "127.0.0.1"),
		...
		makeSession("permplain", roles.RoleCommonUser, "127.0.0.3")
}
...
func TestPermissionEnforcementOnChannelRoutes(t *testing.T) {
	...
	createSearchChannel(t, "perm-tagged", 1, channelcatalog.ChannelStatusEnabled, "default", "gpt-4o", "t1", 5)
	...
	mk := createSearchChannel(t, "perm-mk", 1, channelcatalog.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
...
func TestPermissionUpdateChannelSensitiveGuard(t *testing.T) {
	root, admin, _ := setupPermissionTest(t)
	ch := createSearchChannel(t, "upd-target", 1, channelcatalog.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
...
func TestPermissionOverridesViaUpdateUser(t *testing.T) {
	...
	rec = root.do(http.MethodGet, "/api/user/"+textutil.Int2Str(admin.userID), "")
	require.Equal(t, http.StatusOK, rec.Code, rec.Body.String())
	...
	model.DB.Model(&model.CasbinRule{}).
		Where("ptype = ? AND v0 = ? AND v1 = ? AND v2 = ? AND v3 = ?",
			"p", auth.UserSubject(admin.userID), "channel", "sensitive_write", "allow").
		Count(&count)
	...
	assert.Equal(t, http.StatusForbidden, rec.Code, rec.Body.String())
	model.DB.Model(&model.CasbinRule{}).
		Where("ptype = ? AND v0 = ? AND v1 = ?", "p", auth.UserSubject(admin.userID), "channel").
		Count(&count)
	...
	model.DB.Model(&model.CasbinRule{}).
		Where("ptype = ? AND v0 = ? AND v1 = ? AND v2 = ? AND v3 = ?",
			"p", auth.UserSubject(admin.userID), "channel", "read", "deny").
		Count(&count)
	assert.Equal(t, int64(1), count)
}
