...
func setPaymentCompliance(t *testing.T, confirmed bool) {
	...
	version := ""
	if confirmed {
		version = billingsvc.CurrentPaymentComplianceTermsVersion
	}
...
func TestPaymentComplianceConfirmationContract(t *testing.T) {
	...
	assert.False(t, billingsvc.PaymentComplianceConfirmed())

	// Dashboard PATs authenticate as root but cannot perform this ceremony.
	pat, err := authsvc.GenerateUserAccessToken(rootID)
	...
	assert.Equal(t, billingsvc.CurrentPaymentComplianceTermsVersion, data["terms_version"])
	assert.Equal(t, float64(rootID), data["confirmed_by"])
	assert.NotZero(t, data["confirmed_at"])
	assert.True(t, billingsvc.PaymentComplianceConfirmed())
	assert.Equal(t, "true", setting.GetOption(setting.PaymentComplianceConfirmedOption))
	assert.Equal(t, billingsvc.CurrentPaymentComplianceTermsVersion, setting.GetOption(setting.PaymentComplianceTermsVersionOption))
...
func TestRootOptionSecurityContract(t *testing.T) {
	...
	assert.Equal(t, http.StatusOK, rec.Code)
	assert.Equal(t, billingsvc.ErrPaymentComplianceRequired.Error(), decodeBody(t, rec)["message"])
}
