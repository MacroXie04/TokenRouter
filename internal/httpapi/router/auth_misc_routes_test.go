...
func TestRatioConfigRouteContract(t *testing.T) {
	_, do, _ := setupAuthMiscRoutes(t, roles.RoleRootUser)
...
func TestTwoFALoginRouteContract(t *testing.T) {
	handler, _, _ := setupAuthMiscRoutes(t, roles.RoleRootUser)
	...
	require.NoError(t, err)
	user := model.User{
		Username: "route-twofa-user", Password: hash, Role: roles.RoleCommonUser,
		Status: model.UserStatusEnabled, Group: userssvc.GroupDefault, Quota: 1000, AuthVersion: 1,
	}
	require.NoError(t, model.DB.Create(&user).Error)
	secret, _, err := authsvc.GenerateTwoFASecret(user.Id, user.Username)
	require.NoError(t, err)
	code, err := totp.GenerateCode(secret, time.Now())
	require.NoError(t, err)
	_, err = authsvc.EnableTwoFA(user.Id, code)
	require.NoError(t, err)
	...
	require.NoError(t, model.DB.Where("token_hash = ?", cryptoutil.SHA256Hex(flowToken)).First(&flow).Error)
...
func TestPasskeyLoginBeginRouteContract(t *testing.T) {
	handler, _, _ := setupAuthMiscRoutes(t, roles.RoleRootUser)
	t.Setenv("ANONYMOUS_REQUEST_BODY_LIMIT_KB", "1")
	require.NoError(t, authsvc.InitWebAuthn())
	...
	var flow model.AuthFlow
	require.NoError(t, model.DB.Where("token_hash = ?", cryptoutil.SHA256Hex(flowToken)).First(&flow).Error)
	assert.Equal(t, authsvc.PasskeyPurposeLogin, flow.Purpose)
...
func TestEpayNotifyGETRouteContract(t *testing.T) {
	...
	callbackPath := func(tradeNo, gatewayTradeNo string) string {
		t.Helper()
		_, err := billingsvc.CreateTopUpWithTradeNo(
			userID, 2, 2, "alipay", billingsvc.PaymentProviderEpay, tradeNo,
		)
		require.NoError(t, err)
	...
	assert.Equal(t, before.Quota+2*quotamath.QuotaPerUnit, afterCompliance.Quota,
	...
	assert.Equal(t, afterCompliance.Quota+2*quotamath.QuotaPerUnit, afterCatalog.Quota,
...
func configureAuthMiscGroups(t *testing.T, userID int) {
	...
	billingsvc.SetGroupRatios(map[string]float64{"default": 1, "vip": 2, "staff": 3, "hidden": 4})
	billingsvc.SetGroupGroupRatios(map[string]map[string]float64{
...
func TestPublicUserGroupsRouteContract(t *testing.T) {
	handler, _, userID := setupAuthMiscRoutes(t, roles.RoleRootUser)
...
func TestSelfUserGroupsRouteContract(t *testing.T) {
	handler, do, userID := setupAuthMiscRoutes(t, roles.RoleRootUser)
	configureAuthMiscGroups(t, userID)
...
func TestEnableTwoFARouteContract(t *testing.T) {
	handler, do, userID := setupAuthMiscRoutes(t, roles.RoleRootUser)
	require.NoError(t, model.DB.AutoMigrate(&model.TwoFABackupCode{}))
	secret, _, err := authsvc.GenerateTwoFASecret(userID, "route-enable-user")
	require.NoError(t, err)
	code, err := totp.GenerateCode(secret, time.Now())
	require.NoError(t, err)
	...
	enabled, err := authsvc.TwoFAStatusChecked(userID)
	require.NoError(t, err)
...
func TestEnableTwoFARouteReturnsRecoveryMaterialWhenDatabaseDropsAfterCommit(t *testing.T) {
	_, do, userID := setupAuthMiscRoutes(t, roles.RoleRootUser)
	require.NoError(t, model.DB.AutoMigrate(&model.TwoFABackupCode{}))
	secret, _, err := authsvc.GenerateTwoFASecret(userID, "post-commit-outage-user")
	require.NoError(t, err)
	code, err := totp.GenerateCode(secret, time.Now())
	require.NoError(t, err)
...
func TestGetAllModelsMetaRouteContract(t *testing.T) {
	handler, do, _ := setupAuthMiscRoutes(t, roles.RoleRootUser)
	...
	commonUser := model.User{
		Username: "models-common-user", Password: "unused", Role: roles.RoleCommonUser,
		Status: model.UserStatusEnabled, Group: userssvc.GroupDefault, AuthVersion: 1,
	}
	require.NoError(t, model.DB.Create(&commonUser).Error)
	_, commonAccess, _, err := authsvc.CompleteLogin(&commonUser, "127.0.0.1", "ua", "test")
	require.NoError(t, err)
