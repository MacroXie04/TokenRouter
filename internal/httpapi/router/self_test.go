...
func setupSelfTest(t *testing.T) (http.Handler, func(method, path, body string) *httptest.ResponseRecorder, int, string) {
	...
	passwordHash, err := cryptoutil.PasswordHash("old-password")
	require.NoError(t, err)
...
func TestUpdateSelfPasswordRotatesAuthAndKeepsCurrentSession(t *testing.T) {
	...
	otherSID, _, _, err := authsvc.CompleteLogin(&user, "127.0.0.2", "other", "test")
	require.NoError(t, err)
	...
	assert.Equal(t, authsvc.SessionStatusActive, current.Status)
	...
	assert.Equal(t, authsvc.SessionStatusRevoked, other.Status)
...
func TestUpdateSelfRejectsUnverifiedEmailAndBadPasswordProof(t *testing.T) {
	...
	assert.True(t, cryptoutil.PasswordVerify("old-password", user.Password))
	assert.EqualValues(t, 1, user.AuthVersion)
}

func TestUpdateSelfPersistsBoundedLanguageAndSidebarPreferences(t *testing.T) {
	_, do, userID, _ := setupSelfTest(t)
	_, err := userssvc.UpdateUserBillingPreference(userID, userssvc.BillingPreferenceWalletFirst)
	require.NoError(t, err)
	...
	assert.Equal(t, userssvc.BillingPreferenceWalletFirst, settings["billing_preference"],
	...
	assert.Equal(t, userssvc.BillingPreferenceWalletFirst, visibleSettings["billing_preference"])
...
func TestSelfAffAndTransfer(t *testing.T) {
	...
	require.NoError(t, setting.UpdateOptions(map[string]string{
		...
		setting.PaymentComplianceTermsVersionOption: billingsvc.CurrentPaymentComplianceTermsVersion,
	}))
...
func TestUpdateUserSettingAndReminderOverride(t *testing.T) {
	...
	var got model.User
	require.NoError(t, model.DB.First(&got, userId).Error)
	assert.Equal(t, 5000, userssvc.QuotaWarningThreshold(&got))
}
...
func TestRevokeOtherSessions(t *testing.T) {
	...
	var user model.User
	require.NoError(t, model.DB.First(&user, userId).Error)
	otherSID, _, _, err := authsvc.CompleteLogin(&user, "127.0.0.1", "ua", "test")
	require.NoError(t, err)
	...
	assert.Equal(t, authsvc.SessionStatusActive, bySID[sid].Status, "current session must survive")
	assert.Equal(t, authsvc.SessionStatusRevoked, bySID[otherSID].Status)
...
func TestTelegramBindFlow(t *testing.T) {
	...
	require.NoError(t, model.DB.Where("token_hash = ?", cryptoutil.SHA256Hex(token)).First(&telegramFlow).Error)
