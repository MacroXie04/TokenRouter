...
func signedWaffoCreateResponse(t *testing.T, keys waffoTestKeys, request billingsvc.WaffoCheckoutRequest, acquiringID, paymentURL string) *http.Response {
	t.Helper()
	action, err := jsonutil.Marshal(map[string]string{"actionType": "WEB", "webUrl": paymentURL})
	require.NoError(t, err)
...
func waffoWebhookPayload(t *testing.T, order model.TopUp, status, amount, currency, merchantID, acquiringID string) []byte {
	...
	body, err := jsonutil.Marshal(map[string]any{
		...
		"result": map[string]any{
			...
			"paymentInfo":  map[string]any{"productName": billingsvc.WaffoProductOneTimePayment},
		},
	})
	require.NoError(t, err)
	return body
...
func TestWaffoHTTPWireSnapshotAndRetryableSettlementEndToEnd(t *testing.T) {
	...
	assert.Equal(t, billingsvc.PaymentProviderWaffo, order.PaymentProvider)
	...
	assert.JSONEq(t, `{"message":"success"}`, recorder.Body.String())
	require.NoError(t, model.DB.First(&user, userID).Error)
	assert.Equal(t, 1000+10*quotamath.QuotaPerUnit, user.Quota)
	var logs int64
...
func TestWaffoCheckoutFailuresRemainDurableOrFailTerminally(t *testing.T) {
	...
	restore := commerce.SetWaffoTransportForTesting(waffoRoundTripFunc(func(request *http.Request) (*http.Response, error) {
		...
		require.NoError(t, jsonutil.Unmarshal(body, &checkout))
		switch mode {
		case "transport":
	...
	for _, currentMode := range []string{"transport", "rejected", "business-rejected", "unknown-status", "invalid-response-signature", "unsafe-url", "oversized"} {
		...
		if currentMode == "rejected" || currentMode == "business-rejected" {
			...
			assert.Equal(t, billingsvc.TopUpStatusPending, order.Status)
			assert.Equal(t, billingsvc.WaffoReconciliationCreate, order.ReconciliationState)
		}
	}
...
func TestWaffoWebhookSignatureBoundsAndTerminalStatuses(t *testing.T) {
	...
	require.NoError(t, err)
	order, _, err := billingsvc.CreateBoundWaffoTopUp(billingsvc.WaffoCheckoutSpec{
	...
	assert.JSONEq(t, `{"message":"success"}`, recorder.Body.String())
	require.NoError(t, model.DB.First(&stored, order.Id).Error)
	assert.Equal(t, billingsvc.TopUpStatusFailed, stored.Status)
	var user model.User
	require.NoError(t, model.DB.First(&user, userID).Error)
	assert.Equal(t, 1000, user.Quota)
	...
	unknown, err := jsonutil.Marshal(map[string]any{"eventType": "REFUND_NOTIFICATION"})
	require.NoError(t, err)
...
func TestWaffoClientBoundaryAndTopUpInfoAreInjectedAndAdvertised(t *testing.T) {
	...
	for _, raw := range methods {
		...
		if method["type"] == billingsvc.PaymentMethodWaffo {
			found = true
		}
	}
	assert.True(t, found)
