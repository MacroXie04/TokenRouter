...
func GetRelayModelPolicy(c *gin.Context) policy.TokenModelPolicy {
	if value, ok := c.Get(relayModelPolicyContext); ok {
		if policy, ok := value.(policy.TokenModelPolicy); ok {
			return policy
		}
	}
	return policy.NewTokenModelPolicy(GetRelayToken(c))
}
...
func RequireJimengModel() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if requestContext.Action == jimeng.FetchAction {
			persistedModel, found, err := jimengTaskOriginModel(requestctx.GetUserId(c), requestContext.Request.TaskID)
			if err != nil {
