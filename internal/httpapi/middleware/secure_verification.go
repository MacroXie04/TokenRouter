...
func SecureVerificationRequired() gin.HandlerFunc {
	return func(c *gin.Context) {
		if !RequireSecurityProof(c, auth.SecurityProofScopeChannelKeyRead,
...
func GetSessionAuthIdentity(c *gin.Context) (auth.SessionIdentity, bool) {
...
func RequireSecurityProof(c *gin.Context, requiredScope string, allowedMethods []string) bool {
	...
	if _, err := auth.VerifySecurityProof(raw, identity, requiredScope, allowedMethods); err != nil {
		switch {
		case errors.Is(err, auth.ErrSecurityProofExpired):
			securityProofError(c, "SECURITY_PROOF_EXPIRED", "安全验证已过期")
		case errors.Is(err, auth.ErrProofScope):
			securityProofError(c, "SECURITY_PROOF_SCOPE_MISMATCH", "安全验证范围不匹配")
		case errors.Is(err, auth.ErrProofMethod):
...
// ("<sid>.<refresh>") — shared with internal/httpapi/handlers/accounts/session.go.
