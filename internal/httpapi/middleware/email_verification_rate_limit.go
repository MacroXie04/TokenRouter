...
var emailVerificationFallback = cache.NewRateLimiter(
...
func EmailVerificationRateLimit() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		count, err := cache.Store.Incr(ctx, storeKey)
		if err == nil {
			if count == 1 {
				_ = cache.Store.Expire(ctx, storeKey, emailVerificationWindowSeconds*time.Second)
			}
			if count <= emailVerificationMaxRequests {
				c.Next()
				return
			}
			ttl, _ := cache.Store.TTL(ctx, storeKey)
