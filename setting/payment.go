...
const (
	...
	MaxTopUpReferenceAmount  = common.MaxQuota / int64(common.QuotaPerUnit)
)
...
func parsePayMethods(raw string) ([]map[string]string, error) {
	...
	if err := common.UnmarshalJsonStr(raw, &methods); err != nil || methods == nil || len(methods) > maxPayMethods {
		return nil, fmt.Errorf("%w: %s", ErrInvalidPaymentSetting, PayMethodsOption)
	}
...
func StripeConfigured() bool {
	...
	return validSecret && strings.TrimSpace(common.GetEnv("STRIPE_WEBHOOK_SECRET", "")) != ""
}
...
func buildCreemConfig(options map[string]string) (CreemConfig, error) {
	...
	if err := common.Unmarshal([]byte(rawProducts), &wires); err != nil || len(wires) > maxCreemProducts {
		return CreemConfig{}, fmt.Errorf("%w: %s", ErrInvalidPaymentSetting, CreemProductsOption)
	}
	...
	for _, wire := range wires {
		...
		if productID == "" || productID != strings.TrimSpace(productID) || len(productID) > maxCreemProductIDBytes ||
			...
			len(name) > maxCreemProductName || strings.ContainsRune(name, '\x00') || wire.Quota <= 0 || wire.Quota > common.MaxQuota {
...
func minimumTopUpOption(key string, defaultValue int64) (int64, error) {
	...
	return minimum * int64(common.QuotaPerUnit), nil
}
