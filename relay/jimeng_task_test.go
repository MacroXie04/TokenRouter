...
func TestJimengEncryptionKeyringSupportsRotationAndLegacyCiphertext(t *testing.T) {
	...
	t.Setenv("SESSION_SECRET", oldSessionSecret)
	legacyCiphertext, err := common.EncryptByAES("legacy-provider-secret")
	require.NoError(t, err)
...
func TestJimengProviderPayloadPersistenceIsCanonicalAndPrivate(t *testing.T) {
	...
	assert.Contains(t, string(stored), common.NormalizeProviderCorrelationID("provider-request-id"))
	assert.Contains(t, string(stored), `"status":"done"`)

	privateData := jimengTaskPrivateData{
		RelayReservationID: common.BestEffortUUID(), UpstreamTaskID: "provider-task-id",
	}
...
func TestJimengV2PrivateDataFailsClosedForLegacyJSONReaders(t *testing.T) {
	privateData := jimengTaskPrivateData{
		RelayReservationID: common.BestEffortUUID(), BillingSource: service.BillingSourceWallet,
	...
	require.Error(t, common.UnmarshalJsonStr(encoded, &legacy),
	...
	legacyJSON, err := common.Marshal(privateData)
	require.NoError(t, err)
...
func TestJimengDurableTextBoundsPreventMySQLRecoveryDeadEnds(t *testing.T) {
	...
	combinedPrivate := jimengTaskPrivateData{
		...
		RelayReservationID: common.BestEffortUUID(),
	...
	for low < high {
		...
		candidateRaw, marshalErr := common.Marshal(combinedResult)
	...
	combinedRaw, err := common.Marshal(combinedResult)
	require.NoError(t, err)
...
func TestJimengRecoveryCleanupPrioritizesOldAndTerminalRecords(t *testing.T) {
	...
	require.NoError(t, db.Create(&model.JimengTaskOperation{
		TaskID: activeTaskID, ReservationID: common.BestEffortUUID(), UserID: 1, ChannelID: 1,
	...
	require.NoError(t, db.Create(&model.JimengTaskOperation{
		TaskID: terminalTaskID, ReservationID: common.BestEffortUUID(), UserID: 1, ChannelID: 1,
...
func TestPreparedJimengRecoveryCannotAssumeProviderAcceptance(t *testing.T) {
	privateJSON, err := marshalJimengTaskPrivateData(jimengTaskPrivateData{
		BillingSource: service.BillingSourceWallet, FundingReserved: 5,
...
func TestLegacyJimengSubscriptionSettlementUsesCapturedFundingEpoch(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			fundingRequestID := "legacy-" + common.BestEffortUUID()
			preConsume := model.SubscriptionPreConsumeRecord{
				...
				Status: service.SubscriptionPreConsumeStatusConsumed,
			}
			require.NoError(t, db.Create(&preConsume).Error)
			privateData := jimengTaskPrivateData{
				UpstreamTaskID: "legacy-provider-id", BillingSource: service.BillingSourceSubscription,
			...
			assert.Equal(t, service.SubscriptionPreConsumeStatusSettled, preConsume.Status,
