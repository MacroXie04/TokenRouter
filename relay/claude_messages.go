...
func RelayClaudeMessages(c *gin.Context) {
	...
	defer cancel()
	body, err := common.ReadAllLimited(c.Request.Body, maxClaudeMessagesRequestBodyBytes)
	if err != nil {
	...
	if err := relayAndSettleWithDispatch(c, info, dispatchClaudeUpstream); err != nil {
		common.SysError("Claude relay lifecycle failed: " + err.Error())
		return
	}
}
...
func dispatchClaudeUpstream(c *gin.Context, info *RelayInfo) (*protocolkit.Usage, error) {
	...
	switch {
	case channelType == constant.ChannelTypeAnthropic:
		usage, err = sendClaudeNative(c, info, base, apiKey)
	case channelType == constant.ChannelTypeAws:
	...
	case channelType == constant.ChannelTypeMoonshot:
		usage, err = sendClaudeViaMoonshot(c, info, base, apiKey)
	case channelType == constant.ChannelTypeMiniMax:
		usage, err = sendClaudeViaMiniMax(c, info, base, apiKey)
	case channelType == constant.ChannelTypeZhipuV4:
		usage, err = sendClaudeViaZhipuV4(c, info, base, apiKey)
	case channelType == constant.ChannelTypeBaiduV2:
		usage, err = sendClaudeViaOpenAI(c, info, base, apiKey)
	case channelType == constant.ChannelTypeVolcEngine:
		usage, err = sendClaudeViaVolcEngine(c, info, base, apiKey)
	case channelType == constant.ChannelTypeAli:
	...
	case constant.IsOpenAICompatibleChannelType(channelType):
		usage, err = sendClaudeViaOpenAI(c, info, base, apiKey)
	default:
...
func sendClaudeViaAWS(c *gin.Context, info *RelayInfo, base, apiKey string) (*protocolkit.Usage, error) {
	...
	adaptor := GetAdaptor(constant.ChannelTypeAws)
	if adaptor == nil {
...
func sendClaudeViaZhipuV4(c *gin.Context, info *RelayInfo, base, apiKey string) (*protocolkit.Usage, error) {
	...
	adaptor := GetAdaptor(constant.ChannelTypeZhipuV4)
	if adaptor == nil {
...
func sendClaudeViaAli(c *gin.Context, info *RelayInfo, base, apiKey string) (*protocolkit.Usage, error) {
	...
	adaptor := GetAdaptor(constant.ChannelTypeAli)
	if adaptor == nil {
...
func sendClaudeViaMoonshot(c *gin.Context, info *RelayInfo, base, apiKey string) (*protocolkit.Usage, error) {
	...
	adaptor := GetAdaptor(constant.ChannelTypeMoonshot)
	if adaptor == nil {
...
func sendClaudeViaMiniMax(c *gin.Context, info *RelayInfo, base, apiKey string) (*protocolkit.Usage, error) {
	...
	adaptor := GetAdaptor(constant.ChannelTypeMiniMax)
	if adaptor == nil {
...
func sendClaudeViaAdvancedCustom(c *gin.Context, info *RelayInfo) (*protocolkit.Usage, error) {
	...
	call, err := callAdvancedCustomNative(c, info, constant.RelayFormatClaude, nativeBody, info.Request)
	if err != nil {
		return nil, err
	}
...
func sendClaudeViaOpenAI(c *gin.Context, info *RelayInfo, base, apiKey string) (*protocolkit.Usage, error) {
	...
	defer resp.Body.Close()
	if constant.ChannelType(info.Channel.Type) == constant.ChannelTypeAli &&
...
func claudeStreamFromOpenAI(c *gin.Context, resp *http.Response, info *RelayInfo) (*protocolkit.Usage, error) {
	...
	msgID := "msg_" + common.BestEffortRandomAlphanumeric(24)
