...
func init() {
	service.RegisterAsyncTaskReconciler(reconcileAsyncKlingTasks)
	service.RegisterAsyncTaskPromoter(PromoteKlingTaskRecoveryJournalsContext)
}
...
func RelayKlingTask(c *gin.Context) {
	raw, err := common.ReadAllLimited(c.Request.Body, kling.MaxRequestBodyBytes)
	if err != nil {
	...
	if err != nil || !enabled {
		if err != nil {
			common.SysError("resolve Kling pricing: " + err.Error())
		}
	...
	if err != nil {
		...
		if service.IsSubscriptionFundingErr(err) || errors.Is(err, service.ErrInsufficientQuota) ||
			...
			common.SysError("create atomic Kling reservation: " + err.Error())
		}
	...
	defer func() {
		...
		if cleanupErr := refundKlingTask(&task, reservation, nil,
			...
			common.SysError("Kling reservation cleanup failed for " + task.TaskID + ": " + cleanupErr.Error())
		}
	}()
	if err := markKlingTaskDispatching(&task, reservation); err != nil {
		common.SysError("mark Kling dispatch: " + err.Error())
	...
	if submitErr != nil {
		if !kling.SubmitWasDispatched(submitErr) || definitiveKlingRejection(submitErr) {
			if refundErr := refundKlingTask(&task, reservation, nil,
				...
				common.SysError("persist rejected Kling task " + task.TaskID + ": " + refundErr.Error())
		...
		if persistErr := markKlingAmbiguousDispatch(&task, reservation.ReservationID(), klingUnknownDispatchReason); persistErr != nil {
			common.SysError("persist ambiguous Kling task " + task.TaskID + ": " + persistErr.Error())
	...
	if journalErr != nil {
		common.SysError("persist accepted Kling recovery journal " + task.TaskID + ": " + journalErr.Error())
	}
	var commitErr error
	...
	if commitErr != nil {
		common.SysError("commit accepted Kling task " + task.TaskID + ": " + commitErr.Error())
		...
		if fallbackErr != nil {
			common.SysError("persist accepted Kling fallback " + task.TaskID + ": " + fallbackErr.Error())
			if journalErr != nil {
				data["recovery_degraded"] = true
			}
		} else if cleanupErr := removeKlingRecoveryJournal(task.TaskID); cleanupErr != nil {
			common.SysError("remove accepted Kling recovery journal " + task.TaskID + ": " + cleanupErr.Error())
		}
		writeKlingTaskError(c, http.StatusAccepted, "task_commit_pending",
	...
	if task.Status == model.TaskStatusSuccess {
		if deliverErr := service.DeliverAuditLogOutboxEvent(klingAuditEventID(reservation.ReservationID())); deliverErr != nil {
			common.SysError("deliver Kling consume audit: " + deliverErr.Error())
		}
	}
...
func RelayKlingTaskFetch(c *gin.Context) {
	...
	result := model.DB.Where("task_id = ? AND user_id = ? AND platform = ?", taskID,
		common.GetUserId(c), klingTaskPlatform).Limit(2).Find(&tasks)
	if result.Error != nil {
...
func selectKlingChannel(groups []string, modelName string) (*model.Channel, string, error) {
	...
	for attempt := 0; attempt < 64; attempt++ {
		...
		if channel != nil && channel.Id > 0 && constant.ChannelType(channel.Type) == constant.ChannelTypeKling {
