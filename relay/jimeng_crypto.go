...
func jimengEncryptionKeyring() ([]jimengEncryptionKey, error) {
	...
	if _, configured := seen[jimengSessionKeyID]; !configured {
		keys = append(keys, jimengEncryptionKey{id: jimengSessionKeyID, secret: common.SessionSecret()})
	}
	return keys, nil
}
...
func jimengDecrypt(encoded string) (string, error) {
	...
	if plaintext, legacyErr := common.DecryptByAES(encoded); legacyErr == nil {
