...
func newSunoTaskFixture(t *testing.T, upstream *httptest.Server) sunoTaskFixture {
	...
	require.NoError(t, service.UpdateGroupRatioOption(`{"default":1,"vip":1}`))
	...
	user := model.User{
		Username: "suno-owner", Password: "x", Role: constant.RoleCommonUser,
	...
	token := model.Token{
		...
		Status: service.TokenStatusEnabled, RemainQuota: 2_000_000,
	}
	require.NoError(t, db.Create(&token).Error)
	weight := uint(1)
	channel := model.Channel{
		Name: "suno-mock", Type: int(constant.ChannelTypeSunoAPI), Key: "upstream-suno-key",
...
func createPreparedSunoTaskForRecovery(
	...
	require.True(t, ok)
	pricing, enabled, err := service.ResolveReferenceAsyncTaskBillingPlan(modelName, "default", "default")
	require.NoError(t, err)
...
func createSunoPollManualReviewFixture(
	t *testing.T,
) (sunoTaskFixture, *model.Task, *service.RelayQuotaReservation, model.TaskOperation) {
	t.Helper()
...
func TestRetrySunoTaskManualReviewRejectsAmbiguousOrCrossPlatformState(t *testing.T) {
	...
	t.Run("exact platform allowlist", func(t *testing.T) {
		...
		assert.Nil(t, result)
		assert.ErrorIs(t, err, service.ErrRelayQuotaReviewNotFound)
		var eventCount int64
		require.NoError(t, fixture.db.Model(&model.RelayQuotaReservationReviewEvent{}).
			Where("reservation_id = ?", reservation.ReservationID()).Count(&eventCount).Error)
		assert.Zero(t, eventCount)
...
func TestSunoFetchOwnerGroupAndModelAuthorization(t *testing.T) {
	...
	otherToken := model.Token{UserId: other.Id, Key: "sk-other-suno", Name: "other", Status: service.TokenStatusEnabled, RemainQuota: 1000}
	...
	modelToken := model.Token{UserId: fixture.user.Id, Key: "sk-suno-lyrics-only", Name: "limited",
		Status: service.TokenStatusEnabled, RemainQuota: 1000, ModelLimitsEnabled: true, ModelLimits: "suno_lyrics"}
	...
	groupToken := model.Token{UserId: fixture.user.Id, Key: "sk-suno-vip", Name: "vip",
		Status: service.TokenStatusEnabled, RemainQuota: 1000, Group: "vip"}
...
func TestSunoPreparedRecoveryRefundAndLeaseFence(t *testing.T) {
	...
	for index := 0; index < cap(wins); index++ {
		go func() {
			_, ok, _ := claimSunoTaskOperation(context.Background(), &candidate, common.NowTimestamp())
			wins <- ok
		}()
	}
	winnerCount := 0
