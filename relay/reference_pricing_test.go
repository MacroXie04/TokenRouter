...
func TestOrdinaryRelayReferenceFixedPriceUsesSnapshotAndWritesExactAccounting(t *testing.T) {
	...
	assert.Equal(t, service.BillingModeReference, other["billing_mode"])
	...
	nextPlan, enabled, err := service.ResolveOrdinaryReferenceBillingPlan(
...
func TestReferencePricingLogNeverMixesHotReloadedGroupRatios(t *testing.T) {
	...
	plan, enabled, err := service.ResolveOrdinaryReferenceBillingPlan("accounting-model", "default", "vip")
	require.NoError(t, err)
	require.True(t, enabled)
	...
	other := buildLogOther(nil, &RelayInfo{
		...
		ReferencePricing: map[string]service.ReferenceBillingPlan{"vip": plan},
	}, nil)
