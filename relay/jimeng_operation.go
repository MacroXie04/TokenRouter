...
func createJimengReservedTask(
	...
	return service.NewRelayQuotaReservationWithPersistence(
...
// unknown submission.
func markJimengTaskSafeToRefund(task *model.Task, reservation *service.RelayQuotaReservation) error {
...
func settleJimengAcceptedTask(
	...
	err := reservation.SettleWithChannelAndPersistence(
		...
		func(tx *gorm.DB) error {
			...
			return service.EnqueueAuditLogTx(tx, auditEntry)
		},
	)
	if err != nil {
...
func newJimengConsumeAudit(
	...
	if privateData.BillingSource == service.BillingSourceSubscription && privateData.FundingReserved > 0 {
	...
	if len(upstreamRaw) > 0 && common.Unmarshal(upstreamRaw, &accepted) == nil {
		upstreamRequestID = common.NormalizeProviderCorrelationID(accepted.RequestFingerprint)
		if upstreamRequestID == "" {
			upstreamRequestID = common.NormalizeProviderCorrelationID(accepted.RequestID)
		}
	}
	return &model.Log{
...
func init() {
	service.RegisterJimengTaskPromoter(PromoteJimengTaskRecoveryContext)
	service.RegisterJimengTaskReconciler(reconcileJimengTaskOperationsDatabaseContext)
}
...
func jimengRecoveryPassDuration() time.Duration {
	passSeconds := common.GetEnvInt("JIMENG_RECOVERY_PASS_SECONDS", jimengDefaultPassSeconds)
...
func renewJimengTaskOperationLease(operation *model.JimengTaskOperation, floor int64) (int64, error) {
	...
	if result.RowsAffected != 1 {
		return 0, service.ErrRelayQuotaReservationBusy
	}
		...
		if journalErr == nil {
			...
			if envelope.Status == model.TaskStatusFailure {
				...
				if removeErr := removeJimengRecovery(task.TaskID); removeErr != nil {
					common.SysError("Jimeng recovery journal cleanup remains pending for " + task.TaskID)
				}
				return nil
			}
...
func pollClaimedJimengTask(
	...
	maxPollDays := common.GetEnvInt("JIMENG_MAX_POLL_DAYS", jimengDefaultMaxPollDays)
...
func failClaimedJimengOperation(operation *model.JimengTaskOperation, now int64, cause error) error {
	...
	if result.RowsAffected != 1 {
		return errors.Join(safeCause, service.ErrRelayQuotaReservationBusy)
	}
	if manualReview {
		common.SysError("Jimeng recovery requires manual review for " + operation.TaskID + " (" + safeError + ")")
	} else {
		common.SysError("Jimeng recovery failed for " + operation.TaskID + " (" + safeError + ")")
	}
	return safeCause
}
...
func releaseClaimedJimengOperation(operation *model.JimengTaskOperation, now int64, cause error) error {
	...
	if result.RowsAffected != 1 {
		return errors.Join(cause, service.ErrRelayQuotaReservationBusy)
	}
	return cause
}
