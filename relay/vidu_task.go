...
func init() {
	service.RegisterAsyncTaskPromoter(PromoteViduTaskRecoveryJournalsContext)
	service.RegisterAsyncTaskReconciler(reconcileAsyncViduTasks)
}
...
func RelayViduTask(c *gin.Context) {
	raw, err := common.ReadAllLimited(c.Request.Body, vidu.MaxRequestBodyBytes)
	if err != nil {
	...
	if err != nil || !enabled {
		if err != nil {
			common.SysError("resolve Vidu pricing: " + err.Error())
		}
	...
	if err != nil {
		...
		if service.IsSubscriptionFundingErr(err) || errors.Is(err, service.ErrInsufficientQuota) ||
			...
			common.SysError("create atomic Vidu reservation: " + err.Error())
		}
	...
	defer func() {
		...
		if cleanupErr := refundRejectedViduTask(&task, reservation, "Vidu submission stopped before provider dispatch"); cleanupErr != nil {
			common.SysError("Vidu reservation cleanup failed for " + task.TaskID + ": " + cleanupErr.Error())
		}
	}()
	if err := markViduTaskDispatching(&task, reservation); err != nil {
		common.SysError("mark Vidu dispatch: " + err.Error())
	...
	if submitErr != nil {
		if !vidu.SubmitWasDispatched(submitErr) || definitiveViduRejection(submitErr) {
			if refundErr := refundRejectedViduTask(&task, reservation, "Vidu provider rejected submission"); refundErr != nil {
				common.SysError("persist rejected Vidu task " + task.TaskID + ": " + refundErr.Error())
		...
		if persistErr := markViduAmbiguousDispatch(&task, reservation.ReservationID(), viduUnknownDispatchReason); persistErr != nil {
			common.SysError("persist ambiguous Vidu task " + task.TaskID + ": " + persistErr.Error())
	...
	if provider == nil || strings.TrimSpace(provider.ProviderTaskID) == "" {
		if persistErr := markViduAmbiguousDispatch(&task, reservation.ReservationID(), viduUnknownDispatchReason); persistErr != nil {
			common.SysError("persist Vidu missing-id submission " + task.TaskID + ": " + persistErr.Error())
		}
		writeViduTaskError(c, http.StatusAccepted, "submit_outcome_unknown", viduUnknownDispatchReason,
			gin.H{"task_id": task.TaskID, "status": "unknown"})
		return
	...
	if journalErr != nil {
		common.SysError("persist accepted Vidu recovery journal " + task.TaskID + ": " + journalErr.Error())
	}
	if err := settleAcceptedViduTask(&task, reservation, provider.ProviderTaskID, model.TaskOperationDispatching, ""); err != nil {
		common.SysError("settle accepted Vidu task " + task.TaskID + ": " + err.Error())
		...
		if fallbackErr != nil {
			common.SysError("persist accepted Vidu fallback " + task.TaskID + ": " + fallbackErr.Error())
			if journalErr != nil {
				data["recovery_degraded"] = true
			}
		} else if cleanupErr := removeViduRecoveryJournal(task.TaskID); cleanupErr != nil {
			common.SysError("remove accepted Vidu recovery journal " + task.TaskID + ": " + cleanupErr.Error())
		}
		writeViduTaskError(c, http.StatusAccepted, "task_commit_pending",
	...
	if deliverErr := service.DeliverAuditLogOutboxEvent(viduAuditEventID(reservation.ReservationID())); deliverErr != nil {
		common.SysError("deliver Vidu consume audit: " + deliverErr.Error())
	}
...
func RelayViduTaskFetch(c *gin.Context) {
	...
	if err := model.DB.Where("task_id = ? AND user_id = ? AND platform = ?", taskID,
		common.GetUserId(c), viduTaskPlatform).Limit(2).Find(&tasks).Error; err != nil {
...
func relayViduTaskContent(c *gin.Context, task *model.Task) {
	...
	if _, err := io.Copy(c.Writer, io.LimitReader(response.Body, maxContentBytes)); err != nil {
		common.SysError("stream Vidu content for " + task.TaskID + ": " + err.Error())
	}
}
...
func selectViduChannel(groups []string, modelName string) (*model.Channel, string, error) {
	...
	for attempt := 0; attempt < 64; attempt++ {
		...
		if channel != nil && channel.Id > 0 && constant.ChannelType(channel.Type) == constant.ChannelTypeVidu {
