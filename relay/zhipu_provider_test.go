...
func configureZhipuIntegration(t *testing.T, upstreamURL string, channelType constant.ChannelType, upstreamModel string) (string, int, model.Channel) {
	t.Helper()
	...
	if channelType == constant.ChannelTypeZhipu {
...
func TestZhipuLegacyAndV4OrdinaryRelayWireAndSettlementEndToEnd(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "legacy chat", channelType: constant.ChannelTypeZhipu,
		...
		{
			name: "v4 chat", channelType: constant.ChannelTypeZhipuV4,
		...
		{
			name: "v4 embedding", channelType: constant.ChannelTypeZhipuV4,
...
func TestZhipuLegacyAndV4StreamingWireUsageAndSettlementEndToEnd(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			name: "legacy", channelType: constant.ChannelTypeZhipu, upstreamModel: "chatglm_pro",
		...
		{
			name: "v4", channelType: constant.ChannelTypeZhipuV4, upstreamModel: "glm-4-flash",
...
func TestZhipuV4ImageDownloadNormalizationAndSettlementEndToEnd(t *testing.T) {
	...
	key, userID, channel := configureZhipuIntegration(t, upstream.URL, constant.ChannelTypeZhipuV4, "cogview-4")
...
func TestZhipuV4NativeClaudeNonStreamAndStreamSettlementEndToEnd(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			key, userID := setupClaudeRelay(t, upstream.URL, int(constant.ChannelTypeZhipuV4), "gpt-provider-contract")
			var channel model.Channel
			require.NoError(t, model.DB.Where("name = ?", "mock").First(&channel).Error)
			require.NoError(t, model.DB.Model(&channel).Updates(map[string]any{
...
func TestZhipuErrorsRefundAndUnsupportedModesNeverContactUpstream(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			name: "mapped v4 provider error", channelType: constant.ChannelTypeZhipuV4, upstreamModel: "glm-4-plus",
		...
		{
			name: "malformed legacy credential", channelType: constant.ChannelTypeZhipu, upstreamModel: "chatglm_pro",
			path: "/v1/chat/completions", body: `{"model":"gpt-provider-contract","messages":[{"role":"user","content":"hello"}]}`,
			wantStatus: http.StatusInternalServerError, invalidCredential: true,
		},
		{
			name: "legacy embedding unsupported", channelType: constant.ChannelTypeZhipu, upstreamModel: "chatglm_pro",
			path: "/v1/embeddings", body: `{"model":"gpt-provider-contract","input":"hello"}`,
			wantStatus: http.StatusInternalServerError,
		},
		{
			name: "v4 image edit unsupported", channelType: constant.ChannelTypeZhipuV4, upstreamModel: "cogview-4",
