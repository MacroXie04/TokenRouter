...
func RetryHailuoTaskManualReview(reservationID string, operatorUserID int) (*HailuoTaskManualReviewRetryResult, error) {
	...
	err = model.DB.Transaction(func(tx *gorm.DB) error {
		...
		if len(tasks) != 1 {
			return fmt.Errorf("%w: Hailuo task identity is unavailable or ambiguous", service.ErrRelayQuotaReviewUnsafeRetry)
		}
		task := &tasks[0]
		...
		if record.Status != model.RelayQuotaReservationStatusSettled ||
			...
			return fmt.Errorf("%w: Hailuo task is not awaiting a polling retry", service.ErrRelayQuotaReviewInvalidState)
		}
...
func validateActiveHailuoTaskPollReview(record *model.RelayQuotaReservationRecord, task *model.Task,
	...
	if record == nil || task == nil || operation == nil ||
		...
		return fmt.Errorf("%w: Hailuo polling review tuple is inconsistent", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || privateData.RelayReservationID != record.ReservationID || privateData.SettlementPending ||
		...
		return fmt.Errorf("%w: Hailuo recovery metadata does not match accounting", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	if _, err := validateHailuoTaskPricingSnapshot(task, privateData, record.ActualQuota); err != nil {
		return fmt.Errorf("%w: Hailuo pricing snapshots do not match accounting", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	settled, err := privateData.Pricing.SettlementQuota(0)
	if err != nil || settled != record.ActualQuota {
		return fmt.Errorf("%w: Hailuo pricing snapshot does not match accounting", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	baseURL, err := hailuo.EffectiveBaseURL(privateData.ChannelBaseURL)
	if err != nil || baseURL != privateData.ChannelBaseURL {
		return fmt.Errorf("%w: Hailuo provider endpoint is invalid", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || strings.TrimSpace(key) == "" || strings.ContainsAny(key, "\r\n") {
		return fmt.Errorf("%w: Hailuo channel credential is not recoverable", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	for _, encrypted := range []string{operation.EncryptedProviderTaskID, privateData.EncryptedProviderTaskID} {
		...
		if err != nil || !validHailuoRecoveryOpaqueID(providerID, hailuo.MaxProviderTaskIDBytes) {
			return fmt.Errorf("%w: Hailuo provider id is not recoverable", service.ErrRelayQuotaReviewUnsafeRetry)
		}
	}
	return nil
}
