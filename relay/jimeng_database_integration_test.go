...
func TestJimengExternalDatabaseAtomicLifecycle(t *testing.T) {
	...
	token := model.Token{
		...
		Status: service.TokenStatusEnabled, RemainQuota: 100,
	}
	require.NoError(t, model.DB.Create(&token).Error)
	channel := model.Channel{
		Name: "jimeng-db-int-" + suffix, Key: "access|secret",
		Type: int(constant.ChannelTypeJimeng), Status: constant.ChannelStatusEnabled,
	}
	...
	privateData.EncryptedChannelKey, err = common.EncryptByAES(channel.Key)
	require.NoError(t, err)
