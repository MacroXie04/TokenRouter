...
func RetryGeminiVeoTaskManualReview(reservationID string, operatorUserID int) (*GeminiVeoTaskManualReviewRetryResult, error) {
	...
	err = model.DB.Transaction(func(tx *gorm.DB) error {
		...
		if len(tasks) != 1 {
			return fmt.Errorf("%w: GeminiVeo task identity is unavailable or ambiguous", service.ErrRelayQuotaReviewUnsafeRetry)
		}
		task := &tasks[0]
		...
		if record.Status != model.RelayQuotaReservationStatusSettled ||
			...
			return fmt.Errorf("%w: GeminiVeo task is not awaiting a polling retry", service.ErrRelayQuotaReviewInvalidState)
		}
...
func validateActiveGeminiVeoTaskPollReview(record *model.RelayQuotaReservationRecord, task *model.Task,
	...
	if record == nil || task == nil || operation == nil ||
		...
		return fmt.Errorf("%w: GeminiVeo polling review tuple is inconsistent", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || privateData.RelayReservationID != record.ReservationID || privateData.SettlementPending ||
		...
		return fmt.Errorf("%w: GeminiVeo recovery metadata does not match accounting", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	if _, err := validateGeminiVeoTaskPricingSnapshot(task, privateData, record.ActualQuota); err != nil {
		return fmt.Errorf("%w: GeminiVeo pricing snapshots do not match accounting", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	properties, propertiesErr := decodeGeminiVeoTaskProperties(task.Properties)
	if propertiesErr != nil {
		return fmt.Errorf("%w: Gemini Veo pricing properties are invalid", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || settled != record.ActualQuota {
		return fmt.Errorf("%w: GeminiVeo pricing snapshot does not match accounting", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if !ok || descriptor.Family != properties.Family ||
		...
		return fmt.Errorf("%w: GeminiVeo provider endpoint is invalid", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || credential == "" || len(credential) > descriptor.MaxCredentialBytes {
		return fmt.Errorf("%w: GeminiVeo channel credential is not recoverable", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	for _, encrypted := range []string{operation.EncryptedProviderTaskID, privateData.EncryptedProviderTaskID} {
		...
		if err != nil || descriptor.ValidateProviderTaskID(privateData.RoutingSnapshot,
			...
			return fmt.Errorf("%w: GeminiVeo provider id is not recoverable", service.ErrRelayQuotaReviewUnsafeRetry)
		}
	}
	return nil
}
...
func verifyGeminiVeoTaskManualReviewRetry(reservationID, eventID string) (*GeminiVeoTaskManualReviewRetryResult, error) {
	...
	if len(operations) != 1 {
		return nil, service.ErrRelayQuotaReviewInvalidState
	}
	operation := operations[0]
	var task model.Task
