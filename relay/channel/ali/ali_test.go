...
func aliMeta(mode constant.RelayMode, format constant.RelayFormat, modelName string) *relaycommon.Meta {
	return &relaycommon.Meta{
		Channel: &model.Channel{Type: int(constant.ChannelTypeAli)},
...
func TestAliURLsHeadersModesAndModelCatalog(t *testing.T) {
	tests := []struct {
		...
		{name: "chat", mode: constant.RelayModeChatCompletions, format: constant.RelayFormatOpenAI, model: "qwen-plus", path: "/gateway/compatible-mode/v1/chat/completions"},
		{name: "completions", mode: constant.RelayModeCompletions, format: constant.RelayFormatOpenAI, model: "qwen-plus", path: "/gateway/compatible-mode/v1/completions"},
		{name: "embeddings", mode: constant.RelayModeEmbeddings, format: constant.RelayFormatEmbedding, model: "text-embedding-v1", path: "/gateway/compatible-mode/v1/embeddings"},
		{name: "rerank", mode: constant.RelayModeRerank, format: constant.RelayFormatRerank, model: "gte-rerank-v2", path: "/gateway/api/v1/services/rerank/text-rerank/text-rerank"},
		{name: "responses", mode: constant.RelayModeResponses, format: constant.RelayFormatOpenAIResponses, model: "qwen-plus", path: "/gateway/api/v2/apps/protocols/compatible-mode/v1/responses"},
		{name: "sync generation", mode: constant.RelayModeImagesGenerations, format: constant.RelayFormatOpenAIImage, model: "qwen-image", path: "/gateway/api/v1/services/aigc/multimodal-generation/generation"},
		{name: "async generation", mode: constant.RelayModeImagesGenerations, format: constant.RelayFormatOpenAIImage, model: "wanx-v1", path: "/gateway/api/v1/services/aigc/text2image/image-synthesis", async: true},
		{name: "old Wan edit", mode: constant.RelayModeImagesEdits, format: constant.RelayFormatOpenAIImage, model: "wanx-style-repaint-v1", path: "/gateway/api/v1/services/aigc/image2image/image-synthesis", async: true},
		{name: "new Wan edit", mode: constant.RelayModeImagesEdits, format: constant.RelayFormatOpenAIImage, model: "wan2.6-image", path: "/gateway/api/v1/services/aigc/image-generation/generation", async: true},
		{name: "Qwen edit", mode: constant.RelayModeImagesEdits, format: constant.RelayFormatOpenAIImage, model: "qwen-image-edit", path: "/gateway/api/v1/services/aigc/multimodal-generation/generation"},
		{name: "native Claude", mode: constant.RelayModeChatCompletions, format: constant.RelayFormatClaude, model: "Qwen3-Max", path: "/gateway/apps/anthropic/v1/messages"},
		{name: "converted Claude", mode: constant.RelayModeChatCompletions, format: constant.RelayFormatClaude, model: "other-model", path: "/gateway/compatible-mode/v1/chat/completions"},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			meta := aliMeta(test.mode, test.format, test.model)
			if test.mode == constant.RelayModeChatCompletions {
				meta.IsStream = true
...
func TestAliPluginHeaderCredentialAndFailClosedModes(t *testing.T) {
	meta := aliMeta(constant.RelayModeChatCompletions, constant.RelayFormatOpenAI, "qwen-plus")
	...
	for _, invalid := range []*relaycommon.Meta{
		aliMeta(constant.RelayModeModerations, constant.RelayFormatOpenAI, "qwen-plus"),
		aliMeta(constant.RelayModeAudioSpeech, constant.RelayFormatOpenAIAudio, "qwen-plus"),
		aliMeta(constant.RelayModeChatCompletions, constant.RelayFormatGemini, "qwen-plus"),
		aliMeta(constant.RelayModeResponsesCompact, constant.RelayFormatOpenAIResponsesCompaction, "qwen-plus"),
		aliMeta(constant.RelayModeRerank, constant.RelayFormatRerank, "gte-rerank-v2"),
	} {
		if invalid.Mode == constant.RelayModeRerank {
	...
	badBase := aliMeta(constant.RelayModeChatCompletions, constant.RelayFormatOpenAI, "qwen-plus")
...
func TestAliTextEmbeddingResponsesAndRerankConversion(t *testing.T) {
	textMeta := aliMeta(constant.RelayModeChatCompletions, constant.RelayFormatOpenAI, "alias")
	...
	for _, test := range []struct {
		...
		{constant.RelayModeEmbeddings, constant.RelayFormatEmbedding, map[string]any{"model": "alias", "input": []any{"one", "two"}, "group": "drop"}},
		{constant.RelayModeResponses, constant.RelayFormatOpenAIResponses, map[string]any{"model": "alias", "input": "hello", "instructions": "brief", "group": "drop"}},
	} {
	...
	rerank := aliMeta(constant.RelayModeRerank, constant.RelayFormatRerank, "alias")
...
func TestAliNativeClaudeRequestUsesMappedModel(t *testing.T) {
	meta := aliMeta(constant.RelayModeChatCompletions, constant.RelayFormatClaude, "client-model")
...
func TestAliImageRequestConversion(t *testing.T) {
	sync := aliMeta(constant.RelayModeImagesGenerations, constant.RelayFormatOpenAIImage, "qwen-image")
	n := 2
	sync.Request.N = &n
	...
	async := aliMeta(constant.RelayModeImagesGenerations, constant.RelayFormatOpenAIImage, "wanx-v1")
...
func TestAliMultipartImageEditConversion(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.model, func(t *testing.T) {
			...
			meta := aliMeta(constant.RelayModeImagesEdits, constant.RelayFormatOpenAIImage, test.model)
...
func TestAliRerankResponseUsageErrorMappingAndBounds(t *testing.T) {
	...
	meta := aliMeta(constant.RelayModeRerank, constant.RelayFormatRerank, "gte-rerank-v2")
	adaptor := initializedAdaptor(meta)
...
func TestAliAsyncImagePollAndB64FetchAreBoundedAndCredentialScoped(t *testing.T) {
	...
	meta := aliMeta(constant.RelayModeImagesGenerations, constant.RelayFormatOpenAIImage, "wanx-v1")
