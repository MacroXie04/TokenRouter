...
func (a *Adaptor) GetRequestURL(meta *relaycommon.Meta) (string, error) {
	...
	path := ""
	if a.relayFormat(meta) == constant.RelayFormatClaude {
...
func (a *Adaptor) SetupRequestHeader(req *http.Request, meta *relaycommon.Meta) error {
	...
	if a.mode == constant.RelayModeImagesGenerations && !a.imageSync ||
		a.mode == constant.RelayModeImagesEdits && isWanModel(imageRoutingModel(meta)) {
...
func (a *Adaptor) ConvertRequest(meta *relaycommon.Meta) ([]byte, error) {
	...
	if a.relayFormat(meta) == constant.RelayFormatClaude && SupportsAnthropicMessages(meta.ModelName) {
...
func (a *Adaptor) validate(meta *relaycommon.Meta) error {
	...
	case constant.RelayFormatClaude:
		supported = a.mode == constant.RelayModeChatCompletions
	case constant.RelayFormatOpenAI:
		supported = a.mode == constant.RelayModeChatCompletions || a.mode == constant.RelayModeCompletions
	case constant.RelayFormatEmbedding:
		supported = a.mode == constant.RelayModeEmbeddings
	case constant.RelayFormatRerank:
		supported = a.mode == constant.RelayModeRerank
	case constant.RelayFormatOpenAIResponses:
		supported = a.mode == constant.RelayModeResponses
	case constant.RelayFormatOpenAIImage:
		supported = a.mode == constant.RelayModeImagesGenerations || a.mode == constant.RelayModeImagesEdits
	default:
