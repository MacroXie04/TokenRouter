...
func imageRequestIsSynchronous(meta *relaycommon.Meta) bool {
	...
	if meta.Mode == constant.RelayModeImagesEdits && isWanModel(modelName) {
		return false
	}
...
func convertImageRequest(meta *relaycommon.Meta, synchronous bool) ([]byte, error) {
	...
	if meta.Mode == constant.RelayModeImagesEdits && isMultipart(meta.RequestContentType) {
		return convertMultipartImageEdit(meta)
	}
	if meta.Mode == constant.RelayModeImagesGenerations && isMultipart(meta.RequestContentType) {
