...
func (a *Adaptor) providerType(meta *relaycommon.Meta) constant.ChannelType {
	if a.ChannelType != constant.ChannelTypeUnknown {
		return a.ChannelType
	}
	if meta != nil && meta.Channel != nil {
		return constant.ChannelType(meta.Channel.Type)
	}
	return constant.ChannelTypeOpenAI
}

func providerDefaultBaseURL(channelType constant.ChannelType) string {
	index := int(channelType)
	if index >= 0 && index < len(constant.ChannelBaseURLs) {
		return strings.TrimSpace(constant.ChannelBaseURLs[index])
	}
	return ""
}

func validateProviderMode(channelType constant.ChannelType, mode constant.RelayMode) error {
	var supported bool
	switch channelType {
	case constant.ChannelTypePerplexity:
		supported = mode == constant.RelayModeChatCompletions || mode == constant.RelayModeResponses
	case constant.ChannelTypeJina:
		supported = mode == constant.RelayModeRerank || mode == constant.RelayModeEmbeddings
	case constant.ChannelTypeSubmodel:
		supported = mode == constant.RelayModeChatCompletions || mode == constant.RelayModeCompletions
	default:
		return nil
	}
	if !supported {
		return fmt.Errorf("%s channel does not support relay mode %d", constant.ChannelTypeName(channelType), mode)
	}
	return nil
}
...
func applyProviderRequest(channelType constant.ChannelType, mode constant.RelayMode, stream bool, body map[string]any) error {
	if mode == constant.RelayModeCompletions && emptyJSONArray(body["messages"]) {
...
func applyJinaRequest(mode constant.RelayMode, body map[string]any) {
	switch mode {
...
func applyXAIRequest(mode constant.RelayMode, body map[string]any) {
	model, _ := body["model"].(string)
...
func applySiliconFlowRequest(mode constant.RelayMode, body map[string]any) {
...
func normalizeProviderUsage(channelType constant.ChannelType, mode constant.RelayMode, usage *protocolkit.Usage) bool {
	if usage == nil {
		return false
	}
	switch channelType {
	...
	case constant.ChannelTypeXai:
		...
		return true
	case constant.ChannelTypeJina:
		if mode == constant.RelayModeRerank && usage.TotalTokens > 0 && usage.PromptTokens == 0 && usage.CompletionTokens == 0 {
