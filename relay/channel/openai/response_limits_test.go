...
func TestOpenAIResponseLimitSelection(t *testing.T) {
	adaptor := &Adaptor{}
	for _, mode := range []constant.RelayMode{constant.RelayModeEmbeddings, constant.RelayModeImagesGenerations, constant.RelayModeImagesEdits} {
		adaptor.Mode = mode
		assert.Equal(t, relaycommon.MaxUpstreamLargeJSONBodyBytes, adaptor.bufferedResponseLimit())
	}
	adaptor.Mode = constant.RelayModeAudioSpeech
	assert.Equal(t, relaycommon.MaxUpstreamBinaryBodyBytes, adaptor.bufferedResponseLimit())
	adaptor.Mode = constant.RelayModeChatCompletions
