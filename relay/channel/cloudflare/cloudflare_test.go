...
func cloudflareMeta(mode constant.RelayMode, format constant.RelayFormat) *relaycommon.Meta {
	return &relaycommon.Meta{
		Channel:   &model.Channel{Type: int(constant.ChannelCloudflare), Other: "account_123"},
		Mode:      mode,
		Format:    format,
...
func TestCloudflareURLsAuthenticationAndModeGates(t *testing.T) {
	tests := []struct {
		...
		{name: "chat", mode: constant.RelayModeChatCompletions, format: constant.RelayFormatOpenAI, expected: "https://api.cloudflare.com/client/v4/accounts/account_123/ai/v1/chat/completions"},
		{name: "embeddings", mode: constant.RelayModeEmbeddings, format: constant.RelayFormatEmbedding, expected: "https://api.cloudflare.com/client/v4/accounts/account_123/ai/v1/embeddings"},
		{name: "responses", mode: constant.RelayModeResponses, format: constant.RelayFormatOpenAIResponses, expected: "https://api.cloudflare.com/client/v4/accounts/account_123/ai/v1/responses"},
		{name: "completion", mode: constant.RelayModeCompletions, format: constant.RelayFormatOpenAI, expected: "https://api.cloudflare.com/client/v4/accounts/account_123/ai/run/@cf/meta/mapped-model"},
		{name: "transcription", mode: constant.RelayModeAudioTranscription, format: constant.RelayFormatOpenAIAudio, expected: "https://api.cloudflare.com/client/v4/accounts/account_123/ai/run/@cf/meta/mapped-model"},
		{name: "translation custom base", mode: constant.RelayModeAudioTranslation, format: constant.RelayFormatOpenAIAudio, base: "https://gateway.example/root/", expected: "https://gateway.example/root/client/v4/accounts/account_123/ai/run/@cf/meta/mapped-model"},
	}
	...
	stream := cloudflareMeta(constant.RelayModeChatCompletions, constant.RelayFormatOpenAI)
	...
	for _, invalidBase := range []string{
		...
		meta := cloudflareMeta(constant.RelayModeChatCompletions, constant.RelayFormatOpenAI)
	...
	invalidModel := cloudflareMeta(constant.RelayModeCompletions, constant.RelayFormatOpenAI)
	invalidModel.ModelName = "@cf/meta/../escape"
	adaptor.Init(invalidModel)
	_, err := adaptor.GetRequestURL(invalidModel)
	require.Error(t, err)

	missingKey := cloudflareMeta(constant.RelayModeChatCompletions, constant.RelayFormatOpenAI)
	missingKey.APIKey = " "
	adaptor.Init(missingKey)
	require.Error(t, adaptor.SetupRequestHeader(request, missingKey))

	for _, invalid := range []*relaycommon.Meta{
		cloudflareMeta(constant.RelayModeRerank, constant.RelayFormatRerank),
		cloudflareMeta(constant.RelayModeChatCompletions, constant.RelayFormatClaude),
	} {
	...
	streamingEmbedding := cloudflareMeta(constant.RelayModeEmbeddings, constant.RelayFormatEmbedding)
...
func TestCloudflareRequestConversionUsesMappedModelAndNativeRunShape(t *testing.T) {
	...
	zero := 0
	meta = cloudflareMeta(constant.RelayModeCompletions, constant.RelayFormatOpenAI)
	...
	embedding := cloudflareMeta(constant.RelayModeEmbeddings, constant.RelayFormatEmbedding)
	...
	responses := cloudflareMeta(constant.RelayModeResponses, constant.RelayFormatOpenAIResponses)
	...
	tooLarge := cloudflareMeta(constant.RelayModeChatCompletions, constant.RelayFormatOpenAI)
...
func TestCloudflareAudioRequestExtractsOneBoundedFile(t *testing.T) {
	...
	meta := cloudflareMeta(constant.RelayModeAudioTranscription, constant.RelayFormatOpenAIAudio)
...
func TestCloudflareNonStreamResponsesNormalizeUsageAndMappedModel(t *testing.T) {
	...
	completion := cloudflareMeta(constant.RelayModeCompletions, constant.RelayFormatOpenAI)
	...
	audio := cloudflareMeta(constant.RelayModeAudioTranscription, constant.RelayFormatOpenAIAudio)
...
func TestCloudflareErrorsAndResponseBounds(t *testing.T) {
	...
	streamMeta := cloudflareMeta(constant.RelayModeChatCompletions, constant.RelayFormatOpenAI)
