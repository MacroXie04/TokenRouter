	...
	case "multipart/form-data":
		...
		for {
			...
			if part.FormName() == "model" && part.FileName() == "" {
				value, readErr := common.ReadAllLimited(part, maxModelBytes)
				_ = part.Close()
				if readErr != nil {
...
func prepareMultipart(
	...
	for {
		...
		data, readErr := common.ReadAllLimited(part, MaxRequestBodyBytes)
		_ = part.Close()
		if readErr != nil {
			return nil, readErr
		}
...
func EffectiveBaseURL(configured string, channelType int) (string, error) {
	...
	if base == "" && channelType >= 0 && channelType < len(constant.ChannelBaseURLs) {
		base = strings.TrimRight(constant.ChannelBaseURLs[channelType], "/")
	}
...
func endpointURL(baseURL, path string) (string, error) {
	base, err := EffectiveBaseURL(baseURL, int(constant.ChannelTypeSora))
	if err != nil {
		return "", err
	}
