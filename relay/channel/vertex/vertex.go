...
func (a *Adaptor) Init(meta *relaycommon.Meta) {
	...
	a.relayMode = constant.RelayModeUnknown
	a.credentials = nil
	a.keyType = ""
	a.apiKey = ""
	if meta != nil {
...
func (a *Adaptor) ConvertRequest(meta *relaycommon.Meta) ([]byte, error) {
	...
	case requestModeOpenSource:
		delegate := &openai.Adaptor{ChannelType: constant.ChannelTypeOpenAI, Format: constant.RelayFormatOpenAI, Mode: constant.RelayModeChatCompletions}
...
func (a *Adaptor) validateMeta(meta *relaycommon.Meta) error {
	...
	if mode != constant.RelayModeChatCompletions && mode != constant.RelayModeGemini {
...
func isNativeGeminiRequest(meta *relaycommon.Meta) bool {
	...
	return meta.Mode == constant.RelayModeGemini || strings.Contains(meta.RequestPath, "/models/")
}
...
func isNativeClaudeRequest(meta *relaycommon.Meta) bool {
	...
	return meta.Format == constant.RelayFormatClaude || strings.TrimRight(meta.RequestPath, "/") == "/v1/messages"
}
...
func validateOpenAIRequest(meta *relaycommon.Meta) error {
	...
	for name, value := range map[string]*int{
		...
		if value != nil && (*value < 1 || int64(*value) > appcommon.MaxQuota) {
...
func validateGeminiRequest(request *protocolkit.GeminiChatRequest) error {
	...
	if config := request.GenerationConfig; config != nil {
		...
		if config.TopK != nil && (math.IsNaN(*config.TopK) || math.IsInf(*config.TopK, 0) || *config.TopK < 0 || *config.TopK > float64(appcommon.MaxQuota)) {
			return errors.New("Vertex AI topK is outside the supported range")
		}
		if config.MaxOutputTokens != nil && (*config.MaxOutputTokens < 1 || int64(*config.MaxOutputTokens) > appcommon.MaxQuota) {
...
func validateClaudeRequest(request *protocolkit.ClaudeRequest) error {
	...
	if request.MaxTokens < 1 || int64(request.MaxTokens) > appcommon.MaxQuota {
	...
	if request.TopK != nil && (*request.TopK < 0 || int64(*request.TopK) > appcommon.MaxQuota) {
		return errors.New("Vertex AI Claude top_k is outside the supported range")
	}
	if request.Thinking != nil && (request.Thinking.BudgetTokens < 0 || int64(request.Thinking.BudgetTokens) > appcommon.MaxQuota) {
