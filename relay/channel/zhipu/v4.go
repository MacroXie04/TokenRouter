...
func (a *V4Adaptor) DoResponse(c *gin.Context, response *http.Response, meta *relaycommon.Meta) (*protocolkit.Usage, error) {
	...
	if a.format == constant.RelayFormatClaude {
		return v4ClaudeResponse(c, response, meta)
	}
	if a.mode == constant.RelayModeImagesGenerations {
...
func (a *V4Adaptor) validate(meta *relaycommon.Meta) error {
	...
	case constant.RelayFormatOpenAIImage:
		supported = a.mode == constant.RelayModeImagesGenerations
	default:
	...
	if meta.IsStream && a.mode != constant.RelayModeChatCompletions {
		return fmt.Errorf("Zhipu v4 relay mode %d does not support streaming", a.mode)
	}
	if a.mode == constant.RelayModeImagesGenerations && isMultipart(meta.RequestContentType) {
