...
func codexMeta(mode constant.RelayMode) *relaycommon.Meta {
...
func TestCodexResponsesContract(t *testing.T) {
	meta := codexMeta(constant.RelayModeResponses)
	adaptor := &Adaptor{}
	adaptor.Init(meta)
...
func TestCodexModeAndCredentialValidation(t *testing.T) {
	meta := codexMeta(constant.RelayModeChatCompletions)
	adaptor := &Adaptor{}
	adaptor.Init(meta)
	_, err := adaptor.GetRequestURL(meta)
	assert.Error(t, err)

	meta.Mode = constant.RelayModeResponses
	adaptor.Init(meta)
...
func TestCodexCompactAndAlphaSearchPaths(t *testing.T) {
	for _, test := range []struct {
		...
		{constant.RelayModeResponsesCompact, "/backend-api/codex/responses/compact"},
		{constant.RelayModeAlphaSearch, "/backend-api/codex/alpha/search"},
	} {
