...
func (a *Adaptor) ConvertRequest(meta *relaycommon.Meta) ([]byte, error) {
	...
	if a.mode != constant.RelayModeResponses &&
		a.mode != constant.RelayModeResponsesCompact &&
		a.mode != constant.RelayModeAlphaSearch {
	...
	delete(body, "group")
	if a.mode == constant.RelayModeResponses || a.mode == constant.RelayModeResponsesCompact {
