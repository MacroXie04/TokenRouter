...
const (
	...
	MaxUsageTokens         = int64(appcommon.MaxQuota)
...
func (a *Adaptor) validate(meta *relaycommon.Meta) (Credential, error) {
	...
	if format != constant.RelayFormatOpenAI && format != constant.RelayFormatClaude {
		return Credential{}, fmt.Errorf("AWS channel does not support relay format %q", format)
	}
	if IsNovaModel(meta.ModelName) && format != constant.RelayFormatOpenAI {
