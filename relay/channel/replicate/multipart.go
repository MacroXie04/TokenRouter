...
func newAuxiliaryHTTPClient() *http.Client {
	return &http.Client{
		...
		Transport: &http.Transport{
			DialContext:            appcommon.SafeDialContext,
			TLSClientConfig:        &tls.Config{MinVersion: tls.VersionTLS12, InsecureSkipVerify: appcommon.GetEnvBool("TLS_INSECURE_SKIP_VERIFY", false)},
...
func parseMultipartImage(raw []byte, contentType string) (multipartImage, error) {
	...
	for {
		...
		data, err := appcommon.ReadAllLimited(part, maxEditImageBytes)
		if err != nil {
