...
func replicateMeta(mode constant.RelayMode) *relaycommon.Meta {
	one := 1
	...
	return &relaycommon.Meta{
		Context: context.Background(), Channel: &model.Channel{Type: int(constant.ChannelTypeReplicate)},
		Mode: mode, Format: constant.RelayFormatOpenAIImage, ModelName: DefaultModel,
...
func TestReplicateURLHeadersModesAndCatalog(t *testing.T) {
	meta := replicateMeta(constant.RelayModeImagesGenerations)
	adaptor := &Adaptor{}
	adaptor.Init(meta)
	...
	for _, test := range []struct {
		...
		{name: "chat mode", mutate: func(meta *relaycommon.Meta) { meta.Mode = constant.RelayModeChatCompletions }},
		{name: "wrong format", mutate: func(meta *relaycommon.Meta) { meta.Format = constant.RelayFormatOpenAI }},
		...
		t.Run("reject "+test.name, func(t *testing.T) {
			invalid := replicateMeta(constant.RelayModeImagesGenerations)
			test.mutate(invalid)
			candidate := &Adaptor{}
...
func TestReplicateGenerationConversionAndBounds(t *testing.T) {
	...
	custom := replicateMeta(constant.RelayModeImagesGenerations)
...
func TestReplicateMultipartEditUploadsOneValidatedImage(t *testing.T) {
	...
	meta := replicateMeta(constant.RelayModeImagesEdits)
	...
	wrongType := replicateMeta(constant.RelayModeImagesEdits)
	...
	invalidAfterUploadRisk := replicateMeta(constant.RelayModeImagesEdits)
...
func TestReplicateResponseConversionURLBase64AndFailures(t *testing.T) {
	...
	b64Meta := replicateMeta(constant.RelayModeImagesGenerations)
