...
func ParseFinalUnitDeduction(raw string) (int, bool) {
	...
	if exponent == math.MaxInt64 {
		return int(common.MaxQuota), true
	}
	...
	if scale > int64(len(strconv.FormatInt(common.MaxQuota, 10))) {
		return int(common.MaxQuota), true
	}
	...
	maxText := strconv.FormatInt(common.MaxQuota, 10)
	...
	if err != nil || integerValue >= common.MaxQuota {
		return int(common.MaxQuota), true
	}
