...
func cozeMeta() *relaycommon.Meta {
	return &relaycommon.Meta{
		...
		Channel: &model.Channel{Type: int(constant.ChannelTypeCoze), Other: "bot_123"},
...
func TestCozeCatalogURLHeadersAndSupportedSurface(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"completion mode", func(meta *relaycommon.Meta) { meta.Mode = constant.RelayModeCompletions }},
		{"embedding mode", func(meta *relaycommon.Meta) { meta.Mode = constant.RelayModeEmbeddings }},
		{"image mode", func(meta *relaycommon.Meta) { meta.Mode = constant.RelayModeImagesGenerations }},
		{"responses mode", func(meta *relaycommon.Meta) { meta.Mode = constant.RelayModeResponses }},
		{"Claude format", func(meta *relaycommon.Meta) { meta.Format = constant.RelayFormatClaude }},
...
func TestCozeResponseBoundsAndUsageValidation(t *testing.T) {
	...
	invalidUsage := []cozeUsage{
		{InputCount: -1},
		{OutputCount: int(appcommon.MaxQuota) + 1},
		{TokenCount: int(appcommon.MaxQuota) + 1},
		{InputCount: int(appcommon.MaxQuota), OutputCount: 1},
	}
