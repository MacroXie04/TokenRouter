...
func (a *Adaptor) validate(meta *relaycommon.Meta) error {
	...
	if mode != constant.RelayModeChatCompletions || meta.Format != constant.RelayFormatOpenAI {
...
func addUsage(total *protocolkit.Usage, update sparkUsage) error {
	...
	for _, value := range values {
		if int64(*value.current) > appcommon.MaxQuota-int64(value.delta) {
...
func newProductionDialer() *websocket.Dialer {
	return &websocket.Dialer{
		NetDialContext: appcommon.SafeDialContext,
