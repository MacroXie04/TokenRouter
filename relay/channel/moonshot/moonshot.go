...
func (a *Adaptor) ConvertRequest(meta *relaycommon.Meta) ([]byte, error) {
	...
	if a.mode != constant.RelayModeChatCompletions || !strings.EqualFold(meta.ModelName, "kimi-k2.6") {
		return body, nil
	}
...
func validateMode(mode constant.RelayMode, format constant.RelayFormat) error {
	switch format {
	...
	case constant.RelayFormatOpenAI:
		if mode != constant.RelayModeChatCompletions && mode != constant.RelayModeCompletions {
...
func moonshotOpenAIPath(mode constant.RelayMode) string {
	switch mode {
...
func normalizeNonStreamBody(body []byte, meta *relaycommon.Meta, mode constant.RelayMode) ([]byte, *protocolkit.Usage, int, bool, error) {
...
func estimateCompletionTokens(body []byte, mode constant.RelayMode) int {
	if mode == constant.RelayModeCompletions {
