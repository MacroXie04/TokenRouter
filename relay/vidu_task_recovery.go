...
func settlePendingViduOperation(task *model.Task, reservation *service.RelayQuotaReservation, operation *model.TaskOperation) error {
	if operation.EncryptedProviderTaskID == "" {
	...
	if err != nil {
		return markViduOperationManualReview(operation, "Vidu provider id cannot be decrypted", common.NowTimestamp())
	}
...
func refundPreparedViduTaskWithLease(task *model.Task, reservation *service.RelayQuotaReservation,
