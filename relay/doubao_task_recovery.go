...
func finishPolledDoubaoTask(
	...
	if err == nil {
		if provider.Status == doubao.StatusSucceeded {
			if deliverErr := service.DeliverAuditLogOutboxEvent(doubaoAuditEventID(operation.ReservationID)); deliverErr != nil {
				common.SysError("deliver recovered Doubao consume audit: " + deliverErr.Error())
			}
		}
		return nil
	}
	...
	if fallbackErr := persistDoubaoProviderStatePending(task, operation.ReservationID, provider,
		...
		common.SysError("Doubao terminal accounting remains pending for " + task.TaskID + ": " + err.Error())
		return nil
	} else {
