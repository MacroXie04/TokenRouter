...
func TestAdvancedCustomOpenAIWireAndSettlement(t *testing.T) {
	...
	service.SetModelPriceRegistry(map[string]service.ModelPrice{"gpt-provider-contract": {Prompt: 1, Completion: 2}})
	...
	require.NoError(t, model.DB.Model(&channel).Updates(map[string]any{
		"type": int(constant.ChannelTypeAdvancedCustom), "settings": settings,
...
func TestAdvancedCustomEmbeddingWireUsageAndSettlement(t *testing.T) {
	previousPrices := service.ExportedModelPrices()
	previousRatios := service.ExportedGroupRatios()
	service.SetModelPriceRegistry(map[string]service.ModelPrice{"text-embedding-3-small": {Prompt: 1}})
	service.SetGroupRatios(map[string]float64{"default": 1})
	t.Cleanup(func() {
		service.SetModelPriceRegistry(previousPrices)
		service.SetGroupRatios(previousRatios)
