...
func TestBuildRealtimeUpstreamUsesProviderURLAndAuthentication(t *testing.T) {
	t.Run("OpenAI GA bearer", func(t *testing.T) {
		channel := &model.Channel{Type: int(constant.ChannelTypeOpenAI), BaseURL: "https://api.example.test", Key: "openai-key"}
	...
	t.Run("OpenAI legacy browser subprotocol", func(t *testing.T) {
		channel := &model.Channel{Type: int(constant.ChannelTypeOpenAI), BaseURL: "https://api.example.test", Key: "preview-key"}
	...
	t.Run("Azure deployment and API key", func(t *testing.T) {
		channel := &model.Channel{
			Type: int(constant.ChannelTypeAzure), BaseURL: "https://resource.openai.azure.com",
	...
	t.Run("invalid Azure version", func(t *testing.T) {
		channel := &model.Channel{Type: int(constant.ChannelTypeAzure), BaseURL: "https://resource.openai.azure.com", Key: "azure-key"}
	...
	t.Run("unsupported provider fails closed", func(t *testing.T) {
		channel := &model.Channel{Type: int(constant.ChannelTypeAws), BaseURL: "https://api.openai.com", Key: "aws-secret"}
	...
	t.Run("compatible provider without a base cannot inherit OpenAI", func(t *testing.T) {
		channel := &model.Channel{Type: int(constant.ChannelTypeXinference), Key: "private-provider-secret"}
		_, _, err := buildRealtimeUpstream(channel, "model", "", "")
		require.Error(t, err)
...
func realtimeTestDependencies(upstreamURL string, state *fakeRealtimeState) realtimeDependencies {
	...
	return realtimeDependencies{
		selectChannel: func(_, _ string) (*model.Channel, error) {
			return &model.Channel{Id: 9, Type: int(constant.ChannelTypeOpenAI), BaseURL: upstreamURL, Key: "upstream-key"}, nil
		},
...
func startRealtimeGateway(t *testing.T, deps realtimeDependencies) (*httptest.Server, *model.Token, <-chan struct{}) {
	...
	token := &model.Token{Id: 3, UserId: 2, Name: "test-token", Key: "sk-test", Status: service.TokenStatusEnabled}
	done := make(chan struct{})
	r := gin.New()
	r.GET("/v1/realtime", func(c *gin.Context) {
		common.SetUserId(c, token.UserId)
		common.SetUsername(c, "alice")
		common.SetUserGroup(c, "default")
		common.SetRequestId(c, "request-1")
		middleware.SetupRelayTokenContext(c, token)
		c.Set(common.ContextKeyGroup, service.GroupDefault)
...
func TestRealtimeRejectsQuotaBeforeEitherHandshake(t *testing.T) {
	...
	deps := realtimeDependencies{
		selectChannel: func(_, _ string) (*model.Channel, error) {
			return &model.Channel{Id: 1, Type: int(constant.ChannelTypeOpenAI), BaseURL: "http://unused.test", Key: "key"}, nil
		},
		...
		reserve: func(_ int, _ *model.Token, _ int) (realtimeReservation, error) {
			return nil, service.ErrInsufficientTokenQuota
		},
...
func TestRealtimeSelectsAuthorizedAutoGroupAndUsesItForBilling(t *testing.T) {
	...
	token := &model.Token{Id: 1, UserId: 7, Group: service.GroupAuto}
	router := gin.New()
	router.GET("/v1/realtime", func(c *gin.Context) {
		common.SetUserId(c, token.UserId)
		common.SetUserGroup(c, "member")
		middleware.SetupRelayTokenContext(c, token)
		middleware.SetRelayGroupPolicy(c, service.RelayGroupPolicy{Groups: []string{"staff", "vip"}, Auto: true})
		relayWebSocket(c, deps)
	})
	recorder := httptest.NewRecorder()
...
func TestProductionRealtimePricingUsesUserGroupSpecialRatio(t *testing.T) {
	...
	previousGroups := service.ExportedGroupRatios()
	previousSpecial := service.ExportedGroupGroupRatios()
	t.Cleanup(func() {
		service.SetModelPriceRegistry(previousPrices)
		service.SetGroupRatios(previousGroups)
		service.SetGroupGroupRatios(previousSpecial)
	})
	service.SetModelPriceRegistry(map[string]service.ModelPrice{
		"special-realtime-model": {Prompt: 2, Completion: 4},
	})
	service.SetGroupRatios(map[string]float64{"vip": 2})
	service.SetGroupGroupRatios(map[string]map[string]float64{"member": {"vip": 0.5}})
...
func TestRealtimeAccountsEachUniqueResponseAndRefundsDisconnectReservation(t *testing.T) {
	...
	assert.Equal(t, common.NormalizeProviderCorrelationID("resp-1"), logs[0].upstreamRequestId)
	...
	assert.Equal(t, common.NormalizeProviderCorrelationID("sk-test-super-secret-provider-response-id"), logs[1].upstreamRequestId)
