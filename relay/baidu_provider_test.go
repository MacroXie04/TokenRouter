...
func assertBaiduSettled(t *testing.T, key string, userID, channelID, promptTokens, completionTokens int) {
	t.Helper()
	wantQuota := service.ComputeQuota("gpt-provider-contract", "default", promptTokens, completionTokens)
	var log model.Log
...
func assertBaiduEmbeddingSettled(t *testing.T, key string, userID, channelID, promptTokens int) {
	t.Helper()
	wantQuota := service.ComputeQuota("text-embedding-3-small", "default", promptTokens, 0)
	var log model.Log
	require.NoError(t, model.LOG_DB.Where("user_id = ? AND type = ?", userID, service.LogTypeConsume).First(&log).Error)
...
func TestBaiduLegacyChatAndEmbeddingWireSettlementEndToEnd(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			key, userID, channel := configureBaiduIntegration(t, upstream.URL+"/gateway", constant.ChannelTypeBaidu,
...
func TestBaiduLegacyAndV2StreamsSettleTerminalUsageEndToEnd(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			name: "legacy", channelType: constant.ChannelTypeBaidu, upstreamModel: "ERNIE-3.5-8K",
		...
		{
			name: "v2", channelType: constant.ChannelTypeBaiduV2, upstreamModel: "ernie-4.0-turbo-8k",
		...
		t.Run(test.name, func(t *testing.T) {
			...
			upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
				...
				if test.channelType == constant.ChannelTypeBaidu {
...
func TestBaiduV2SearchAndNativeClaudeUseOpenAIWireAndSettle(t *testing.T) {
	...
	for _, nativeClaude := range []bool{false, true} {
		...
		t.Run(name, func(t *testing.T) {
			...
			var userID int
			if nativeClaude {
				key, userID = setupClaudeRelay(t, upstream.URL+"/gateway", int(constant.ChannelTypeBaiduV2), "gpt-provider-contract")
			} else {
			...
			require.NoError(t, model.DB.Where("name = ?", "mock").First(&channel).Error)
			require.NoError(t, model.DB.Model(&channel).Updates(map[string]any{
				"type": constant.ChannelTypeBaiduV2, "key": "v2-token|application-id",
...
func TestBaiduErrorsAndUnsupportedModesRefundWithoutCredentialLeak(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			name: "v2 mapped error", channelType: constant.ChannelTypeBaiduV2,
		...
		{
			name: "v2 embedding unsupported", channelType: constant.ChannelTypeBaiduV2,
			path: "/v1/embeddings", body: `{"model":"gpt-provider-contract","input":"hello"}`,
			upstreamModel: "ernie-4.0-turbo-8k", credential: "v2-secret-token|application-id",
			wantStatus: http.StatusInternalServerError,
		},
		{
			name: "legacy Claude unsupported", channelType: constant.ChannelTypeBaidu,
