...
var veoTaskHandlers = struct {
	sync.RWMutex
	byChannelType map[constant.ChannelType]VeoTaskChannelHandler
}{byChannelType: make(map[constant.ChannelType]VeoTaskChannelHandler)}

var veoTaskProviders = struct {
	sync.RWMutex
	byChannelType map[constant.ChannelType]VeoTaskProviderDescriptor
	byPlatform    map[string]VeoTaskProviderDescriptor
	byFamily      map[string]VeoTaskProviderDescriptor
}{
	byChannelType: make(map[constant.ChannelType]VeoTaskProviderDescriptor),
...
func veoTaskProviderByChannelType(channelType constant.ChannelType) (VeoTaskProviderDescriptor, bool) {
	veoTaskProviders.RLock()
	defer veoTaskProviders.RUnlock()
...
func RegisterVeoTaskChannelHandler(channelType constant.ChannelType, handler VeoTaskChannelHandler) {
...
func veoTaskHandler(channelType constant.ChannelType) VeoTaskChannelHandler {
...
func relayVeoTask(c *gin.Context, raw []byte, contentType string) bool {
	...
	if common.GetUserId(c) <= 0 || middleware.GetRelayToken(c) == nil {
	...
	handler := veoTaskHandler(constant.ChannelType(channel.Type))
	if handler == nil {
...
func selectVeoTaskChannel(groups []string, modelName string) (*model.Channel, string, error) {
	...
	for attempt := 0; attempt < 128; attempt++ {
		...
		if veoTaskHandler(constant.ChannelType(channel.Type)) != nil {
