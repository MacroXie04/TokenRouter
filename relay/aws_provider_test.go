...
func TestAWSRelayReservesBeforeIOAndSettlesClaudeSemanticUsage(t *testing.T) {
	...
	service.SetModelPriceRegistry(map[string]service.ModelPrice{"accounting-model": {Prompt: 1, Completion: 1}})
...
func newRelayContextFromRequest(t *testing.T, request *http.Request, recorder *httptest.ResponseRecorder, token *model.Token) *gin.Context {
	...
	common.SetUserId(context, token.UserId)
	common.SetUsername(context, "accounting-user")
	common.SetUserGroup(context, "default")
