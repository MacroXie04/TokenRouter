...
func ResolveJimengRelayQuotaReservationReview(
	...
	case model.RelayQuotaReservationStatusSettled:
		if resolution != service.RelayQuotaReviewResolutionSettle {
			return nil, fmt.Errorf("%w: reservation was settled", service.ErrRelayQuotaReviewInvalidState)
		}
	case model.RelayQuotaReservationStatusRefunded:
		if resolution != service.RelayQuotaReviewResolutionRefund {
			return nil, fmt.Errorf("%w: reservation was refunded", service.ErrRelayQuotaReviewInvalidState)
		}
	default:
		return nil, fmt.Errorf("%w: reservation is %s", service.ErrRelayQuotaReviewInvalidState, initial.Status)
	}
	...
	if initial.Status == model.RelayQuotaReservationStatusDispatched {
		...
		if err := model.DB.Where("task_id = ?", operation.TaskID).First(&task).Error; err != nil {
			return nil, fmt.Errorf("%w: Jimeng task is unavailable", service.ErrRelayQuotaReviewUnsafeRetry)
		}
...
func resolveJimengManualReviewTx(
	...
	if err := tx.Where("reservation_id = ?", reservationID).First(&record).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return jimengManualReviewTxOutcome{}, service.ErrRelayQuotaReviewNotFound
		}
	...
	if !jimengReviewSourceRecordMatches(expectedSource, &record) ||
		...
		return jimengManualReviewTxOutcome{}, service.ErrRelayQuotaReservationBusy
	}
	...
	if resolution == service.RelayQuotaReviewResolutionSettle {
		...
		if err := service.EnqueueAuditLogTx(tx, auditEntry); err != nil {
			return jimengManualReviewTxOutcome{}, err
		}
	}
...
func validateTerminalJimengManualReviewTx(
	...
	event, found, err := service.FindRelayQuotaReservationResolutionEventTx(
...
func classifyJimengManualReviewResolutionError(err error) error {
	...
	if errors.Is(err, service.ErrRelayQuotaOperation) || errors.Is(err, service.ErrRelayQuotaManualReview) {
		return fmt.Errorf("%w: durable reservation changed", service.ErrRelayQuotaReviewInvalidState)
	}
	return err
}
