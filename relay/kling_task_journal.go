...
func persistKlingRecoveryJournal(record klingRecoveryJournalRecord) error {
	...
	data, err := common.Marshal(klingRecoveryJournalDiskEnvelope{
...
func loadKlingRecoveryJournal(taskID string) (klingRecoveryJournalRecord, error) {
	...
	if err := common.Unmarshal(data, &disk); err != nil || disk.Version != klingRecoveryJournalDiskVersion ||
	...
	if err := common.Unmarshal([]byte(plaintext), &record); err != nil || record.TaskID != taskID || record.Action != action {
