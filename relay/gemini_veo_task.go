...
func init() {
	...
	service.RegisterAsyncTaskPromoter(PromoteGeminiVeoTaskRecoveryJournalsContext)
	service.RegisterAsyncTaskReconciler(reconcileAsyncGeminiVeoTasks)
}
...
func relayVeoTaskWithProvider(c *gin.Context, selection VeoTaskChannelSelection) {
	...
	descriptor, registered := veoTaskProviderByChannelType(constant.ChannelType(selection.Channel.Type))
	if !registered {
		writeGeminiVeoTaskError(c, http.StatusBadRequest, "invalid_request", "Veo channel selection is invalid", nil)
		return
	}
	...
	if err != nil || !enabled {
		if err != nil {
			common.SysError("resolve GeminiVeo pricing: " + err.Error())
		}
	...
	if err != nil {
		...
		if service.IsSubscriptionFundingErr(err) || errors.Is(err, service.ErrInsufficientQuota) ||
			...
			common.SysError("create atomic GeminiVeo reservation: " + err.Error())
		}
	...
	defer func() {
		...
		if cleanupErr := refundRejectedGeminiVeoTask(&task, reservation, "GeminiVeo submission stopped before provider dispatch"); cleanupErr != nil {
			common.SysError("GeminiVeo reservation cleanup failed for " + task.TaskID + ": " + cleanupErr.Error())
		}
	}()
	if err := markGeminiVeoTaskDispatching(&task, reservation); err != nil {
		common.SysError("mark GeminiVeo dispatch: " + err.Error())
	...
	if submitErr != nil {
		if !descriptor.SubmitWasDispatched(submitErr) || definitiveGeminiVeoRejection(submitErr) {
			if refundErr := refundRejectedGeminiVeoTask(&task, reservation, "GeminiVeo provider rejected submission"); refundErr != nil {
				common.SysError("persist rejected GeminiVeo task " + task.TaskID + ": " + refundErr.Error())
		...
		if persistErr := markGeminiVeoAmbiguousDispatch(&task, reservation.ReservationID(), geminiVeoUnknownDispatchReason); persistErr != nil {
			common.SysError("persist ambiguous GeminiVeo task " + task.TaskID + ": " + persistErr.Error())
	...
	if provider == nil || descriptor.ValidateProviderTaskID(routingSnapshot, mappedModel, provider.ProviderTaskID) != nil {
		if persistErr := markGeminiVeoAmbiguousDispatch(&task, reservation.ReservationID(), geminiVeoUnknownDispatchReason); persistErr != nil {
			common.SysError("persist GeminiVeo missing-id submission " + task.TaskID + ": " + persistErr.Error())
		}
		writeGeminiVeoTaskError(c, http.StatusAccepted, "submit_outcome_unknown", geminiVeoUnknownDispatchReason,
			gin.H{"task_id": task.TaskID, "status": "unknown"})
		return
	...
	if journalErr != nil {
		common.SysError("persist accepted GeminiVeo recovery journal " + task.TaskID + ": " + journalErr.Error())
	}
	if err := settleAcceptedGeminiVeoTask(&task, reservation, provider.ProviderTaskID, model.TaskOperationDispatching, ""); err != nil {
		common.SysError("settle accepted GeminiVeo task " + task.TaskID + ": " + err.Error())
		...
		if fallbackErr != nil {
			common.SysError("persist accepted GeminiVeo fallback " + task.TaskID + ": " + fallbackErr.Error())
			if journalErr != nil {
				data["recovery_degraded"] = true
			}
		} else if cleanupErr := removeGeminiVeoRecoveryJournal(task.TaskID); cleanupErr != nil {
			common.SysError("remove accepted GeminiVeo recovery journal " + task.TaskID + ": " + cleanupErr.Error())
		}
		writeGeminiVeoTaskError(c, http.StatusAccepted, "task_commit_pending",
	...
	if deliverErr := service.DeliverAuditLogOutboxEvent(geminiVeoAuditEventID(reservation.ReservationID())); deliverErr != nil {
		common.SysError("deliver GeminiVeo consume audit: " + deliverErr.Error())
	}
...
func RelayGeminiVeoTaskFetch(c *gin.Context) {
	...
	if err := model.DB.Where("task_id = ? AND user_id = ? AND platform IN ?", taskID,
		common.GetUserId(c), model.VeoTaskOperationPlatforms()).Limit(2).Find(&tasks).Error; err != nil {
...
func streamGeminiVeoContentResponse(c *gin.Context, task *model.Task, response *http.Response) {
	...
	if _, err := io.Copy(c.Writer, response.Body); err != nil {
		common.SysError("stream GeminiVeo content for " + task.TaskID + ": " + err.Error())
	}
}
