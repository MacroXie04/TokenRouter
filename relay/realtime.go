...
import (
	...
	openaichannel "github.com/tokenrouter/tokenrouter/relay/channel/openai"
...
type realtimePricingSnapshot struct {
	...
	allowZero        bool
	reference        bool
	referencePlan    service.ReferenceBillingPlan
}
...
func productionRealtimeDependencies() realtimeDependencies {
	return realtimeDependencies{
		selectChannel: func(group, modelName string) (*model.Channel, error) {
			return service.GetRandomSatisfiedChannel(group, modelName, nil, nil)
		},
		reserve: func(userId int, token *model.Token, quota int) (realtimeReservation, error) {
			return service.NewRelayQuotaReservation(userId, token, quota)
		},
		reserveFree: func(userId int, token *model.Token, quota int, freeModel bool) (realtimeReservation, error) {
			return service.NewRelayQuotaReservationWithFreeModel(userId, token, quota, freeModel)
		},
		dial: wsDialer.Dial,
		resolvePricing: func(userId int, modelName, userGroup, usingGroup string) (realtimePricingSnapshot, error) {
			plan, enabled, err := service.ResolveOrdinaryReferenceBillingPlanForUser(
			...
			if service.ShouldSkipOrdinaryFreeModelPreConsume(modelName, userGroup, usingGroup) {
				return realtimePricingSnapshot{freeModel: true, allowZero: true}, nil
			}
			quota, clamp, quotaErr := service.ComputeBillingQuotaForUser(modelName, userGroup, usingGroup, false, &protocolkit.Usage{
		...
		reservationQuota: func(modelName, userGroup, usingGroup string) (int, error) {
			quota, clamp, err := service.ComputeBillingQuotaForUser(modelName, userGroup, usingGroup, false, &protocolkit.Usage{
				PromptTokens: defaultRealtimePreConsumedTokens,
				TotalTokens:  defaultRealtimePreConsumedTokens,
			}, billingexpr.RequestInput{})
			if err != nil {
		...
		recordLog: func(entry realtimeLogEntry) error {
			return service.RecordConsumeLogChecked(
...
func relayWebSocket(c *gin.Context, deps realtimeDependencies) {
	...
	for _, candidate := range groups {
		...
		if selectErr != nil && !errors.Is(selectErr, service.ErrChannelNotFound) {
			err = selectErr
			break
		}
	}
	...
	if err != nil {
		...
		if errors.Is(err, service.ErrInsufficientTokenQuota) ||
			errors.Is(err, service.ErrInsufficientQuota) || service.IsSubscriptionFundingErr(err) {
	...
	accountant := newRealtimeAccountant(realtimeAccountantConfig{
		...
		log: realtimeLogEntry{
			userId: userId, username: common.GetUsername(c),
			tokenName: common.GetString(c, common.ContextKeyTokenName),
			modelName: modelName, channelId: channel.Id, userGroup: userGroup, group: group,
			ip: c.ClientIP(), requestId: common.GetRequestId(c), tokenId: token.Id,
		},
		startedAt: deps.now(),
	})
	...
	if err := reservation.MarkDispatched(); err != nil {
		...
		common.SysError("realtime dispatch accounting failed: " + combined.Error())
	...
	if upstreamResp != nil && upstreamResp.Body != nil {
		if closeErr := upstreamResp.Body.Close(); closeErr != nil {
			common.SysError("realtime upstream handshake response close failed: " + closeErr.Error())
		}
	}
	if err != nil {
	...
	if err != nil {
		if closeErr := upstreamConn.Close(); closeErr != nil {
			common.SysError("realtime upstream close after client handshake failure: " + closeErr.Error())
		}
	...
	if err := clientConn.Close(); err != nil && !errors.Is(err, http.ErrServerClosed) {
		common.SysError("realtime client close failed: " + err.Error())
	}
	if err := upstreamConn.Close(); err != nil && !errors.Is(err, http.ErrServerClosed) {
		common.SysError("realtime upstream close failed: " + err.Error())
	}
	second := <-results
	...
	if first.err != nil && !isNormalWebSocketClose(first.err) {
		common.SysError("realtime " + first.direction + " stopped: " + realtimeErrorForLog(first.err))
	}
	if second.err != nil && !isNormalWebSocketClose(second.err) {
		common.SysError("realtime " + second.direction + " stopped: " + realtimeErrorForLog(second.err))
	}
...
func refundRealtimeAccountant(accountant *realtimeAccountant, reason string) {
	...
	if err := accountant.close(); err != nil {
		common.SysError("realtime " + reason + " refund failed: " + err.Error())
	}
}
...
func (a *realtimeAccountant) processUpstreamMessage(data []byte) (bool, error) {
	...
	responseId = common.NormalizeProviderCorrelationID(responseId)
	if responseId != "" {
...
func (a *realtimeAccountant) settlementQuota(usage normalizedRealtimeUsage) (int, map[string]any, error) {
	...
	settlement, err := a.pricing.referencePlan.SettlementUsageQuota(
		...
		service.ReferenceUsageContext{ForceAudio: true, Realtime: true},
	)
...
func (a *realtimeAccountant) recordUsage(responseId string, usage normalizedRealtimeUsage, quota int, settled realtimeReservation, billingFields map[string]any, fallbackErr, settlementErr error) error {
	...
	if a.pricing.reference {
		...
		ratio, special := service.EffectiveGroupRatio(entry.userGroup, entry.group)
...
func buildRealtimeUpstreamForRequest(
	...
	meta := &relaycommon.Meta{
		Channel: channel, Mode: constant.RelayModeRealtime,
