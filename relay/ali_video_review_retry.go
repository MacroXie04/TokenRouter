...
func RetryAliWanTaskManualReview(reservationID string, operatorUserID int) (*AliWanTaskManualReviewRetryResult, error) {
	...
	err = model.DB.Transaction(func(tx *gorm.DB) error {
		...
		if len(tasks) != 1 {
			return fmt.Errorf("%w: AliWan task identity is unavailable or ambiguous", service.ErrRelayQuotaReviewUnsafeRetry)
		}
		task := &tasks[0]
		...
		if record.Status != model.RelayQuotaReservationStatusSettled ||
			...
			return fmt.Errorf("%w: AliWan task is not awaiting a polling retry", service.ErrRelayQuotaReviewInvalidState)
		}
...
func validateActiveAliWanTaskPollReview(record *model.RelayQuotaReservationRecord, task *model.Task,
	...
	if record == nil || task == nil || operation == nil ||
		...
		return fmt.Errorf("%w: AliWan polling review tuple is inconsistent", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || privateData.RelayReservationID != record.ReservationID || privateData.SettlementPending ||
		...
		return fmt.Errorf("%w: AliWan recovery metadata does not match accounting", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	properties, err := validateAliWanTaskPricingSnapshot(task, privateData, record.ActualQuota)
	if err != nil {
		return fmt.Errorf("%w: AliWan pricing snapshots do not match accounting", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || settled != record.ActualQuota {
		return fmt.Errorf("%w: AliWan pricing snapshot does not match accounting", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	baseURL, err := aliWan.EffectiveBaseURL(privateData.ChannelBaseURL)
	if err != nil || baseURL != privateData.ChannelBaseURL {
		return fmt.Errorf("%w: AliWan provider endpoint is invalid", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	if err != nil || strings.TrimSpace(key) == "" || len(key) > 8<<10 || strings.ContainsAny(key, "\r\n\x00") {
		return fmt.Errorf("%w: AliWan channel credential is not recoverable", service.ErrRelayQuotaReviewUnsafeRetry)
	}
	...
	for _, encrypted := range []string{operation.EncryptedProviderTaskID, privateData.EncryptedProviderTaskID} {
		...
		if err != nil || !validAliWanRecoveryOpaqueID(providerID, aliWan.MaxProviderTaskIDBytes) {
			return fmt.Errorf("%w: AliWan provider id is not recoverable", service.ErrRelayQuotaReviewUnsafeRetry)
		}
	}
	return nil
}
