...
func newVideoTaskFixture(t *testing.T, upstreamURL string) videoTaskFixture {
	...
	user := model.User{
		Username: "video-owner", Password: "x", Role: constant.RoleCommonUser,
	...
	weight := uint(1)
	channel := model.Channel{
		Name: "sora-mock", Type: int(constant.ChannelTypeSora), Key: "upstream-video-key",
...
func TestVideoTaskSubmitPollFetchRemixAndContentLifecycle(t *testing.T) {
	...
	restrictedToken := model.Token{
		UserId: fixture.user.Id, Key: "sk-video-model-restricted", Status: service.TokenStatusEnabled,
	...
	otherToken := model.Token{
		UserId: otherUser.Id, Key: "sk-video-other", Status: service.TokenStatusEnabled, UnlimitedQuota: true,
	}
