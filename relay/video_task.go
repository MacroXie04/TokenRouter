...
func init() {
	...
	service.RegisterAsyncTaskReconciler(reconcileAsyncVideoTasks)
}
...
func RelayVideoTask(c *gin.Context) {
	raw, err := common.ReadAllLimited(c.Request.Body, sora.MaxRequestBodyBytes)
	if err != nil {
	...
	if remix {
		...
		if err != nil || channel.Status != constant.ChannelStatusEnabled || !videoChannelSupported(channel) {
		...
		channelKey = strings.TrimSpace(service.GetChannelKey(channel))
		if channelKey == "" {
	...
	quota, err := service.ComputePerCallQuotaMultiplierForUser(
		originModel, common.GetUserGroup(c), usingGroup, multiplier,
	)
	if err != nil {
		writeVideoTaskError(c, http.StatusBadRequest, "model_price_error", err.Error(), nil)
		return
	}
	freeModel := service.ShouldSkipPerCallFreeModelPreConsume(
		originModel, common.GetUserGroup(c), usingGroup,
	)
	...
	if err != nil {
		...
		if service.IsSubscriptionFundingErr(err) || errors.Is(err, service.ErrInsufficientQuota) ||
			...
			common.SysError("create atomic video reservation: " + err.Error())
		}
		writeVideoTaskError(c, status, code, message, nil)
		return
	}
	...
	defer func() {
		...
		if cleanupErr := refundRejectedVideoTask(&task, reservation, "submission aborted before provider dispatch"); cleanupErr != nil {
			common.SysError("video reservation cleanup failed for " + task.TaskID + ": " + cleanupErr.Error())
		}
	}()
	if err := markVideoTaskDispatching(&task, reservation); err != nil {
		common.SysError("mark video dispatch: " + err.Error())
	...
	if submitErr != nil {
		if definitiveVideoRejection(submitErr) {
			if refundErr := refundRejectedVideoTask(&task, reservation, "video provider rejected submission"); refundErr != nil {
				common.SysError("persist rejected video task " + task.TaskID + ": " + refundErr.Error())
		...
		if settleErr := settleUnknownVideoDispatch(
			...
			common.SysError("persist ambiguous video task " + task.TaskID + ": " + settleErr.Error())
	...
	if journalErr != nil {
		common.SysError("persist accepted video recovery journal " + task.TaskID + ": " + journalErr.Error())
	}
	if err := settleAcceptedVideoTask(
		&task, reservation, provider, providerTaskID, model.TaskOperationDispatching, "",
	); err != nil {
		common.SysError("settle accepted video task " + task.TaskID + ": " + err.Error())
		...
		if fallbackErr != nil {
			common.SysError("persist accepted video fallback " + task.TaskID + ": " + fallbackErr.Error())
			if journalErr != nil {
				data["recovery_degraded"] = true
			}
		} else if cleanupErr := removeVideoRecoveryJournal(task.TaskID); cleanupErr != nil {
			common.SysError("remove accepted video recovery journal " + task.TaskID + ": " + cleanupErr.Error())
		}
		writeVideoTaskError(c, http.StatusAccepted, "task_commit_pending",
	...
	if deliverErr := service.DeliverAuditLogOutboxEvent("video:" + reservation.ReservationID()); deliverErr != nil {
		common.SysError("deliver video consume audit: " + deliverErr.Error())
	}
...
func RelayVideoTaskFetch(c *gin.Context) {
	...
	doubaoLookup := model.DB.Where("task_id = ? AND user_id = ? AND platform IN ?", taskID,
		common.GetUserId(c), model.DoubaoVideoTaskOperationPlatforms()).Limit(2).Find(&doubaoTasks)
	...
	aliWanLookup := model.DB.Where("task_id = ? AND user_id = ? AND platform = ?", taskID,
		common.GetUserId(c), aliWanTaskPlatform).Limit(2).Find(&aliWanTasks)
	...
	viduLookup := model.DB.Where("task_id = ? AND user_id = ? AND platform = ?", taskID,
		common.GetUserId(c), viduTaskPlatform).First(&viduTask).Error
	if viduLookup == nil {
	...
	geminiVeoLookup := model.DB.Where("task_id = ? AND user_id = ? AND platform IN ?", taskID,
		common.GetUserId(c), model.VeoTaskOperationPlatforms()).Limit(2).Find(&geminiVeoTasks)
	...
	hailuoLookup := model.DB.Where("task_id = ? AND user_id = ? AND platform = ?", taskID,
		common.GetUserId(c), hailuoTaskPlatform).Limit(2).Find(&hailuoTasks)
	...
	var task model.Task
	err := model.DB.Where("task_id = ? AND user_id = ?", taskID, common.GetUserId(c)).First(&task).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
...
func VideoProxy(c *gin.Context) {
	...
	if err != nil {
		common.SysError("fetch video content for " + task.TaskID + ": " + err.Error())
	...
	if _, err := io.Copy(c.Writer, response.Body); err != nil {
		common.SysError("stream video content for " + task.TaskID + ": " + err.Error())
	}
}
...
func videoChannelSupported(channel *model.Channel) bool {
	...
	typeID := constant.ChannelType(channel.Type)
	return typeID == constant.ChannelTypeSora || typeID == constant.ChannelTypeOpenAI
}
