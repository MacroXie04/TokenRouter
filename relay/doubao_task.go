...
func init() {
	service.RegisterAsyncTaskReconciler(reconcileAsyncDoubaoTasks)
	service.RegisterAsyncTaskPromoter(PromoteDoubaoTaskRecoveryJournalsContext)
}
...
func RelayDoubaoTask(c *gin.Context) {
	raw, err := common.ReadAllLimited(c.Request.Body, doubao.MaxRequestBodyBytes)
	if err != nil {
	...
	pricing, enabled, err := service.ResolveReferenceAsyncTaskBillingPlanForUser(
		userID, originModel, common.GetUserGroup(c), usingGroup,
	)
	if err != nil || !enabled {
		if err != nil {
			common.SysError("resolve Doubao pricing: " + err.Error())
		}
	...
	if err != nil {
		...
		if service.IsSubscriptionFundingErr(err) || errors.Is(err, service.ErrInsufficientQuota) ||
			...
			common.SysError("create atomic Doubao reservation: " + err.Error())
		}
	...
	defer func() {
		...
		if cleanupErr := refundDoubaoTask(&task, reservation, nil,
			...
			common.SysError("Doubao reservation cleanup failed for " + task.TaskID + ": " + cleanupErr.Error())
		}
	}()
	if err := markDoubaoTaskDispatching(&task, reservation); err != nil {
		common.SysError("mark Doubao dispatch: " + err.Error())
	...
	if submitErr != nil {
		if !doubao.SubmitWasDispatched(submitErr) || definitiveDoubaoRejection(submitErr) {
			if refundErr := refundDoubaoTask(&task, reservation, nil,
				...
				common.SysError("persist rejected Doubao task " + task.TaskID + ": " + refundErr.Error())
		...
		if persistErr := markDoubaoAmbiguousDispatch(&task, reservation.ReservationID(), doubaoUnknownDispatchReason); persistErr != nil {
			common.SysError("persist ambiguous Doubao task " + task.TaskID + ": " + persistErr.Error())
	...
	if journalErr != nil {
		common.SysError("persist accepted Doubao recovery journal " + task.TaskID + ": " + journalErr.Error())
	}
	var commitErr error
	...
	if commitErr != nil {
		common.SysError("commit accepted Doubao task " + task.TaskID + ": " + commitErr.Error())
		...
		if fallbackErr != nil {
			common.SysError("persist accepted Doubao fallback " + task.TaskID + ": " + fallbackErr.Error())
			if journalErr != nil {
				data["recovery_degraded"] = true
			}
		} else if cleanupErr := removeDoubaoRecoveryJournal(task.TaskID); cleanupErr != nil {
			common.SysError("remove accepted Doubao recovery journal " + task.TaskID + ": " + cleanupErr.Error())
		}
		writeDoubaoTaskError(c, http.StatusAccepted, "task_commit_pending",
	...
	if task.Status == model.TaskStatusSuccess {
		if deliverErr := service.DeliverAuditLogOutboxEvent(doubaoAuditEventID(reservation.ReservationID())); deliverErr != nil {
			common.SysError("deliver Doubao consume audit: " + deliverErr.Error())
		}
	}
...
func RelayDoubaoTaskFetch(c *gin.Context) {
	...
	result := model.DB.Where("task_id = ? AND user_id = ? AND platform IN ?", taskID,
		common.GetUserId(c), model.DoubaoVideoTaskOperationPlatforms()).Limit(2).Find(&tasks)
	if result.Error != nil {
...
func relayDoubaoTaskContent(c *gin.Context, task *model.Task) {
	...
	if _, err := io.Copy(c.Writer, io.LimitReader(response.Body, maxContentBytes)); err != nil {
		common.SysError("stream Doubao content for " + task.TaskID + ": " + err.Error())
	}
}
...
func selectDoubaoChannel(groups []string, modelName string) (*model.Channel, string, error) {
	...
	for attempt := 0; attempt < 64; attempt++ {
		...
		if channel != nil && channel.Id > 0 &&
			(constant.ChannelType(channel.Type) == constant.ChannelTypeDoubaoVideo ||
				constant.ChannelType(channel.Type) == constant.ChannelTypeVolcEngine) {
...
func applyDoubaoVideoInputRatio(
	...
	ratio float64,
) (service.ReferenceAsyncTaskBillingPlan, error) {
	...
	if multiplier.LessThanOrEqual(decimal.Zero) || multiplier.GreaterThan(decimal.NewFromInt(100)) {
		return service.ReferenceAsyncTaskBillingPlan{}, errors.New("Doubao video-input price ratio is invalid")
	}
	if pricing.UseFixedPrice {
		value, err := decimal.NewFromString(pricing.FixedPrice)
		if err != nil {
			return service.ReferenceAsyncTaskBillingPlan{}, errors.New("Doubao fixed price is invalid")
		}
		pricing.FixedPrice = value.Mul(multiplier).String()
	} else {
		value, err := decimal.NewFromString(pricing.ModelRatio)
		if err != nil {
			return service.ReferenceAsyncTaskBillingPlan{}, errors.New("Doubao model ratio is invalid")
		}
		pricing.ModelRatio = value.Mul(multiplier).String()
	}
	if err := pricing.Validate(); err != nil {
		return service.ReferenceAsyncTaskBillingPlan{}, err
	}
	return pricing, nil
}
