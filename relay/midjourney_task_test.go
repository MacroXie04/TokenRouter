...
func newMidjourneyTaskFixture(t *testing.T, provider *mockMidjourneyProvider) midjourneyTaskFixture {
	...
	for _, modelName := range midjourney.Models {
		modes[modelName] = service.BillingModeReference
	...
	user := model.User{Username: "midjourney-owner", Password: "x", Role: constant.RoleCommonUser,
	...
	weight := uint(1)
	channel := model.Channel{Name: "midjourney-mock", Type: int(constant.ChannelTypeMidjourney),
		Key: "upstream-midjourney-key", Status: constant.ChannelStatusEnabled,
...
func createPreparedMidjourneyTaskForRecovery(t *testing.T, fixture midjourneyTaskFixture,
	action midjourney.Action) (*model.Task, *service.RelayQuotaReservation) {
	t.Helper()
