...
func configureGeminiVeoLifecycleFixture(t *testing.T) relayAccountingFixture {
	...
	require.NoError(t, model.DB.Model(&model.Channel{}).Where("id = ?", fixture.channel.Id).
		Updates(map[string]any{
			"type": int(constant.ChannelTypeGemini), "key": geminiVeoTestCredential,
	...
	require.NoError(t, model.DB.Create(&model.Ability{Group: service.GroupDefault,
...
func TestGeminiVeoDefinitiveRejectionRefundsAndAmbiguityNeverReplays(t *testing.T) {
	...
	t.Run("ambiguous dispatch", func(t *testing.T) {
		...
		require.NoError(t, err)
		assert.Equal(t, service.RelayQuotaReviewKindGeminiVeoTask, review.ReviewKind)
...
func createPreparedGeminiVeoTaskForRecovery(t *testing.T,
	fixture relayAccountingFixture) (*model.Task, *service.RelayQuotaReservation) {
	t.Helper()
	plan, enabled, err := service.ResolveReferenceAsyncTaskBillingPlan(
		geminiVeoTestOriginModel, service.GroupDefault, service.GroupDefault)
	require.NoError(t, err)
	...
	routing, err := geminiVeoProviderDescriptor().PrepareRoutingSnapshot(&model.Channel{
		Id: fixture.channel.Id, Type: int(constant.ChannelTypeGemini), BaseURL: geminiVeoTestBaseURL,
	...
	task := &model.Task{CreatedAt: now, UpdatedAt: now, TaskID: taskID,
		...
		Group: service.GroupDefault, ChannelId: fixture.channel.Id, Quota: quota,
...
func TestGeminiVeoPricingSnapshotRejectsTamperingAndArithmeticOverflow(t *testing.T) {
	plan := service.ReferenceAsyncTaskBillingPlan{Version: 1, ModelName: geminiVeoTestOriginModel,
	...
	overflow := geminiVeoPricingSnapshot{Plan: service.ReferenceAsyncTaskBillingPlan{
