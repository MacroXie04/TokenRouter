...
func init() {
	service.RegisterAsyncTaskPromoter(PromoteMidjourneyTaskRecoveryJournalsContext)
	service.RegisterAsyncTaskReconciler(reconcileAsyncMidjourneyTasks)
}
...
func RelayMidjourney(c *gin.Context) {
	...
	case constant.RelayModeUnknown:
...
func submitMidjourneyTask(c *gin.Context, mode constant.RelayMode) {
	operation := midjourneyOperationForMode(mode)
	if operation == "" {
		writeMidjourneyError(c, http.StatusBadRequest, 4, "unknown Midjourney operation")
		return
	}
	raw, err := common.ReadAllLimited(c.Request.Body, midjourney.MaxRequestBodyBytes)
	if err != nil {
	...
	if prepared.Action == midjourney.ActionInpaint || prepared.Action == midjourney.ActionCustomZoom {
		pricing = service.ReferenceAsyncTaskBillingPlan{
			Version: 1, ModelName: prepared.Model, GroupRatio: "0", UseFixedPrice: true, FixedPrice: "0",
			FreeModel: service.ShouldSkipExplicitFreeModelPreConsume(prepared.Model),
		}
	}
	...
	if err != nil {
		...
		if service.IsSubscriptionFundingErr(err) || errors.Is(err, service.ErrInsufficientQuota) ||
			...
			common.SysError("create atomic Midjourney reservation: " + err.Error())
		}
	...
	if err := markMidjourneyTaskDispatching(&task, reservation); err != nil {
		common.SysError("mark Midjourney dispatch: " + err.Error())
	...
	if submitErr != nil {
		if midjourney.IsDefinitiveRejection(submitErr) {
			...
			if err := refundMidjourneyTask(&task, reservation, nil, code,
				...
				common.SysError("refund rejected Midjourney task: " + err.Error())
		...
		if !midjourney.SubmitWasDispatched(submitErr) {
			if err := refundMidjourneyTask(&task, reservation, nil, 5,
				...
				common.SysError("refund undispatched Midjourney task: " + err.Error())
				writeMidjourneyError(c, http.StatusInternalServerError, 4, "submission failed and quota cleanup is pending")
				return
			}
			finalized = true
		...
		if err := markMidjourneyAmbiguousDispatch(&task, reservation.ReservationID(), midjourneyUnknownDispatchReason); err != nil {
			common.SysError("persist ambiguous Midjourney dispatch: " + err.Error())
		}
		finalized = true
	...
	if journalErr != nil {
		common.SysError("persist accepted Midjourney recovery journal " + task.TaskID + ": " + journalErr.Error())
	}
	...
	if err != nil {
		common.SysError("persist accepted Midjourney task: " + err.Error())
		fallbackErr := persistAcceptedMidjourneyFallback(&task, reservation.ReservationID(),
			providerState.ProviderTaskID)
		if fallbackErr != nil {
			common.SysError("persist accepted Midjourney fallback " + task.TaskID + ": " + fallbackErr.Error())
		} else if cleanupErr := removeMidjourneyRecoveryJournal(task.TaskID); cleanupErr != nil {
			common.SysError("remove accepted Midjourney recovery journal " + task.TaskID + ": " + cleanupErr.Error())
		}
	...
	finalized = true
	if status == model.TaskStatusSuccess {
		_ = service.DeliverAuditLogOutboxEvent("mj:" + reservation.ReservationID())
	}
	properties := any(nil)
...
func handleMidjourneyUploadDispatch(c *gin.Context, prepared *midjourney.PreparedRequest,
	task *model.Task, reservation *service.RelayQuotaReservation, baseURL, channelKey string) {
	...
	if err := settleSuccessfulMidjourneyTask(task, reservation, state, response.Code,
		...
		common.SysError("settle Midjourney upload: " + err.Error())
...
func resolveMidjourneySubmitChannel(c *gin.Context, prepared *midjourney.PreparedRequest, groups []string) (
	*model.Channel, string, string, string, error,
	...
	if prepared.OriginalTaskID == "" {
		...
		key := service.GetChannelKey(channel)
	...
	if err := model.DB.Where("id = ? AND status = ?", parent.ChannelId, constant.ChannelStatusEnabled).
...
func loadOwnedMidjourneyTaskCredentials(c *gin.Context, taskID string, groups []string) (
	*model.Task, *model.Midjourney, midjourneyTaskProperties, midjourneyTaskPrivateData, string, string, error,
	...
	if err := model.DB.Where("task_id = ? AND user_id = ? AND platform = ?", taskID,
		common.GetUserId(c), midjourneyTaskPlatform).First(&task).Error; err != nil {
...
func fetchMidjourneyTask(c *gin.Context) {
	...
	if err := model.DB.Where("task_id = ? AND user_id = ? AND platform = ?", taskID,
		common.GetUserId(c), midjourneyTaskPlatform).First(&generic).Error; err != nil {
		writeMidjourneyError(c, http.StatusBadRequest, 4, "task_no_found")
		return
	}
...
func fetchMidjourneyTasks(c *gin.Context) {
	raw, err := common.ReadAllLimited(c.Request.Body, midjourneyFetchRequestMaxBytes)
	if err != nil {
		status := http.StatusBadRequest
		if errors.Is(err, common.ErrBodyTooLarge) {
			status = http.StatusRequestEntityTooLarge
	...
	if err := model.DB.Where("task_id IN ? AND user_id = ? AND platform = ?", request.IDs,
		common.GetUserId(c), midjourneyTaskPlatform).Find(&generic).Error; err != nil {
...
func fetchMidjourneyImageSeed(c *gin.Context) {
	...
	var channel model.Channel
	if err := model.DB.Where("id = ? AND status = ?", task.ChannelId, constant.ChannelStatusEnabled).
...
func isMidjourneyChannelType(channelType int) bool {
	t := constant.ChannelType(channelType)
	return t == constant.ChannelTypeMidjourney || t == constant.ChannelTypeMidjourneyPlus
}
...
func midjourneyOperationForMode(mode constant.RelayMode) string {
	switch mode {
	case constant.RelayModeMidjourneyAction:
		return "action"
	case constant.RelayModeMidjourneyShorten:
		return "shorten"
	case constant.RelayModeMidjourneyModal:
		return "modal"
	case constant.RelayModeMidjourneyImagine:
		return "imagine"
	case constant.RelayModeMidjourneyChange:
		return "change"
	case constant.RelayModeMidjourneySimpleChange:
	...
	case constant.RelayModeMidjourneyVideo:
		return "video"
	case constant.RelayModeSwapFace:
		return "swap"
	case constant.RelayModeMidjourneyUpload:
