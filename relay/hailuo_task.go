...
func init() {
	service.RegisterAsyncTaskPromoter(PromoteHailuoTaskRecoveryJournalsContext)
	service.RegisterAsyncTaskReconciler(reconcileAsyncHailuoTasks)
}
...
func RelayHailuoTask(c *gin.Context) {
	raw, err := common.ReadAllLimited(c.Request.Body, hailuo.MaxRequestBodyBytes)
	if err != nil {
	...
	if err != nil || !enabled {
		if err != nil {
			common.SysError("resolve Hailuo pricing: " + err.Error())
		}
	...
	if err != nil {
		...
		if service.IsSubscriptionFundingErr(err) || errors.Is(err, service.ErrInsufficientQuota) ||
			...
			common.SysError("create atomic Hailuo reservation: " + err.Error())
		}
	...
	defer func() {
		...
		if cleanupErr := refundRejectedHailuoTask(&task, reservation, "Hailuo submission stopped before provider dispatch"); cleanupErr != nil {
			common.SysError("Hailuo reservation cleanup failed for " + task.TaskID + ": " + cleanupErr.Error())
		}
	}()
	if err := markHailuoTaskDispatching(&task, reservation); err != nil {
		common.SysError("mark Hailuo dispatch: " + err.Error())
	...
	if submitErr != nil {
		if !hailuo.SubmitWasDispatched(submitErr) || definitiveHailuoRejection(submitErr) {
			if refundErr := refundRejectedHailuoTask(&task, reservation, "Hailuo provider rejected submission"); refundErr != nil {
				common.SysError("persist rejected Hailuo task " + task.TaskID + ": " + refundErr.Error())
		...
		if persistErr := markHailuoAmbiguousDispatch(&task, reservation.ReservationID(), hailuoUnknownDispatchReason); persistErr != nil {
			common.SysError("persist ambiguous Hailuo task " + task.TaskID + ": " + persistErr.Error())
	...
	if provider == nil || strings.TrimSpace(provider.ProviderTaskID) == "" {
		if persistErr := markHailuoAmbiguousDispatch(&task, reservation.ReservationID(), hailuoUnknownDispatchReason); persistErr != nil {
			common.SysError("persist Hailuo missing-id submission " + task.TaskID + ": " + persistErr.Error())
		}
		writeHailuoTaskError(c, http.StatusAccepted, "submit_outcome_unknown", hailuoUnknownDispatchReason,
			gin.H{"task_id": task.TaskID, "status": "unknown"})
		return
	...
	if journalErr != nil {
		common.SysError("persist accepted Hailuo recovery journal " + task.TaskID + ": " + journalErr.Error())
	}
	if err := settleAcceptedHailuoTask(&task, reservation, provider.ProviderTaskID, model.TaskOperationDispatching, ""); err != nil {
		common.SysError("settle accepted Hailuo task " + task.TaskID + ": " + err.Error())
		...
		if fallbackErr != nil {
			common.SysError("persist accepted Hailuo fallback " + task.TaskID + ": " + fallbackErr.Error())
			if journalErr != nil {
				data["recovery_degraded"] = true
			}
		} else if cleanupErr := removeHailuoRecoveryJournal(task.TaskID); cleanupErr != nil {
			common.SysError("remove accepted Hailuo recovery journal " + task.TaskID + ": " + cleanupErr.Error())
		}
		writeHailuoTaskError(c, http.StatusAccepted, "task_commit_pending",
	...
	if deliverErr := service.DeliverAuditLogOutboxEvent(hailuoAuditEventID(reservation.ReservationID())); deliverErr != nil {
		common.SysError("deliver Hailuo consume audit: " + deliverErr.Error())
	}
...
func RelayHailuoTaskFetch(c *gin.Context) {
	...
	if err := model.DB.Where("task_id = ? AND user_id = ? AND platform = ?", taskID,
		common.GetUserId(c), hailuoTaskPlatform).Limit(2).Find(&tasks).Error; err != nil {
...
func relayHailuoTaskContent(c *gin.Context, task *model.Task) {
	...
	if _, err := io.Copy(c.Writer, io.LimitReader(response.Body, maxContentBytes)); err != nil {
		common.SysError("stream Hailuo content for " + task.TaskID + ": " + err.Error())
	}
}
...
func selectHailuoChannel(groups []string, modelName string) (*model.Channel, string, error) {
	...
	for attempt := 0; attempt < 64; attempt++ {
		...
		if channel != nil && channel.Id > 0 && constant.ChannelType(channel.Type) == constant.ChannelTypeMiniMax {
