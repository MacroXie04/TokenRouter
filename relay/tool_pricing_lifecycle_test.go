...
func TestOrdinaryRelayRejectsModelQuotaClampAfterAcceptedWork(t *testing.T) {
	...
	err := relayAndSettleWithDispatch(c, info, func(c *gin.Context, _ *RelayInfo) (*protocolkit.Usage, error) {
		...
		return &protocolkit.Usage{PromptTokens: int(common.MaxQuota), TotalTokens: int(common.MaxQuota)}, nil
	})
...
func TestOrdinaryRelayRejectsCombinedModelAndToolQuotaOverflow(t *testing.T) {
	fixture := newRelayAccountingFixture(t, int(common.MaxQuota), int(common.MaxQuota))
	service.SetModelPriceRegistry(map[string]service.ModelPrice{
	...
	assertAcceptedWorkRemainsUnsettled(t, fixture, int(common.MaxQuota), int(common.MaxQuota))
}
