...
func init() {
	service.RegisterAsyncTaskPromoter(PromoteAliWanTaskRecoveryJournalsContext)
	service.RegisterAsyncTaskReconciler(reconcileAsyncAliWanTasks)
}
...
func RelayAliWanTask(c *gin.Context) {
	raw, err := common.ReadAllLimited(c.Request.Body, aliWan.MaxRequestBodyBytes)
	if err != nil {
	...
	basePricing, enabled, err := service.ResolveReferenceAsyncTaskBillingPlanForUser(
		userID, originModel, common.GetUserGroup(c), usingGroup,
	)
	if err != nil || !enabled {
		if err != nil {
			common.SysError("resolve AliWan pricing: " + err.Error())
		}
	...
	if err != nil {
		...
		if service.IsSubscriptionFundingErr(err) || errors.Is(err, service.ErrInsufficientQuota) ||
			...
			common.SysError("create atomic AliWan reservation: " + err.Error())
		}
	...
	defer func() {
		...
		if cleanupErr := refundRejectedAliWanTask(&task, reservation, "AliWan submission stopped before provider dispatch"); cleanupErr != nil {
			common.SysError("AliWan reservation cleanup failed for " + task.TaskID + ": " + cleanupErr.Error())
		}
	}()
	if err := markAliWanTaskDispatching(&task, reservation); err != nil {
		common.SysError("mark AliWan dispatch: " + err.Error())
	...
	if submitErr != nil {
		if !aliWan.SubmitWasDispatched(submitErr) || definitiveAliWanRejection(submitErr) {
			if refundErr := refundRejectedAliWanTask(&task, reservation, "AliWan provider rejected submission"); refundErr != nil {
				common.SysError("persist rejected AliWan task " + task.TaskID + ": " + refundErr.Error())
		...
		if persistErr := markAliWanAmbiguousDispatch(&task, reservation.ReservationID(), aliWanUnknownDispatchReason); persistErr != nil {
			common.SysError("persist ambiguous AliWan task " + task.TaskID + ": " + persistErr.Error())
	...
	if provider == nil || strings.TrimSpace(provider.ProviderTaskID) == "" {
		if persistErr := markAliWanAmbiguousDispatch(&task, reservation.ReservationID(), aliWanUnknownDispatchReason); persistErr != nil {
			common.SysError("persist AliWan missing-id submission " + task.TaskID + ": " + persistErr.Error())
		}
		writeAliWanTaskError(c, http.StatusAccepted, "submit_outcome_unknown", aliWanUnknownDispatchReason,
			gin.H{"task_id": task.TaskID, "status": "unknown"})
		return
	...
	if journalErr != nil {
		common.SysError("persist accepted AliWan recovery journal " + task.TaskID + ": " + journalErr.Error())
	}
	if err := settleAcceptedAliWanTask(&task, reservation, provider.ProviderTaskID, model.TaskOperationDispatching, ""); err != nil {
		common.SysError("settle accepted AliWan task " + task.TaskID + ": " + err.Error())
		...
		if fallbackErr != nil {
			common.SysError("persist accepted AliWan fallback " + task.TaskID + ": " + fallbackErr.Error())
			if journalErr != nil {
				data["recovery_degraded"] = true
			}
		} else if cleanupErr := removeAliWanRecoveryJournal(task.TaskID); cleanupErr != nil {
			common.SysError("remove accepted AliWan recovery journal " + task.TaskID + ": " + cleanupErr.Error())
		}
		writeAliWanTaskError(c, http.StatusAccepted, "task_commit_pending",
	...
	if deliverErr := service.DeliverAuditLogOutboxEvent(aliWanAuditEventID(reservation.ReservationID())); deliverErr != nil {
		common.SysError("deliver AliWan consume audit: " + deliverErr.Error())
	}
...
func RelayAliWanTaskFetch(c *gin.Context) {
	...
	if err := model.DB.Where("task_id = ? AND user_id = ? AND platform = ?", taskID,
		common.GetUserId(c), aliWanTaskPlatform).Limit(2).Find(&tasks).Error; err != nil {
...
func relayAliWanTaskContent(c *gin.Context, task *model.Task) {
	...
	if _, err := io.Copy(c.Writer, io.LimitReader(response.Body, maxContentBytes)); err != nil {
		common.SysError("stream AliWan content for " + task.TaskID + ": " + err.Error())
	}
}
...
func selectAliWanChannel(groups []string, modelName string) (*model.Channel, string, error) {
	...
	for attempt := 0; attempt < 64; attempt++ {
		...
		if channel != nil && channel.Id > 0 && constant.ChannelType(channel.Type) == constant.ChannelTypeAli {
