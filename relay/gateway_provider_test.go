...
func gatewayChannelTypes() []constant.ChannelType {
	return []constant.ChannelType{constant.ChannelTypeSub2API, constant.ChannelTypeNewAPI}
}

func TestGatewayProvidersOpenAIChatLifecycle(t *testing.T) {
	gin.SetMode(gin.TestMode)
	for _, channelType := range gatewayChannelTypes() {
		t.Run(constant.ChannelTypeName(channelType), func(t *testing.T) {
...
func TestNewAPIGatewayGeminiStreamPreservesURIAndSettles(t *testing.T) {
	...
	key := setupChannelIntegration(t, upstream.URL, constant.ChannelTypeNewAPI, "gemini-stream")
...
func TestNewAPIGatewayAcceptedMalformedResponseSettlesWithoutRetry(t *testing.T) {
	...
	service.SetModelPriceRegistry(map[string]service.ModelPrice{"claude-malformed": {Prompt: 2, Completion: 4}})
	...
	key, _ := setupClaudeRelay(t, upstream.URL, int(constant.ChannelTypeNewAPI), "claude-malformed")
	request := httptest.NewRequest(http.MethodPost, "/v1/messages",
...
func TestNewAPIGatewayDefinitiveRejectionRefunds(t *testing.T) {
	...
	key := setupChannelIntegration(t, upstream.URL, constant.ChannelTypeNewAPI, "gemini-rejected")
