...
func geminiVeoProviderDescriptor() VeoTaskProviderDescriptor {
	return VeoTaskProviderDescriptor{
		ChannelType: constant.ChannelTypeGemini, Platform: model.TaskOperationPlatformGeminiVeo,
		...
		CaptureCredential: func(channel *model.Channel) (string, error) {
			credential := strings.TrimSpace(service.GetChannelKey(channel))
		...
		PrepareRoutingSnapshot: func(channel *model.Channel, _ string) (string, error) {
			if channel == nil || constant.ChannelType(channel.Type) != constant.ChannelTypeGemini {
			...
			encoded, err := common.Marshal(geminiVeoRoutingSnapshot{BaseURL: baseURL})
