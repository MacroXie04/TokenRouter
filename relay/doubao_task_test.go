...
func configureDoubaoLifecycleFixture(t *testing.T, fixed bool, channelType constant.ChannelType) relayAccountingFixture {
	t.Helper()
	...
	for _, modelName := range doubao.ModelList() {
		require.NoError(t, model.DB.Create(&model.Ability{
			Group: service.GroupDefault, Model: modelName, ChannelId: fixture.channel.Id,
...
func TestDoubaoGenericRoutesPreserveSelectedPlatformAndFetchLocally(t *testing.T) {
	tests := []struct {
		name       string
		channel    constant.ChannelType
		platform   string
		submitPath string
		fetchPath  string
	}{
		{"VolcEngine legacy route", constant.ChannelTypeVolcEngine, model.TaskOperationPlatformVolcEngine,
			"/v1/video/generations", "/v1/video/generations/"},
		{"VolcEngine OpenAI route", constant.ChannelTypeVolcEngine, model.TaskOperationPlatformVolcEngine,
			"/v1/videos", "/v1/videos/"},
		{"Doubao video legacy route", constant.ChannelTypeDoubaoVideo, model.TaskOperationPlatformDoubaoVideo,
			"/v1/video/generations", "/v1/video/generations/"},
		{"Doubao video OpenAI route", constant.ChannelTypeDoubaoVideo, model.TaskOperationPlatformDoubaoVideo,
			"/v1/videos", "/v1/videos/"},
	}
...
func TestDoubaoDefinitiveRejectionRefundsButAmbiguousDispatchStaysHeld(t *testing.T) {
	t.Run("definitive provider rejection", func(t *testing.T) {
		fixture := configureDoubaoLifecycleFixture(t, true, constant.ChannelTypeDoubaoVideo)
		var calls atomic.Int32
	...
	t.Run("transport ambiguity requires explicit review", func(t *testing.T) {
		fixture := configureDoubaoLifecycleFixture(t, true, constant.ChannelTypeVolcEngine)
		...
		assert.Equal(t, service.RelayQuotaReviewKindDoubaoVideoTask, review.ReviewKind)
		assert.Equal(t, model.TaskOperationPlatformVolcEngine, review.TaskPlatform)
		assert.True(t, review.ResolutionRequired)
		assert.ElementsMatch(t, []string{service.RelayQuotaReviewResolutionSettle,
			service.RelayQuotaReviewResolutionRefund}, review.ResolutionOptions)
		...
		resolved, err := ResolveDoubaoRelayQuotaReservationReview(
			reservation.ReservationID, 9101, service.RelayQuotaReviewResolutionRefund,
		)
		require.NoError(t, err)
		...
		replay, err := ResolveDoubaoRelayQuotaReservationReview(
			reservation.ReservationID, 9102, service.RelayQuotaReviewResolutionRefund,
		)
		require.NoError(t, err)
...
func TestDoubaoPollingUsesImmutableFixedAndRatioPricing(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			fixture := configureDoubaoLifecycleFixture(t, test.fixed, constant.ChannelTypeVolcEngine)
			...
			var logs []model.Log
			require.NoError(t, model.LOG_DB.Where("type = ?", service.LogTypeConsume).Find(&logs).Error)
			...
			require.NoError(t, model.LOG_DB.Model(&model.Log{}).
				Where("type = ?", service.LogTypeConsume).Count(&logCount).Error)
...
func TestDoubaoCompletedContentAndFetchAreOwnerAndPlatformScoped(t *testing.T) {
	...
	require.NoError(t, model.DB.Create(&model.Task{
		CreatedAt: common.NowTimestamp(), UpdatedAt: common.NowTimestamp(), TaskID: foreignID,
		Platform: model.TaskOperationPlatformKling, UserId: fixture.user.Id, Group: service.GroupDefault,
...
func createPreparedDoubaoTaskForRecovery(
	...
	fixture relayAccountingFixture,
	platform string,
) (*model.Task, *service.RelayQuotaReservation) {
	t.Helper()
	pricing, enabled, err := service.ResolveReferenceAsyncTaskBillingPlan(
		doubaoLifecycleBaseModel, service.GroupDefault, service.GroupDefault,
	)
	require.NoError(t, err)
	...
	task := &model.Task{
		...
		UserId: fixture.user.Id, Group: service.GroupDefault, ChannelId: fixture.channel.Id,
