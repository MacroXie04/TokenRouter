...
func GetRelayFormat(channelType constant.ChannelType, mode constant.RelayMode) constant.RelayFormat {
	...
	case constant.RelayModeAlphaSearch:
		return constant.RelayFormatOpenAIAlphaSearch
...
func SanitizeUpstreamError(err error, secrets ...string) error {
	...
	copy.Body = appcommon.RedactSensitiveText(body)
	return &copy
}
...
func WriteUpstreamError(c *gin.Context, err error) {
	...
	msg := appcommon.RedactSensitiveText(err.Error())
	body := ""
	var ue *UpstreamError
	if errors.As(err, &ue) && ue != nil {
		...
		if int64(len(ue.Body)) > MaxUpstreamErrorBodyBytes {
			...
			body = appcommon.RedactSensitiveText(ue.Body)
			msg = body
		}
	}
	if body != "" {
