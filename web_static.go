...
func newEmbeddedWebHandler(dist fs.FS) (*embeddedWebHandler, error) {
	...
	if common.GetEnvBool("GLOBAL_WEB_RATE_LIMIT_ENABLE", true) {
		...
		handler.limiter = common.NewRateLimiter(limit, time.Duration(durationSeconds)*time.Second, "web")
	}
	return handler, nil
}

func boundedEnvInt(name string, fallback, minimum, maximum int) int {
	value := common.GetEnvInt(name, fallback)
...
func (handler *embeddedWebHandler) allow(c *gin.Context) bool {
	...
	if err != nil {
		...
		common.Logger.Warn("web rate limiter store error; failing open", "err", err.Error())
		return true
	}
	if allowed {
		return true
...
func injectAnalytics(document string) string {
	...
	if gaID, ok := safeAnalyticsID(common.GetEnv("GOOGLE_ANALYTICS_ID", "")); ok {
	...
	if websiteID, ok := safeAnalyticsID(common.GetEnv("UMAMI_WEBSITE_ID", "")); ok {
		if scriptURL, valid := safeAnalyticsScriptURL(common.GetEnv("UMAMI_SCRIPT_URL", "https://analytics.umami.is/script.js")); valid {
