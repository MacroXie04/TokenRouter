...
func createChannelBalanceFixture(t *testing.T, channelType constant.ChannelType, settingJSON string) *model.Channel {
	t.Helper()
...
func TestFetchChannelBalanceProviderContracts(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "AIProxy", channelType: constant.ChannelTypeAIProxy,
		...
		{
			name: "API2GPT", channelType: constant.ChannelTypeAPI2GPT,
		...
		{
			name: "OpenRouter", channelType: constant.ChannelTypeOpenRouter,
			wantURL:    "https://openrouter.ai/api/v1/credits",
			wantHeader: "Authorization", wantHeaderValue: "Bearer primary-secret", absentHeader: "Api-Key",
			body: `{"data":{"total_credits":12.5,"total_usage":3}}`, wantBalance: 9.5,
		},
		{
			name: "Moonshot", channelType: constant.ChannelTypeMoonshot,
		...
		{
			name: "configured balance_url", channelType: constant.ChannelTypeAzure,
...
func TestFetchChannelBalancePreservesFiniteNegativeProviderBalances(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "numeric response", channelType: constant.ChannelTypeAPI2GPT, body: `{"total_remaining":-2.5}`, want: -2.5},
		{name: "text response", channelType: constant.ChannelTypeSiliconFlow, body: `{"code":20000,"data":{"totalBalance":"-4.75"}}`, want: -4.75},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
...
func TestFetchChannelBalanceRejectsMalformedProviderResponsesWithoutMutation(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "malformed JSON", channelType: constant.ChannelTypeAIProxy, body: `{"success":`},
		{name: "AIProxy reports failure", channelType: constant.ChannelTypeAIProxy, body: `{"success":false,"error_code":401}`},
		{name: "AIProxy omits points", channelType: constant.ChannelTypeAIProxy, body: `{"success":true,"data":{}}`},
		{name: "API2GPT omits remaining", channelType: constant.ChannelTypeAPI2GPT, body: `{}`},
		{name: "AIGC2D oversized value", channelType: constant.ChannelTypeAIGC2D, body: `{"total_available":1000000000000001}`},
		{name: "SiliconFlow reports failure", channelType: constant.ChannelTypeSiliconFlow, body: `{"code":40001,"data":{"totalBalance":"1"}}`},
		{name: "SiliconFlow non-finite text", channelType: constant.ChannelTypeSiliconFlow, body: `{"code":20000,"data":{"totalBalance":"NaN"}}`},
		{name: "DeepSeek has no CNY", channelType: constant.ChannelTypeDeepSeek, body: `{"balance_infos":[{"currency":"USD","total_balance":"1"}]}`},
		{name: "DeepSeek cardinality is bounded", channelType: constant.ChannelTypeDeepSeek, body: `{"balance_infos":[` + strings.Join(tooManyDeepSeekBalances, ",") + `]}`},
		{name: "DeepSeek non-finite text", channelType: constant.ChannelTypeDeepSeek, body: `{"balance_infos":[{"currency":"CNY","total_balance":"+Inf"}]}`},
		{name: "OpenRouter omits usage", channelType: constant.ChannelTypeOpenRouter, body: `{"data":{"total_credits":10}}`},
		{name: "OpenRouter non-finite number", channelType: constant.ChannelTypeOpenRouter, body: `{"data":{"total_credits":1e9999,"total_usage":1}}`},
		{name: "Moonshot reports failure", channelType: constant.ChannelTypeMoonshot, body: `{"code":1,"status":false,"data":{"available_balance":73}}`},
		{name: "Moonshot omits balance", channelType: constant.ChannelTypeMoonshot, body: `{"code":0,"status":true,"data":{}}`},
	}
...
func TestFetchChannelBalanceRejectsMissingOpenAIFieldsWithoutMutation(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			channel := createChannelBalanceFixture(t, constant.ChannelTypeOpenAI, "")
			calls := 0
			useChannelBalanceTransport(t, func(request *http.Request) (*http.Response, error) {
				calls++
...
func TestFetchChannelBalanceRejectsMalformedConfiguredURLWithoutFallback(t *testing.T) {
	...
	for _, settingJSON := range settings {
		channel := createChannelBalanceFixture(t, constant.ChannelTypeDeepSeek, settingJSON)
		called := false
...
func TestFetchChannelBalanceCustomChannelWithoutBaseURLDoesNotFallBackToOpenAI(t *testing.T) {
	setupChannelBalanceTestDB(t)
	channel := createChannelBalanceFixture(t, constant.ChannelTypeCustom, "")
	called := false
	useChannelBalanceTransport(t, func(*http.Request) (*http.Response, error) {
		called = true
		return nil, errors.New("must not be called")
...
func TestFetchChannelBalanceRejectsOversizedResponseWithoutMutation(t *testing.T) {
	setupChannelBalanceTestDB(t)
	channel := createChannelBalanceFixture(t, constant.ChannelTypeAPI2GPT, "")
	useChannelBalanceTransport(t, func(request *http.Request) (*http.Response, error) {
...
func TestFetchChannelBalanceSanitizesTransportFailureAndDoesNotMutate(t *testing.T) {
	...
	channel := createChannelBalanceFixture(t, constant.ChannelTypeAzure, settingJSON)
	useChannelBalanceTransport(t, func(*http.Request) (*http.Response, error) {
...
func TestFetchChannelBalanceHTTPFailureDoesNotMutate(t *testing.T) {
	setupChannelBalanceTestDB(t)
	channel := createChannelBalanceFixture(t, constant.ChannelTypeAIGC2D, "")
	useChannelBalanceTransport(t, func(request *http.Request) (*http.Response, error) {
...
func TestFetchChannelBalanceMissingRowIsNotReportedAsSuccess(t *testing.T) {
	setupChannelBalanceTestDB(t)
	channel := &model.Channel{
		Id: 999, Type: int(constant.ChannelTypeAPI2GPT), Key: "primary-secret",
...
func TestUpdateAllChannelsBalancesHonorsAutoBanAndSkipsMultiKey(t *testing.T) {
	...
	channels := []*model.Channel{
		{
			Type: int(constant.ChannelTypeAPI2GPT), Name: "auto-ban-off", Key: "off-secret",
			Status: constant.ChannelStatusEnabled, AutoBan: &autoBanOff, Balance: 10, BalanceUpdatedTime: 10,
		},
		{
			Type: int(constant.ChannelTypeAPI2GPT), Name: "auto-ban-on", Key: "on-secret",
			Status: constant.ChannelStatusEnabled, AutoBan: &autoBanOn, Balance: 10, BalanceUpdatedTime: 10,
		},
		{
			Type: int(constant.ChannelTypeAPI2GPT), Name: "multi-key", Key: "multi-one\nmulti-two",
			Status: constant.ChannelStatusEnabled, AutoBan: &autoBanOn, Balance: 10, BalanceUpdatedTime: 10,
	...
	assert.Equal(t, constant.ChannelStatusEnabled, stored[0].Status)
	assert.Equal(t, -1.0, stored[0].Balance)
	assert.Equal(t, constant.ChannelStatusAutoDisabled, stored[1].Status)
	assert.Equal(t, -1.0, stored[1].Balance)
	assert.Equal(t, constant.ChannelStatusEnabled, stored[2].Status)
...
func TestUpdateAllChannelsBalancesStopsOnCancellation(t *testing.T) {
	...
	for _, key := range []string{"first-secret", "second-secret"} {
		require.NoError(t, model.DB.Create(&model.Channel{
			Type: int(constant.ChannelTypeAPI2GPT), Name: key, Key: key,
			Status: constant.ChannelStatusEnabled, Balance: 10, BalanceUpdatedTime: 10,
		}).Error)
	}
	ctx, cancel := context.WithCancel(context.Background())
	t.Cleanup(cancel)
...
func TestUpdateAllChannelsBalancesRejectsOverlappingSweeps(t *testing.T) {
	setupChannelBalanceTestDB(t)
	require.NoError(t, model.DB.Create(&model.Channel{
		Type: int(constant.ChannelTypeAPI2GPT), Name: "blocking", Key: "blocking-secret",
		Status: constant.ChannelStatusEnabled, Balance: 10, BalanceUpdatedTime: 10,
	}).Error)
