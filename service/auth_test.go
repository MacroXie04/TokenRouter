...
func TestPasswordLoginInvalidInputsAndStoredHashesUseBcryptSentinel(t *testing.T) {
	validHash, err := common.PasswordHash("password123")
	require.NoError(t, err)
	...
	highCostHash := loginPasswordSentinelAtCost(common.PasswordBcryptTargetCost + 1)
	...
	require.NoError(t, model.DB.Create(&model.User{
		Username: "oauth-only-login", Password: "", Role: constant.RoleCommonUser,
		Status: model.UserStatusEnabled, Group: "default", AuthVersion: 1,
	}).Error)
...
func TestPasswordLoginVerificationPlanEqualizesEveryAcceptedCost(t *testing.T) {
	for storedCost := bcrypt.MinCost; storedCost <= common.PasswordBcryptTargetCost; storedCost++ {
		...
		assert.Equal(t, 1<<common.PasswordBcryptTargetCost, totalWork,
			"accepted cost %d must receive exactly the target work class", storedCost)
	}

	for _, invalidHash := range []string{"", "not-a-bcrypt-hash", loginPasswordSentinelAtCost(common.PasswordBcryptTargetCost + 1)} {
		plan, parsedCost, valid := loginPasswordVerificationPlan(invalidHash)
		assert.False(t, valid)
		assert.Equal(t, common.PasswordBcryptTargetCost, parsedCost)
		require.Len(t, plan, 1)
		assert.False(t, plan[0].primary)
		cost, err := bcrypt.Cost([]byte(plan[0].hash))
		require.NoError(t, err)
		assert.Equal(t, common.PasswordBcryptTargetCost, cost)
	}
}
...
func TestPasswordLoginUpgradesLowCostHashWithCompareAndSwap(t *testing.T) {
	...
	require.NoError(t, err)
	user := &model.User{
		Username: "legacy-low-cost", Password: string(legacyHash), Role: constant.RoleCommonUser,
		Status: model.UserStatusEnabled, Group: "default", AuthVersion: 1,
	}
	...
	require.NoError(t, err)
	assert.Equal(t, common.PasswordBcryptTargetCost, upgradedCost)
	assert.True(t, common.PasswordVerify("legacy-password", stored.Password))

	replacement, err := common.PasswordHash("new-password")
	require.NoError(t, err)
	stale := stored
