...
func validateChannelForCreate(channel *model.Channel) error {
	...
	if channel.Status == 0 {
		channel.Status = constant.ChannelStatusEnabled
	}
	if channel.Status != constant.ChannelStatusEnabled &&
		channel.Status != constant.ChannelStatusAutoDisabled &&
		channel.Status != constant.ChannelStatusManuallyDisabled {
...
func validateChannelJSONObject(field, value string) error {
	...
	if err := common.UnmarshalJsonStr(value, &object); err != nil || object == nil {
