...
func newWalletViolationFeeFixture(t *testing.T, channelType constant.ChannelType) violationFeeFixture {
	t.Helper()
	...
	channel := model.Channel{
		...
		Status: constant.ChannelStatusEnabled, UsedQuota: 7,
	}
...
func violationFeeInput(fixture violationFeeFixture) GrokViolationFeeInput {
	return GrokViolationFeeInput{
		...
		ChannelID:     fixture.channel.Id, ModelName: "grok-4", Group: GroupDefault,
...
func TestChargeGrokViolationFeeRejectsUnsafeAuditMetadataBeforeIntent(t *testing.T) {
	fixture := newWalletViolationFeeFixture(t, constant.ChannelTypeXai)
		...
		},
		{
			...
			mutate: func(fixture violationFeeFixture) {
				require.NoError(t, fixture.db.Model(&model.Channel{}).Where("id = ?", fixture.channel.Id).
					Update("type", int(constant.ChannelTypeOpenAI)).Error)
			},
		},
		{
...
func TestChargeGrokViolationFeeUsesOriginalSubscriptionEpoch(t *testing.T) {
	...
	subscription := model.UserSubscription{
		...
		StartTime: common.NowTimestamp() - 60, EndTime: common.NowTimestamp() + 3600,
	...
	require.NoError(t, db.Create(&token).Error)
	channel := model.Channel{Name: "xai-sub", Type: int(constant.ChannelTypeXai), Key: "upstream", Status: constant.ChannelStatusEnabled}
	require.NoError(t, db.Create(&channel).Error)
	reservation, err := NewRelayQuotaReservation(user.Id, &token, 5)
	require.NoError(t, err)
	require.NoError(t, reservation.MarkDispatched())
