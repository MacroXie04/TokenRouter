...
func DecideChannelHealthTransition(policy setting.ChannelReliabilitySetting, input ChannelHealthPolicyInput) ChannelStatusTransition {
	...
	if input.AllowDisable && input.AutoBan && input.Status == constant.ChannelStatusEnabled &&
		policy.AutomaticDisableChannelEnabled && (failureReason != "" || latencyExceeded) {
		decision.NextStatus = constant.ChannelStatusAutoDisabled
	...
	if input.Failure == nil && !latencyExceeded && !input.LocalError &&
		policy.AutomaticEnableChannelEnabled && input.Status == constant.ChannelStatusAutoDisabled {
		decision.NextStatus = constant.ChannelStatusEnabled
...
func SelectChannelsForReliabilityTest(channels []model.Channel, mode string) ([]model.Channel, error) {
	...
	for i := range channels {
		...
		if !channelHealthSupported(constant.ChannelType(channel.Type)) {
			continue
		}
		if mode == setting.ChannelTestModeAutoBanOnly && !channelAutoBanEnabled(channel) {
			continue
		}
		if mode == setting.ChannelTestModePassiveRecovery && channel.Status != constant.ChannelStatusAutoDisabled {
			continue
		}
