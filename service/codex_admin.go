...
func defaultCodexAdminRuntime() codexAdminRuntime {
	return codexAdminRuntime{
		...
		now:           time.Now,
		requestID:     common.SecureRandomUUID,
	}
}
...
func codexRequestURL(baseURL, endpoint string) (string, error) {
	...
	// is enabled.
	if parsed.Scheme != "https" && !common.SSRFDisabled() {
...
func refreshCodexOAuthToken(
	...
	body, err := common.ReadAllLimited(resp.Body, codexOAuthResponseLimit)
	if err != nil {
...
func loadCodexChannelCredential(channelID int, requireUsageFields bool) (*model.Channel, *CodexOAuthKey, string, error) {
	...
	if channel.Type != int(constant.ChannelTypeCodex) {
...
func fetchCodexWham(
	...
	defer resp.Body.Close()
	responseBody, err := common.ReadAllLimited(resp.Body, codexWhamResponseLimit)
	if err != nil {
...
func consumeCodexWhamRateLimitResetCredit(
	...
	body, err := common.Marshal(map[string]string{"redeem_request_id": requestID})
	if err != nil {
