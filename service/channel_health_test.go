...
func TestChannelHealthPolicyGatesAndPassiveMode(t *testing.T) {
	...
	channel := model.Channel{
		Name: "policy", Type: int(constant.ChannelTypeOpenAI), Key: "k",
		Status: constant.ChannelStatusEnabled, BaseURL: "https://health.example.test",
		TestModel: "gpt-4", AutoBan: &autoBan,
	}
	require.NoError(t, model.DB.Create(&channel).Error)

	success, _, err := testAndRecordChannelContextWithMode(
		context.Background(), &channel, common.NowTimestamp(), false,
	)
	...
	success, _, err = testAndRecordChannelContextWithMode(
		context.Background(), &stored, common.NowTimestamp(), true,
	)
	assert.False(t, success)
	require.NoError(t, err)
	require.NoError(t, model.DB.First(&stored, channel.Id).Error)
...
func TestChannelHealthLocalErrorsNeverDisableAndKeywordsCanDisable(t *testing.T) {
	...
	autoBan := 1
	local := model.Channel{
		Name: "unsupported", Type: int(constant.ChannelTypeMidjourney), Status: constant.ChannelStatusEnabled,
		TestModel: "mj", AutoBan: &autoBan,
	}
	require.NoError(t, model.DB.Create(&local).Error)
	success, _, err := testAndRecordChannel(&local, common.NowTimestamp())
	assert.False(t, success)
	require.NoError(t, err)
	var stored model.Channel
	...
	keyword := model.Channel{
		Name: "keyword", Type: int(constant.ChannelTypeOpenAI), Key: "k",
		Status: constant.ChannelStatusEnabled, BaseURL: "https://health.example.test",
		TestModel: "gpt-4", AutoBan: &autoBan,
	}
	require.NoError(t, model.DB.Create(&keyword).Error)
	success, _, err = testAndRecordChannel(&keyword, common.NowTimestamp())
	assert.False(t, success)
	require.NoError(t, err)
	stored = model.Channel{}
	require.NoError(t, model.DB.First(&stored, keyword.Id).Error)
	assert.Equal(t, constant.ChannelStatusAutoDisabled, stored.Status)
}
...
func TestRunChannelHealthTestsReturnsPersistenceFailuresAndContinues(t *testing.T) {
	...
	first := model.Channel{Name: "first", Type: int(constant.ChannelTypeOpenAI), Key: "k", Status: constant.ChannelStatusEnabled, BaseURL: upstream.URL, TestModel: "gpt-4"}
	second := model.Channel{Name: "second", Type: int(constant.ChannelTypeOpenAI), Key: "k", Status: constant.ChannelStatusEnabled, BaseURL: upstream.URL, TestModel: "gpt-4"}
...
func TestChannelHealthStatusAndAbilityWriteRollBackTogether(t *testing.T) {
	...
	autoBan := 1
	channel := model.Channel{
		Name: "rollback", Type: int(constant.ChannelTypeOpenAI), Key: "k", Status: constant.ChannelStatusEnabled,
	...
	success, _, err := testAndRecordChannel(&channel, common.NowTimestamp())
	assert.False(t, success)
	...
	assert.Equal(t, constant.ChannelStatusEnabled, gotChannel.Status)
	...
	require.NoError(t, model.DB.First(&gotChannel, channel.Id).Error)
	require.NoError(t, model.DB.First(&gotAbility, ability.Group, ability.Model, ability.ChannelId).Error)
	assert.Equal(t, constant.ChannelStatusAutoDisabled, gotChannel.Status)
...
func TestChannelHealthUsesProviderWireContractAndLeastCredential(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Anthropic", channelType: constant.ChannelTypeAnthropic, key: "anthropic-secret",
			mappedModel: "claude-upstream", wantPath: "/v1/messages",
			wantHeader: "x-api-key", wantHeaderValue: "anthropic-secret",
		},
		{
			name: "Gemini", channelType: constant.ChannelTypeGemini, key: "gemini-secret",
		...
		{
			name: "Azure", channelType: constant.ChannelTypeAzure, key: "azure-secret", other: "2025-04-01-preview",
		...
		{
			name: "Zhipu v4", channelType: constant.ChannelTypeZhipuV4, key: "zhipu-v4-secret",
		...
		{
			name: "MiniMax", channelType: constant.ChannelTypeMiniMax, key: "minimax-secret",
		...
		{
			name: "Submodel", channelType: constant.ChannelTypeSubmodel, key: "submodel-secret",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
				...
				if test.channelType != constant.ChannelTypeGemini {
				...
				if test.channelType == constant.ChannelTypeCohere {
				...
				if test.channelType == constant.ChannelTypeAli {
					assert.Equal(t, "application/json", request.Header.Get("Accept"))
					assert.EqualValues(t, 1, payload["max_tokens"])
					assert.Len(t, payload["messages"], 1)
				}
				if test.channelType == constant.ChannelTypeZhipuV4 || test.channelType == constant.ChannelTypeMiniMax {
					assert.Equal(t, "application/json", request.Header.Get("Accept"))
					assert.EqualValues(t, 1, payload["max_tokens"])
					assert.Len(t, payload["messages"], 1)
				}
			...
			channel := model.Channel{
				...
				Status: constant.ChannelStatusEnabled, BaseURL: upstream.URL, TestModel: "client-model",
...
func TestZhipuLegacyChannelHealthUsesSignedV3Contract(t *testing.T) {
	...
	channel := model.Channel{
		Name: "zhipu-v3-health", Type: int(constant.ChannelTypeZhipu), Key: "health-id.health-secret",
		Status: constant.ChannelStatusEnabled, BaseURL: upstream.URL, TestModel: "client-model",
...
func TestZhipuChannelHealthRequiresCredentialBeforeDispatch(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "legacy", channelType: constant.ChannelTypeZhipu, modelName: "chatglm_pro"},
		{name: "v4", channelType: constant.ChannelTypeZhipuV4, modelName: "glm-4-plus"},
	} {
		t.Run(test.name, func(t *testing.T) {
...
func TestCohereChannelHealthRequiresCredentialBeforeDispatch(t *testing.T) {
	request, err := buildChannelHealthRequest(t.Context(), &model.Channel{
		Type: int(constant.ChannelTypeCohere), BaseURL: "https://cohere.example",
	}, "command-r-plus")
...
func TestAWSChannelHealthUsesBedrockWireAndLeastCredential(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			channel := &model.Channel{
				Type: int(constant.ChannelTypeAws), Key: test.credential,
	...
	request, err := buildChannelHealthRequest(t.Context(), &model.Channel{
		Type: int(constant.ChannelTypeAws), BaseURL: "https://bedrock.example.test",
...
func TestMiniMaxChannelHealthRequiresCredentialBeforeDispatch(t *testing.T) {
	request, err := buildChannelHealthRequest(t.Context(), &model.Channel{
		Type: int(constant.ChannelTypeMiniMax), BaseURL: "https://api.minimax.chat",
	}, "MiniMax-M2.7")
	require.ErrorContains(t, err, "API key is empty")
	assert.Nil(t, request)
}
...
func TestAliChannelHealthRequiresCredentialBeforeDispatch(t *testing.T) {
	request, err := buildChannelHealthRequest(t.Context(), &model.Channel{
		Type: int(constant.ChannelTypeAli), BaseURL: "https://dashscope.example",
	}, "qwen-plus")
	require.ErrorContains(t, err, "API key is empty")
	assert.Nil(t, request)
}
...
func TestDifyChannelHealthUsesChatMessagesWithoutForwardingModel(t *testing.T) {
	channel := &model.Channel{
		Type: int(constant.ChannelTypeDify), BaseURL: "https://api.dify.test/gateway", Key: "dify-secret",
...
func TestCozeChannelHealthUsesBotContract(t *testing.T) {
	channel := &model.Channel{
		Type: int(constant.ChannelTypeCoze), BaseURL: "https://api.coze.test/gateway", Key: "coze-secret",
...
func TestMokaAIChannelHealthUsesEmbeddingContract(t *testing.T) {
	channel := &model.Channel{
		Type: int(constant.ChannelTypeMokaAI), BaseURL: "https://moka.example/gateway", Key: "moka-secret",
...
func TestPaLMChannelHealthUsesGenerateMessageContract(t *testing.T) {
	request, err := buildChannelHealthRequest(t.Context(), &model.Channel{
		Type: int(constant.ChannelTypePaLM), BaseURL: "https://palm.example/gateway", Key: "palm-secret",
	...
	_, err = buildChannelHealthRequest(t.Context(), &model.Channel{
		Type: int(constant.ChannelTypePaLM), BaseURL: "https://palm.example", Key: "",
	}, "PaLM-2")
	assert.ErrorContains(t, err, "API key")
}
...
func TestBaiduChannelHealthUsesProviderAuthenticationAndWireContracts(t *testing.T) {
	...
	legacy, err := buildChannelHealthRequest(t.Context(), &model.Channel{
		Type: int(constant.ChannelTypeBaidu), BaseURL: upstream.URL + "/gateway", Key: "client-id|client-secret",
	}, "ERNIE-4.0-8K")
	require.NoError(t, err)
	...
	v2, err := buildChannelHealthRequest(t.Context(), &model.Channel{
		Type: int(constant.ChannelTypeBaiduV2), BaseURL: "https://qianfan.example/gateway", Key: "bearer-token|app-id",
	}, "ernie-4.0-turbo-8k")
	require.NoError(t, err)
...
func TestBaiduV2ChannelHealthRequiresCredentialBeforeDispatch(t *testing.T) {
	request, err := buildChannelHealthRequest(t.Context(), &model.Channel{
		Type: int(constant.ChannelTypeBaiduV2), BaseURL: "https://qianfan.example",
	}, "ernie-4.0-turbo-8k")
...
func TestChannelHealthCurrentMultiKeyNeverTransmitsCollectionOrDisabledKey(t *testing.T) {
	...
	channel := model.Channel{
		Name: "multi-health", Type: int(constant.ChannelTypeOpenAI),
		Key: "disabled-secret\nenabled-secret", ChannelInfo: `{"is_multi_key":true,"multi_key_size":2,"multi_key_status_list":{"0":2,"1":1}}`,
		Status: constant.ChannelStatusEnabled, BaseURL: upstream.URL, TestModel: "gpt-4",
	}
	require.NoError(t, model.DB.Create(&channel).Error)
	success, _, message := TestChannelHealthDetailed(channel.Id, "")
	assert.True(t, success, message)
...
func TestScheduledHealthSkipsUnsupportedProviderWithoutAutoDisable(t *testing.T) {
	...
	autoBan := 1
	channel := model.Channel{
		Name: "unsupported-task", Type: int(constant.ChannelTypeJimeng), Key: "provider-secret",
		Status: constant.ChannelStatusEnabled, BaseURL: upstream.URL, TestModel: "jimeng-video", AutoBan: &autoBan,
	}
	require.NoError(t, model.DB.Create(&channel).Error)
	...
	assert.Equal(t, constant.ChannelStatusEnabled, channel.Status)
