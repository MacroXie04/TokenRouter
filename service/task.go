...
func RunScheduledWithLeaseContext(taskType string, interval time.Duration, fn func(context.Context) error) error {
	...
	if taskType == "" || interval <= 0 || fn == nil {
		...
		common.SysError("leased task failed: " + err.Error())
		return err
	}
	...
	runnerID, err := taskLeaseOwnerToken(common.GetEnv("NODE_NAME", "tokenrouter-node-1"))
	if err != nil {
...
func runWithLeaseContext(
	...
	defer func() {
		if releasePending {
			if releaseErr := ReleaseTaskLock(taskType, leaseOwner); releaseErr != nil {
				common.SysError("release abandoned " + taskType + " lease: " + releaseErr.Error())
			}
		}
	}()
	taskID, err := common.SecureRandomUUID()
	if err != nil {
	...
	if finalErr != nil {
		common.SysError("leased task failed: " + finalErr.Error())
	}
	return finalErr
}
...
func logScheduledTaskError(taskType, operation string, err error) error {
	...
	common.SysError("scheduled task failed: " + wrapped.Error())
	return wrapped
}
...
func boundedPeriodicTaskError(err error) string {
	...
	const maxBytes = 1024
	message := common.RedactSensitiveText(err.Error())
