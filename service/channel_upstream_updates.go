...
func ValidateChannelOtherSettingsForType(raw string, channelType int) error {
	...
	if encoded, exists := rawFields["advanced_custom"]; exists && string(encoded) != "null" {
		if err := common.Unmarshal(encoded, &advanced); err != nil || advanced == nil {
	...
	if channelType == int(constant.ChannelTypeAdvancedCustom) {
		if advanced == nil {
...
func decodeChannelUpstreamModelSettings(raw string) (ChannelUpstreamModelSettings, map[string]json.RawMessage, error) {
	...
	if err := common.Unmarshal([]byte(trimmed), &rawFields); err != nil {
		return settings, nil, errors.New("channel settings must be a JSON object")
	}
	if rawFields == nil {
		return settings, nil, errors.New("channel settings must be a JSON object")
	}
	if err := common.Unmarshal([]byte(trimmed), &settings); err != nil {
...
func encodeChannelUpstreamModelSettings(settings ChannelUpstreamModelSettings, rawFields map[string]json.RawMessage) (string, error) {
	...
	if err := common.Unmarshal(encoded, &knownFields); err != nil {
		return "", err
	}
...
func detectChannelUpstreamModelUpdates(ctx context.Context, channelID int, force, allowAutoApply, refreshCache, requireEnabled bool) (*ChannelUpstreamDetection, bool, error) {
	...
	if requireEnabled && snapshot.Status != constant.ChannelStatusEnabled {
	...
	if !force {
		minimum := int64(common.GetEnvInt("CHANNEL_UPSTREAM_MODEL_UPDATE_MIN_CHECK_INTERVAL_SECONDS", int(defaultUpstreamModelMinCheckSeconds)))
		if minimum < 0 {
	...
	err = model.DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if requireEnabled && current.Status != constant.ChannelStatusEnabled {
...
func applyChannelUpstreamModelUpdates(ctx context.Context, channelID int, addInput, ignoreInput, removeInput []string, refreshCache, requireEligible bool) (*ChannelUpstreamApplication, error) {
	...
	err = model.DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		if requireEligible && (channel.Status != constant.ChannelStatusEnabled || !settings.CheckEnabled) {
			return errChannelUpstreamBulkIneligible
		}
		...
		settings.LastCheckTime = common.NowTimestamp()
...
func ApplyAllChannelUpstreamModelUpdates(ctx context.Context) (*ApplyAllChannelUpstreamSummary, error) {
	...
	for {
		...
		query := model.DB.WithContext(ctx).Where("status = ?", constant.ChannelStatusEnabled).
...
func RunChannelUpstreamModelUpdateTask(ctx context.Context, force, allowAutoApply bool, report func(processed, total int) error) (ChannelUpstreamUpdateSummary, error) {
	...
	var total int64
	if err := model.DB.WithContext(ctx).Model(&model.Channel{}).Where("status = ?", constant.ChannelStatusEnabled).Count(&total).Error; err != nil {
		return summary, err
	}
	lastID := 0
	processed := 0
	cacheRefreshNeeded := false
	pollingIntervalSeconds := common.GetEnvInt("POLLING_INTERVAL", 0)
	...
	for {
		...
		var channels []model.Channel
		query := model.DB.WithContext(ctx).Where("status = ? AND id > ?", constant.ChannelStatusEnabled, lastID).
		...
		for i := range channels {
			...
			if err != nil {
				...
				common.SysError(fmt.Sprintf("upstream model detection failed for channel %d", channels[i].Id))
				continue
			}
