...
func MinimumPotentialToolSurcharge(
	...
	quota, err = common.QuotaFromDecimalStrict(total.Round(0))
	if err != nil || quota < 0 {
		if err == nil {
			err = ErrInvalidQuota
		}
...
func (counter *ToolUsageCounter) Settle(groupRatio float64) (ToolPricingSettlement, error) {
	...
	quotaPerUnit := decimal.NewFromInt(int64(common.QuotaPerUnit))
	...
	quota, clamp := common.QuotaFromDecimalChecked(rounded)
	if clamp != nil {
