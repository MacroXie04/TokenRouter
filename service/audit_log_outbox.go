...
func EnqueueAuditLogTx(tx *gorm.DB, logEntry *model.Log) error {
	...
	entry := *logEntry
	entry.Id = 0
	payload, err := common.Marshal(&auditLogOutboxPayload{EventID: *entry.AuditEventId, Log: entry})
	if err != nil {
		return err
	}
...
func DeliverAuditLogOutboxContext(ctx context.Context) error {
	...
	workerUUID, err := common.SecureRandomUUID()
	if err != nil {
		return err
	}
...
func claimAuditLogOutboxContext(ctx context.Context, id int64, workerID string, now int64) (*model.AuditLogOutbox, bool, error) {
	...
	leaseToken, err := common.SecureRandomUUID()
	if err != nil {
...
func auditLogPayloadReceipt(payload string) string {
	return auditLogPayloadReceiptPrefix + common.SHA256Hex(payload)
}
...
func auditLogOutboxBatchSize() int {
	size := common.GetEnvInt("AUDIT_LOG_OUTBOX_BATCH_SIZE", defaultAuditLogOutboxBatchSize)
	if size < 1 || size > 1000 {
...
func auditLogOutboxLeaseSeconds() int64 {
	seconds := int64(common.GetEnvInt("AUDIT_LOG_OUTBOX_LEASE_SECONDS", int(defaultAuditLogOutboxLeaseSeconds)))
...
func auditLogSinkTimeout() time.Duration {
	seconds := common.GetEnvInt("AUDIT_LOG_SINK_TIMEOUT_SECONDS", defaultAuditLogSinkTimeoutSeconds)
