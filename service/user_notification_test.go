...
func TestUpdateUserNotificationSettingsPreservesIndependentDataAndWriteOnlySecrets(t *testing.T) {
	user := setupUserNotificationTest(t, constant.RoleAdminUser,
		`{"billing_preference":"wallet_only","language":"fr","future":{"keep":true}}`)
	enabled := true
	require.NoError(t, UpdateUserNotificationSettings(user.Id, constant.RoleAdminUser, UserNotificationSettingsInput{
	...
	err = UpdateUserNotificationSettings(user.Id, constant.RoleAdminUser, UserNotificationSettingsInput{
...
func TestUpdateUserNotificationSettingsRejectsUnsafeDestinationsAtomically(t *testing.T) {
	user := setupUserNotificationTest(t, constant.RoleCommonUser,
	...
	for _, input := range tests {
		require.Error(t, UpdateUserNotificationSettings(user.Id, constant.RoleCommonUser, input), "%+v", input)
	}
