...
func CreateTopUp(userId int, amount int64, money float64, paymentMethod, paymentProvider string) (*model.TopUp, error) {
	tradeNo, err := common.SecureRandomAlphanumeric(32)
	if err != nil {
		return nil, err
	}
...
func topUpCreditQuota(amount int64) (int64, error) {
	unit := int64(common.QuotaPerUnit)
	if amount <= 0 || unit <= 0 || amount > common.MaxQuota/unit {
		return 0, ErrTopUpQuotaOverflow
	}
	credit := amount * unit
	if credit <= 0 || credit > common.MaxQuota {
		return 0, ErrTopUpQuotaOverflow
	}
	return credit, nil
}
...
func calculateTopUpMoney(amount int64, group string, unitPrice float64) (float64, error) {
	if amount <= 0 || amount > common.MaxQuota || !finitePositiveMoney(unitPrice) {
		return 0, ErrTopUpPricingInvalid
	}
	ratio, err := validatedTopUpGroupRatio(group)
	if err != nil {
	...
	if setting.GetQuotaDisplayType() == setting.QuotaDisplayTypeTokens {
		dAmount = dAmount.Div(decimal.NewFromInt(common.QuotaPerUnit))
	}
	payMoney := dAmount.
...
func completeTopUp(userId int, tradeNo string, amount int64, stripePayment *stripeSettlement, checkoutBinding *stripeCheckoutBinding, creemPayment *CreemSettlement) error {
	if userId <= 0 || strings.TrimSpace(tradeNo) == "" || amount <= 0 || amount > common.MaxQuota {
	...
	err := model.DB.Transaction(func(tx *gorm.DB) error {
		...
		query := subscriptionLockForUpdate(tx).
		...
		if creemPayment != nil {
			if order.CreditQuotaVersion != CreemTopUpCreditQuotaVersion || order.CreditQuota != order.Amount ||
				order.CreditQuota <= 0 || order.CreditQuota > common.MaxQuota {
		...
		var user model.User
		if err := subscriptionLockForUpdate(tx).Select("id", "quota", "stripe_customer").First(&user, userId).Error; err != nil {
			return err
		}
		if stripePayment != nil {
	...
	if credited {
		if err := DeliverAuditLogOutboxEvent(auditEventID); err != nil {
			common.SysError(fmt.Sprintf("top-up audit delivery deferred trade_no=%s user_id=%d: %v", tradeNo, userId, err))
		}
	}
	return nil
}
...
func markTopUpReconciliation(tradeNo, state, detail string) error {
	...
	return model.DB.Transaction(func(tx *gorm.DB) error {
		var order model.TopUp
		if err := subscriptionLockForUpdate(tx).Select("id", "status", "reconciliation_state", "reconciliation_detail").
