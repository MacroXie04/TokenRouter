...
func TestLogCleanupRejectsFutureTargetsBeforeEnqueueOrDeletion(t *testing.T) {
	initTaskDB(t)
	future := common.NowTimestamp() + 3600
	...
	payload, err := common.Marshal(logCleanupPayload{TargetTimestamp: future, BatchSize: logCleanupBatchSize})
	require.NoError(t, err)
...
func TestLogCleanupRejectsOversizedPersistedBatchBeforeDeletion(t *testing.T) {
	initTaskDB(t)
	payload, err := common.Marshal(logCleanupPayload{
		TargetTimestamp: common.NowTimestamp() - 1,
...
func TestStaleSystemTaskLeaseTakeoverFailsPreviousRunAtomically(t *testing.T) {
	...
	require.NoError(t, model.DB.Model(&model.SystemTaskLock{}).
		...
		Update("locked_until", common.NowTimestamp()-1).Error)
...
func TestStaleSystemTaskCleanupUsesLockExpiryNotTaskTimestamp(t *testing.T) {
	...
	require.NoError(t, model.DB.Model(&model.SystemTask{}).Where("id = ?", task.ID).Update("updated_at", common.NowTimestamp()).Error)
	require.NoError(t, model.DB.Model(&model.SystemTaskLock{}).
...
func TestSystemTaskLeaseRejectsWrongOwnerAndExpiredOwner(t *testing.T) {
	...
	require.ErrorIs(t, model.RenewSystemTaskLock(claimed.TaskID, "runner-other", common.NowTimestamp()+600), model.ErrSystemTaskLockLost)
...
func TestSystemTaskHeartbeatRenewsLease(t *testing.T) {
	...
	oldUntil := common.NowTimestamp() + 2
	require.NoError(t, model.DB.Model(&model.SystemTaskLock{}).
		Where("task_id = ?", claimed.TaskID).
...
func TestSystemTaskCleanupLeavesGenericRunWithLeaseLockAlone(t *testing.T) {
	...
	lock := model.SystemTaskLock{
		...
		LockedUntil: common.NowTimestamp() - 1,
		UpdatedAt:   common.NowTimestamp() - 10,
	}
...
func TestChannelTestTaskNotifyFlagDeliversRootCompletion(t *testing.T) {
	...
	require.NoError(t, model.DB.Create(&model.User{
		...
		Role: constant.RoleRootUser, Status: model.UserStatusEnabled,
	...
	require.NoError(t, notifyRootChannelStatus(7, "Primary", constant.ChannelStatusAutoDisabled, "余额不足"))
	assert.Equal(t, []string{"root@example.test", "root@example.test"}, mailer.to)
	assert.Equal(t, []string{"通道测试完成", "通道「Primary」（#7）已被禁用"}, mailer.subject)

	result, err = runChannelTestTask(context.Background(), &model.SystemTask{Payload: `{"notify":false}`})
	require.NoError(t, err)
	assert.JSONEq(t, `{"tested":0,"succeeded":0,"failed":0,"disabled":0,"enabled":0}`, result)
	assert.Len(t, mailer.to, 2, "notify=false must not deliver another message")
}
...
func TestChannelTestTaskRejectsMalformedOrUnknownModeWithoutProbing(t *testing.T) {
	...
	channel := model.Channel{
		Name: "must-not-probe", Type: int(constant.ChannelTypeOpenAI), Key: "secret",
		Status: constant.ChannelStatusEnabled, BaseURL: "https://invalid.example.test", TestModel: "gpt-4",
	}
