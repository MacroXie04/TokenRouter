...
func TestDifyDoesNotAdvertiseAnUpstreamModelCatalog(t *testing.T) {
	assert.False(t, channelSupportsUpstreamModelDiscovery(constant.ChannelTypeDify),
...
func TestAdvancedCustomModelDiscoverySupportsQueryAndDefaultBearerAuth(t *testing.T) {
	...
	t.Run("query auth on an absolute route", func(t *testing.T) {
		...
		t.Cleanup(server.Close)
		channel := &model.Channel{
			Type: int(constant.ChannelTypeAdvancedCustom), BaseURL: "https://base.invalid", Key: "secret-key",
	...
	t.Run("nil auth defaults to bearer and a client placeholder is skipped", func(t *testing.T) {
		...
		t.Cleanup(server.Close)
		channel := &model.Channel{
			Type: int(constant.ChannelTypeAdvancedCustom), BaseURL: server.URL, Key: "secret-key",
	...
	for name, settings := range invalidSettings {
		t.Run(name, func(t *testing.T) {
			_, err := FetchUpstreamModelsForChannel(&model.Channel{
				Type: int(constant.ChannelTypeAdvancedCustom), BaseURL: "https://base.invalid", Key: "secret-key", OtherSettings: settings,
			})
			require.Error(t, err)
...
func TestAdvancedCustomQueryTransportErrorRedactsCredential(t *testing.T) {
	...
	channel := &model.Channel{
		Type: int(constant.ChannelTypeAdvancedCustom), BaseURL: "http://127.0.0.1:1", Key: secret,
		OtherSettings: advancedCustomDiscoverySettings("/models",
...
func TestProviderSpecificModelDiscoveryPaths(t *testing.T) {
	...
	tests := []struct {
		...
		{constant.ChannelTypeAli, "/compatible-mode/v1/models"},
		{constant.ChannelTypeZhipuV4, "/api/paas/v4/models"},
		{constant.ChannelTypeVolcEngine, "/v1/models"},
		{constant.ChannelTypeMoonshot, "/v1/models"},
	}
	for _, test := range tests {
		t.Run(constant.ChannelTypeName(test.channelType), func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestSpecialProviderModelDiscoveryPlans(t *testing.T) {
	tests := []struct {
		...
		{constant.ChannelTypeZhipuV4, "glm-coding-plan", "https://open.bigmodel.cn/api/coding/paas/v4/models"},
		{constant.ChannelTypeMoonshot, "kimi-coding-plan", "https://api.kimi.com/coding/v1/models"},
		{constant.ChannelTypeVolcEngine, "doubao-coding-plan", "https://ark.cn-beijing.volces.com/api/coding/v3/v1/models"},
	}
	for _, test := range tests {
...
func TestModelDiscoveryHeaderOverrideValidation(t *testing.T) {
	base := &model.Channel{Type: int(constant.ChannelTypeOpenAI), BaseURL: "https://example.com", Key: "secret"}
