...
func TestRelayAccountingExternalDatabaseConcurrency(t *testing.T) {
	...
	suffix := common.BestEffortRandomAlphanumeric(10)
	user := model.User{
...
func TestSystemTaskExternalDatabaseLeaseAndCadence(t *testing.T) {
	...
	suffix := common.BestEffortRandomAlphanumeric(12)
...
func TestAuditOutboxExternalDatabaseLeaseAndDelivery(t *testing.T) {
	...
	concurrentEventID := common.BestEffortUUID()
	...
	content := "external-outbox-" + common.BestEffortRandomAlphanumeric(12)
...
func exerciseTaskOperationAccountingLifecycle(t *testing.T) {
	...
	const quota = 10
	suffix := common.BestEffortRandomAlphanumeric(12)
	platform := strconv.Itoa(int(constant.ChannelTypeSora))
	now, err := model.DatabaseUnixTimestamp(model.DB)
	require.NoError(t, err)
	...
	channel := model.Channel{
		...
		Type: int(constant.ChannelTypeSora), Status: constant.ChannelStatusEnabled,
	}
	require.NoError(t, model.DB.Create(&channel).Error)

	task := model.Task{
		CreatedAt: now, UpdatedAt: now, TaskID: taskID,
		Platform: platform, UserId: user.Id, Group: GroupDefault, ChannelId: channel.Id,
