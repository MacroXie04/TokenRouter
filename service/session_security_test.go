...
func TestRefreshRotationDoesNotDependOnPostCASDatabaseReads(t *testing.T) {
	...
	claims, err := common.ParseJWTSigned(result.AccessToken, common.SessionSecret())
	require.NoError(t, err)
...
func sessionFixture(t *testing.T, username string) (*model.User, string, string, string, *common.JWTClaims) {
	t.Helper()
	initAuthDB(t)
...
func TestSessionAccessTokenCarriesAndValidatesLiveSessionIdentity(t *testing.T) {
	...
	assert.True(t, common.IsDashboardAccessToken(claims))
...
func TestSessionAccessTokenRejectsIdentityAndVersionMismatches(t *testing.T) {
	...
	tests := []struct {
		name   string
		mutate func(*common.JWTClaims)
	}{
		{name: "wrong user", mutate: func(c *common.JWTClaims) { c.UserID = other.Id }},
		{name: "wrong sid", mutate: func(c *common.JWTClaims) { c.SessionID = otherSID }},
		{name: "missing sid", mutate: func(c *common.JWTClaims) { c.SessionID = "" }},
		{name: "wrong user auth version", mutate: func(c *common.JWTClaims) { c.UserAuthVersion++ }},
		{name: "wrong session version", mutate: func(c *common.JWTClaims) { c.SessionVersion++ }},
		{name: "wrong token use", mutate: func(c *common.JWTClaims) { c.TokenUse = "security_proof" }},
		{name: "expired claims", mutate: func(c *common.JWTClaims) {
	...
	tests := []struct {
		...
		{name: "expired session", mutate: func(t *testing.T, _ *model.User, sid string) {
			require.NoError(t, model.DB.Model(&model.UserSession{}).Where("sid = ?", sid).
				UpdateColumn("expires_at", common.NowTimestamp()-1).Error)
		}},
...
func TestExpiredSignedSessionAccessTokenIsRejected(t *testing.T) {
	...
	raw, err := common.GenerateSessionJWT(user.Id, user.Role, sid, claims.UserAuthVersion, claims.SessionVersion,
		common.SessionSecret(), -time.Second)
	require.NoError(t, err)
	_, err = common.ParseJWT(raw, common.SessionSecret())
	assert.Error(t, err)
}
...
func TestRefreshRotationImmediatelyInvalidatesPreviousAccessToken(t *testing.T) {
	...
	assert.ErrorIs(t, err, ErrSessionRevoked)
	newClaims, err := common.ParseJWT(newAccess, common.SessionSecret())
	require.NoError(t, err)
...
func TestLegacyRefreshDigestRotatesForwardToKeyedDigest(t *testing.T) {
	...
	require.NoError(t, model.DB.Model(&model.UserSession{}).Where("sid = ?", sid).
		UpdateColumn("refresh_hash", common.SHA256Hex(refresh)).Error)
	...
	assert.NotEqual(t, common.SHA256Hex(rotatedRefresh), session.RefreshHash)
	assert.Equal(t, common.SHA256Hex(refresh), session.PreviousRefreshHash)
	assert.InDelta(t, common.NowTimestamp()+int64(RefreshReplayWindow/time.Second), session.PreviousValidUntil, 1)
}
...
func TestConcurrentRefreshAndRevokeCannotLeaveUsableAccessToken(t *testing.T) {
	...
	for _, raw := range []string{originalAccess, refreshedAccess} {
		...
		claims, err := common.ParseJWT(raw, common.SessionSecret())
		require.NoError(t, err)
		_, _, err = ValidateAccessTokenClaims(claims)
		assert.ErrorIs(t, err, ErrSessionRevoked)
	}
