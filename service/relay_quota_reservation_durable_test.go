	...
	for _, test := range cases {
		t.Run(test.name, func(t *testing.T) {
			...
			subscription := model.UserSubscription{
				...
				EndTime: common.NowTimestamp() + 3600, Status: SubscriptionStatusActive,
...
func TestRelayQuotaChannelFailureRollsBackAndRecoversAllAccounting(t *testing.T) {
	...
	channel := model.Channel{
		Name: "saturated-channel", Key: "upstream", Status: constant.ChannelStatusEnabled,
...
func TestRelayQuotaReservationManualReviewCapsPersistedError(t *testing.T) {
	...
	require.Error(t, markInvalidRelayQuotaReservationForReview(&record, longError, common.NowTimestamp()))
	record = loadRelayQuotaReservationRecord(t, reservation.ReservationID())
