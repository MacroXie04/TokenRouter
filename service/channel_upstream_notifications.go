...
func buildChannelUpstreamNotificationContent(summary channelUpstreamNotificationSummary) string {
	...
	if failedChannels > 0 {
		...
		for _, channelID := range summary.FailedChannelIDs[:shown] {
			ids = append(ids, common.Int2Str(channelID))
		}
		content.WriteString("\n\n失败渠道 ID：")
...
func notifyChannelUpstreamWatchers(summary channelUpstreamNotificationSummary) {
	...
	if err := model.DB.Select("id", "email", "email_verified", "role", "status", "setting").
		Where("status = ? AND role >= ?", model.UserStatusEnabled, constant.RoleAdminUser).
		Find(&users).Error; err != nil {
		common.SysError("query upstream model notification watchers failed")
		return
	}
	...
	for i := range users {
		...
		if err := SendUserNotification(user.Id, user.Email, user.EmailVerified, settings, UserNotification{
			Type: "channel_update", Title: "上游模型巡检通知", Content: content,
		}); err != nil {
			common.SysError(fmt.Sprintf("notify upstream model watcher %d failed", user.Id))
		}
	}
}
