...
func TestFetchUpstreamModelsUsesProviderContractAndLeastCredential(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			channel: model.Channel{Type: int(constant.ChannelTypeOpenAI), Key: "disabled-key\nenabled-key",
		...
		{
			name: "Anthropic", channel: model.Channel{Type: int(constant.ChannelTypeAnthropic), Key: "anthropic-key"},
		...
		{
			name: "Gemini", channel: model.Channel{Type: int(constant.ChannelTypeGemini), Key: "gemini-key"},
		...
		{
			name: "Azure", channel: model.Channel{Type: int(constant.ChannelTypeAzure), Key: "azure-key", Other: "2025-04-01-preview"},
		...
		{
			name: "Codex",
			channel: model.Channel{Type: int(constant.ChannelTypeCodex),
...
func TestFetchUpstreamModelsFailsClosedForUnsupportedProvider(t *testing.T) {
	...
	_, err := FetchUpstreamModelsForChannel(&model.Channel{
		Type: int(constant.ChannelTypeJimeng), BaseURL: upstream.URL,
