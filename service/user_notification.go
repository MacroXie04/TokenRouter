...
func UpdateUserNotificationSettings(userID, callerRole int, input UserNotificationSettingsInput) error {
	if input.QuotaWarningThreshold <= 0 || !common.QuotaWithinBounds(input.QuotaWarningThreshold) {
	...
	return updateUserSettingsChecked(userID, func(settings *UserSettings, storedRole int) error {
		...
		if callerRole >= constant.RoleAdminUser && storedRole >= constant.RoleAdminUser && input.UpstreamModelUpdateNotifyEnabled != nil {
...
func SendUserNotification(userID int, accountEmail string, accountEmailVerified bool, settings UserSettings, notification UserNotification) error {
	...
	case NotifyTypeEmail:
		...
		return Mail.Send(destination, notification.Title, notification.Content)
...
func NotifyRootUser(notification UserNotification) error {
	...
	result := model.DB.Select("id", "email", "email_verified", "setting").
		Where("status = ? AND role = ?", model.UserStatusEnabled, constant.RoleRootUser).
...
func notifyRootChannelStatus(channelID int, channelName string, status int, reason string) error {
	...
	} else if status != constant.ChannelStatusEnabled {
		return errors.New("channel notification status is invalid")
	}
	return NotifyRootUser(UserNotification{
...
func sendWebhookUserNotification(settings UserSettings, notification UserNotification) error {
	...
	payload, err := common.Marshal(webhookNotificationPayload{
...
func performUserNotificationRequest(method, destination string, headers http.Header, payload []byte) error {
	...
	if _, err := common.ReadAllLimited(response.Body, maxNotificationResponseBytes); err != nil {
