...
func TestCreateRedemptionBatchRejectsUnsafeParameters(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "oversized quota", userID: 1, quota: int(common.MaxQuota) + 1, count: 1},
...
func TestTopUpCompleteIdempotent(t *testing.T) {
	...
	var got model.User
	require.NoError(t, model.DB.First(&got, u.Id).Error)
	assert.Equal(t, 1000*common.QuotaPerUnit, got.Quota)
	var logs int64
...
func TestTopUpCreditSnapshotOverflowAndLegacyPendingFailClosed(t *testing.T) {
	initWalletDB(t)
	user := newUser(t, 0)
	_, err := CreateTopUpWithTradeNo(user.Id, common.MaxQuota/int64(common.QuotaPerUnit)+1,
...
func TestConcurrentTopUpCompletionCreditsExactlyOnce(t *testing.T) {
	...
	var got model.User
	require.NoError(t, model.DB.First(&got, u.Id).Error)
	assert.Equal(t, 100*common.QuotaPerUnit, got.Quota)
...
func TestTopUpCompletionRollsBackStatusWhenCreditFails(t *testing.T) {
	...
	require.NoError(t, model.DB.First(&got, u.Id).Error)
	assert.Equal(t, 100*common.QuotaPerUnit, got.Quota, "rollback must leave the order safely retryable")
}
...
func TestCreateTopUpRejectsOutOfDomainFinancialInputs(t *testing.T) {
	...
	for name, create := range map[string]func() (*model.TopUp, error){
		...
		"large amount":   func() (*model.TopUp, error) { return CreateTopUp(u.Id, common.MaxQuota+1, 1, "stripe", "stripe") },
