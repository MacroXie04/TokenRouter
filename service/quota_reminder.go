...
func checkAndSendQuotaReminder(userID, subscriptionID int) {
	...
	if err := SendUserNotification(delivery.user.Id, delivery.user.Email, delivery.user.EmailVerified,
		delivery.settings, UserNotification{Type: "quota_exceed", Title: title, Content: body}); err != nil {
...
func reserveQuotaReminder(userID, subscriptionID int) (*quotaReminderDelivery, int, error) {
	...
	err := model.DB.Transaction(func(tx *gorm.DB) error {
		query := subscriptionLockForUpdate(tx).Select(
			"id", "email", "email_verified", "quota", "quota_reminder_at", "setting",
		).Where("id = ?", userID)
		if err := query.First(&delivery.user).Error; err != nil {
			return err
		}
		delivery.settings = parseUserSettings(delivery.user.Setting)
		threshold = globalThreshold
		if override := delivery.settings.QuotaWarningThreshold; override > 0 {
			threshold = override
		}
		if threshold <= 0 || !common.QuotaWithinBounds(threshold) || now-delivery.user.QuotaReminderAt < 3600 {
		...
		if subscriptionID > 0 {
			...
			if !common.QuotaWithinBounds(delivery.user.Quota) {
				return errQuotaReminderNotNeeded
			}
