...
func TestUserUsableGroupsApplySpecialDirectivesWithoutBypassingPositiveRatios(t *testing.T) {
	...
	assert.False(t, IsUserSelectableGroup("team", GroupAuto))
	...
	assert.Equal(t, "Automatic for team", groups[GroupAuto])
	assert.False(t, IsUserSelectableGroup("team", GroupAuto), "auto remains a presentation marker, never a concrete relay group")
}
...
func TestResolveRelayGroupPolicyRevalidatesAndFailsClosed(t *testing.T) {
	...
	auto := &model.Token{Group: GroupAuto, AutoGroups: `["staff","vip","default"]`, CrossGroupRetry: true}
...
func TestResolveRelayGroupPolicyInheritsOrderedGlobalAndRejectsEmpty(t *testing.T) {
	...
	policy, err := ResolveRelayGroupPolicy("default", &model.Token{Group: GroupAuto})
	require.NoError(t, err)
	assert.Equal(t, []string{"vip", "default"}, policy.Groups)

	require.NoError(t, setting.UpdateOption(setting.AutoGroupsOption, `[]`))
	_, err = ResolveRelayGroupPolicy("default", &model.Token{Group: GroupAuto})
