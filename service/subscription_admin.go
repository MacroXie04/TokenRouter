...
func authorizeSubscriptionTargetTx(tx *gorm.DB, userID int, operatorRole *int) error {
	...
	var user model.User
	if err := subscriptionLockForUpdate(tx).Select("id", "role").Where("id = ?", userID).First(&user).Error; err != nil {
		return err
	}
...
func validateSubscriptionPlanFinancialPayload(p *model.SubscriptionPlan) error {
	...
	if p.TotalAmount > common.MaxQuota {
		return fmt.Errorf("总额度不能超过%d", common.MaxQuota)
	}
	return nil
}
...
func validateSubscriptionPlanPayload(p *model.SubscriptionPlan) error {
	...
	if _, err := calcPlanEndTime(time.Unix(common.NowTimestamp(), 0), p); err != nil {
...
func ResolveLegacySubscriptionEntitlementReview(operatorUserID, subscriptionID int, resolution LegacySubscriptionEntitlementResolution) error {
	...
	err := model.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := subscriptionLockForUpdate(tx).Select("id").Where("id = ?", identity.UserId).First(&targetUser).Error; err != nil {
			return err
		}
		...
		if !IsRoot(operator.Role) {
			return errors.New("root operator required")
		}
		var sub model.UserSubscription
		if err := subscriptionLockForUpdate(tx).Where("id = ?", subscriptionID).First(&sub).Error; err != nil {
			return err
		}
	...
	if err := DeliverAuditLogOutboxEvent(auditEventID); err != nil {
		common.SysError(fmt.Sprintf("legacy subscription resolution audit delivery deferred subscription_id=%d: %v", subscriptionID, err))
	}
	return nil
}
...
func updateSubscriptionResetState(tx *gorm.DB, sub *model.UserSubscription, amountUsed, lastResetTime, nextResetTime, now int64, incrementUsageEpoch bool) error {
	...
	if amountUsed < 0 || amountUsed > common.MaxQuota {
