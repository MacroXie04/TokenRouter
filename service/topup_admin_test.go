...
func TestListUserTopUpsScopesOwnerWindowPaginationAndSearch(t *testing.T) {
	...
	require.NoError(t, model.DB.Model(&model.TopUp{}).Where("trade_no = ?", "owner-old").
		Update("create_time", common.NowTimestamp()-userTopUpHistoryWindowSeconds-1).Error)
...
func TestManualCompleteTopUpRecoveryBoundsStatusAndIdempotency(t *testing.T) {
	...
	oversized := model.TopUp{UserId: user.Id, Amount: common.MaxQuota + 1, Money: 1, TradeNo: "manual-oversized", Status: TopUpStatusPending}
	...
	assert.Equal(t, 100*common.QuotaPerUnit, credited.Quota,
...
func TestConcurrentManualCompleteTopUpCreditsExactlyOnce(t *testing.T) {
	...
	assert.Equal(t, 100*common.QuotaPerUnit, userAfter.Quota)
	var logs int64
