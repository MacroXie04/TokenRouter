...
func TestRecordConsumeLogCheckedNormalizesProviderControlledRequestID(t *testing.T) {
	...
	assert.Equal(t, common.NormalizeProviderCorrelationID("req_safe-123"), got.UpstreamRequestId)
	...
	assert.Equal(t, common.NormalizeProviderCorrelationID("req_safe-123"), logs[0].UpstreamRequestId)
