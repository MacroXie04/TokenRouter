...
func TestImportLegacyOneAPIRemapsOwnershipAndInviters(t *testing.T) {
	...
	require.NoError(t, legacyDB.Create(&model.Token{UserId: invitee.Id, Key: "sk-remapped-owner", Name: "remapped", Status: TokenStatusEnabled}).Error)
...
func TestImportLegacyOneAPIRollsBackEveryEntityOnTargetWriteFailure(t *testing.T) {
	...
	require.NoError(t, legacyDB.Create(&model.Token{UserId: legacyUser.Id, Key: "sk-rollback", Name: "rollback", Status: TokenStatusEnabled}).Error)
...
func TestImportLegacyOneAPIPreflightsPasswordHashesBeforeTargetMutation(t *testing.T) {
	...
	require.NoError(t, legacyDB.Create(&model.Token{
		UserId: valid.Id, Key: "sk-password-preflight", Name: "password-preflight", Status: TokenStatusEnabled,
	}).Error)
	...
	counts, err := ImportLegacyOneAPI(legacyPath)
	require.ErrorIs(t, err, common.ErrPasswordHashCostUnsupported)
...
func TestLegacyPasswordHashPreflightAllowsOAuthOnlyAndBoundedBcrypt(t *testing.T) {
	...
	require.ErrorIs(t, err, common.ErrPasswordHashInvalid)
