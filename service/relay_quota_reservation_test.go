...
func TestRelayQuotaReservationPersistsUnlimitedTokenAndChannelUsage(t *testing.T) {
	...
	channel := model.Channel{Name: "accounted-channel", Key: "upstream", Status: constant.ChannelStatusEnabled, UsedQuota: 7}
...
func TestRelayQuotaReservationSettlesUserAndTokenAtomically(t *testing.T) {
	...
	require.NoError(t, RecordConsumeLogChecked(
		...
		true, 0, GroupDefault, "127.0.0.1", "request-1", "response-1",
...
func TestRelayQuotaReservationRejectsQuotaAndCounterOverflow(t *testing.T) {
	...
	if strconv.IntSize > 32 {
		_, err = NewRelayQuotaReservation(user.Id, &token, int(common.MaxQuota+1))
		assert.ErrorIs(t, err, ErrInvalidQuota)
	}

	require.NoError(t, db.Model(&user).Update("used_quota", int(common.MaxQuota)).Error)
	reservation, err := NewRelayQuotaReservation(user.Id, &token, 10)
	require.NoError(t, err)
	...
	require.NoError(t, db.First(&token, token.Id).Error)
	assert.Equal(t, 90, user.Quota)
	assert.Equal(t, int(common.MaxQuota), user.UsedQuota)
...
func TestRelayQuotaReservationTokenUsageOverflowRollsBackUser(t *testing.T) {
	...
	token := model.Token{
		...
		RemainQuota: 100, UsedQuota: int(common.MaxQuota),
	}
	require.NoError(t, db.Create(&token).Error)
	...
	assert.Zero(t, user.RequestCount)
	assert.Equal(t, 90, token.RemainQuota)
	assert.Equal(t, int(common.MaxQuota), token.UsedQuota)
}
...
func TestRelayQuotaReservationRefundRejectsTokenBalanceOverflow(t *testing.T) {
	...
	reservation, err := NewRelayQuotaReservation(user.Id, &token, 10)
	require.NoError(t, err)
	require.NoError(t, db.Model(&token).Update("remain_quota", int(common.MaxQuota)).Error)
	...
	assert.Equal(t, int(common.MaxQuota), token.RemainQuota, "token quota must never wrap or exceed the cap")
...
func TestRecordConsumeLogCheckedFailsClosedWithoutAuthoritativeClock(t *testing.T) {
	...
	err := RecordConsumeLogChecked(
		...
		0, GroupDefault, "127.0.0.1", "request", "response", 1, nil,
	)
