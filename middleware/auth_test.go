...
func TestTokenAuthDynamicallyInheritsCurrentUserGroup(t *testing.T) {
	...
	service.SetGroupRatios(map[string]float64{"default": 1, "vip": 2})
	...
	token := model.Token{
		UserId: user.Id, Key: "sk-dynamic-group", Status: service.TokenStatusEnabled,
		UnlimitedQuota: true, Group: "",
	}
	require.NoError(t, model.DB.Create(&token).Error)
	require.NoError(t, service.SetUserGroup(user.Id, "vip"))

	router := gin.New()
	router.Use(TokenAuth())
	observedUserGroup := ""
	router.GET("/", func(c *gin.Context) {
		observedUserGroup = common.GetUserGroup(c)
	...
	require.NoError(t, service.SetUserGroup(user.Id, ""))
	recorder = httptest.NewRecorder()
	router.ServeHTTP(recorder, request)
	require.Equal(t, http.StatusOK, recorder.Code, recorder.Body.String())
...
func TestTokenAuthRevalidatesExplicitAndAutoGroups(t *testing.T) {
	...
	explicit := model.Token{UserId: user.Id, Key: "sk-explicit-group", Status: service.TokenStatusEnabled, UnlimitedQuota: true, Group: "vip"}
	auto := model.Token{UserId: user.Id, Key: "sk-auto-group", Status: service.TokenStatusEnabled, UnlimitedQuota: true, Group: service.GroupAuto, AutoGroups: `["vip","default"]`}
...
func TestTokenOrUserAuthKeepsRelayAndDashboardCredentialsSeparated(t *testing.T) {
	...
	token := model.Token{
		UserId: owner.Id, Key: "sk-video-content", Status: service.TokenStatusEnabled,
	...
	router.GET("/content", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{
			"user_id": common.GetUserId(c), "has_relay_token": GetRelayToken(c) != nil,
		})
	})
