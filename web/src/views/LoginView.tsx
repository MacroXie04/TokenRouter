...
import { authGet, authPost, isCanceledAuthRequest } from '../features/auth/auth-api';
import {
  normalizeTelegramBotName,
  TelegramLoginWidget,
} from '../features/auth/TelegramLoginWidget';
import { parseTurnstileConfig, TurnstileWidget, turnstileParams } from '../features/security/TurnstileWidget';
import {
...
const MAX_OAUTH_ERROR_MESSAGE_CHARACTERS = 512;
const MAX_USERNAME_CHARACTERS = 64;
const MAX_PASSWORD_CHARACTERS = 64;
const MAX_EMAIL_CHARACTERS = 50;
const MAX_LOGIN_IDENTIFIER_BYTES = 50;

function safeOAuthDisplayText(
  value: string,
...
function safeOAuthCredentialText(value: string, maximumCharacters: number): string | null {
...
function oauthErrorClearedLocation(pathname: string, search: string, hash: string): string {
  if (
    search.length > MAX_AUTH_CALLBACK_QUERY_CHARACTERS
    || hasUnsafeAuthText(search)
    || hasMalformedAuthQueryEncoding(search)
...
function initialAffiliateCode(): string {
  if (
...
function replaceBrowserLocation(target: string): void {
    ...
    <AuthLayout title={pageTitle} description={pageDescription}>
      ...
      {wechatOpen && (
        <div
          ...
          role="presentation"
          onMouseDown={(event) => { if (event.target === event.currentTarget) closeWeChat(); }}
        >
          <form
            ref={wechatDialogRef}
            className="modal"
          ...
          >
            ...
            {authStatus.wechat_qrcode && <img src={authStatus.wechat_qrcode} alt={t('WeChat authorization QR code')} />}
            ...
            <label>
              ...
              <input
                value={wechatCode}
                onChange={(event) => setWechatCode(event.target.value.slice(0, MAX_WECHAT_CODE_CHARACTERS))}
                required
                maxLength={MAX_WECHAT_CODE_CHARACTERS}
                autoComplete="one-time-code"
              />
            </label>
            {wechatError && <p className="error" role="alert">{wechatError}</p>}
            <div className="modal-actions">
              <button type="button" className="link" onClick={closeWeChat}>{t('Close')}</button>
      ...
      {telegramOpen && (
        <div
          ...
          role="presentation"
          onMouseDown={(event) => {
            if (!telegramPending && event.target === event.currentTarget) closeTelegram();
          }}
        >
          <section
          ...
          >
            ...
            />
            {telegramError && <p className="error" role="alert">{telegramError}</p>}
            <div className="modal-actions">
              <button type="button" className="link" disabled={telegramPending} onClick={closeTelegram}>{t('Close')}</button>
            </div>
          </section>
        </div>
      )}
