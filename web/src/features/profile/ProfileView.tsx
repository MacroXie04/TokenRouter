...
interface OAuthOverview {
  catalog: OAuthCatalog;
  bindings: CustomOAuthBinding[];
}

const LANGUAGE_OPTIONS: Array<{ code: ProfileLanguage; label: string }> = [
...
const NOTIFICATION_OPTIONS: Array<{ value: ProfileNotificationSettings['notifyType']; label: string }> = [
...
const MAX_NOTIFICATION_QUOTA = 2_147_483_647;
...
function enabledSidebarModules(): SidebarModules {
  return {
...
function sidebarPreferenceEnabled(modules: SidebarModules, section: keyof SidebarModules, key: string): boolean {
...
function defaultExternalNavigate(target: string): void {
...
function validEmailAddress(value: string): string | null {
...
function hasUnsafeNotificationText(value: string): boolean {
...
function notificationLoopbackHost(hostname: string): boolean {
...
function validNotificationURL(value: string, kind: 'webhook' | 'bark' | 'gotify'): boolean {
  if (value === '') return true;
...
function validWriteOnlyCredential(value: string, maximumBytes: number): boolean {
  return utf8Bytes(value) <= maximumBytes && !hasUnsafeNotificationText(value);
}

function validProofCode(value: string): boolean {
  return /^\d{6}$/.test(value.trim());
}

function validFactorCode(value: string): boolean {
...
export function ProfileView({
  ...
  const renderBackupCodes = (): ReactNode => {
    ...
    return (
      <aside className="user-subpanel" aria-labelledby="profile-backup-codes-title">
        ...
        <button type="button" onClick={hideBackupCodes} disabled={!backupCodesSaved}>{t('Hide backup codes')}</button>
      </aside>
    );
  };
  ...
  const renderSessions = (): ReactNode => {
    ...
    return (
      <>
        ...
        <button type="button" onClick={() => { void signOutOtherSessions(); }} disabled={busy !== null || sessions.data.length < 2}>
        ...
        <ul className="key-list">
          {sessions.data.map((session) => (
            <li key={session.sid}>
              ...
              <button type="button" className="link" onClick={() => { void signOutSession(session); }} disabled={busy !== null}>
