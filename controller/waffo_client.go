...
type WaffoOrderClient interface {
	CreateOrder(context.Context, setting.WaffoConfig, service.WaffoCheckoutRequest) (WaffoCreateOrderResult, error)
}
...
func (waffoHTTPOrderClient) CreateOrder(ctx context.Context, config setting.WaffoConfig, checkout service.WaffoCheckoutRequest) (WaffoCreateOrderResult, error) {
	...
	body, err := common.Marshal(checkout)
	if err != nil || len(body) == 0 || len(body) > waffoCreateRequestLimit {
		return WaffoCreateOrderResult{}, service.ErrWaffoCheckoutBindingMismatch
	}
	...
	responseBody, err := common.ReadAllLimited(response.Body, waffoCreateResponseLimit)
	if err != nil {
		return WaffoCreateOrderResult{}, err
	}
	...
	if common.Unmarshal(responseBody, &decoded) != nil || len(decoded.Code) > 64 || len(decoded.Message) > 2048 {
	...
	if !service.ValidWaffoCreateIdentifier(decoded.Data.PaymentRequestID) ||
		!service.ValidWaffoCreateIdentifier(decoded.Data.MerchantOrderID) ||
		!service.ValidWaffoCreateIdentifier(decoded.Data.AcquiringOrderID) || len(decoded.Data.OrderStatus) > 64 ||
...
func parseWaffoPaymentURL(action string) (string, error) {
	if action == "" || len(action) > waffoCreateOrderActionLimit || strings.ContainsRune(action, '\x00') {
		return "", service.ErrWaffoCheckoutBindingMismatch
	}
	...
	if common.Unmarshal([]byte(action), &decoded) != nil || len(decoded.ActionType) > 64 {
		return "", service.ErrWaffoCheckoutBindingMismatch
	}
