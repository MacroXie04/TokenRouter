...
func setupTopupTest(t *testing.T) (http.Handler, func(method, path, body string) *httptest.ResponseRecorder, int) {
	...
	service.SetTopUpGroupRatios(map[string]float64{"default": 1.0, "vip": 2.0})
...
func TestTopUpEpayPayAndNotifyContract(t *testing.T) {
	...
	assert.Equal(t, int64(10*common.QuotaPerUnit), order.CreditQuota)
	...
	assert.Equal(t, service.TopUpStatusPending, stillPending.Status)
	...
	assert.Equal(t, before.Quota+10*common.QuotaPerUnit, u.Quota, "top-up credits Amount * QuotaPerUnit")
...
func TestTokenDisplayEpayAndStripePreserveMoneyToQuotaContract(t *testing.T) {
	...
	displayAmount := int64(2 * common.QuotaPerUnit)
...
func TestTopUpRequestsFailClosedOnUnsafeAmountsAndPricing(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			path: "/api/user/amount", body: fmt.Sprintf(`{"amount":%d}`, common.MaxQuota+1),
			message: "error",
		},
		{
			name: "epay amount above quota domain",
			path: "/api/user/pay", body: fmt.Sprintf(`{"amount":%d,"payment_method":"alipay"}`, common.MaxQuota+1),
			message: "error",
		},
		{
			name: "stripe amount above quota domain",
			path: "/api/user/stripe/amount", body: fmt.Sprintf(`{"amount":%d}`, common.MaxQuota+1),
			message: "error",
		},
		{
			name: "NaN epay price",
		...
		},
		{
			...
			path: "/api/user/amount", body: fmt.Sprintf(`{"amount":%d}`, common.MaxQuota),
			configure: func(t *testing.T) {
		...
		},
		{
			...
			configure: func(t *testing.T) {
				...
				require.NoError(t, setting.UpdateOption(setting.MinTopUpOption,
					fmt.Sprintf("%d", common.MaxQuota/int64(common.QuotaPerUnit)+1)))
			},
			message: "error",
		},
		{
			...
			configure: func(t *testing.T) {
				service.SetTopUpGroupRatios(map[string]float64{"default": math.NaN()})
...
func TestTopUpStripePayContract(t *testing.T) {
	...
	assert.Equal(t, service.StripeCheckoutBindingVersion, order.ProviderBindingVersion)
	...
	assert.Equal(t, service.StripeOrderTypeWallet, metadata["order_type"])
...
func TestTopUpStripeAmbiguousCheckoutOutcomeStaysPending(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			reconciliation: service.StripeReconciliationCreationUnknown,
		},
		{
			...
			reconciliation: service.StripeReconciliationBindingMismatch,
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
