...
func GetRedemption(c *gin.Context) {
	...
	redemption, err := service.GetRedemptionByID(id)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": err.Error()})
		return
	}
...
func UpdateRedemption(c *gin.Context) {
	...
	if _, err := service.GetRedemptionByID(req.Id); err != nil {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": err.Error()})
		return
	}
	if statusOnly == "" {
	...
	updated, err := service.UpdateRedemption(req.Id, statusOnly != "", req.Name, req.Quota, req.ExpiredTime, req.Status)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": err.Error()})
		return
	}
	service.RecordSystemLog(common.GetUserId(c), service.LogTypeManage,
		"redemption.update id="+common.Int2Str(req.Id))
...
func DeleteInvalidRedemption(c *gin.Context) {
	rows, err := service.DeleteInvalidRedemptions()
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": err.Error()})
		return
	}
...
func DeleteRedemption(c *gin.Context) {
	...
	if err := service.DeleteRedemptionByID(id); err != nil {
		c.JSON(http.StatusOK, gin.H{"success": false, "message": err.Error()})
		return
	}
