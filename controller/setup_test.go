...
func TestPostSetupConcurrentSingleWinner(t *testing.T) {
	...
	var rootCount int64
	require.NoError(t, db.Model(&model.User{}).Where("role >= ?", constant.RoleRootUser).Count(&rootCount).Error)
...
func TestPostSetupFailureRollsBackSingletonClaim(t *testing.T) {
	...
	require.NoError(t, db.Create(&model.User{
		Username: "taken-name", Password: "irrelevant", Role: constant.RoleCommonUser,
...
func TestPostSetupExistingRootPersistsModeWithoutReplacingAccount(t *testing.T) {
	...
	require.NoError(t, db.Where("role >= ?", constant.RoleRootUser).Find(&roots).Error)
...
func TestPostSetupCommittedStateWinsOverUnrelatedCacheRefreshFailure(t *testing.T) {
	...
	require.NoError(t, db.Model(&model.User{}).
		Where("username = ? AND role >= ?", "durable-root", constant.RoleRootUser).
