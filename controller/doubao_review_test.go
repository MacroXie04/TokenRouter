...
func TestDoubaoManualReviewResolutionRouteUsesExactProviderTuple(t *testing.T) {
	...
	token := model.Token{
		UserId: rootID, Key: "sk-doubao-review-route", Status: service.TokenStatusEnabled, RemainQuota: 100,
	}
	require.NoError(t, model.DB.Create(&token).Error)
	channel := model.Channel{
		Name: "doubao-review-route", Key: "provider-secret", Type: int(constant.ChannelTypeVolcEngine),
		Status: constant.ChannelStatusEnabled, BaseURL: "https://ark.example",
	}
	...
	assert.Equal(t, service.RelayQuotaReviewKindDoubaoVideoTask, detailData["review_kind"])
