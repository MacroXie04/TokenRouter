...
func TestOllamaManagementRoutesUseNativeWireAndExactResponses(t *testing.T) {
	...
	channel := createSearchChannel(t, "ollama-ops", int(constant.ChannelTypeOllama), constant.ChannelStatusEnabled, "default", "llama3.2:latest", "", 0)
...
func TestOllamaManagementRoutesValidateChannelAndKeepCredentialsPrivate(t *testing.T) {
	...
	_, do, _ := setupChannelRead(t, constant.RoleRootUser)
	ollamaChannel := createSearchChannel(t, "ollama-error", int(constant.ChannelTypeOllama), constant.ChannelStatusEnabled, "default", "model", "", 0)
	require.NoError(t, model.DB.Model(&ollamaChannel).Updates(map[string]any{
		"base_url": upstream.URL,
		"key":      "ollama-operation-secret",
	}).Error)
	wrongType := createSearchChannel(t, "not-ollama", int(constant.ChannelTypeOpenAI), constant.ChannelStatusEnabled, "default", "model", "", 0)
