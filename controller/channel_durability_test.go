...
func TestUpdateChannelRollsBackWhenAbilityRebuildFails(t *testing.T) {
	_, do, _ := setupChannelRead(t, constant.RoleRootUser)
	channel := createSearchChannel(t, "atomic-update", int(constant.ChannelTypeOpenAI), constant.ChannelStatusEnabled, "default", "old-model", "", 0)
...
func TestDeleteChannelRollsBackWhenAbilityDeleteFails(t *testing.T) {
	_, do, _ := setupChannelRead(t, constant.RoleRootUser)
	channel := createSearchChannel(t, "atomic-delete", int(constant.ChannelTypeOpenAI), constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
	require.NoError(t, model.DB.Create(&model.Ability{
	...
	rec := do(http.MethodDelete, "/api/channel/"+common.Int2Str(channel.Id), "")
	assert.Equal(t, http.StatusInternalServerError, rec.Code, rec.Body.String())
...
func TestAddChannelReturnsAbilityCacheRefreshFailure(t *testing.T) {
	...
	require.NoError(t, service.SyncAbilityCache(), "a later cache refresh can safely retry")
}

func TestDeleteChannelCacheRefreshFailureIsExplicitAndRetryable(t *testing.T) {
	_, do, _ := setupChannelRead(t, constant.RoleRootUser)
	channel := createSearchChannel(t, "delete-cache-refresh", int(constant.ChannelTypeOpenAI), constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
	require.NoError(t, model.DB.Create(&model.Ability{
		Group: "default", Model: "gpt-4o", ChannelId: channel.Id, Enabled: true, Weight: 1,
	}).Error)
	...
	path := "/api/channel/" + common.Int2Str(channel.Id)
...
func TestChannelStatusRollsBackWhenAbilityWriteFailsAndRetries(t *testing.T) {
	_, do, _ := setupChannelRead(t, constant.RoleRootUser)
	channel := createSearchChannel(t, "status-rollback", int(constant.ChannelTypeOpenAI), constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
	...
	path := "/api/channel/" + common.Int2Str(channel.Id) + "/status"
	...
	require.NoError(t, model.DB.First(&gotChannel, channel.Id).Error)
	require.NoError(t, model.DB.First(&gotAbility, ability.Group, ability.Model, ability.ChannelId).Error)
	assert.Equal(t, constant.ChannelStatusManuallyDisabled, gotChannel.Status)
	assert.False(t, gotAbility.Enabled)
}

func TestTagEditRollsBackAbilityRebuildFailureAndRetries(t *testing.T) {
	_, do, _ := setupChannelRead(t, constant.RoleRootUser)
	channel := createSearchChannel(t, "tag-rollback", int(constant.ChannelTypeOpenAI), constant.ChannelStatusEnabled, "default", "old-model", "atomic", 0)
	tag := "atomic"
...
func TestTagStatusRollsBackAbilityWriteFailureAndRetries(t *testing.T) {
	_, do, _ := setupChannelRead(t, constant.RoleRootUser)
	channel := createSearchChannel(t, "tag-status-rollback", int(constant.ChannelTypeOpenAI), constant.ChannelStatusEnabled, "default", "gpt-4o", "atomic-status", 0)
	tag := "atomic-status"
...
func TestBatchDeleteRollsBackAfterAbilityDeleteAndRetries(t *testing.T) {
	_, do, _ := setupChannelRead(t, constant.RoleRootUser)
	channel := createSearchChannel(t, "batch-delete-rollback", int(constant.ChannelTypeOpenAI), constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
	ability := model.Ability{Group: "default", Model: "gpt-4o", ChannelId: channel.Id, Enabled: true, Weight: 1}
	require.NoError(t, model.DB.Create(&ability).Error)
...
func TestFixAbilitiesRollsBackFailedRebuildAndRetries(t *testing.T) {
	_, do, _ := setupChannelRead(t, constant.RoleRootUser)
	channel := createSearchChannel(t, "fix-rollback", int(constant.ChannelTypeOpenAI), constant.ChannelStatusEnabled, "default", "new-model", "", 0)
...
func TestCopyChannelRollsBackAbilityCreationFailureAndRetries(t *testing.T) {
	_, do, _ := setupChannelRead(t, constant.RoleRootUser)
	origin := createSearchChannel(t, "copy-rollback", int(constant.ChannelTypeOpenAI), constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
	...
	path := "/api/channel/copy/" + common.Int2Str(origin.Id)
...
func TestChannelTestDoesNotClaimSuccessWhenResultPersistenceFails(t *testing.T) {
	...
	channel := createSearchChannel(t, "health-write", int(constant.ChannelTypeOpenAI), constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
	...
	rec := do(http.MethodGet, "/api/channel/test/"+common.Int2Str(channel.Id), "")
	require.Equal(t, http.StatusOK, rec.Code, rec.Body.String())
	body := decodeBody(t, rec)
