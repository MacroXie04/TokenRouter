...
func TestLoginSessionEndpointsEnforceOwnership(t *testing.T) {
	...
	ownedSID, _, err := service.CreateSession(&actor, "127.0.0.2", "owned", "test")
	...
	foreignSID, _, err := service.CreateSession(&foreign, "127.0.0.3", "foreign", "test")
	require.NoError(t, err)
	...
	assert.Equal(t, service.SessionStatusActive, foreignSession.Status)
	...
	assert.Equal(t, service.SessionStatusRevoked, ownedSession.Status)
...
func TestRevokedSessionJWTFailsImmediatelyWhilePATRemainsIndependent(t *testing.T) {
	...
	require.NoError(t, err)
	revoked, err := service.RevokeUserSession(userID, sid, "test")
	require.NoError(t, err)
...
func TestLogoutCannotRevokeArbitrarySID(t *testing.T) {
	...
	foreign := model.User{Username: "logout-foreign", Password: "pw", Role: 1,
		Status: model.UserStatusEnabled, AuthVersion: 1}
	require.NoError(t, model.DB.Create(&foreign).Error)
	foreignSID, foreignRefresh, err := service.CreateSession(&foreign, "127.0.0.4", "foreign", "test")
	require.NoError(t, err)
	...
	bearerSID, bearerAccess, _, err := service.CompleteLogin(&foreign, "127.0.0.5", "bearer", "test")
	require.NoError(t, err)
	req := httptest.NewRequest(http.MethodPost, "/api/user/auth/logout", nil)
