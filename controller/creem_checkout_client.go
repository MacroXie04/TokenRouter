...
func createCreemCheckout(ctx context.Context, config setting.CreemConfig, snapshot service.CreemCheckoutRequest) (*creemCheckoutResponse, error) {
	...
	if err != nil || len(payload) == 0 || len(payload) > 64<<10 {
		return nil, service.ErrCreemCheckoutBindingMismatch
	}
	...
	body, err := common.ReadAllLimited(response.Body, creemCheckoutResponseLimit)
	if err != nil {
		return nil, err
	}
	...
	if common.Unmarshal(body, &result) != nil || !validCreemCheckoutURL(result.CheckoutURL) || !service.ValidCreemCheckoutID(result.ID) {
		return nil, service.ErrCreemCheckoutBindingMismatch
	}
	return &result, nil
}
