...
func initializeRelayModelCatalog(t *testing.T) int {
	...
	user := model.User{
		...
		Role: constant.RoleCommonUser, Status: model.UserStatusEnabled, Group: service.GroupDefault,
	}
	require.NoError(t, db.Create(&user).Error)
	channels := []model.Channel{
		{Type: int(constant.ChannelTypeOpenAI), Key: "openai", Name: "openai", Status: constant.ChannelStatusEnabled},
		{Type: int(constant.ChannelTypeAnthropic), Key: "anthropic", Name: "anthropic", Status: constant.ChannelStatusEnabled},
	}
	require.NoError(t, db.Create(&channels).Error)
	require.NoError(t, db.Create(&[]model.Ability{
		{Group: service.GroupDefault, Model: "gpt-4", ChannelId: channels[0].Id, Enabled: true},
		{Group: service.GroupDefault, Model: "gpt-4o", ChannelId: channels[0].Id, Enabled: true},
...
func TestRelayModelCatalogUnionsAuthorizedAutoGroups(t *testing.T) {
	...
	router.Use(func(c *gin.Context) {
		common.SetUserId(c, userID)
		middleware.SetupRelayTokenContext(c, &model.Token{UserId: userID, Group: service.GroupAuto})
		middleware.SetRelayGroupPolicy(c, service.RelayGroupPolicy{Groups: []string{"vip", service.GroupDefault}, Auto: true})
		c.Next()
	})
