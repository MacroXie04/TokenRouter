...
func TestRefreshedAuthCookiesRejectInvalidExpiryBeforeWriting(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "longer than refresh ttl", expiresAt: 100 + int64(service.RefreshTokenTTL/time.Second) + 1, validatedAt: 100},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			assert.True(t, errors.Is(err, service.ErrSessionExpiryInvalid))
