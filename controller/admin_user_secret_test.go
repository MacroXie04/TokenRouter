...
func TestAdminUserResponsesNeverExposePasswordHashesOrPATs(t *testing.T) {
	...
	target := createManagedUser(t, "secret-response-target", constant.RoleCommonUser, model.UserStatusEnabled, "default")
	...
	for _, path := range []string{"/api/user", "/api/user/" + common.Int2Str(target.Id)} {
		...
		require.NoError(t, common.UnmarshalJsonStr(settingsJSON, &visibleSettings))
