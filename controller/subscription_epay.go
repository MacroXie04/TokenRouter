...
func SubscriptionRequestEpay(c *gin.Context) {
	...
	tradeNo := service.NewSubscriptionEpayTradeNo(userID, time.Now().Unix(), suffix)
	order, err := service.CreateBoundEpaySubscriptionOrder(userID, plan.Id, req.PaymentMethod, tradeNo)
	if err != nil {
	...
	uri, params, err := client.Purchase(&epay.PurchaseArgs{
		...
		Money:          service.FormatPayMoney(order.Money),
		Device:         epay.PC,
		NotifyUrl:      notifyURL,
		ReturnUrl:      returnURL,
	})
	if err != nil || !validAbsoluteEpayURL(uri) {
		if statusErr := service.ExpireSubscriptionOrder(order.TradeNo, service.PaymentProviderEpay); statusErr != nil {
			common.SysError("EPay subscription checkout failure status update failed trade_no=" + order.TradeNo + ": " + statusErr.Error())
		}
...
func validAbsoluteEpayURLWithQuery(raw string, allowQuery bool) bool {
	if raw == "" || raw != strings.TrimSpace(raw) || strings.Contains(raw, `\`) ||
		boundedPublicStatusText(raw, maxStatusServerURLBytes, "") == "" {
		return false
	}
...
func SubscriptionEpayNotify(c *gin.Context) {
	...
	if !ok || service.CompleteBoundEpaySubscriptionOrder(info.ServiceTradeNo, payload, info.Type, info.Money) != nil {
		_, _ = c.Writer.Write([]byte("fail"))
		return
	}
