...
func TestConfiguredGroupsContractAndAuthorization(t *testing.T) {
	handler, do, _ := setupChannelRead(t, constant.RoleAdminUser)
	previous := service.ExportedGroupRatios()
	service.SetGroupRatios(map[string]float64{"vip": 2, "default": 1, "staff": 0.5})
	t.Cleanup(func() { service.SetGroupRatios(previous) })
	...
	_, commonDo, _ := setupChannelRead(t, constant.RoleCommonUser)
