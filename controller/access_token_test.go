...
func TestGenerateAccessTokenAndAuth(t *testing.T) {
	r, do, userId, _ := setupAuthAdjacent(t, constant.RoleCommonUser)
...
func TestAccessTokenAuthenticatesAdminRoutes(t *testing.T) {
	r, do, _, _ := setupAuthAdjacent(t, constant.RoleRootUser)
	rec := do(http.MethodGet, "/api/user/token", "")
	require.Equal(t, http.StatusOK, rec.Code)
	var body struct {
		Data string `json:"data"`
...
func TestGenerateAccessTokenUserRateLimit(t *testing.T) {
	...
	model.DB = db
	model.LOG_DB = db
	user := model.User{Username: "patrl", Password: "pw", Role: constant.RoleCommonUser,
