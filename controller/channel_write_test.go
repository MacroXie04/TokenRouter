...
func createWriteChannel(t *testing.T, name string, status int, group, models, tag string, priority int64) model.Channel {
	t.Helper()
	ch := createSearchChannel(t, name, int(constant.ChannelTypeOpenAI), status, group, models, tag, priority)
	return ch
}
...
func TestChannelStatusUpdateEndpoints(t *testing.T) {
	...
	ch := createWriteChannel(t, "statusch", constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
	...
	rec := do(http.MethodPost, "/api/channel/"+common.Int2Str(ch.Id)+"/status", `{"status":3}`)
	require.Equal(t, http.StatusOK, rec.Code, rec.Body.String())
	body := decodeBody(t, rec)
	assert.Equal(t, true, body["success"])
	assert.Equal(t, true, body["data"])
	assert.Equal(t, constant.ChannelStatusManuallyDisabled, channelByID(t, ch.Id).Status)
	...
	rec = do(http.MethodPost, "/api/channel/"+common.Int2Str(ch.Id)+"/status", `{"status":4}`)
	assert.Equal(t, http.StatusBadRequest, rec.Code)

	// Batch: two channels, one already disabled -> changed count 1.
	ch2 := createWriteChannel(t, "statusch2", constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
	...
	assert.Equal(t, constant.ChannelStatusManuallyDisabled, channelByID(t, ch2.Id).Status)
...
func TestChannelDeleteDisabledEndpoint(t *testing.T) {
	...
	createWriteChannel(t, "delme1", constant.ChannelStatusAutoDisabled, "default", "gpt-4o", "", 0)
	createWriteChannel(t, "delme2", constant.ChannelStatusManuallyDisabled, "default", "gpt-4o", "", 0)
...
func TestChannelTagEndpoints(t *testing.T) {
	_, do, _ := setupChannelRead(t, constant.RoleRootUser)
	fast1 := createWriteChannel(t, "fast1", constant.ChannelStatusEnabled, "default", "gpt-4o", "fast", 10)
	fast2 := createWriteChannel(t, "fast2", constant.ChannelStatusEnabled, "default", "claude-sonnet", "fast", 20)
	slow := createWriteChannel(t, "slow1", constant.ChannelStatusEnabled, "default", "gpt-4o-mini", "slow", 5)
	...
	assert.Equal(t, constant.ChannelStatusManuallyDisabled, channelByID(t, fast2.Id).Status)
	assert.Equal(t, constant.ChannelStatusEnabled, channelByID(t, slow.Id).Status)
	...
	assert.Equal(t, true, decodeBody(t, rec)["success"])
	assert.Equal(t, constant.ChannelStatusEnabled, channelByID(t, fast1.Id).Status)
...
func TestChannelDeleteBatchEndpoint(t *testing.T) {
	_, do, _ := setupChannelRead(t, constant.RoleRootUser)
	ch1 := createWriteChannel(t, "batch1", constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
	ch2 := createWriteChannel(t, "batch2", constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
	ch3 := createWriteChannel(t, "batch3", constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
...
func TestChannelFixAbilitiesEndpoint(t *testing.T) {
	_, do, _ := setupChannelRead(t, constant.RoleRootUser)
	enabled := createWriteChannel(t, "fixme", constant.ChannelStatusEnabled, "default,vip", "gpt-4o, claude-sonnet", "", 7)
	disabled := createWriteChannel(t, "fixoff", constant.ChannelStatusManuallyDisabled, "default", "gpt-4o-mini", "", 0)
...
func TestChannelFetchModelsEndpoints(t *testing.T) {
	...
	ch := createWriteChannel(t, "fetchch", constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
	require.NoError(t, model.DB.Model(&ch).Updates(map[string]any{"base_url": up.URL, "key": "sk-up"}).Error)

	// Fetch from a stored channel.
	rec := do(http.MethodGet, "/api/channel/fetch_models/"+common.Int2Str(ch.Id), "")
	require.Equal(t, http.StatusOK, rec.Code, rec.Body.String())
	body := decodeBody(t, rec)
	assert.Equal(t, true, body["success"])
	...
	defer down.Close()
	badCh := createWriteChannel(t, "fetchbad", constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
	require.NoError(t, model.DB.Model(&badCh).Updates(map[string]any{"base_url": down.URL, "key": "k"}).Error)
	rec = do(http.MethodGet, "/api/channel/fetch_models/"+common.Int2Str(badCh.Id), "")
	body = decodeBody(t, rec)
	assert.Equal(t, false, body["success"])
...
func TestChannelBatchTagAndTagModels(t *testing.T) {
	...
	ch2 := createWriteChannel(t, "tagme2", constant.ChannelStatusEnabled, "default", "m1", "old", 0)
...
func TestChannelCopyEndpoint(t *testing.T) {
	_, do, _ := setupChannelRead(t, constant.RoleRootUser)
	orig := createWriteChannel(t, "origin", constant.ChannelStatusEnabled, "default", "gpt-4o", "cp", 9)
	require.NoError(t, model.DB.Model(&orig).Updates(map[string]any{"balance": 42.5, "used_quota": 100}).Error)

	rec := do(http.MethodPost, "/api/channel/copy/"+common.Int2Str(orig.Id), "")
	require.Equal(t, http.StatusOK, rec.Code, rec.Body.String())
	body := decodeBody(t, rec)
	assert.Equal(t, true, body["success"])
	data, ok := body["data"].(map[string]any)
	...
	rec = do(http.MethodPost, "/api/channel/copy/"+common.Int2Str(orig.Id)+"?reset_balance=false&suffix=_v2", "")
	require.Equal(t, http.StatusOK, rec.Code)
...
func TestChannelMultiKeyManageEndpoint(t *testing.T) {
	_, do, _ := setupChannelRead(t, constant.RoleRootUser)
	multikey := createWriteChannel(t, "mk", constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
	...
	plain := createWriteChannel(t, "plain", constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
	...
	autoCh := createWriteChannel(t, "autok", constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
...
func TestChannelBalanceEndpoints(t *testing.T) {
	...
	ch := createWriteChannel(t, "balch", constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
	...
	rec := do(http.MethodGet, "/api/channel/update_balance/"+common.Int2Str(ch.Id), "")
	require.Equal(t, http.StatusOK, rec.Code, rec.Body.String())
	body := decodeBody(t, rec)
	assert.Equal(t, true, body["success"])
	...
	other := createWriteChannel(t, "otherch", constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
	require.NoError(t, model.DB.Model(&other).Updates(map[string]any{"type": int(constant.ChannelTypeAnthropic), "base_url": up.URL, "key": "k"}).Error)
	rec = do(http.MethodGet, "/api/channel/update_balance/"+common.Int2Str(other.Id), "")
	body = decodeBody(t, rec)
	assert.Equal(t, false, body["success"])
	assert.Equal(t, "尚未实现", body["message"])

	// Multi-key channels refuse balance queries.
	mk := createWriteChannel(t, "mkbal", constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
	...
	rec = do(http.MethodGet, "/api/channel/update_balance/"+common.Int2Str(mk.Id), "")
	...
	defer zeroUp.Close()
	zch := createWriteChannel(t, "zeroch", constant.ChannelStatusEnabled, "default", "gpt-4o", "", 0)
	...
	assert.Equal(t, constant.ChannelStatusAutoDisabled, channelByID(t, zch.Id).Status, "zero balance disables the channel")
