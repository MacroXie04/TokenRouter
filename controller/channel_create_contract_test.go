...
func TestChannelCreateAcceptsReferenceEnvelopeAndPersistsProviderFields(t *testing.T) {
	...
	assert.Equal(t, constant.ChannelStatusManuallyDisabled, channel.Status)
...
func TestChannelCreateBatchAndMultiKeyModesAreBoundedAndSecretSafe(t *testing.T) {
	...
	require.NoError(t, common.UnmarshalJsonStr(multi.ChannelInfo, &info))
...
func TestChannelUpdatePersistsEveryClassifiedProviderField(t *testing.T) {
	_, do, _ := setupChannelRead(t, constant.RoleRootUser)
	channel := createSearchChannel(t, "full-update", int(constant.ChannelTypeOpenAI),
