...
func WeChatAuth(c *gin.Context) {
	...
	wechatId, err := service.GetWeChatIdByCode(query.Get("code"))
	if err != nil {
		c.JSON(http.StatusOK, gin.H{"message": "微信登录失败", "success": false})
		return
	}
	...
	sid, access, refresh, accessExpiresAt, err := service.CompleteLoginWithExpiry(
...
func wechatLoginOrRegister(wechatId string) (*model.User, bool, error) {
	if user, err := service.FindUserByProviderIdentity("wechat", wechatId); err == nil {
	...
	usernameSuffix, err := common.SecureRandomAlphanumeric(8)
	if err != nil {
		return nil, false, err
	}
	user, created, err := service.LoginOrBindUser("wechat", &service.ProviderUser{
...
func WeChatBind(c *gin.Context) {
	...
	if err := service.BindProviderToSession("wechat", &service.ProviderUser{ProviderID: wechatId}, userId, identity.SessionID); err != nil {
