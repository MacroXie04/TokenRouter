...
func TestUserTopUpRedeemsServerIssuedCodeExactlyOnce(t *testing.T) {
	...
	redemption := model.Redemption{
		...
		Status: service.RedemptionStatusEnabled, Name: "secure", Quota: 125,
	}
...
func TestPaymentComplianceGateCoversEveryRedemptionAndAffiliateMutation(t *testing.T) {
	...
	redemption := model.Redemption{
		...
		Status: service.RedemptionStatusEnabled, Name: "compliance", Quota: 125,
	}
	require.NoError(t, model.DB.Create(&redemption).Error)
	...
	for _, path := range []string{"/api/user/topup", "/api/user/redemption/redeem"} {
		...
		assert.Contains(t, rec.Body.String(), service.ErrPaymentComplianceRequired.Error())
	}
	...
	assert.Equal(t, service.RedemptionStatusEnabled, storedRedemption.Status)
