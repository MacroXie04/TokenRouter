...
func TestStripeWebhookCompletion(t *testing.T) {
	...
	order, err := service.CreateBoundStripeTopUpWithTradeNo(user.Id, 1000, 10.0, "USD", "ref_stripe-completion-order")
	require.NoError(t, err)
	...
	assert.Equal(t, service.TopUpStatusPending, pending.Status)
	assert.Equal(t, service.StripeReconciliationPaymentMismatch, pending.ReconciliationState)
	...
	assert.Equal(t, 1000*common.QuotaPerUnit, got.Quota, "webhook must credit immutable internal quota")
	...
	assert.Equal(t, 1000*common.QuotaPerUnit, got2.Quota, "duplicate webhook must not double-credit")
}
...
func TestStripeWebhookReturnsRetryableFailureWithoutPartialCredit(t *testing.T) {
	...
	order, err := service.CreateBoundStripeTopUpWithTradeNo(user.Id, 750, 7.5, "USD", "ref_stripe-retry-order")
	require.NoError(t, err)
	require.NoError(t, service.BindStripeTopUpSession(order.TradeNo, "cs_wallet_retry"))
	...
	require.NoError(t, db.First(&unchanged, user.Id).Error)
	assert.Equal(t, 750*common.QuotaPerUnit, unchanged.Quota)
}
...
func TestStripeWebhookPaymentStateMachine(t *testing.T) {
	...
	require.NoError(t, db.Create(&user).Error)
	pending, err := service.CreateBoundStripeTopUpWithTradeNo(user.Id, 600, 6, "USD", "ref_stripe-delayed-order")
	require.NoError(t, err)
	require.NoError(t, service.BindStripeTopUpSession(pending.TradeNo, "cs_wallet_delayed"))
	r := router.SetUpRouter()
	...
	assert.Equal(t, service.TopUpStatusPending, gotOrder.Status)
	...
	require.NoError(t, db.First(&gotUser, user.Id).Error)
	require.NoError(t, db.First(&gotOrder, pending.Id).Error)
	assert.Equal(t, 600*common.QuotaPerUnit, gotUser.Quota)
	assert.Equal(t, service.TopUpStatusSuccess, gotOrder.Status)

	failed, err := service.CreateBoundStripeTopUpWithTradeNo(user.Id, 700, 7, "USD", "ref_stripe-failed-order")
	require.NoError(t, err)
	require.NoError(t, service.BindStripeTopUpSession(failed.TradeNo, "cs_wallet_failed"))
	...
	assert.Equal(t, service.TopUpStatusFailed, gotOrder.Status)
	require.NoError(t, db.First(&gotUser, user.Id).Error)
	assert.Equal(t, 600*common.QuotaPerUnit, gotUser.Quota, "a failed delayed payment must not grant quota")
}
...
func TestStripeWebhookRequiresDisjointTypeModeAndSessionBindings(t *testing.T) {
	...
	order, err := service.CreateBoundStripeTopUpWithTradeNo(user.Id, 100, 1, "USD", "ref_routing_wallet")
	require.NoError(t, err)
	require.NoError(t, service.BindStripeTopUpSession(order.TradeNo, "cs_routing_wallet"))
	...
	require.NoError(t, db.First(&stored, order.Id).Error)
	assert.Equal(t, service.TopUpStatusPending, stored.Status)
	assert.Equal(t, service.StripeReconciliationBindingMismatch, stored.ReconciliationState)

	legacy, err := service.CreateStripeTopUpWithTradeNo(user.Id, 50, .5, "USD", "ref_legacy_webhook_pending")
	require.NoError(t, err)
	...
	assert.Equal(t, service.StripeReconciliationLegacyBinding, stored.ReconciliationState)

	require.NoError(t, db.Model(&model.TopUp{}).Where("id = ?", legacy.Id).Update("status", service.TopUpStatusSuccess).Error)
