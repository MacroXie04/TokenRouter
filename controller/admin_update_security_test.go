...
func TestGenericAdminUserUpdateCannotEscalateRoles(t *testing.T) {
	...
	target := createManagedUser(t, "role-escalation-target", constant.RoleCommonUser, model.UserStatusEnabled, "default")

	// An admin cannot rewrite a lower-role account into a root account.
	rec := admin.do(http.MethodPut, "/api/user", `{"id":`+common.Int2Str(target.Id)+`,"role":100}`)
	assert.Equal(t, http.StatusBadRequest, rec.Code, rec.Body.String())
	var unchanged model.User
	require.NoError(t, model.DB.First(&unchanged, target.Id).Error)
	assert.Equal(t, constant.RoleCommonUser, unchanged.Role)

	// An admin cannot mutate itself or a peer through the generic editor.
	rec = admin.do(http.MethodPut, "/api/user", `{"id":`+common.Int2Str(admin.userID)+`,"quota":0}`)
	assert.Equal(t, http.StatusForbidden, rec.Code, rec.Body.String())

	// Even root uses the explicit, audited role workflow.
	rec = root.do(http.MethodPut, "/api/user", `{"id":`+common.Int2Str(target.Id)+`,"role":10}`)
	assert.Equal(t, http.StatusBadRequest, rec.Code, rec.Body.String())
	require.NoError(t, model.DB.First(&unchanged, target.Id).Error)
	assert.Equal(t, constant.RoleCommonUser, unchanged.Role)
}
...
func TestGenericAdminUserUpdateRejectsCorruptQuotaAndStatus(t *testing.T) {
	...
	target := createManagedUser(t, "invalid-update-target", constant.RoleCommonUser, model.UserStatusEnabled, "default")
	originalQuota := target.Quota

	for _, body := range []string{
		`{"id":` + common.Int2Str(target.Id) + `,"quota":-1}`,
		`{"id":` + common.Int2Str(target.Id) + `,"quota":2147483648}`,
		`{"id":` + common.Int2Str(target.Id) + `,"status":999}`,
	} {
