...
func assertOAuthFlowPending(t *testing.T, token string) *model.AuthFlow {
	t.Helper()
	flow, err := service.PeekAuthFlow(token, service.AuthFlowPurposeOAuth)
...
func TestOAuthBindCallbackRequiresExactLiveRequestSession(t *testing.T) {
	...
	otherOwnerSID, otherOwnerAccess, otherOwnerRefresh, err := service.CompleteLogin(
	...
	other := model.User{Username: "oauth-callback-other", Password: "pw", Role: constant.RoleCommonUser,
		Status: model.UserStatusEnabled, Quota: 1000, AuthVersion: 1}
	require.NoError(t, model.DB.Create(&other).Error)
	sid, access, refresh, err := service.CompleteLogin(&other, "127.0.0.8", "other", "test")
	require.NoError(t, err)
	...
	_, err = service.PeekAuthFlow(flowToken, service.AuthFlowPurposeOAuth)
	assert.ErrorIs(t, err, service.ErrInvalidFlowToken)
...
func TestOAuthStartCarriesOnlyOneBoundedAffiliateCode(t *testing.T) {
	...
	require.NoError(t, err)
	flow, err := service.PeekAuthFlow(location.Query().Get("state"), service.AuthFlowPurposeOAuth)
	require.NoError(t, err)
...
func TestOAuthCallbackProviderDenialConsumesStateWithoutReflectingDescription(t *testing.T) {
	...
	assert.Zero(t, behavior.tokenCalls.Load())
	assert.Zero(t, behavior.userInfoCalls.Load())
	_, err = service.PeekAuthFlow(state, service.AuthFlowPurposeOAuth)
	assert.ErrorIs(t, err, service.ErrInvalidFlowToken)
}
