...
func TestRatioSyncChannelsContractAndRootAuthorization(t *testing.T) {
	handler, do, _ := setupChannelRead(t, constant.RoleRootUser)
	highPriority := int64(10)
	explicit := model.Channel{
		Type: int(constant.ChannelTypeCustom), Key: "channel-secret-explicit", Name: "explicit",
		Status: constant.ChannelStatusEnabled, BaseURL: "https://ratios.example/api", Priority: &highPriority,
	}
	require.NoError(t, model.DB.Create(&explicit).Error)
	defaultURL := model.Channel{
		Type: int(constant.ChannelTypeOpenAI), Key: "channel-secret-default", Name: "default-url",
		Status: constant.ChannelStatusManuallyDisabled,
	}
	require.NoError(t, model.DB.Create(&defaultURL).Error)
	withoutURL := model.Channel{
		Type: int(constant.ChannelTypeCustom), Key: "channel-secret-hidden", Name: "no-url",
		Status: constant.ChannelStatusEnabled,
	}
	require.NoError(t, model.DB.Create(&withoutURL).Error)
	credentialedURL := model.Channel{
		Type: int(constant.ChannelTypeCustom), Key: "another-channel-secret", Name: "unsafe-url",
		Status: constant.ChannelStatusEnabled, BaseURL: "https://user:embedded-url-secret@example.com",
	}
	...
	assert.EqualValues(t, constant.ChannelStatusManuallyDisabled, defaultItem["status"])
	...
	_, adminDo, _ := setupChannelRead(t, constant.RoleAdminUser)
...
func TestRatioSyncFetchContractWithStoredChannelIDs(t *testing.T) {
	...
	channel := model.Channel{
		Type: int(constant.ChannelTypeCustom), Key: "must-not-be-sent", Name: "stored-source",
		Status: constant.ChannelStatusEnabled, BaseURL: server.URL,
	}
...
func TestRatioSyncOpenRouterCredentialIsBoundToStoredDestination(t *testing.T) {
	...
	channel := model.Channel{
		Type: int(constant.ChannelTypeOpenRouter), Key: credential, Name: "stored-openrouter",
		Status: constant.ChannelStatusEnabled, BaseURL: stored.URL,
	}
	require.NoError(t, model.DB.Create(&channel).Error)
	payload := fmt.Sprintf(`{
		"upstreams":[{
...
func TestRatioSyncOpenRouterCredentialIsNotForwardedAcrossRedirect(t *testing.T) {
	...
	channel := model.Channel{
		Type: int(constant.ChannelTypeOpenRouter), Key: credential, Name: "redirecting-openrouter",
		Status: constant.ChannelStatusEnabled, BaseURL: redirector.URL,
	}
	require.NoError(t, model.DB.Create(&channel).Error)
