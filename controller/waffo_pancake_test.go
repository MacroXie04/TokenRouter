...
type controllerPancakeGateway struct {
	mu               sync.Mutex
	checkoutRequests []service.WaffoPancakeCheckoutRequest
	checkoutErr      error
	storeNames       []string
	productCalls     []map[string]string
	published        []string
	catalog          *service.WaffoPancakeCatalog
}

func (gateway *controllerPancakeGateway) CreateCheckout(_ context.Context, _ setting.WaffoPancakeConfig, request service.WaffoPancakeCheckoutRequest) (*service.WaffoPancakeCheckoutSession, error) {
	gateway.mu.Lock()
	...
	return &service.WaffoPancakeCheckoutSession{
...
func (gateway *controllerPancakeGateway) ListCatalog(context.Context, setting.WaffoPancakeConfig) (*service.WaffoPancakeCatalog, error) {
	gateway.mu.Lock()
	defer gateway.mu.Unlock()
...
func (verifier *controllerPancakeVerifier) Verify(payload []byte, _ string, expectedEnvironment string) (*service.WaffoPancakeWebhookEvent, error) {
	verifier.mu.Lock()
...
func controllerPancakeEvent(tradeNo string, userID int, amount string) *service.WaffoPancakeWebhookEvent {
	return &service.WaffoPancakeWebhookEvent{
		...
		EventID: "delivery_pancake_123", StoreID: controllerPancakeStoreID, StoreName: "Store", Mode: service.WaffoPancakeModeTest,
...
func TestWaffoPancakeWalletAndSubscriptionHTTPContractEndToEnd(t *testing.T) {
	...
	restoreGateway := service.SetWaffoPancakeGatewayForTesting(gateway)
	restoreVerifier := service.SetWaffoPancakeWebhookVerifierForTesting(verifier)
	...
	assert.Equal(t, service.PaymentMethodWaffoPancake, methods[len(methods)-1].(map[string]any)["type"])
	...
	assert.Equal(t, service.PaymentProviderWaffoPancake, walletOrder.PaymentProvider)
...
func TestWaffoPancakeWebhookAvailabilitySignatureAndBodyLimits(t *testing.T) {
	...
	restore := service.SetWaffoPancakeWebhookVerifierForTesting(verifier)
	t.Cleanup(restore)
...
func TestWaffoPancakeCheckoutFailureClassification(t *testing.T) {
	...
	gateway := &controllerPancakeGateway{checkoutErr: service.ErrWaffoPancakeTransport}
	restore := service.SetWaffoPancakeGatewayForTesting(gateway)
	...
	require.NoError(t, model.DB.Where("user_id = ? AND payment_provider = ?", userID, service.PaymentProviderWaffoPancake).
		Order("id desc").First(&wallet).Error)
	assert.Equal(t, service.TopUpStatusPending, wallet.Status)
	assert.Equal(t, service.WaffoPancakeCreateUnknown, wallet.ReconciliationState)
	assert.Nil(t, wallet.ProviderSessionId)

	gateway = &controllerPancakeGateway{checkoutErr: &service.WaffoPancakeGatewayError{Status: http.StatusBadRequest, DefinitelyRejected: true}}
	restore = service.SetWaffoPancakeGatewayForTesting(gateway)
	...
	assert.Equal(t, service.TopUpStatusFailed, subscription.Status)
...
func TestWaffoPancakeRootAdministrationContract(t *testing.T) {
	...
	gateway := &controllerPancakeGateway{catalog: &service.WaffoPancakeCatalog{Stores: []service.WaffoPancakeCatalogStore{{
		ID: controllerPancakeStoreID, Name: "Store", Status: "active", ProdEnabled: true,
		OnetimeProducts: []service.WaffoPancakeCatalogProduct{{ID: controllerPancakeProductID, Name: "Wallet", Status: "active"}, {ID: controllerPancakePlanID, Name: "Plan", Status: "active"}},
	}}}}
	restore := service.SetWaffoPancakeGatewayForTesting(gateway)
	t.Cleanup(restore)
